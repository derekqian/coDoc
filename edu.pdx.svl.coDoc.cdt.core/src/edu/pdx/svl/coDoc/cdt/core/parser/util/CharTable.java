...
public class CharTable extends HashTable {
	...
	protected final int hash(char[] source, int start, int length) {
		return CharArrayUtils.hash(source, start, length)
				& ((keyTable.length * 2) - 1);
	}
		...
		if (hashTable != null) {
			...
			if ((currEntry + 1) >= capacity()) {
				...
				if (capacity() > minHashSize) {
					// if we grew from list to hash, then recurse and add as a
					// hashtable
	...
	protected void removeEntry(int i) {
		// Remove the entry from the keyTable, shifting everything over if
		// necessary
		...
		// Make sure you remove the value before calling super where currEntry
		// will change
		removeEntry(i, hash);
	}
