...
public class DefaultProblemHandler {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.IProblemHandler#ruleOnProblem(edu.pdx.svl.coDoc.cdt.core.parser.IProblem)
	 */
	public static boolean ruleOnProblem(IProblem p, ParserMode mode) {
		if (p == null)
			return true;
		if (p.checkCategory(IProblem.SCANNER_RELATED)
				|| p.checkCategory(IProblem.PREPROCESSOR_RELATED)) {
			switch (p.getID()) {
