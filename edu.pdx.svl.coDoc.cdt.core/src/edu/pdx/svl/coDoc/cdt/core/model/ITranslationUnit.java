...
/**
 * Represents an entire C translation unit (<code>.c</code> source file). The
 * children are of type <code>IStructureElement</code>, <code>IInclude</code>,
 * etc.. and appear in the order in which they are declared in the source. If a
 * <code>.c</code> file cannot be parsed, its structure remains unknown. Use
 * <code>ICElement.isStructureKnown</code> to determine whether this is the
 * case.
 */
public interface ITranslationUnit extends ICElement, IParent, IOpenable,
		ISourceReference, ISourceManipulation {
	/**
	 * Creates and returns an include declaration in this translation unit with
	 * the given name.
	 * <p>
	 * Optionally, the new element can be positioned before the specified
	 * sibling. If no sibling is specified, the element will be inserted as the
	 * last import declaration in this translation unit.
	 * <p>
	 * If the translation unit already includes the specified include
	 * declaration, the import is not generated (it does not generate
	 * duplicates).
	 * 
	 * @param name
	 *            the name of the include declaration to add (For example:
	 *            <code>"stdio.h"</code> or <code>"sys/types.h"</code>)
	 * @param sibling
	 *            the existing element which the include declaration will be
	 *            inserted immediately before (if <code> null </code>, then
	 *            this include will be inserted as the last include declaration.
	 * @param monitor
	 *            the progress monitor to notify
	 * @return the newly inserted include declaration (or the previously
	 ...
	 *                <ul>
	 *                <li> This C element does not exist or the specified
	 ...
	 *                translation unit (INVALID_SIBLING)
	 *                <li> The name is not a valid import name (INVALID_NAME)
	 *                </ul>
	 */
	IInclude createInclude(String name, boolean isStd, ICElement sibling,
			IProgressMonitor monitor) throws CModelException;

	/**
	 * Creates and returns a using declaration/directive in this translation
	 * unit
	 * 
	 * 
	 * @param name
	 *            the name of the using
	 * @param monitor
	 *            the progress monitor to notify
	 * @return the newly inserted namespace declaration (or the previously
	 *         existing one in case attempting to create a duplicate)
	 * 
	 * @exception CModelException
	 *                if the element could not be created. Reasons include:
	 *                <ul>
	 *                <li>This C element does not exist
	 ...
	 *                </ul>
	 */
	IUsing createUsing(String name, boolean isDirective, ICElement sibling,
			IProgressMonitor monitor) throws CModelException;
	 ...
	 *                </ul>
	 */
	INamespace createNamespace(String namespace, ICElement sibling,
			IProgressMonitor monitor) throws CModelException;

	/**
	 * Finds the shared working copy for this element, given a
	 * <code>IBuffer</code> factory. If no working copy has been created for
	 ...
	 * @return the found shared working copy for this element, <code>null</code>
	 *         if none
	 * @see IBufferFactory
	 * @since 2.0
	 */
	...
	/**
	 * Returns the smallest element within this translation unit that includes
	 ...
	 * the source range of this translation unit.
	 * 
	 * @param line
	 *            a position inside the translation unit
	 * @return the innermost C element enclosing a given source position or
	 *         <code>null</code> if none (excluding the translation unit).
	 * @exception CModelException
	 *                if the translation unit does not exist or if an exception
	 ...
	 * 
	 * @param position
	 *            a source position inside the translation unit
	 * @return the innermost C element enclosing a given source position or
	 *         <code>null</code> if none (excluding the translation unit).
	 * @exception CModelException
	 *                if the translation unit does not exist or if an exception
	...
	/**
	 * Returns the elements within this translation unit that includes the given
	 * source position (that is, a method, field, etc.), or an empty array if
	 * there are no elements other than the translation unit itself at the given
	 * position, or if the given position is not within the source range of this
	 * translation unit. You have this behavior when at expansion of a macro.
	 * 
	 * @param position
	 *            a source position inside the translation unit
	 * @return the innermost C element enclosing a given source position or
	...
	/**
	 * Returns the include declaration in this translation unit with the given
	 * name.
	 * 
	 * @param the
	 *            name of the include to find (For example:
	 *            <code>"stdio.h"</code> or <code>"sys/types.h"</code>)
	 * @return a handle onto the corresponding include declaration. The include
	 *         declaration may or may not exist.
	 */
	IInclude getInclude(String name);

	/**
	 * Returns the include declarations in this translation unit in the order in
	 ...
	 * based on the same original translation unit AND was using the same buffer
	 * factory (i.e. as defined by <code>Object#equals</code>).
	 * <p>
	 * The life time of a shared working copy is as follows:
	 * <ul>
	 * <li>The first call to <code>getSharedWorkingCopy(...)</code> creates a
	 * new working copy for this element</li>
	 ...
	 * working copy, i.e. if the working copy is closed then reopened, this
	 ...
	 * <p>
	 * When the shared working copy instance is created, an ADDED ICElementDelta
	 * is reported on this working copy.
	 * 
	 * @param monitor
	 ...
	 *            content of the working copy or <code>null</code> if the
	...
	/**
	 * Returns the first using in this translation unit with the name This is a
	 * handle-only method. The namespace declaration may or may not exist.
	 * 
	 * @param name
	 *            the name of the namespace declaration (For example,
	 *            <code>"std"</code>)
	 */
	IUsing getUsing(String name);

	/**
	 * Returns the usings in this translation unit in the order in which they
	...
	/**
	 * Returns the first namespace declaration in this translation unit with the
	 * given name This is a handle-only method. The namespace declaration may or
	 * may not exist.
	 * 
	 * @param name
	 *            the name of the namespace declaration (For example,
	...
	/**
	 * Returns the namespace declarations in this translation unit in the order
	 ...
	 * @return IWorkingCopy
	 */
	IWorkingCopy getWorkingCopy(IProgressMonitor monitor, IBufferFactory factory)
			throws CModelException;
	 ...
	 * 
	 * @deprecated this is currently only used by the core tests. It should be
	...
	/**
	 * Used by contributed languages' model builders to indicate whether or not
	 * the parse of a translation unit was successful.
	 * 
	 * @param wasSuccessful
	 * 
