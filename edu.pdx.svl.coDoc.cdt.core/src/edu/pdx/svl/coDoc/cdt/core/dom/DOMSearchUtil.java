...
/**
 * Utility class to have commonly used algorithms in one place for searching
 * with the DOM.
 * 
 * @author dsteffle
 */
public class DOMSearchUtil {
	...
	public static ParserLanguage getLanguageFromFile(IFile file) {
		...
		IContentType contentType = CCorePlugin.getContentType(project, file
	...
	public static ParserLanguage getLanguage(IPath path, IProject project) {
		...
		String id = null;
		IContentType contentType = CCorePlugin.getContentType(project, path
				.lastSegment());
		if (contentType != null) {
	...
	/**
	 * This is used to get the names from the TU that the IASTName searchName
	 * belongs to.
	 * 
	 * @param searchName
	 *            the IASTName whose references/delcarations are to be retrieved
	 * @param limitTo
	 *            used to specify whether to get declarations, references, or
	 *            both, one of: ( CSearchPattern.DECLARATION |
	 *            CSearchPattern.REFERENCES | CSearchPattern.ALL_OCCURRENCES )
	 * @return IASTName[] declarations, references, or both depending on limitTo
	 *         that correspond to the IASTName searchName searched for
	 */
	public static IASTName[] getNamesFromDOM(IASTName searchName, int limitTo) {
		...
		if (binding instanceof PDOMBinding) {
			try {
				...
				names = (IASTName[]) pdomNames.toArray(new IASTName[pdomNames
						.size()]);
			...
			if (names == null || names.length == 0) { // try alternate
														// strategies
				try {
					// fix for 86829, 95224
					if ((binding instanceof ICPPConstructor || (binding instanceof ICPPMethod && ((ICPPMethod) binding)
							.isDestructor()))
							&& binding.getScope() instanceof ICPPClassScope) {
						binding = ((ICPPClassScope) binding.getScope())
								.getClassType();
						names = getNames(tu, binding, limitTo);
					}
	...
	private static IASTName[] getNames(IASTTranslationUnit tu,
			IBinding binding, int limitTo) {
		IASTName[] names = null;
		if (limitTo == DECLARATIONS || limitTo == DECLARATIONS_DEFINITIONS) {
			...
			names = tu.getDeclarations(binding);
			names = (IASTName[]) ArrayUtil.addAll(IASTName.class, names, tu
					.getReferences(binding));
		} else { // assume ALL
			names = tu.getDeclarations(binding);
			names = (IASTName[]) ArrayUtil.addAll(IASTName.class, names, tu
					.getReferences(binding));
