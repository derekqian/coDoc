...
/**
 * Represents the outcome of an C model operation. Status objects are used
 ...
 * </p>
 * <p>
 * A C model status may also carry additional information (that is, in addition
 * to the information defined in <code>IStatus</code>):
 * <ul>
...
public interface ICModelStatus extends IStatus {
	/**
	 * Returns any C elements associated with the failure (see specification of
	 * the status code), or an empty array if no elements are related to this
	...
	/**
	 * Returns whether this status indicates that a C model element does not
