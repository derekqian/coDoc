...
public class ASTUtil {
	public static String[] getTemplateParameters(
	...
	public static String[] getTemplateParameters(Iterator templateParams) {
		...
		while (templateParams.hasNext()) {
			...
			IASTTemplateParameter parameter = (IASTTemplateParameter) templateParams
					.next();
			IASTTemplateParameter.ParamKind kind = parameter
					.getTemplateParameterKind();
			if (kind == IASTTemplateParameter.ParamKind.CLASS) {
				if ((parameter.getIdentifier() != null)
						&& (parameter.getIdentifier().length() != 0)) {
	...
	public static String getType(IASTAbstractDeclaration declaration) {
		...
		if (declaration instanceof IASTParameterDeclaration)
			type
					.append(getInitializerClause((IASTParameterDeclaration) declaration));
		return type.toString();
	}

	public static String getInitializerClause(
	...
	public static String getPointerToFunctionType(
			IASTAbstractDeclaration declaration) {
		StringBuffer type = new StringBuffer();
		...
		if (po != null) {
			...
			String[] parameters = getParameterTypes(
					declaration.getParameters(), false /*
														 * replace with
														 * takeVarArgs() later
														 */);
	...
	public static String getElaboratedTypeSignature(
			IASTElaboratedTypeSpecifier elab) {
		StringBuffer type = new StringBuffer();
	...
	public static String[] getFunctionParameterTypes(
			IASTFunction functionDeclaration) {
	...
	public static String[] getParameterTypes(Iterator parameters,
		...
		while (parameters.hasNext()) {
			IASTParameterDeclaration param = (IASTParameterDeclaration) parameters
					.next();
	...
	/**
	 * Return a string for the given expression. Expressions having an extension
	 * kind should provide their own toString method which will be called by
	 * this.
	 * 
	 * @param expression
	 * @return
	...
	public static String getExpressionString(IASTExpression expression) {
		...
		IASTNewExpressionDescriptor descriptor = expression
				.getNewExpressionDescriptor();
		IASTTypeId typeId = expression.getTypeId();

		if (literal != null && !literal.equals(EMPTY_STRING)
				&& (idExpression == null || idExpression.equals(EMPTY_STRING)))
			return getLiteralExpression(expression);

		if (idExpression != null && !idExpression.equals(EMPTY_STRING)
				&& lhs == null)
	...
	private static String getLiteralExpression(IASTExpression expression) {
		...
		if (kind != Kind.PRIMARY_CHAR_LITERAL
				&& kind != Kind.PRIMARY_STRING_LITERAL)
		...
		if (kind == Kind.PRIMARY_CHAR_LITERAL) {
			quoted = (literalString.charAt(0) == literalString
					.charAt(literalString.length() - 1) && literalString
					.charAt(0) == '\'');
			if (!quoted)
				buffer.append('\'');
	...
	private static String getBinaryExpression(IASTExpression expression) {
		...
		if (kind != Kind.EXPRESSIONLIST && kind != Kind.PM_DOTSTAR
				&& kind != Kind.PM_ARROWSTAR && kind != Kind.POSTFIX_SUBSCRIPT
		...
		if (kind == Kind.ANDEXPRESSION || kind == Kind.EXPRESSIONLIST
				|| kind == Kind.EXCLUSIVEOREXPRESSION
				|| kind == Kind.PM_DOTSTAR || kind == Kind.PM_ARROWSTAR
				...
				|| kind == Kind.ADDITIVE_PLUS || kind == Kind.ADDITIVE_MINUS
				...
				|| kind == Kind.SHIFT_LEFT || kind == Kind.SHIFT_RIGHT) {
	...
	private static String getUnaryExpression(IASTExpression expression) {
		...
		if (kind == Kind.UNARY_SIZEOF_UNARYEXPRESSION) {
			...
			buffer.append(' ');
				...
				|| kind == Kind.UNARY_DECREMENT || kind == Kind.THROWEXPRESSION) {
			buffer.append(ASTUtil.getStringForKind(kind));
		} else if (kind == Kind.UNARY_INCREMENT)
			buffer.append("++"); //$NON-NLS-1$
		else if (kind == Kind.DELETE_VECTORCASTEXPRESSION
				|| kind == Kind.DELETE_CASTEXPRESSION) {
		...
		if (kind == Kind.POSTFIX_INCREMENT || kind == Kind.POSTFIX_DECREMENT) {
			buffer.append(ASTUtil.getStringForKind(kind));
		}
	...
	static {
		expressionKindStringMap.put(IASTExpression.Kind.POSTFIX_DYNAMIC_CAST,
				Keywords.DYNAMIC_CAST);
		expressionKindStringMap.put(IASTExpression.Kind.POSTFIX_STATIC_CAST,
				Keywords.STATIC_CAST);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_REINTERPRET_CAST,
				Keywords.REINTERPRET_CAST);
		expressionKindStringMap.put(IASTExpression.Kind.POSTFIX_CONST_CAST,
				Keywords.CONST_CAST);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_CHAR, Keywords.CHAR);
		expressionKindStringMap
				.put(IASTExpression.Kind.POSTFIX_SIMPLETYPE_WCHART,
						Keywords.WCHAR_T);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_BOOL, Keywords.BOOL);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_SHORT, Keywords.SHORT);
		expressionKindStringMap.put(IASTExpression.Kind.POSTFIX_SIMPLETYPE_INT,
				Keywords.INT);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_LONG, Keywords.LONG);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_SIGNED, Keywords.SIGNED);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_UNSIGNED,
				Keywords.UNSIGNED);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_FLOAT, Keywords.FLOAT);
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_SIMPLETYPE_DOUBLE, Keywords.DOUBLE);
		expressionKindStringMap.put(IASTExpression.Kind.THROWEXPRESSION,
				Keywords.THROW);
		expressionKindStringMap.put(IASTExpression.Kind.ANDEXPRESSION, "&"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.UNARY_AMPSND_CASTEXPRESSION, "&"); //$NON-NLS-1$
		...
		expressionKindStringMap.put(IASTExpression.Kind.RELATIONAL_LESSTHAN,
				"<"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.RELATIONAL_LESSTHANEQUALTO, "<="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.RELATIONAL_GREATERTHANEQUALTO, ">="); //$NON-NLS-1$
		...
		expressionKindStringMap.put(IASTExpression.Kind.EQUALITY_NOTEQUALS,
				"!="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.UNARY_STAR_CASTEXPRESSION, "*"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.MULTIPLICATIVE_MULTIPLY, "*"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.UNARY_PLUS_CASTEXPRESSION, "+"); //$NON-NLS-1$
		expressionKindStringMap.put(IASTExpression.Kind.ADDITIVE_PLUS, "+"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.UNARY_MINUS_CASTEXPRESSION, "-"); //$NON-NLS-1$
		expressionKindStringMap.put(IASTExpression.Kind.ADDITIVE_MINUS, "-"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.UNARY_NOT_CASTEXPRESSION, "!"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.UNARY_TILDE_CASTEXPRESSION, "~"); //$NON-NLS-1$
		expressionKindStringMap.put(IASTExpression.Kind.UNARY_DECREMENT, "--"); //$NON-NLS-1$
		expressionKindStringMap
				.put(IASTExpression.Kind.POSTFIX_DECREMENT, "--"); //$NON-NLS-1$
		expressionKindStringMap.put(IASTExpression.Kind.UNARY_INCREMENT, "++"); //$NON-NLS-1$
		expressionKindStringMap
				.put(IASTExpression.Kind.POSTFIX_INCREMENT, "++"); //$NON-NLS-1$
		...
		expressionKindStringMap.put(IASTExpression.Kind.MULTIPLICATIVE_MODULUS,
				"%"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_DOT_TEMPL_IDEXPRESS, "."); //$NON-NLS-1$	
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_DOT_IDEXPRESSION, "."); //$NON-NLS-1$   |
		expressionKindStringMap.put(IASTExpression.Kind.POSTFIX_DOT_DESTRUCTOR,
				"."); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_ARROW_TEMPL_IDEXP, "->"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_ARROW_DESTRUCTOR, "->"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.POSTFIX_ARROW_IDEXPRESSION, "->"); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_NORMAL, "="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_MULT, "*="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_DIV, "/="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_MOD, "%="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_PLUS, "+="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_MINUS, "-="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_RSHIFT, ">>="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_LSHIFT, "<<="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_AND, "&="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_XOR, "^="); //$NON-NLS-1$
		expressionKindStringMap.put(
				IASTExpression.Kind.ASSIGNMENTEXPRESSION_OR, "|="); //$NON-NLS-1$
