...
/**
 * Represent the root C element corresponding to the workspace. Since there is
 * only one such root element, it is commonly referred to as <em>the</em> C
 * model element. The C model element needs to be opened before it can be
 * navigated or manipulated. The C model element has no parent (it is the root
 * of the C element hierarchy). Its children are <code>ICProject</code>s.
 * <p>
 * This interface provides methods for performing copy, move, rename, and delete
 * operations on multiple C elements.
 * </p>
 * 
...
public interface ICModel extends ICElement, IParent, IOpenable {
	 ...
	 *                </ul>
	 */
	void copy(ICElement[] elements, ICElement[] containers,
	 ...
	 *                </ul>
	 */
	void delete(ICElement[] elements, boolean force, IProgressMonitor monitor)
	 ...
	 */
	void move(ICElement[] elements, ICElement[] containers,
			ICElement[] siblings, String[] renamings, boolean replace,
			IProgressMonitor monitor) throws CModelException;
	 ...
	 *                </ul>
	 */
	void rename(ICElement[] elements, ICElement[] destinations, String[] names,
