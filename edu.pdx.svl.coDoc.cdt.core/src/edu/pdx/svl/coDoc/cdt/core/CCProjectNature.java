...
public class CCProjectNature extends CProjectNature {
	...
	public static final String CC_NATURE_ID = CCorePlugin.PLUGIN_ID
			+ ".ccnature"; //$NON-NLS-1$
	...
	/**
	 * Checks to ensure that a cnature already exists, if not throw a
	 * CoreException. Does NOT add a default builder
	 * 
	...
	public void configure() throws CoreException {
		if (!getProject().hasNature(CProjectNature.C_NATURE_ID)) {
			IStatus status = new Status(IStatus.ERROR, CCorePlugin.PLUGIN_ID,
					...
					"No Nature", null); // $NON_NLS //$NON-NLS-1$
