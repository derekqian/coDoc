 ...
 * tree of C elements. The changes to the elements are described by the
...
public class ElementChangedEvent extends EventObject {
	 ...
	 * creations, deletions, and modifications to one or more C element(s)
	 * expressed as a hierarchical C element delta as returned by
	 * <code>getDelta()</code>.
	 * 
	 ...
	 * contains information relative to the previous CModel operations (in other
	 * words, it ignores the possible resources which have changed outside C
	 * operations). In particular, it is possible that the CModel be
	 * inconsistent with respect to resources, which got modified outside CModel
	 * operations (it will only be fully consistent once the POST_CHANGE
	 ...
	 * @see #getDelta()
	 * @since 2.0
	 * @deprecated - no longer used, such deltas are now notified during
	 *             POST_CHANGE
	 */
	 ...
	 * somewhere in function body No global data affected for any C element but
	 * element offsets should be recalculated now.
	 * 
	 * Note: usually, CShifData object is sent with this event as ICElementDelta
	 * 
	 * @see CShiftData
	 */
