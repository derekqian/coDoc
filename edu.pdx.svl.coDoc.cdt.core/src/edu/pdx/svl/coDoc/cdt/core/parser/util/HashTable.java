...
public class HashTable implements Cloneable {
	...
	public Object clone() {
		...
		if (hashTable != null) {
			...
			System.arraycopy(hashTable, 0, newTable.hashTable, 0,
					hashTable.length);
			System.arraycopy(nextTable, 0, newTable.nextTable, 0,
					nextTable.length);
		}
	...
	protected void rehash() {
		...
		// clear the table (don't call clear() or else the subclasses stuff will
		// be cleared too)
		for (int i = 0; i < capacity(); i++) {
			hashTable[2 * i] = 0;
			hashTable[2 * i + 1] = 0;
