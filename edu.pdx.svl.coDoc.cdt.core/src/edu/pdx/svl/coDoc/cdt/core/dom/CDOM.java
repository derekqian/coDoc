...
import edu.pdx.svl.coDoc.cdt.internal.core.dom.InternalASTServiceProvider;
import edu.pdx.svl.coDoc.cdt.internal.core.dom.PartialWorkingCopyCodeReaderFactory;
import edu.pdx.svl.coDoc.cdt.internal.core.dom.SavedCodeReaderFactory;
import edu.pdx.svl.coDoc.cdt.internal.core.dom.WorkingCopyCodeReaderFactory;
 ...
 * @author jcamelon
 * 
 * This class serves as the manager of the AST/DOM mechanisms for the CDT. It
 * should be eventually added to CCorePlugin for startup.
 */
public class CDOM implements IASTServiceProvider {
	...
	/**
	 * Constant <code>PARSE_SAVED_RESOURCES</code> - Parse saved resources in
	 * the workspace
	 */
	public static final int PARSE_SAVED_RESOURCES = 0;

	/**
	 * Constant <code>PARSE_WORKING_COPY_WITH_SAVED_INCLUSIONS</code> - Parse
	 * working copy for translation unit, saved resources for all header files.
	 */
	public static final int PARSE_WORKING_COPY_WITH_SAVED_INCLUSIONS = 1;

	/**
	 * Constant <code>PARSE_WORKING_COPY_WHENEVER_POSSIBLE</code> - Parse
	 * working copy whenever possible for both header files and the file in
	 * question as a translation unit.
	 */
	public static final int PARSE_WORKING_COPY_WHENEVER_POSSIBLE = 2;

	/**
	 * <code>provider</code> is registered by the UI as a
	 * IWorkingCopyProvider.
	 */
	private IWorkingCopyProvider provider;

	/**
	 * This is the factory function that returns an ICodeReaderFactory instance
	 * based upon the key provided.
	 * 
	 * @param key
	 *            one of PARSE_SAVED_RESOURCES,
	 *            PARSE_WORKING_COPY_WITH_SAVED_INCLUSIONS,
	 *            PARSE_WORKING_COPY_WHENEVER_POSSIBLE
	 * @return an implementation that works according to the key specified or
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.IASTServiceProvider#getTranslationUnit(org.eclipse.core.resources.IFile)
	 */
	 ...
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.IASTServiceProvider#getTranslationUnit(org.eclipse.core.resources.IFile,
	 *      edu.pdx.svl.coDoc.cdt.core.dom.ICodeReaderFactory)
	 */
	public IASTTranslationUnit getTranslationUnit(IFile fileToParse,
	...
	public ASTCompletionNode getCompletionNode(IFile fileToParse, int offset,
		...
		// TODO - At this time, we purely delegate blindly
		// In the future, we may need to delegate based upon context provided
		return defaultService.getCompletionNode(fileToParse, offset,
				fileCreator);
	}
	...
	public ASTCompletionNode getCompletionNode(IStorage fileToParse,
		...
		// TODO - At this time, we purely delegate blindly
		// In the future, we may need to delegate based upon context provided
		return defaultService.getCompletionNode(fileToParse, project, offset,
				fileCreator);
	}

	/**
	 * This method allows a UI component to register its IWorkingCopyProvider to
	 * the CDOM.
	 * 
	 * @param workingCopyProvider -
	 *            UI components buffer manager
	 */
