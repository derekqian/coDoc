...
/**
 * Destroys a working copy (remove it from its cache if it is shared) and signal
 * its removal through a delta.
 */
public class DestroyWorkingCopyOperation extends CModelOperation {
	 ...
	 *                if setting the source of the original translation unit
	 *                fails
	 */
	protected void executeOperation() throws CModelException {
		...
		Map perFactoryWorkingCopies = (Map) sharedWorkingCopies
				.get(workingCopy.bufferFactory);
		if (perFactoryWorkingCopies != null) {
			if (perFactoryWorkingCopies.remove(originalElement) != null) {
				// System.out.println("Destroying shared working copy " +
				// workingCopy.toStringWithAncestors());//$NON-NLS-1$
				// CModelManager.getDefault().fire(delta,
