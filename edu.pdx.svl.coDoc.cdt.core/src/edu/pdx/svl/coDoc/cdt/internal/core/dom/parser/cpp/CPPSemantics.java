...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTCastExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFieldReference;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTLiteralExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IPointerType;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IProblemBinding;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IQualifierType;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTFieldDesignator;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTExplicitTemplateInstantiation;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTFieldReference;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTForStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTNamedTypeSpecifier;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTNewExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTUsingDeclaration;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTUsingDirective;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTWhileStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPClassTemplate;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPFunctionTemplate;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPFunctionType;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPMember;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPPointerToMemberType;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPTemplateTemplateParameter;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTCompositeTypeSpecifier.ICPPASTBaseSpecifier;
...
import edu.pdx.svl.coDoc.cdt.core.parser.util.ObjectSet;
import edu.pdx.svl.coDoc.cdt.core.parser.util.ArrayUtil.ArrayWrapper;
...
public class CPPSemantics {
	...
	protected static final ASTNodeProperty STRING_LOOKUP_PROPERTY = new ASTNodeProperty(
			"CPPSemantics.STRING_LOOKUP_PROPERTY - STRING_LOOKUP"); //$NON-NLS-1$

	public static final char[] EMPTY_NAME_ARRAY = new char[0];

	public static final String EMPTY_NAME = ""; //$NON-NLS-1$

	public static final char[] OPERATOR_ = new char[] { 'o', 'p', 'e', 'r',
			'a', 't', 'o', 'r', ' ' }; //$NON-NLS-1$
	...
	static protected class LookupData {
		...
		public ObjectSet visited = ObjectSet.EMPTY_SET; // used to ensure we
														// don't visit things
														// more than once

		public ObjectSet inheritanceChain; // used to detect circular
											// inheritance
		...
		public boolean includeBlockItem(IASTNode item) {
			if (astName.getPropertyInParent() == STRING_LOOKUP_PROPERTY)
				return true;
			if ((astName != null && astName.getParent() instanceof IASTIdExpression)
					|| item instanceof ICPPASTNamespaceDefinition
					|| (item instanceof IASTSimpleDeclaration && ((IASTSimpleDeclaration) item)
		...
		public boolean forDefinition() {
			...
			if (p1 instanceof IASTDeclarator
					&& p2 instanceof IASTSimpleDeclaration) {
			...
			if (p1 instanceof ICPPASTNamedTypeSpecifier
				...
				if (p2 instanceof ICPPASTFunctionDeclarator) {
					...
					if (names.length >= 2 && names[names.length - 1] == astName)
						return CPPVisitor.isConstructor(
								names[names.length - 2], (IASTDeclarator) p2);
				} else if (p2 instanceof ICPPASTNamedTypeSpecifier) {
					IASTNode p3 = p2.getParent();
					return p3 instanceof IASTTypeId
							&& p3.getParent() instanceof ICPPASTNewExpression;
		...
		private boolean checkWholeClassScope() {
			...
			IASTNode parent = astName.getParent();
			while (parent != null
					&& !(parent instanceof IASTFunctionDefinition)) {
		...
		/**
		 * an IType[] of function arguments, inluding the implied object
		 * argument
		 * 
		 * @return
		 */
		public IType getImpliedObjectArgument() {
			...
			if (astName != null) {
				...
				if (prop == IASTFieldReference.FIELD_NAME
						|| (prop == STRING_LOOKUP_PROPERTY && tempName
								.getParent() instanceof ICPPASTFieldReference)) {
					ICPPASTFieldReference fieldRef = (ICPPASTFieldReference) tempName
							.getParent();
					implied = CPPVisitor.getExpressionType(fieldRef
							.getFieldOwner());
					if (fieldRef.isPointerDereference()
							&& implied instanceof IPointerType) {
						try {
				...
				} else if (prop == STRING_LOOKUP_PROPERTY
						&& tempName.getParent() instanceof IASTArraySubscriptExpression) {
					IASTExpression exp = ((IASTArraySubscriptExpression) tempName
							.getParent()).getArrayExpression();
		...
		public boolean forFriendship() {
			...
			if (node instanceof ICPPASTDeclSpecifier
					&& node.getParent() instanceof IASTDeclaration) {
			...
			if (decl instanceof IASTSimpleDeclaration) {
				...
				if (!((ICPPASTDeclSpecifier) simple.getDeclSpecifier())
						.isFriend())
					return false;
				if (dtor != null)
					return true;
				...
				if (!((ICPPASTDeclSpecifier) fnDef.getDeclSpecifier())
						.isFriend())
					return false;
				return (dtor != null);
			}
		...
		public boolean checkClassContainingFriend() {
			...
			while (p != null) {
				...
				if (prop == ICPPASTTemplateId.TEMPLATE_ID_ARGUMENT
						|| prop == IASTDeclarator.DECLARATOR_NAME)
					return false;
				if (p instanceof IASTDeclarator
						&& !(((IASTDeclarator) p).getName() instanceof ICPPASTQualifiedName))
					return false;
				if (p instanceof IASTDeclaration) {
					if (prop == IASTCompositeTypeSpecifier.MEMBER_DECLARATION) {
						if (p instanceof IASTSimpleDeclaration) {
							ICPPASTDeclSpecifier declSpec = (ICPPASTDeclSpecifier) ((IASTSimpleDeclaration) p)
									.getDeclSpecifier();
							return declSpec.isFriend();
						} else if (p instanceof IASTFunctionDefinition) {
							ICPPASTDeclSpecifier declSpec = (ICPPASTDeclSpecifier) ((IASTFunctionDefinition) p)
									.getDeclSpecifier();
							return declSpec.isFriend();
						}
					} else {
		...
		public boolean preferTemplateFunctions() {
			if (astName == null)
				return false;
			return (astName instanceof ICPPASTTemplateId || astName
					.getPropertyInParent() == ICPPASTTemplateId.TEMPLATE_NAME);
		}
	}
	...
	static protected class Cost {
		...
		public int compare(Cost cost) throws DOMException {
			...
			if (userDefined != 0 || cost.userDefined != 0) {
				...
				// else they are the same constructor/conversion operator and
				// are ranked
			...
			if (result == 0) {
				...
				} else if ((cost.qualification == qualification)
						&& qualification == 0) {
					return 0;
				} else {
	...
	static protected IBinding resolveBinding(IASTName name) {
		// 1: get some context info off of the name to figure out what kind of
		// lookup we want
	...
	private static IBinding postResolution(IBinding binding, LookupData data) {
		...
		if (binding == null && data.checkClassContainingFriend()) {
			// 3.4.1-10 if we don't find a name used in a friend declaration in
			// the member declaration's class
			...
			if (parent instanceof ICPPASTCompositeTypeSpecifier) {
				IScope scope = ((ICPPASTCompositeTypeSpecifier) parent)
						.getScope();
				try {
					lookup(data, scope);
					binding = resolveAmbiguities(data, data.astName);
		...
		if (binding instanceof ICPPClassTemplate) {
			...
			if (prop != ICPPASTQualifiedName.SEGMENT_NAME
				...
				try {
					IASTNode def = ((ICPPInternalBinding) binding)
							.getDefinition();
					if (def != null) {
						def = def.getParent();
						IASTNode parent = data.astName.getParent();
						while (parent != null) {
							if (parent == def) {
								binding = CPPTemplates
										.instantiateWithinClassTemplate((ICPPClassTemplate) binding);
								break;
							}
		...
		if (binding instanceof ICPPClassType && data.considerConstructors) {
			...
			if (data.astName instanceof ICPPASTTemplateId
					&& cls instanceof ICPPInternalTemplate) {
				ICPPASTTemplateId id = (ICPPASTTemplateId) data.astName;
				IType[] args = CPPTemplates.createTypeArray(id
						.getTemplateArguments());
				IBinding inst = ((ICPPInternalTemplate) cls).instantiate(args);
				cls = inst instanceof ICPPClassType ? (ICPPClassType) inst
						: cls;
			}
			if (cls != null) {
				try {
					...
					if (ctors.length > 0
							&& !(ctors[0] instanceof IProblemBinding)) {
						// then use the class scope to resolve which one.
						binding = ((ICPPClassScope) cls.getCompositeScope())
								.getBinding(data.astName, true);
					}
				} catch (DOMException e) {
					binding = e.getProblem();
				}
		...
		if (name.getParent() instanceof ICPPASTTemplateId) {
			if (binding instanceof ICPPTemplateInstance) {
				IBinding b = binding;
				binding = ((ICPPTemplateInstance) binding)
		...
		if (name.getParent() instanceof ICPPASTQualifiedName) {
			IASTName[] ns = ((ICPPASTQualifiedName) name.getParent())
					.getNames();
		...
		if (binding != null) {
			if (name.getPropertyInParent() == IASTNamedTypeSpecifier.NAME
					&& !(binding instanceof IType || binding instanceof ICPPConstructor)) {
				...
				if (parent instanceof IASTTypeId
					...
					binding = new ProblemBinding(data.astName,
							IProblemBinding.SEMANTIC_INVALID_TYPE, data.name());
				}
			}
		}
		...
		if (binding == null || binding instanceof IProblemBinding) {
			...
			if (pdom != null) {
				binding = ((IPDOMResolver) pdom.getAdapter(IPDOMResolver.class))
						.resolveBinding(name);
			}
			...
			if (binding == null) {
				if (name instanceof ICPPASTQualifiedName
						&& data.forDefinition())
					binding = new ProblemBinding(
							data.astName,
							IProblemBinding.SEMANTIC_MEMBER_DECLARATION_NOT_FOUND,
							data.name());
				else
					binding = new ProblemBinding(data.astName,
							IProblemBinding.SEMANTIC_NAME_NOT_FOUND, data
									.name());
			}
		}
		return binding;
	...
	static private CPPSemantics.LookupData createLookupData(IASTName name,
			boolean considerAssociatedScopes) {
		CPPSemantics.LookupData data = new CPPSemantics.LookupData(name);
		IASTNode parent = name.getParent();

		if (name instanceof ICPPASTTemplateId) {
			data.templateArguments = ((ICPPASTTemplateId) name)
					.getTemplateArguments();
		}
		...
		if (parent instanceof IASTDeclarator
				&& parent.getPropertyInParent() == IASTSimpleDeclaration.DECLARATOR) {
			IASTSimpleDeclaration simple = (IASTSimpleDeclaration) parent
					.getParent();
		...
		if (parent instanceof ICPPASTFunctionDeclarator) {
			data.functionParameters = ((ICPPASTFunctionDeclarator) parent)
					.getParameters();
		} else if (parent instanceof IASTIdExpression) {
			ASTNodeProperty prop = parent.getPropertyInParent();
			if (prop == IASTFunctionCallExpression.FUNCTION_NAME) {
				parent = parent.getParent();
				IASTExpression exp = ((IASTFunctionCallExpression) parent)
						.getParameterExpression();
				if (exp instanceof IASTExpressionList)
					data.functionParameters = ((IASTExpressionList) exp)
							.getExpressions();
				else if (exp != null)
			...
			IASTExpression exp = ((IASTFunctionCallExpression) parent
					.getParent()).getParameterExpression();
			if (exp instanceof IASTExpressionList)
				data.functionParameters = ((IASTExpressionList) exp)
						.getExpressions();
			else if (exp != null)
			...
			if (val instanceof IASTExpressionList)
				data.functionParameters = ((IASTExpressionList) val)
						.getExpressions();
			else if (val != null)
	...
	static private void getAssociatedScopes(IType t, ObjectSet namespaces,
			ObjectSet classes) throws DOMException {
		// 3.4.2-2
		if (t instanceof ICPPClassType) {
			...
			getAssociatedScopes(getUltimateType(ft.getReturnType(), true),
					namespaces, classes);
			IType[] ps = ft.getParameterTypes();
			for (int i = 0; i < ps.length; i++) {
				getAssociatedScopes(getUltimateType(ps[i], true), namespaces,
						classes);
			}
			...
			getAssociatedScopes(getUltimateType(((ICPPPointerToMemberType) t)
					.getType(), true), namespaces, classes);
		}
		return;
	}
	...
	static private ICPPScope getLookupScope(IASTName name) throws DOMException {
		...
		if (parent instanceof ICPPASTBaseSpecifier) {
			...
			IASTFunctionDeclarator dtor = (IASTFunctionDeclarator) initializer
					.getParent();
		...
		if (scope instanceof ICPPScope)
			...
			return new CPPScope.CPPScopeProblem(((IProblemBinding) scope)
					.getASTNode(), IProblemBinding.SEMANTIC_BAD_SCOPE,
					((IProblemBinding) scope).getNameCharArray());
		return new CPPScope.CPPScopeProblem(name,
				IProblemBinding.SEMANTIC_BAD_SCOPE, name.toCharArray());
	}

	private static void mergeResults(LookupData data, Object results,
			boolean scoped) {
		if (!data.prefixLookup) {
			if (results instanceof IBinding) {
				data.foundItems = ArrayUtil.append(Object.class,
						(Object[]) data.foundItems, results);
			} else if (results instanceof Object[]) {
				data.foundItems = ArrayUtil.addAll(Object.class,
						(Object[]) data.foundItems, (Object[]) results);
			}
		} else {
			data.foundItems = mergePrefixResults(
					(CharArrayObjectMap) data.foundItems, results, scoped);
		}
	}
	 ...
	 * @param dest
	 * @param source :
	 *            either Object[] or CharArrayObjectMap
	 * @param scoped
	 * @return
	 */
	private static Object mergePrefixResults(CharArrayObjectMap dest,
			Object source, boolean scoped) {
		if (source == null)
			return dest;
		CharArrayObjectMap resultMap = (dest != null) ? dest
				: new CharArrayObjectMap(2);
		...
		for (int i = 0; i < size; i++) {
			char[] key = (map != null) ? map.keyAt(i)
					: (objs[i] instanceof IBinding) ? ((IBinding) objs[i])
							.getNameCharArray() : ((IASTName) objs[i])
							.toCharArray();
			...
			if (idx == -1) {
				...
				if (obj instanceof Object[]) {
					if (so instanceof IBinding || so instanceof IASTName)
						obj = ArrayUtil
								.append(Object.class, (Object[]) obj, so);
					else
						obj = ArrayUtil.addAll(Object.class, (Object[]) obj,
								(Object[]) so);
				} else {
					if (so instanceof IBinding || so instanceof IASTName)
						...
						temp[0] = obj;
						obj = ArrayUtil.addAll(Object.class, temp,
								(Object[]) so);
					}
				}
	...
	static protected void lookup(CPPSemantics.LookupData data, Object start)
			throws DOMException {
		...
		while (scope != null) {
			...
			if (!data.usingDirectivesOnly) {
				if (scope.isFullyCached() && !data.prefixLookup
						&& data.astName != null) {
					IBinding binding = data.prefixLookup ? null : scope
							.getBinding(data.astName, true);
					if (binding != null
							&& (CPPSemantics.declaredBefore(binding,
									data.astName) || (scope instanceof ICPPClassScope && data.checkWholeClassScope))) {
						mergeResults(data, binding, true);
					}
				} else {
					if (!data.prefixLookup && data.astName != null) {
						IBinding b = scope.getBinding(data.astName, false);
						if (b instanceof CPPImplicitFunction
								|| b instanceof CPPImplicitTypedef
								|| b instanceof PDOMBinding)
							// TODO the PDOMBinding thing is a kludge
							mergeResults(data, b, true);
					}
					mergeResults(data, lookupInScope(data, scope, blockItem),
							true);
				}
				...
				if ((!data.hasResults() || data.prefixLookup)
					...
					directives.array = ((ICPPNamespaceScope) scope)
							.getUsingDirectives();
					if (directives.array != null) {
						for (int i = 0; i < directives.array.length; i++) {
							if (!CPPSemantics.declaredBefore(
									directives.array[i], blockItem)) {
								directives.array[i] = null;
								directives.array = ArrayUtil.trim(
										IASTNode.class, directives.array);
							}
						}
					}
				}
			...
			if (!data.ignoreUsingDirectives) {
				...
				if (data.prefixLookup || !data.hasResults()) {
					...
					if (directives != null && directives.array != null
							&& directives.array.length != 0)
						processDirectives(data, scope, directives.array);

					while (!data.usingDirectives.isEmpty()
							&& data.usingDirectives.get(scope) != null) {
						transitives = lookupInNominated(data, scope,
								transitives);

						if (!data.qualified()
								|| (data.prefixLookup || !data.hasResults())) {
							processDirectives(data, scope, transitives);
						}
					}
				}
			...
			if ((!data.prefixLookup && (data.problem != null || data
					.hasResults()))
			...
			if (!data.prefixLookup
					&& (data.problem != null || data.hasResults()))
				return;
			...
			if (parentScope instanceof ICPPTemplateScope) {
				...
				if (parent != null) {
					...
					if (templateScope.getTemplateDefinition() == ((ICPPTemplateScope) parentScope)
	...
	private static Object lookupInParents(CPPSemantics.LookupData data,
			ICPPScope lookIn) throws DOMException {
		ICPPBase[] bases = null;
		...
		for (int i = 0; i < size; i++) {
			...
			if (!bases[i].isVirtual() || !data.visited.containsKey(parent)) {
				...
				// if the inheritanceChain already contains the parent, then
				// that
				// is circular inheritance
				if (!data.inheritanceChain.containsKey(parent)) {
					// is this name define in this scope?
					if (data.astName != null && !data.prefixLookup
							&& parent.isFullyCached())
					...
					if (inherited == null || data.prefixLookup) {
						...
						if (inherited != null) {
							inherited = mergePrefixResults(null, inherited,
									true);
							inherited = mergePrefixResults(
									(CharArrayObjectMap) inherited,
					...
					data.problem = new ProblemBinding(null,
							IProblemBinding.SEMANTIC_CIRCULAR_INHERITANCE, cls
									.getNameCharArray());
					return null;
				}
			}
						...
						if (result instanceof Object[]) {
							...
							for (int j = 0; j < r.length && r[j] != null; j++) {
								if (checkForAmbiguity(data, r[j], inherited)) {
									data.problem = new ProblemBinding(
											data.astName,
	...
	public static void visitVirtualBaseClasses(LookupData data,
			ICPPClassType cls) {
		ICPPBase[] bases = null;
		try {
			bases = cls.getBases();
	...
	private static boolean checkForAmbiguity(LookupData data, Object n,
			Object names) throws DOMException {
		...
		IBinding binding = (n instanceof IBinding) ? (IBinding) n
				: ((IASTName) n).resolveBinding();
		...
		while (o != null) {
			IBinding b = (o instanceof IBinding) ? (IBinding) o
					: ((IASTName) o).resolveBinding();

			if (b instanceof ICPPUsingDeclaration) {
				objs = ArrayUtil.append(Object.class, objs,
						((ICPPUsingDeclaration) b).getDelegates());
			} else {
				...
				boolean ok = false;
				// 3.4.5-4 if the id-expression in a class member access is a
				// qualified id... the result
				...
				// it is not ambiguous if they are the same thing and it is
				// static or an enumerator
				if (binding instanceof IEnumerator
						|| (binding instanceof IFunction && ((ICPPInternalFunction) binding)
								.isStatic(false))
						|| (binding instanceof IVariable && ((IVariable) binding)
								.isStatic())) {
					ok = true;
				}
				if (!ok)
	...
	static private void processDirectives(CPPSemantics.LookupData data,
			IScope scope, Object[] directives) throws DOMException {
		...
		for (int i = 0; i < size && directives[i] != null; i++) {
			...
			if (d instanceof ICPPASTUsingDirective) {
				binding = ((ICPPASTUsingDirective) d).getQualifiedName()
						.resolveBinding();
			} else if (d instanceof ICPPASTNamespaceDefinition) {
				binding = ((ICPPASTNamespaceDefinition) d).getName()
						.resolveBinding();
			}
			...
			if (!data.visited.containsKey(temp)) {
				...
				// data.usingDirectives is a map from enclosing scope to a
				// IScope[]
				// of namespaces to consider when we reach that enclosing scope
				IScope[] scopes = (IScope[]) (data.usingDirectives.isEmpty() ? null
						: data.usingDirectives.get(enclosing));
				scopes = (IScope[]) ArrayUtil
						.append(IScope.class, scopes, temp);
	...
	static private ICPPScope getClosestEnclosingScope(IScope scope1,
	 ...
	 * @throws DOMException
	 */
	static protected IASTName[] lookupInScope(CPPSemantics.LookupData data,
			ICPPScope scope, IASTNode blockItem) throws DOMException {
		...
		if (data.associated.containsKey(scope)) {
			// we are looking in scope, remove it from the associated scopes
			// list
		...
		if (parent instanceof IASTCompoundStatement) {
			...
			// 9-2 a class name is also inserted into the scope of the class
			// itself
			...
			CPPNamespace namespace = (CPPNamespace) ((ICPPASTNamespaceDefinition) parent)
					.getName().resolveBinding();
			namespaceDefs = namespace.getNamespaceDefinitions();

			nodes = ((ICPPASTNamespaceDefinition) namespaceDefs[0].getParent())
					.getDeclarations();
			namespaceIdx = 0;
		...
		int idx = -1;
		boolean checkWholeClassScope = (scope instanceof ICPPClassScope)
				&& data.checkWholeClassScope;
		IASTNode item = (nodes != null ? (nodes.length > 0 ? nodes[++idx]
				: null) : parent);
		...
		while (item != null) {
			if (item instanceof ICPPASTLinkageSpecification) {
				IASTDeclaration[] decls = ((ICPPASTLinkageSpecification) item)
						.getDeclarations();
				if (decls != null && decls.length > 0) {
					nodeStack = (IASTNode[][]) ArrayUtil.append(
							IASTNode[].class, nodeStack, nodes);
					nodeIdxStack = ArrayUtil.setInt(nodeIdxStack,
							++nodeStackPos, idx);
					nodes = ((ICPPASTLinkageSpecification) item)
							.getDeclarations();
					idx = 0;
					item = nodes[idx];
				}
			...
			if (item instanceof ICPPASTUsingDirective
					|| (item instanceof ICPPASTNamespaceDefinition && ((ICPPASTNamespaceDefinition) item)
							.getName().toCharArray().length == 0)) {
				...
				if (possible != null) {
					...
					while (temp != null) {
						...
						if ((checkWholeClassScope || declaredBefore(temp,
								data.astName))
								&& (item != blockItem || data
										.includeBlockItem(item)))

						{
							if (data.considerConstructors
									|| !(temp.getParent() instanceof IASTDeclarator && CPPVisitor
											.isConstructor(scope,
													(IASTDeclarator) temp
															.getParent()))) {
								found = (IASTName[]) ArrayUtil.append(
										IASTName.class, found, temp);
							}
						}
			...
			if (idx > -1 && ++idx < nodes.length) {
				...
				while (item == null) {
					if (namespaceIdx > -1) {
						...
						while (namespaceIdx > -1
								&& namespaceDefs.length > ++namespaceIdx) {
							nodes = ((ICPPASTNamespaceDefinition) namespaceDefs[namespaceIdx]
									.getParent()).getDeclarations();
							if (nodes.length > 0) {
								idx = 0;
								item = nodes[0];
								break;
							...
							&& nodes instanceof IASTParameterDeclaration[]) {
						...
						if (parent instanceof IASTCompoundStatement) {
							nodes = ((IASTCompoundStatement) parent)
									.getStatements();
						}
						if (nodes.length > 0) {
							idx = 0;
	...
	static private Object[] lookupInNominated(CPPSemantics.LookupData data,
			ICPPScope scope, Object[] transitives) throws DOMException {
		if (data.usingDirectives.isEmpty())
			return transitives;
		...
		for (int i = 0; i < directives.length && directives[i] != null; i++) {
			...
			if (!data.visited.containsKey(temp)) {
				...
				if (!found && temp instanceof ICPPNamespaceScope) {
					usings.array = ((ICPPNamespaceScope) temp)
							.getUsingDirectives();
				}

				// only consider the transitive using directives if we are an
				// unqualified
				// lookup, or we didn't find the name in decl
				if (usings.array != null && usings.array.length > 0
						&& (!data.qualified() || !found)) {
					transitives = ArrayUtil.addAll(Object.class, transitives,
							usings.array);
				}
			}
		}
	...
	static private Object collectResult(CPPSemantics.LookupData data,
			ICPPScope scope, IASTNode node, boolean checkAux)
			throws DOMException {
		...
		if (node instanceof ICPPASTTemplateDeclaration)
			...
			if (forStatement.getConditionDeclaration() == null) {
				if (forStatement.getInitializerStatement() instanceof IASTDeclarationStatement)
					declaration = ((IASTDeclarationStatement) forStatement
							.getInitializerStatement()).getDeclaration();
			} else {
				if (forStatement.getInitializerStatement() instanceof IASTDeclarationStatement) {
					Object o = collectResult(data, scope, forStatement
							.getInitializerStatement(), checkAux);
			...
			declaration = ((ICPPASTSwitchStatement) node)
			...
			declaration = ((ICPPASTWhileStatement) node)
			...
			if (dtor != null) { // could be null when content assist in the
								// declSpec
			...
			IASTName name = CPPTemplates
					.getTemplateParameterName((ICPPASTTemplateParameter) node);
			scope.addName(name);
		...
		if (declaration instanceof IASTSimpleDeclaration) {
			...
			ICPPASTDeclSpecifier declSpec = (ICPPASTDeclSpecifier) simpleDeclaration
					.getDeclSpecifier();
					...
					if (!data.typesOnly
						...
						if (nameMatches(data, declaratorName)) {
							if (resultName == null)
								...
								resultArray = new IASTName[] { resultName,
										declaratorName };
							else
								resultArray = (IASTName[]) ArrayUtil.append(
										IASTName.class, resultArray,
										declaratorName);
						}
					}
				}
			...
			if (declarators.length == 0
				...
				if (declarators.length == 0 && /*
												 * compSpec.getKey() ==
												 * IASTCompositeTypeSpecifier.k_union &&
												 */
					...
					for (int i = 0; i < decls.length; i++) {
						...
						if (o instanceof IASTName) {
							...
							if (resultName == null) {
								...
								resultArray = (IASTName[]) ArrayUtil.addAll(
										IASTName.class, resultArray, oa);
							} else {
								resultArray = (IASTName[]) ArrayUtil.addAll(
										IASTName.class, resultArray, oa);
							}
				...
				for (int i = 0; i < list.length; i++) {
					...
					if (!data.typesOnly && nameMatches(data, tempName)) {
						if (resultName == null)
							...
							resultArray = (IASTName[]) ArrayUtil.append(
									IASTName.class, resultArray, tempName);
					}
				}
			}
			if (specName != null) {
				...
				if (nameMatches(data, specName)) {
					if (resultName == null)
						...
						resultArray = (IASTName[]) ArrayUtil.append(
								IASTName.class, resultArray, specName);
				}
			}
			...
			IASTName namespaceName = ((ICPPASTNamespaceDefinition) declaration)
					.getName();
			...
			if (!((ICPPASTDeclSpecifier) functionDef.getDeclSpecifier())
					.isFriend()) {
	...
	private static final boolean nameMatches(LookupData data, IASTName potential) {
		if (potential instanceof ICPPASTQualifiedName) {
			// A qualified name implies the name actually belongs to a different
			// scope, and should
		...
		char[] n = data.name();
		return ((data.prefixLookup && CharArrayUtils.equals(c, 0, n.length, n)) || (!data.prefixLookup && CharArrayUtils
				.equals(c, n)));
	}
	...
	static protected IBinding resolveAmbiguities(IASTName name,
			Object[] bindings) {
		...
		if (bindings == null || bindings.length == 0)
			...
			if (bindings[0] instanceof IBinding)
				return (IBinding) bindings[0];
					...
					&& ((IASTName) bindings[0]).getBinding() != null)
		...
		for (int i = 0; i < bindings.length; i++) {
			if (bindings[i] instanceof IASTName)
				result = (IBinding[]) ArrayUtil.append(IBinding.class, result,
						((IASTName) bindings[i]).resolveBinding());
	...
	static public boolean declaredBefore(Object obj, IASTNode node) {
		if (node == null)
			return true;
		if (node.getPropertyInParent() == STRING_LOOKUP_PROPERTY)
			return true;
		...
		if (nd != null) {
			...
			if (prop == IASTDeclarator.DECLARATOR_NAME
					|| nd instanceof IASTDeclarator) {
				IASTDeclarator dtor = (IASTDeclarator) ((nd instanceof IASTDeclarator) ? nd
						: nd.getParent());
				while (dtor.getParent() instanceof IASTDeclarator)
					dtor = (IASTDeclarator) dtor.getParent();
				...
				if (init != null)
					...
					pointOfDecl = ((ASTNode) dtor).getOffset()
							+ ((ASTNode) dtor).getLength();
			}
	...
	static private IBinding resolveAmbiguities(CPPSemantics.LookupData data,
			IASTName name) throws DOMException {
		if (!data.hasResults() || data.prefixLookup)
			return null;
		...
		for (int i = 0; i < items.length && items[i] != null; i++) {
			...
			if (temp instanceof ICPPUsingDeclaration) {
				...
				if (type == null) {
					type = temp;
				} else if ((temp instanceof ICPPDelegate && ((ICPPDelegate) temp)
						.getBinding() == type)
						|| (type instanceof ICPPDelegate && ((ICPPDelegate) type)
								.getBinding() == temp)
						|| (type instanceof ICPPDelegate
								&& temp instanceof ICPPDelegate && ((ICPPDelegate) type)
								.getBinding() == ((ICPPDelegate) temp)
								.getBinding())) {
					// ok, delegates are synonyms
				} else if (type instanceof ICPPClassTemplate
						&& temp instanceof ICPPSpecialization
						&& ((ICPPSpecialization) temp).getSpecializedBinding() == type) {
					// ok, stay with the template, the specialization, if
					// applicable, will come out during instantiation
				} else if (type != temp
						&& !((IType) type).isSameType((IType) temp)) {
					return new ProblemBinding(data.astName,
							IProblemBinding.SEMANTIC_AMBIGUOUS_LOOKUP, data
									.name());
				}
				...
				if (obj == null)
					obj = temp;
						...
						.getBinding() == obj)
						|| (obj instanceof ICPPDelegate && ((ICPPDelegate) obj)
								.getBinding() == temp)) {
					// ok, delegates are synonyms
				} else if (obj != temp) {
					return new ProblemBinding(data.astName,
		...
		if (data.forUsingDeclaration()) {
			...
			if (obj != null) {
				...
				bindings = (IBinding[]) ArrayUtil.append(IBinding.class,
						bindings, obj);
				bindings = (IBinding[]) ArrayUtil.append(IBinding.class,
						bindings, type);
			} else {
				...
				bindings = (IBinding[]) ArrayUtil.append(IBinding.class,
						bindings, type);
				bindings = (IBinding[]) ArrayUtil.addAll(IBinding.class,
						bindings, fns.keyArray());
			}
			bindings = (IBinding[]) ArrayUtil.trim(IBinding.class, bindings);
			ICPPUsingDeclaration composite = new CPPUsingDeclaration(
					data.astName, bindings);
			return composite;
		}
		...
		if (numTemplateFns > 0) {
			if (data.functionParameters != null && !data.forDefinition()) {
				IFunction[] fs = CPPTemplates.selectTemplateFunctions(
						templateFns, data.functionParameters, data.astName);
		...
		if (numFns > 0) {
			...
			return resolveFunction(data, (IBinding[]) fns
					.keyArray(IBinding.class));
		}

		return obj;
	}

	static private boolean functionHasParameters(IFunction function,
			IASTParameterDeclaration[] params) throws DOMException {
	...
	static private void reduceToViable(LookupData data, IBinding[] functions)
			throws DOMException {
		...
		for (int i = 0; i < size && functions[i] != null; i++) {
			...
			// if there are m arguments in the list, all candidate functions
			// having m parameters
			// are viable
			...
			if (def) {
				// if this is for a definition, we had to match the number of
				// parameters.
				functions[i] = null;
				continue;
			}
	...
	static private boolean isMatchingFunctionDeclaration(IFunction candidate,
			LookupData data) {
		IASTName name = data.astName;
		ICPPASTTemplateDeclaration decl = CPPTemplates
				.getTemplateDeclaration(name);
		if (decl != null && !(candidate instanceof ICPPTemplateDefinition))
			return false;

		if (candidate instanceof ICPPTemplateDefinition
				&& decl instanceof ICPPASTTemplateSpecialization) {
			ICPPFunctionTemplate fn = CPPTemplates.resolveTemplateFunctions(
					new Object[] { candidate }, data.astName);
		...
		try {
			...
			if (dtor.isConst() != ftype.isConst()
					|| dtor.isVolatile() != ftype.isVolatile())
				return false;
			return functionHasParameters(candidate,
					(IASTParameterDeclaration[]) data.functionParameters);
		} catch (DOMException e) {
		}
		return false;
	}
	...
	static private IType[] getTargetParameterTypes(IFunction fn)
			throws DOMException {
		IParameter[] params = fn.getParameters();

		boolean useImplicit = (fn instanceof ICPPMethod && !(fn instanceof ICPPConstructor));
		IType[] result = new IType[useImplicit ? params.length + 1
				: params.length];

		if (useImplicit) {
			ICPPFunctionType ftype = (ICPPFunctionType) ((ICPPFunction) fn)
					.getType();
			if (ftype != null) {
				...
				if (scope instanceof ICPPClassScope) {
					...
					cls = new CPPClassType.CPPClassTypeProblem(scope
							.getPhysicalNode(),
							IProblemBinding.SEMANTIC_BAD_SCOPE, fn
									.getNameCharArray());
				}
				if (cls instanceof ICPPClassTemplate) {
					IBinding within = CPPTemplates
							.instantiateWithinClassTemplate((ICPPClassTemplate) cls);
					if (within instanceof ICPPClassType)
						cls = (ICPPClassType) within;
				}
				IType implicitType = cls;
				if (ftype.isConst() || ftype.isVolatile()) {
					implicitType = new CPPQualifierType(implicitType, ftype
							.isConst(), ftype.isVolatile());
				}
		...
		for (int i = 0; i < params.length; i++)
			result = (IType[]) ArrayUtil.append(IType.class, result, params[i]
					.getType());

		return result;
	}

	static private IBinding resolveFunction(CPPSemantics.LookupData data,
			IBinding[] fns) throws DOMException {
		...
		// reduce our set of candidate functions to only those who have the
		...
		IFunction currFn = null; // the function currently under
									// consideration
		...
		Cost temp = null; // the cost of using a user defined conversion to
							// convert source to target

		boolean hasWorse = false; // currFn has a worse parameter fit than
									// bestFn
		boolean hasBetter = false; // currFn has a better parameter fit than
									// bestFn
		boolean ambiguous = false; // ambiguity, 2 functions are equally good
		boolean currHasAmbiguousParam = false; // currFn has an ambiguous
												// parameter conversion (ok if
												// not bestFn)
		boolean bestHasAmbiguousParam = false; // bestFn has an ambiguous
												// parameter conversion (not ok,
												// ambiguous)

		IType[] sourceParameters = getSourceParameterTypes(data.functionParameters); // the
																						// parameters
																						// the
																						// function
																						// is
		...
		outer: for (int fnIdx = 0; fnIdx < numFns; fnIdx++) {
			...
			if (currFn == null
					|| bestFn == currFn
					|| (bestFn instanceof ICPPDelegate && ((ICPPDelegate) bestFn)
							.getBinding() == currFn)
					|| (currFn instanceof ICPPDelegate && ((ICPPDelegate) currFn)
							.getBinding() == bestFn)) {
				continue;
			}
			...
			numSourceParams = (useImplicitObj) ? sourceParameters.length + 1
					: sourceParameters.length;
			int numTargetParams = 0;

			if (currFnCost == null
					|| currFnCost.length != ((numSourceParams == 0) ? 1
							: numSourceParams)) {
				currFnCost = new Cost[(numSourceParams == 0) ? 1
						: numSourceParams];
			}
			...
			for (int j = 0; j < numSourceParams || j == 0; j++) {
				if (useImplicitObj) {
					source = (j == 0) ? impliedObjectType
							: sourceParameters[j - 1];
				...
				if (useImplicitObj
						&& j == 0
						&& (currFn instanceof ICPPInternalFunction ? ((ICPPInternalFunction) currFn)
								.isStatic(false)
								: currFn.isStatic())) {
					// 13.3.1-4 for static member functions, the implicit object
					// parameter is considered to match any object
					...
					// 12.3-4 At most one user-defined conversion is implicitly
					// applied to
					...
					if ((cost.rank == Cost.NO_MATCH_RANK || cost.rank == Cost.FUZZY_TEMPLATE_PARAMETERS)
						...
						temp = checkUserDefinedConversionSequence(source,
								target);
						if (temp != null) {
							cost = temp;
						}
			...
			hasWorse = false;
			hasBetter = false;
			// In order for this function to be better than the previous best,
			// it must
			// have at least one parameter match that is better that the
			// corresponding
			// match for the other function, and none that are worse.
			int len = (bestFnCost == null || currFnCost.length < bestFnCost.length) ? currFnCost.length
					: bestFnCost.length;
			for (int j = 1; j <= len; j++) {
				...
				// an ambiguity in the user defined conversion sequence is only
				// a problem
				// if this function turns out to be the best.
				currHasAmbiguousParam = (currCost.userDefined == 1);
				if (bestFnCost != null) {
					comparison = currCost.compare(bestFnCost[bestFnCost.length
							- j]);
			...
			// If function has a parameter match that is better than the current
			// best,
			// and another that is worse (or everything was just as good,
			// neither better nor worse).
			// then this is an ambiguity (unless we find something better than
			// both later)
			ambiguous |= (hasWorse && hasBetter) || (!hasWorse && !hasBetter);

			if (!hasWorse) {
				// if they are both template functions, we can order them that
				// way
				boolean bestIsTemplate = (bestFn instanceof ICPPSpecialization && ((ICPPSpecialization) bestFn)
						.getSpecializedBinding() instanceof ICPPFunctionTemplate);
				boolean currIsTemplate = (currFn instanceof ICPPSpecialization && ((ICPPSpecialization) currFn)
						.getSpecializedBinding() instanceof ICPPFunctionTemplate);
				if (bestIsTemplate && currIsTemplate) {
					ICPPFunctionTemplate t1 = (ICPPFunctionTemplate) ((ICPPSpecialization) bestFn)
							.getSpecializedBinding();
					ICPPFunctionTemplate t2 = (ICPPFunctionTemplate) ((ICPPSpecialization) currFn)
							.getSpecializedBinding();
				...
				// we prefer normal functions over template functions, unless we
	...
	/**
	 * 13.4-1 A use of an overloaded function without arguments is resolved in
	 * certain contexts to a function
	 * 
	 * @param data
	 * @param fns
	 * @return
	 */
	private static IBinding resolveTargetedFunction(LookupData data,
			IBinding[] fns) {
		if (fns.length == 1)
			return fns[0];
		...
		return (result != null) ? result : new ProblemBinding(data.astName,
				IProblemBinding.SEMANTIC_AMBIGUOUS_LOOKUP, data.name());
	}
	...
	private static Object getTargetType(LookupData data) {
		...
		while (node != null) {
			...
			if (prop == IASTDeclarator.INITIALIZER) {
				...
				IASTInitializerExpression initExp = (IASTInitializerExpression) node
						.getParent();
			...
			// target is the left side of an assignment
				...
				IASTBinaryExpression binaryExp = (IASTBinaryExpression) node
						.getParent();
			...
			// target is a parameter of a function
					...
					|| (prop == IASTExpressionList.NESTED_EXPRESSION && node
							.getParent().getPropertyInParent() == IASTFunctionCallExpression.PARAMETERS)) {
				// if this function call refers to an overloaded function, there
				// is more than one possiblity
				// for the target type
				...
				if (prop == IASTFunctionCallExpression.PARAMETERS) {
					...
					IASTExpressionList list = (IASTExpressionList) node
							.getParent();
				...
				for (int i = 0; i < types.length && types[i] != null; i++) {
					...
					if (pts.length > idx)
						result = (IType[]) ArrayUtil.append(IType.class,
								result, pts[idx]);
				}
				return result;
			}
			// target is an explicit type conversion
				...
				return CPPVisitor.createType(cast.getTypeId()
			...
			// target is a template non-type parameter (14.3.2-5)
				...
				ICPPTemplateDefinition template = (ICPPTemplateDefinition) id
						.getTemplateName().resolveBinding();
				if (template != null) {
					try {
						ICPPTemplateParameter[] ps = template
								.getTemplateParameters();
						if (i < args.length
								&& i < ps.length
								&& ps[i] instanceof ICPPTemplateNonTypeParameter) {
							return ((ICPPTemplateNonTypeParameter) ps[i])
									.getType();
						}
					} catch (DOMException e) {
						return null;
			...
			// target is the return value of a function, operator or conversion
				...
				IASTDeclarator dtor = ((IASTFunctionDefinition) node)
						.getDeclarator();
				while (dtor.getNestedDeclarator() != null)
					dtor = dtor.getNestedDeclarator();
				IBinding binding = dtor.getName().resolveBinding();
				...
				IASTUnaryExpression parent = (IASTUnaryExpression) node
						.getParent();
	...
	static private IFunctionType[] getPossibleFunctions(
				...
				for (int i = 0; i < items.length; i++) {
					...
					try {
						if (temp instanceof IFunction) {
							result = (IFunctionType[]) ArrayUtil.append(
									IFunctionType.class, result,
									((IFunction) temp).getType());
						} else if (temp instanceof IVariable) {
							IType type = getUltimateType(((IVariable) temp)
									.getType(), false);
							if (type instanceof IFunctionType)
								result = (IFunctionType[]) ArrayUtil.append(
										IFunctionType.class, result, type);
						}
					} catch (DOMException e) {
					}
				}
	...
	static protected Cost checkStandardConversionSequence(IType source,
	...
	static private Cost checkUserDefinedConversionSequence(IType source,
			IType target) throws DOMException {
		Cost cost = null;
		...
		// constructors
		if (t instanceof ICPPClassType) {
			ICPPConstructor[] constructors = ((ICPPClassType) t)
					.getConstructors();
			if (constructors.length > 0) {
				if (constructors.length == 1
						&& constructors[0] instanceof IProblemBinding)
					constructor = null;
				else {
		...
		if (s instanceof ICPPInternalClassType) {
			ICPPMethod[] ops = ((ICPPInternalClassType) s)
					.getConversionOperators();
			if (ops.length > 0 && !(ops[0] instanceof IProblemBinding)) {
				Cost[] costs = null;
				for (int i = 0; i < ops.length; i++) {
					cost = checkStandardConversionSequence(ops[i].getType()
							.getReturnType(), target);
					if (cost.rank != Cost.NO_MATCH_RANK)
						costs = (Cost[]) ArrayUtil.append(Cost.class, costs,
								cost);
				}
				if (costs != null) {
					Cost best = costs[0];
		...
		if (constructorCost != null
				&& constructorCost.rank != Cost.NO_MATCH_RANK
				&& conversionCost != null
				&& conversionCost.rank != Cost.NO_MATCH_RANK) {
	...
	static protected IType getUltimateType(IType type,
		...
		try {
			while (true) {
				...
				else if (stopAtPointerToMember
						&& type instanceof ICPPPointerToMemberType)
					return type;
	...
	static private boolean isCompleteType(IType type) {
		...
		if (type instanceof ICPPClassType
				&& type instanceof ICPPInternalBinding)
	...
	static private Cost lvalue_to_rvalue(IType source, IType target)
		...
		if (target instanceof IPointerType
				&& ((IPointerType) target).getType() instanceof IFunctionType
		...
		// 4.1 if T is a non-class type, the type of the rvalue is the
		// cv-unqualified version of T
		if (source instanceof IQualifierType) {
			...
			if (!(t instanceof ICPPClassType)) {
				...
				&& (((IPointerType) source).isConst() || ((IPointerType) source)
						.isVolatile())) {
	...
	static private void qualificationConversion(Cost cost) throws DOMException {
		...
		while (true) {
			...
			if (op1 == null && op2 == null)
				...
				canConvert = false;
				break;
			} else if (op1 instanceof ICPPPointerToMemberType
					^ op2 instanceof ICPPPointerToMemberType) {
				canConvert = false;
				break;
			}

			// if const is in cv1,j then const is in cv2,j. Similary for
			// volatile
			...
			// if cv1,j and cv2,j are different then const is in every cv2,k for
			// 0<k<j
			if (!constInEveryCV2k
					&& (op1.isConst() != op2.isConst() || op1.isVolatile() != op2
							.isVolatile())) {
				canConvert = false;
				requiredConversion = Cost.NO_MATCH_RANK;
				break;
		...
		if (s instanceof IQualifierType ^ t instanceof IQualifierType) {
			if (t instanceof IQualifierType) {
				...
				if (t instanceof IBasicType
						&& ((IBasicType) t).getType() == IBasicType.t_char
					...
					if (qt instanceof IBasicType) {
						...
						canConvert = (val != null
								&& val instanceof IASTLiteralExpression && ((IASTLiteralExpression) val)
			...
			if (qs.isConst() && !qt.isConst() || qs.isVolatile()
					&& !qt.isVolatile())
				requiredConversion = Cost.NO_MATCH_RANK;
			else if (qs.isConst() == qt.isConst()
					&& qs.isVolatile() == qt.isVolatile())
			...
			for (IType type = s; canConvert == true && i == 1; type = t, i++) {
				while (type instanceof ITypeContainer) {
					if (type instanceof IQualifierType)
						...
						canConvert = !((IPointerType) type).isConst()
								&& !((IPointerType) type).isVolatile();
					}
					if (!canConvert) {
						requiredConversion = Cost.NO_MATCH_RANK;
						break;
	 ...
	 * type, otherwise they can be converted to unsigned int. 4.5-2 wchar_t or
	 * an enumeration can be converted to the first of the following that can
	 * hold it: int, unsigned int, long unsigned long. 4.5-4 bool can be
	 * promoted to int 4.6 float can be promoted to double
	 * @throws DOMException
	 */
	static private void promotion(Cost cost) throws DOMException {
		...
		if (src instanceof IBasicType && trg instanceof IBasicType) {
			...
			if ((tType == IBasicType.t_int && (sType == IBasicType.t_int
					|| // short, long , unsigned etc
					sType == IBasicType.t_char || sType == ICPPBasicType.t_bool
					|| sType == ICPPBasicType.t_wchar_t || sType == IBasicType.t_unspecified))
					|| // treat unspecified as int
					(tType == IBasicType.t_double && sType == IBasicType.t_float)) {
				cost.promotion = 1;
			}
		} else if (src instanceof IEnumeration
				&& trg instanceof IBasicType
				&& (((IBasicType) trg).getType() == IBasicType.t_int || ((IBasicType) trg)
						.getType() == IBasicType.t_unspecified)) {
			cost.promotion = 1;
		}

		cost.rank = (cost.promotion > 0) ? Cost.PROMOTION_RANK
				: Cost.NO_MATCH_RANK;
	}
	...
	static private void conversion(Cost cost) throws DOMException {
		...
		if (src instanceof IBasicType && trg instanceof IPointerType) {
			// 4.10-1 an integral constant expression of integer type that
			// evaluates to 0 can be converted to a pointer type
			...
			while (tPrev instanceof ITypeContainer) {
				...
				while (next instanceof IQualifierType
						|| next instanceof ITypedef) {
					next = ((ITypeContainer) next).getType();
				}
				if (next == t)
					break;
			...
			// 4.10-2 an rvalue of type "pointer to cv T", where T is an object
			// type can be
			...
			// 4.10-3 An rvalue of type "pointer to cv D", where D is a class
			// type can be converted
			// to an rvalue of type "pointer to cv B", where B is a base class
			// of D.
			else if (s instanceof ICPPClassType
					&& tPrev instanceof IPointerType
					&& t instanceof ICPPClassType) {
				temp = hasBaseClass((ICPPClassType) s, (ICPPClassType) t, false);
				cost.rank = (temp > -1) ? Cost.CONVERSION_RANK
						: Cost.NO_MATCH_RANK;
		...
		if (t instanceof IBasicType && s instanceof IBasicType
				|| s instanceof IEnumeration) {
			// 4.7 An rvalue of an integer type can be converted to an rvalue of
			// another integer type.
			// An rvalue of an enumeration type can be converted to an rvalue of
			// an integer type.
			cost.rank = Cost.CONVERSION_RANK;
			cost.conversion = 1;
		} else if (t instanceof IBasicType
				&& ((IBasicType) t).getType() == ICPPBasicType.t_bool
				&& s instanceof IPointerType) {
			// 4.12 pointer or pointer to member type can be converted to an
			// rvalue of type bool
			cost.rank = Cost.CONVERSION_RANK;
			cost.conversion = 1;
		} else if (s instanceof ICPPPointerToMemberType
				&& t instanceof ICPPPointerToMemberType) {
			// 4.11-2 An rvalue of type "pointer to member of B of type cv T",
			// where B is a class type,
			// can be converted to an rvalue of type "pointer to member of D of
			// type cv T" where D is a
			// derived class of B
			...
			if (spm.getType().isSameType(tpm.getType())) {
				temp = hasBaseClass(tpm.getMemberOfClass(), spm
						.getMemberOfClass(), false);
				cost.rank = (temp > -1) ? Cost.CONVERSION_RANK
						: Cost.NO_MATCH_RANK;
				cost.conversion = (temp > -1) ? temp : 0;
				cost.detail = 1;
	...
	static private void derivedToBaseConversion(Cost cost) throws DOMException {
		...
		if (cost.targetHadReference && s instanceof ICPPClassType
				&& t instanceof ICPPClassType) {
	...
	static private int hasBaseClass(IBinding symbol, IBinding base,
			boolean needVisibility) throws DOMException {
		if (symbol == base) {
			return 0;
		}
		...
		for (int i = 0; i < bases.length; i++) {
			...
			if (parent instanceof IType
					&& (((IType) parent).isSameType(clsBase) || (clsBase instanceof ICPPSpecialization && // allow
																											// some
																											// flexibility
																											// with
																											// templates
					((IType) ((ICPPSpecialization) clsBase)
							.getSpecializedBinding())
							.isSameType((IType) parent)))) {
	...
	public static ICPPFunction findOperator(IASTExpression exp,
			ICPPClassType cls) {
		IScope scope = null;
		try {
		...
		if (exp instanceof IASTArraySubscriptExpression) {
			...
			data.forceQualified = true;
			data.functionParameters = new IASTExpression[] { ((IASTArraySubscriptExpression) exp)
	...
	public static IBinding[] findBindings(IScope scope, String name,
			boolean qualified) throws DOMException {
		return findBindings(scope, name.toCharArray(), qualified);
	}

	public static IBinding[] findBindings(IScope scope, char[] name,
			boolean qualified) throws DOMException {
		CPPASTName astName = new CPPASTName();
		...
		if (!map.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (obj instanceof IBinding)
					result = (IBinding[]) ArrayUtil.append(IBinding.class,
							result, obj);
					...
					for (int j = 0; j < objs.length && objs[j] != null; j++) {
						...
						if (item instanceof IBinding)
							result = (IBinding[]) ArrayUtil.append(
									IBinding.class, result, item);
						else if (item instanceof IASTName) {
							IBinding binding = ((IASTName) item)
									.resolveBinding();
							if (binding != null
									&& !(binding instanceof IProblemBinding))
								result = (IBinding[]) ArrayUtil.append(
										IBinding.class, result, binding);
						}
					}
				}
			}
	...
	public static boolean isSameFunction(IFunction function,
			IASTDeclarator declarator) {
		IASTName name = declarator.getName();
		ICPPASTTemplateDeclaration templateDecl = CPPTemplates
				.getTemplateDeclaration(name);
		...
		if (fnIsTemplate && dtorIsTemplate) {
			return CPPTemplates.isSameTemplate(
					(ICPPTemplateDefinition) function, name);
