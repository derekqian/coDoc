...
import edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTNode.LookupKind;
...
public class TypeFilter {
	...
	public boolean willAccept(ITypeInfo.eType type) {
		return (acceptedTypes.containsKey(ITypeInfo.t_any) || acceptedTypes
				.containsKey(type));
	}
	...
	public boolean shouldAccept(ISymbol symbol, ITypeInfo typeInfo) {
		...
		boolean symbolIsMember = container.isType(ITypeInfo.t_class,
				ITypeInfo.t_union);
		boolean symbolIsLocal = container.isType(ITypeInfo.t_constructor,
		...
		if (typeInfo.isType(ITypeInfo.t_function)) {
			...
			return false;
		} else if (typeInfo.isType(ITypeInfo.t_type)
				&& typeInfo.checkBit(ITypeInfo.isTypedef)) {
			...
			return false;
				...
				|| typeInfo.isType(ITypeInfo.t__Bool, ITypeInfo.t_void)) {
			if ((acceptedKinds.containsKey(LookupKind.VARIABLES)
					&& !symbolIsMember && !symbolIsLocal)
					|| (acceptedKinds.containsKey(LookupKind.LOCAL_VARIABLES)
							&& !symbolIsMember && symbolIsLocal)
	...
	private void populatedAcceptedTypes(LookupKind kind) {
		if (kind == LookupKind.ALL) {
			acceptedTypes.put(ITypeInfo.t_any);
		} else if (kind == LookupKind.STRUCTURES) {
			acceptedTypes.put(ITypeInfo.t_class);
			acceptedTypes.put(ITypeInfo.t_struct);
			acceptedTypes.put(ITypeInfo.t_union);
		} else if (kind == LookupKind.STRUCTS) {
			acceptedTypes.put(ITypeInfo.t_struct);
		} else if (kind == LookupKind.UNIONS) {
			acceptedTypes.put(ITypeInfo.t_union);
		} else if (kind == LookupKind.CLASSES) {
			acceptedTypes.put(ITypeInfo.t_class);
		} else if (kind == LookupKind.CONSTRUCTORS) {
			acceptedTypes.put(ITypeInfo.t_constructor);
		} else if (kind == LookupKind.NAMESPACES) {
			acceptedTypes.put(ITypeInfo.t_namespace);
		} else if (kind == LookupKind.ENUMERATIONS) {
			acceptedTypes.put(ITypeInfo.t_enumeration);
		} else if (kind == LookupKind.ENUMERATORS) {
			acceptedTypes.put(ITypeInfo.t_enumerator);
		}
		// else if ( kind == LookupKind.TYPEDEFS ) { acceptedTypes.put(
		// TypeInfo.t_type ); }
