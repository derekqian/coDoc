...
abstract public class CPPScope implements ICPPScope {
	public static class CPPScopeProblem extends ProblemBinding implements
			ICPPScope {
		...
		if (obj != null) {
			if (obj instanceof ObjectSet) {
				...
				for (int i = 0; i < os.size(); i++) {
					...
					if (o instanceof IASTName) {
						...
						if (n instanceof ICPPASTQualifiedName) {
							IASTName[] ns = ((ICPPASTQualifiedName) n)
									.getNames();
							n = ns[ns.length - 1];
						}
				...
				if (binding instanceof ICPPUsingDeclaration) {
					return CPPSemantics.resolveAmbiguities(name,
			...
			if (pdom != null) {
				...
				if (physicalNode instanceof ICPPASTNamespaceDefinition) {
					...
					if (nsbinding instanceof ICPPScope)
						return ((ICPPScope) nsbinding).getBinding(name,
								forceResolve);
				}
			}
		}
		...
		if (obj instanceof ObjectSet) {
			...
			for (int i = set.size() - 1; i > 0; i--) {
				...
				if ((o instanceof IBinding && o == binding)
						|| (o instanceof IASTName && ((IASTName) o)
								.getBinding() == binding)) {
					set.remove(o);
				}
			}
