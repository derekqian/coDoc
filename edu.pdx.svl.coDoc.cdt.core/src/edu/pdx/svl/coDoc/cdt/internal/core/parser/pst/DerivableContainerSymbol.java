 ...
 */
public class DerivableContainerSymbol extends ContainerSymbol implements
	...
	public Object clone() {
		DerivableContainerSymbol copy = (DerivableContainerSymbol) super
				.clone();

		copy._parentScopes = (_parentScopes != Collections.EMPTY_LIST) ? (List) ((ArrayList) _parentScopes)
				.clone()
				: _parentScopes;
		copy._constructors = (_constructors != Collections.EMPTY_LIST) ? (List) ((ArrayList) _constructors)
				.clone()
				: _constructors;
		copy._friends = (_friends != Collections.EMPTY_LIST) ? (List) ((ArrayList) _friends)
				.clone()
				: _friends;
	...
	public ISymbol instantiate(ITemplateSymbol template, ObjectMap argMap)
		...
		DerivableContainerSymbol newSymbol = (DerivableContainerSymbol) super
		...
		for (int i = 0; i < size; i++) {
			...
			if (parent instanceof IDeferredTemplateInstance) {
				template.registerDeferredInstatiation(newSymbol, parent,
						ITemplateSymbol.DeferredKind.PARENT, argMap);
			} else if (parent.isType(ITypeInfo.t_templateParameter)
					&& argMap.containsKey(parent)) {
				ITypeInfo info = (ITypeInfo) argMap.get(parent);
				parent = info.getTypeSymbol();
			}
			newSymbol.addParent(parent, wrapper.isVirtual(), wrapper
					.getAccess(), wrapper.getOffset(), wrapper.getReferences());
		}
	...
	public void instantiateDeferredParent(ISymbol parent,
	 ...
	 * @param map
	 */
	public void discardDeferredParent(IDeferredTemplateInstance parent,
	...
	public void addConstructor(IParameterizedSymbol constructor)
		...
		if (constructors.size() == 0
				|| ParserSymbolTable.isValidOverload(constructors, constructor)) {
			addToConstructors(constructor);
		} else {
			throw new ParserSymbolTableException(
		...
		constructor.setIsTemplateMember(isTemplateMember()
	...
	public void addCopyConstructor() throws ParserSymbolTableException {
		...
		if (getContainingSymbol() instanceof ITemplateSymbol) {
			paramType = TemplateEngine.instantiateWithinTemplateScope(this,
					(ITemplateSymbol) getContainingSymbol());
		}

		ITypeInfo param = getSymbolTable().getTypeInfoProvider().getTypeInfo(
				ITypeInfo.t_type);
		...
		if (constructor == null) {
			constructor = getSymbolTable().newParameterizedSymbol(getName(),
					ITypeInfo.t_constructor);
			constructor.addParameter(this, ITypeInfo.isConst,
					new ITypeInfo.PtrOp(ITypeInfo.PtrOp.t_reference, false,
							false), false);
	 ...
	 */
	public IParameterizedSymbol lookupConstructor(final List parameters)
			throws ParserSymbolTableException {
	 ...
	 * @param obj
	 * @throws ParserSymbolTableException
	 *             9.3.2-1 In the body of a nonstatic member function... the
	 *             type of this of a class X is X*. If the member function is
	 *             declared const, the type of this is const X*, if the member
	 *             function is declared volatile, the type of this is volatile
	 *             X*....
	 */
	private boolean addThis(IParameterizedSymbol obj) {
		...
		if ((!type.isType(ITypeInfo.t_function) && !type
		...
		if (containing.isType(ITypeInfo.t_class, ITypeInfo.t_union)) {
			// check to see if there is already a this object, since using
			// declarations
			...
			try {
				CharArrayObjectMap map = ParserSymbolTable.lookupInContained(
						data, obj);
			...
			// if we didn't find "this" then foundItems will still be null, no
			// need to actually
			// check its contents
			if (!foundThis) {
				ISymbol thisObj = getSymbolTable().newSymbol(
						ParserSymbolTable.THIS, ITypeInfo.t_type);
				...
				thisObj.getTypeInfo().setBit(
						obj.getTypeInfo().checkBit(ITypeInfo.isConst),
						ITypeInfo.isConst);
				thisObj.getTypeInfo().setBit(
						obj.getTypeInfo().checkBit(ITypeInfo.isVolatile),
				...
				try {
					...
					// shouldn't happen because we checked that "this" didn't
					// exist already
					return false;
				}
	...
	public void addFriend(ISymbol friend) {
		...
		if (containing == null) {
			...
			while (enclosing != null
					&& !enclosing.isType(ITypeInfo.t_namespace)) {
	 ...
	 * 
	 * @param name
	 * @return Declaration 7.3.1.2-3 When looking for a prior declaration of a
	 *         class or a function declared as a friend, scopes outside the
	 *         innermost enclosing namespace scope are not considered. 11.4-9 If
	 *         a friend declaration appears in a local class and the name
	 *         specified is an unqualified name, a prior declaration is looked
	...
	public ISymbol lookupForFriendship(char[] name)
		...
		IContainerSymbol enclosing = getContainingSymbol();
		if (enclosing != null
				&& enclosing.isType(ITypeInfo.t_namespace, ITypeInfo.t_union)) {
			while (enclosing != null
					&& (enclosing.getType() != ITypeInfo.t_namespace)) {
				enclosing = enclosing.getContainingSymbol();
			}
		}
	...
	// public AddParentCommand( IDerivableContainerSymbol container,
	...
	// AddConstructorCommand( IParameterizedSymbol newConstr,
	// IDerivableContainerSymbol context ){
	...
	// }
	//			
	// ContentsIterator contents = (ContentsIterator)
	...
	public class ParentWrapper implements
			IDerivableContainerSymbol.IParentSymbol {
	...
	private List _parentScopes = Collections.EMPTY_LIST; // inherited scopes
															// (is base classes)
