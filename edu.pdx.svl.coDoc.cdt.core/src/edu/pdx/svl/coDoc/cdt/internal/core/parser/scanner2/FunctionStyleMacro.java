...
public class FunctionStyleMacro extends ObjectStyleMacro {
	...
	public FunctionStyleMacro(char[] name, char[] expansion, char[][] arglist) {
		...
		if (arglist != null && arglist[0] != null && arglist.length > 0) {
			...
			// if the last element in the list is null then binary search for
			...
			if (arglist[arglist.length - 1] == null) {
				...
				for (int j = arglist.length / 2; last == -1;) {
					if (arglist[j] == null) {
						...
						smallest = j;
						j = smallest + (largest - smallest) / 2;
						if ((j + 1 == arglist.length && arglist[j] != null)
			...
			if (arglist[last] != null
					&& CharArrayUtils.equals(arglist[last], ELLIPSIS_CHARARRAY)) {
				this.hasVarArgs = true;
				varArgsPosition = last;
				// change the arg to __VA_ARGS__ so this will be replaced
				// properly later on...
				arglist[last] = VA_ARGS_CHARARRAY;
			} else if (arglist[last] != null
					&& CharArrayUtils.equals(arglist[last],
							arglist[last].length - ELLIPSIS_CHARARRAY.length,
							ELLIPSIS_CHARARRAY.length, ELLIPSIS_CHARARRAY)) { // if
				// the
				// last
				// 3
				// are
				...
				varArgsPosition = last;
				// change the arg to "argname" instead of "argname..." so
				// argname will be replaced properly later on...
				char[] swap = new char[arglist[last].length
						- ELLIPSIS_CHARARRAY.length];
	...
	public class Expansion {
		...
		public final CharArrayObjectMap definitions = new CharArrayObjectMap(
				FunctionStyleMacro.this.arglist.length);
