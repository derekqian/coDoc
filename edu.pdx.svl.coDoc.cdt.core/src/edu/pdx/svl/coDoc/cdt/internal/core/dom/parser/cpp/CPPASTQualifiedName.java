...
public class CPPASTQualifiedName extends CPPASTNode implements
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTQualifiedName#addName(edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTName)
	 */
	...
	private void removeNullNames() {
		names = (IASTName[]) ArrayUtil.removeNullsAfter(IASTName.class, names,
				namesPos);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTQualifiedName#getNames()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTQualifiedName#isFullyQualified()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTQualifiedName#setFullyQualified(boolean)
	 */
	...
	public boolean isDeclaration() {
		...
		if (parent instanceof IASTNameOwner) {
			...
			if (role == IASTNameOwner.r_reference)
				return false;
			return true;
		}
		return false;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTQualifiedName#isConversionOrOperator()
	 */
	public boolean isConversionOrOperator() {
		IASTName[] nonNullNames = getNames(); // ensure no null names

		int len = nonNullNames.length;
		if (nonNullNames[len - 1] instanceof ICPPASTConversionName
				|| nonNullNames[len - 1] instanceof ICPPASTOperatorName)
			return true;

		// check templateId's name
		if (nonNullNames[len - 1] instanceof ICPPASTTemplateId) {
			IASTName tempName = ((ICPPASTTemplateId) nonNullNames[len - 1])
					.getTemplateName();
			if (tempName instanceof ICPPASTConversionName
					|| tempName instanceof ICPPASTOperatorName)
				return true;
		}
