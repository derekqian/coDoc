...
import edu.pdx.svl.coDoc.cdt.core.model.BufferChangedEvent;
import edu.pdx.svl.coDoc.cdt.core.model.CModelException;
import edu.pdx.svl.coDoc.cdt.core.model.IBuffer;
import edu.pdx.svl.coDoc.cdt.core.model.IBufferChangedListener;
...
public abstract class Openable extends Parent implements IOpenable,
	...
	public Openable(ICElement parent, IPath path, int type) {
		...
		this(parent, ResourcesPlugin.getWorkspace().getRoot()
				.getFileForLocation(path), path.lastSegment(), type);
	}
	...
	public void bufferChanged(BufferChangedEvent event) {
		if (event.getBuffer().isClosed()) {
			CModelManager.getDefault().getElementsOutOfSynchWithBuffers()
					.remove(this);
			getBufferManager().removeBuffer(event.getBuffer());
		} else {
			CModelManager.getDefault().getElementsOutOfSynchWithBuffers().put(
					this, this);
		}
	}
	 ...
	 */
	protected abstract boolean buildStructure(OpenableInfo info,
	...
	public void makeConsistent(IProgressMonitor monitor, boolean forced)
			throws CModelException {
		if (isConsistent()) {
			return;
		}
	...
	protected void generateInfos(Object info, Map newElements,
		...
		if (CModelManager.VERBOSE) {
			System.out
					.println("OPENING Element (" + Thread.currentThread() + "): " + this); //$NON-NLS-1$//$NON-NLS-2$
		}
		...
		boolean isStructureKnown = buildStructure(openableInfo, monitor,
				newElements, getResource());
		openableInfo.setIsStructureKnown(isStructureKnown);

		// remove out of sync buffer for this element
		CModelManager.getDefault().getElementsOutOfSynchWithBuffers().remove(
				this);
