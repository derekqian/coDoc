...
// import edu.pdx.svl.coDoc.cdt.internal.core.pdom.indexer.ctags.CtagsIndexer;
import edu.pdx.svl.coDoc.cdt.internal.core.pdom.indexer.full.PDOMFullIndexer;
import edu.pdx.svl.coDoc.cdt.internal.core.pdom.indexer.nulli.PDOMNullIndexer;
...
/**
 * The PDOM Provider. This is likely temporary since I hope to integrate the
 * PDOM directly into the core once it has stabilized.
 * 
 * @author Doug Schaefer
 */
public class PDOMManager implements IPDOMManager, IElementChangedListener {

	private static final QualifiedName indexerProperty = new QualifiedName(
			CCorePlugin.PLUGIN_ID, "pdomIndexer"); //$NON-NLS-1$

	private static final QualifiedName dbNameProperty = new QualifiedName(
			CCorePlugin.PLUGIN_ID, "pdomName"); //$NON-NLS-1$

	private static final QualifiedName pdomProperty = new QualifiedName(
			CCorePlugin.PLUGIN_ID, "pdom"); //$NON-NLS-1$
	...
	public synchronized IPDOM getPDOM(ICProject project) throws CoreException {
		...
		if (pdom == null) {
			...
			IPath dbPath = CCorePlugin.getDefault().getStateLocation().append(
					dbName);
	...
	public IPDOMIndexer getIndexer(ICProject project) {
		try {
			IProject rproject = project.getProject();
			IPDOMIndexer indexer = (IPDOMIndexer) rproject
					.getSessionProperty(indexerProperty);

			if (indexer == null) {
				indexer = createIndexer(project, "");
			}
			return indexer;
	...
	private void processDelta(ICElementDelta delta) throws CoreException {
		...
		case ICElement.C_PROJECT:
			...
			case ICElementDelta.ADDED:
				createIndexer(project, "");
				// IEclipsePreferences prefs = new
				// ProjectScope(project.getProject()).getNode(CCorePlugin.PLUGIN_ID);
				// prefs.addNodeChangeListener(new
				// IEclipsePreferences.INodeChangeListener() {
				// public void added(NodeChangeEvent event) {
				// String indexerId = event.getParent().get(INDEXER_ID_KEY,
				// null);
				// try {
	...
	public String getIndexerId(ICProject project) throws CoreException {
		IEclipsePreferences prefs = new ProjectScope(project.getProject())
				.getNode(CCorePlugin.PLUGIN_ID);
		if (prefs == null)
		...
		if (indexerId == null) {
			...
			// try {
			// ICDescriptor desc =
			// CCorePlugin.getDefault().getCProjectDescription(project.getProject(),
			// false);
			// if (desc != null) {
			// ICExtensionReference[] ref =
			// desc.get(CCorePlugin.INDEXER_UNIQ_ID);
	...
	// This job is only used when setting during a get. Sometimes the get is
	// being
	...
	private IPDOMIndexer createIndexer(ICProject project, String indexerId)
			throws CoreException {
		...
		// indexerExt.getConfigurationElements();
		...
		IPDOMIndexer indexer = new PDOMFullIndexer();
	...
	IPDOMIndexerTask getNextTask() {
		synchronized (indexerJobMutex) {
			return indexerJobQueue.isEmpty() ? null
	...
	/**
	 * Startup the PDOM. This mainly sets us up to handle model change events.
	 */
	public void startup() {
