...
public class CPPBaseClause implements ICPPBase {
	static public class CPPBaseProblem extends ProblemBinding implements
			ICPPBase {
		...
		public IBinding getBaseClass() {
			if (classProblem == null) {
				classProblem = new CPPClassType.CPPClassTypeProblem(node, id,
						arg);
			}
			return classProblem;
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPBase#getBaseClass()
	 */
	public IBinding getBaseClass() throws DOMException {
		if (baseClass == null) {
			IBinding b = base.getName().resolveBinding();
			while (b instanceof ITypedef
					&& ((ITypedef) b).getType() instanceof IBinding) {
				b = (IBinding) ((ITypedef) b).getType();
			}
			if (b instanceof ICPPClassType
					|| b instanceof ICPPTemplateParameter)
				baseClass = b;

			else if (b instanceof IProblemBinding) {
				baseClass = new CPPClassType.CPPClassTypeProblem(
						base.getName(), ((IProblemBinding) b).getID(), base
								.getName().toCharArray());
			} else {
				baseClass = new CPPClassType.CPPClassTypeProblem(
						base.getName(),
						IProblemBinding.SEMANTIC_NAME_NOT_FOUND, base.getName()
								.toCharArray());
			}
		}
		return baseClass;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPBase#getVisibility()
	 */
	public int getVisibility() {
		int vis = base.getVisibility();

		if (vis == 0) {
			ICPPASTCompositeTypeSpecifier compSpec = (ICPPASTCompositeTypeSpecifier) base
					.getParent();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPBase#isVirtual()
	 */
