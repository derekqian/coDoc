...
public interface ITypeInfo {
	public static class OperatorExpression extends Enum {
		...
		// 5.3.1-1 : The unary * operator, the expression to which it is applied
		// shall be
		// a pointer to an object type or a pointer to a function type and the
		// result
		// is an lvalue refering to the object or function to which the
		// expression points
		public static final OperatorExpression indirection = new OperatorExpression(
				1);

		// 5.3.1-2 : The result of the unary & operator is a pointer to its
		// operand
		public static final OperatorExpression addressof = new OperatorExpression(
				0);

		// 5.2.1 A postfix expression followed by an expression in square
		// brackets is a postfix
		// expression. one of the expressions shall have the type "pointer to T"
		// and the other
		// shall have a enumeration or integral type. The result is an lvalue of
		// type "T"
		public static final OperatorExpression subscript = new OperatorExpression(
				2);
	...
	public static class PtrOp {
		...
		public boolean equals(Object o) {
			...
			return (isConst() == op.isConst()
					&& isVolatile() == op.isVolatile() && getType() == op
					.getType());
		}
	...
	public static final ITypeInfo.eType t_any = new ITypeInfo.eType(-1); // don't
																			// care

	public static final ITypeInfo.eType t_undef = new ITypeInfo.eType(0); // not
																			// specified

	public static final ITypeInfo.eType t_type = new ITypeInfo.eType(1); // Type
																			// Specifier
	...
	public static final ITypeInfo.eType t_templateParameter = new ITypeInfo.eType(
			22);
	 ...
	 * 
	 * @param infoProvider -
	 *            TypeInfoProvider to use if pooling the TypeInfo created, if
	 *            null, pooling is not used. If pooling is used,
	 *            TypeInfoProvider.returnTypeInfo must be called when the
	 *            TypeInfo is no longer needed
	 * @return
	 */
	 ...
	 * upperType (inclusive). upperType of 0 means no range and our type must be
	 * type.
	 */
	public abstract boolean isType(ITypeInfo.eType type,
	 ...
	 * 
	 * @param type
	 * @return boolean return true if our type can hold all the values of the
	 *         passed in type. TODO, for now return true if our type is "larger"
	 *         (based on ordering of the type values)
	 */
