...
public class BasicTokenDuple implements ITokenDuple {
	...
	public ITokenDuple getLastSegment() {
		...
		for (;;) {
			...
			if (token.getType() == IToken.tLT)
				token = TokenFactory.consumeTemplateIdArguments(token,
						getLastToken());
	...
	public ITokenDuple[] getSegments() {
		...
		for (;;) {
			...
			if (startOfSegment == last.getNext()
					&& startOfSegment.getType() != IToken.tEOC) // for the EOC
																// token, next
																// points to
																// itself
			{
			...
			if (token.getType() == IToken.tCOLONCOLON) {
				ITokenDuple d = TokenFactory.createTokenDuple(startOfSegment,
						(prev == null) ? startOfSegment : prev);
				r.add(d);
	...
	public String toString() {
		if (stringRepresentation == null)
			stringRepresentation = createCharArrayRepresentation(firstToken,
					lastToken);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ITokenDuple#getSubDuple(int, int)
	 */
	public ITokenDuple getSubrange(int startIndex, int endIndex) {
		return TokenFactory.createTokenDuple(getToken(startIndex),
				getToken(endIndex));
	}
	...
	public char[] extractNameFromTemplateId() {
		...
		// operators
			...
			CharArrayUtils.overWrite(nameBuffer, idx,
					createCharArrayRepresentation(first, i));
	...
	public boolean contains(ITokenDuple duple) {
		...
		for (IToken current = getFirstToken(); current != null; current = current
				.getNext()) {
