...
public class ASTExpression implements IASTExpression {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression#getLHSExpression()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression#getRHSExpression()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression#getLiteralString()
	 */
	...
	public long evaluateExpression() throws ASTExpressionEvaluationException {
		...
		if (getExpressionKind() == IASTExpression.Kind.PRIMARY_INTEGER_LITERAL) {
			try {
				...
				{
					return Integer
							.parseInt(getLiteralString().substring(2), 16);
				}
		...
		if (getExpressionKind() == IASTExpression.Kind.MULTIPLICATIVE_MULTIPLY)
			return (getLHSExpression().evaluateExpression() * getRHSExpression()
					.evaluateExpression());
		...
		if (getExpressionKind() == IASTExpression.Kind.LOGICALANDEXPRESSION)
			return ((getLHSExpression().evaluateExpression() != 0) && (getRHSExpression()
		...
		if (getExpressionKind() == IASTExpression.Kind.CONDITIONALEXPRESSION) {
			return (getLHSExpression().evaluateExpression() != 0) ? getRHSExpression()
					.evaluateExpression()
					: getThirdExpression().evaluateExpression();
		}
	 ...
	 */
	public ILookupResult lookup(String prefix, LookupKind[] k,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression#freeReferences()
	 */
