...
public class TokenFactory {
	...
	public static ITokenDuple createTokenDuple(IToken first, IToken last,
		...
		if ((first == last) && (templateArgLists == null)
				&& (first instanceof ITokenDuple))
			return (ITokenDuple) first;
	...
	public static ITokenDuple createTokenDuple(ITokenDuple firstDuple,
			ITokenDuple secondDuple) {
		if (secondDuple == null)
			return firstDuple;
		if (firstDuple == null)
			return secondDuple;
			...
			try {
				...
				while (!scopes.isEmpty() && token != last) {
					...
					case IToken.tLBRACKET:
						scopes.addValue(IToken.tLBRACKET);
						break;
					case IToken.tLPAREN:
						scopes.addValue(IToken.tLPAREN);
						break;
					}
				}
			} finally {
