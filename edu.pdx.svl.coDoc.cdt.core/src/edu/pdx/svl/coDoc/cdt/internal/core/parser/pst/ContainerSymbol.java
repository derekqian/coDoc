...
public class ContainerSymbol extends BasicSymbol implements IContainerSymbol {
	...
	public Object clone() {
		...
		copy._usingDirectives = (_usingDirectives != Collections.EMPTY_LIST) ? (List) ((ArrayList) _usingDirectives)
				.clone()
				: _usingDirectives;
	...
	public ISymbol instantiate(ITemplateSymbol template, ObjectMap argMap)
		...
		ContainerSymbol newContainer = (ContainerSymbol) super.instantiate(
				template, argMap);
			...
			while (iter.hasNext()) {
				...
				if (containedSymbol instanceof IUsingDirectiveSymbol) {
					...
					if (!template.getExplicitSpecializations().isEmpty()) {
						...
						if (hasAllParams) {
							ISymbol temp = TemplateEngine
									.checkForTemplateExplicitSpecialization(
											template, symbol, argList);
							if (temp != null)
					...
					if (!template.getDefinitionParameterMap().isEmpty()
							&& template.getDefinitionParameterMap()
									.containsKey(containedSymbol)) {
						ObjectMap defMap = (ObjectMap) template
								.getDefinitionParameterMap().get(
										containedSymbol);
					...
					newSymbol = ((ISymbol) containedSymbol).instantiate(
							template, instanceMap);
					newSymbol.setContainingSymbol(newContainer);
					newContainer._contents.add(newSymbol);

					if (newSymbol instanceof IParameterizedSymbol
							&& newSymbol.isType(ITypeInfo.t_constructor)) {
						collectInstantiatedConstructor((IParameterizedSymbol) containedSymbol);
					} else {
						if (newContainer.getContainedSymbols().containsKey(
								newSymbol.getName())) {
							Object obj = newContainer.getContainedSymbols()
									.get(newSymbol.getName());
							if (obj instanceof List) {
								...
								list.add(obj);
								list.add(newSymbol);
								newContainer.putInContainedSymbols(newSymbol
										.getName(), list);
							}
						} else {
							newContainer.putInContainedSymbols(newSymbol
	...
	protected void collectInstantiatedConstructor(
			IParameterizedSymbol constructor) {
	...
	public void addSymbol(ISymbol obj) throws ParserSymbolTableException {
		...
		if (obj.getType() == ITypeInfo.t_enumerator) {
			...
			if (containing.getType() == ITypeInfo.t_enumeration) {
				...
				// enumerator has the type of its
				// enumeration
				obj.setTypeSymbol(containing);
				// Each enumerator is declared in the scope that immediately
		...
		if (obj.isType(ITypeInfo.t_template)) {
			if (!TemplateEngine.canAddTemplate(containing,
					(ITemplateSymbol) obj)) {
		...
		// 14.6.1-4 A Template parameter shall not be redeclared within its
		// scope.
		if (isTemplateMember() || isType(ITypeInfo.t_template)) {
			if (TemplateEngine.alreadyHasTemplateParameter(this, obj.getName())) {
				throw new ParserSymbolTableException(
						ParserSymbolTableException.r_RedeclaredTemplateParam);
			}
		}

		boolean unnamed = CharArrayUtils.equals(obj.getName(),
		...
		if (origObj != null) {
			...
			boolean validOverride = (!unnamed ? ((origList == null) ? ParserSymbolTable
					.isValidOverload(origDecl, obj)
			...
			if (unnamed || validOverride) {
				if (origList == null) {
					...
					((ContainerSymbol) containing).putInContainedSymbols(obj
							.getName(), origList);
				} else {
					origList.add(obj);
			...
			((ContainerSymbol) containing).putInContainedSymbols(obj.getName(),
					obj);
		}
	...
	public IUsingDirectiveSymbol addUsingDirective(IContainerSymbol namespace)
		...
		if (namespace.getType() != ITypeInfo.t_namespace) {
			throw new ParserSymbolTableException(
					ParserSymbolTableException.r_InvalidUsing);
		}
		...
		IUsingDirectiveSymbol usingDirective = new UsingDirectiveSymbol(
				getSymbolTable(), namespace);
	...
	public IUsingDeclarationSymbol addUsingDeclaration(char[] name,
		...
		// figure out which declaration we are talking about, if it is a set of
		// functions,
		// then they will be in data.foundItems (since we provided no parameter
		// info);
		ISymbol symbol = null;
		ISymbol clone = null;
		int objListSize = 0;
		...
		if (symbol == null
				&& (data.foundItems == null || data.foundItems.isEmpty())) {
			throw new ParserSymbolTableException(
					ParserSymbolTableException.r_InvalidUsing);
		}
		...
		UsingDeclarationSymbol usingDeclaration = new UsingDeclarationSymbol(
				getSymbolTable(), usingRefs, usingDecs);
	 ...
	 *      java.lang.String)
	 */
	public ISymbol elaboratedLookup(final ITypeInfo.eType type, char[] name)
			throws ParserSymbolTableException {
		LookupData data = new LookupData(name) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IContainerSymbol#lookupMemberForDefinition(java.lang.String)
	 */
	/**
	 ...
	 * 
	 * ie: you have this: namespace A{ namespace B{ void f1(int); } using
	 * namespace B; }
	 * 
	 * if you then do this void A::f1(int) { ... } //ill-formed, f1 is not a
	 * member of A but, you can do this (Assuming f1 has been defined elsewhere)
	...
	public ISymbol lookupMemberForDefinition(char[] name,
			final ITypeInfo.eType type) throws ParserSymbolTableException {
		LookupData data = new LookupData(name) {
			public TypeFilter getFilter() {
				if (t == ITypeInfo.t_any)
					return ANY_FILTER;
	...
	public IParameterizedSymbol lookupMethodForDefinition(char[] name,
			final List parameters) throws ParserSymbolTableException {
		LookupData data = new LookupData(name) {
		...
		if (data.foundItems != null) {
			...
			return (IParameterizedSymbol) ((symbol instanceof IParameterizedSymbol) ? symbol
					: null);
		}
	 ...
	 * 
	 * @param name
	 * @return Declaration The name of a class or namespace member can be
	 *         referred to after the :: scope resolution operator applied to a
	 *         nested-name-specifier that nominates its class or namespace.
	 *         During the lookup for a name preceding the ::, object, function
	 *         and enumerator names are ignored. If the name is not a class-name
	...
	public IParameterizedSymbol unqualifiedFunctionLookup(char[] name,
			final List parameters) throws ParserSymbolTableException {
		// figure out the set of associated scopes first, so we can remove those
		// that are searched
		...
		for (int i = 0; i < size; i++) {
			...
			ITypeInfo info = ParserSymbolTable.getFlatTypeInfo(param,
					getSymbolTable().getTypeInfoProvider());
			...
			// are those associated with the member type together with those
			// associated with X
			if (param.hasPtrOperators() && param.getPtrOperators().size() == 1) {
				ITypeInfo.PtrOp op = (ITypeInfo.PtrOp) param.getPtrOperators()
						.get(0);
				if (op.getType() == ITypeInfo.PtrOp.t_pointer
						&& paramType.getContainingSymbol().isType(
								ITypeInfo.t_class, ITypeInfo.t_union)) {
					ParserSymbolTable.getAssociatedScopes(paramType
							.getContainingSymbol(), associated);
				}
			}
		}
		...
		// if we haven't found anything, or what we found is not a class member,
		// consider the
		// associated scopes
		if (found == null
				|| found.getContainingSymbol().getType() != ITypeInfo.t_class) {
			// if( found != null ){
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IContainerSymbol#templateLookup(java.lang.String,
	 *      java.util.List)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IContainerSymbol#lookupTemplateIdForDefinition(java.lang.String,
	 *      java.util.List)
	 */
	...
	public List prefixLookup(final TypeFilter filter, char[] prefix,
			boolean qualified, final List paramList)
			throws ParserSymbolTableException {
		...
		if (filter != null && filter.willAccept(ITypeInfo.t_constructor)
				&& (this instanceof IDerivableContainerSymbol)) {
			if (CharArrayUtils
					.equals(getName(), 0, prefix.length, prefix, true)) {
				List temp = ((IDerivableContainerSymbol) this)
						.getConstructors();
				int size = temp.size();
		...
		for (int i = 0; i < size; i++) {
			...
			if (data.getAmbiguities() != null
					&& data.getAmbiguities().containsKey(key))
				continue;
			...
			if (obj instanceof List) {
				...
				if (paramList != null
						&& ((ISymbol) obj).isType(ITypeInfo.t_function)) {
					if (tempList == null)
	...
	public boolean isVisible(ISymbol symbol, IContainerSymbol qualifyingSymbol) {
		...
		if (node instanceof IASTMember) {
			...
			visibility = ParserSymbolTable.getVisibility(symbol,
					qualifyingSymbol);
			if (visibility == null)
				return false;
			...
			if (isFriendOf((qualifyingSymbol != null) ? qualifyingSymbol
					: symbolContainer)) {
				return true;
			}
			...
			if (visibility == ASTAccessVisibility.PROTECTED) {
				try {
					return (ParserSymbolTable.hasBaseClass(container,
							symbolContainer) >= 0);
				} catch (ParserSymbolTableException e) {
					return false;
				}
	...
	protected boolean isFriendOf(IContainerSymbol symbol) {
		if (symbol instanceof IDerivableContainerSymbol) {
			...
			if (container != null
					&& !container.isType(ITypeInfo.t_class, ITypeInfo.t_union)) {
				container = null;
			}
			...
			for (int i = 0; i < size; i++) {
				...
				if (friend == this || forwardSymbol == this
						|| friend == container
						|| (container != null && forwardSymbol == container)) {
					return true;
				}
			}
		}
	...
	protected class ContentsIterator implements Iterator {
		...
		public boolean hasNext() {
			...
			for (; idx < internalList.size();) {
				IExtensibleSymbol extensible = (IExtensibleSymbol) internalList
						.get(idx++);
			...
			for (; idx < internalList.size();) {
				...
				if (!alreadyReturned.containsKey(extensible)) {
					if (extensible instanceof ISymbol) {
						...
						if (symbol.isForwardDeclaration()
								&& symbol.getForwardSymbol() != null
								&& symbol.getForwardSymbol()
										.getContainingSymbol() == ContainerSymbol.this) {
	...
	// }
	//			
	// //this is an inefficient way of doing this, we can modify the interfaces
	// if the undo starts
	// //being used often.
	// ContentsIterator iter = (ContentsIterator)
	// _context.getContentsIterator();
	// while( iter.hasNext() ){
	...
	// public AddUsingDirectiveCommand( IContainerSymbol container,
	// IUsingDirectiveSymbol directive ){
	// _decl = container;
