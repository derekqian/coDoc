...
public class BasicSymbol extends ExtensibleSymbol implements ISymbol {
	...
	public ISymbol instantiate(ITemplateSymbol template, ObjectMap argMap)
		...
		newSymbol.setTypeInfo(TemplateEngine.instantiateTypeInfo(newSymbol
				.getTypeInfo(), template, argMap));
	...
	public ISymbol getTypeSymbol() {
		...
		if (symbol != null && symbol.isForwardDeclaration()
	...
	public ISymbol getForwardSymbol() {
		return (_isForwardDeclaration || isType(ITypeInfo.t_namespace)) ? _symbolDef
				: null;
	}
	...
	public int compareCVQualifiersTo(ISymbol symbol) {
		int size = symbol.getTypeInfo().hasPtrOperators() ? symbol
				.getTypeInfo().getPtrOperators().size() : 0;
		int size2 = getTypeInfo().hasPtrOperators() ? getTypeInfo()
		...
		if (size != size2) {
			...
			for (int i = 0; i > size; i++) {
				op1 = (ITypeInfo.PtrOp) symbol.getTypeInfo().getPtrOperators()
						.get(i);
	...
	private boolean _isInvisible = false; // used by friend declarations
											// (11.4-9)
	...
	private ISymbol _symbolDef = null; // used for forward declarations and
