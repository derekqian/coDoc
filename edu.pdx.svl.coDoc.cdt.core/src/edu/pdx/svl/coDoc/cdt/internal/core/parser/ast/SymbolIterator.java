...
public class SymbolIterator implements Iterator {
	...
	public boolean hasNext() {
		...
		while (interalIterator.hasNext()) {
			IExtensibleSymbol symbol = (IExtensibleSymbol) interalIterator
					.next();
		...
		while (interalIterator.hasNext()) {
			...
			if (temp.getASTExtension() != null) {
				if (temp.getASTExtension().getPrimaryDeclaration() instanceof IASTOffsetableElement
						&& CharArrayUtils.equals(((IASTOffsetableElement) temp
								.getASTExtension().getPrimaryDeclaration())
								.getFilename(), EMPTY_CHAR_ARRAY))
					continue;
