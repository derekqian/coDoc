...
public class Database {
		...
		if (size > MAX_SIZE)
			...
			throw new CoreException(
					new Status(
							...
							0,
							CCorePlugin
									.getResourceString("pdom.requestTooLarge"), new IllegalArgumentException())); //$NON-NLS-1$
	...
	public void free(int offset) throws CoreException {
		...
		if (blocksize < 0)
			...
			throw new CoreException(new Status(IStatus.ERROR,
					CCorePlugin.PLUGIN_ID, 0, "Already Freed", new Exception()));
	...
	public void reportFreeBlocks() throws CoreException {
		...
		System.out.println("wasted: "
				+ (toc.length * CHUNK_SIZE - (malloced - freed)));
