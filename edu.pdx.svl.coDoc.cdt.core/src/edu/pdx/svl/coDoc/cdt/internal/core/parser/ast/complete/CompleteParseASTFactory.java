...
import edu.pdx.svl.coDoc.cdt.core.parser.ast.ASTSemanticException;
...
import edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTemplateParameter.ParamKind;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.ast.ASTAbstractDeclaration;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.problem.IProblemFactory;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ExtensibleSymbolExtension;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ForewardDeclaredSymbolExtension;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IContainerSymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IDeferredTemplateInstance;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IDerivableContainerSymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IExtensibleSymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IParameterizedSymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ISymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ISymbolASTExtension;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IUsingDeclarationSymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IUsingDirectiveSymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.NamespaceSymbolExtension;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ParserSymbolTableException;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.StandardSymbolExtension;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.TemplateSymbolExtension;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.TypeInfoProvider;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.UndefinedTemplateSymbol;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ISymbolASTExtension.ExtensionException;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.token.TokenFactory;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.util.TraceUtil;

/**
 * @author jcamelon
 * 
 * The CompleteParseASTFactory class creates a complete AST for a given parsed
 * code.
 * 
 */
public class CompleteParseASTFactory extends BaseASTFactory implements
		IASTFactory {
	...
	private final CharArrayObjectMap typeIdCache = new CharArrayObjectMap(
			BUILTIN_TYPE_SIZE);

	private final CharArrayObjectMap simpleTypeSpecCache = new CharArrayObjectMap(
			BUILTIN_TYPE_SIZE);
	...
	public CompleteParseASTFactory(ParserLanguage language, ParserMode mode,
			IASTFactoryExtension extension) {
		super(extension);
	...
	/*
	 * Adds a reference to a reference list Overrides an existing reference if
	 * it has the same name and offset
	 */
	protected void addReference(List references, IASTReference reference) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (ref != null) {
				if ((CharArrayUtils.equals(ref.getNameCharArray(), reference
						.getNameCharArray()))
	...
	/*
	 * Test if the provided list is a valid parameter list Parameters are list
	 * of TypeInfos
	 */
	...
	private ISymbol lookupElement(IContainerSymbol startingScope, char[] name,
			ITypeInfo.eType type, List parameters, LookupType lookupType)
			throws ASTSemanticException {
		return lookupElement(startingScope, name, type, parameters, null,
				lookupType);
	}

	private ISymbol lookupElement(IContainerSymbol startingScope, char[] name,
			ITypeInfo.eType type, List parameters, List arguments,
			LookupType lookupType) throws ASTSemanticException {
		ISymbol result = null;
		if (startingScope == null)
			return null;
		try {
			if ((type == ITypeInfo.t_function)
					|| (type == ITypeInfo.t_constructor)) {
				...
				if (validParameterList(parameters))
					if (type == ITypeInfo.t_constructor) {
						...
						result = startingDerivableScope
								.lookupConstructor(parameters);
					} else {
						if (arguments != null)
							result = startingScope.lookupFunctionTemplateId(
									name, parameters, arguments,
									(lookupType == LookupType.FORDEFINITION));
						else if (lookupType == LookupType.QUALIFIED)
							result = startingScope.qualifiedFunctionLookup(
									name, parameters);
						else if (lookupType == LookupType.UNQUALIFIED
								|| lookupType == LookupType.FORPARENTSCOPE)
							result = startingScope.unqualifiedFunctionLookup(
									name, parameters);
						else if (lookupType == LookupType.FORDEFINITION)
							result = startingScope.lookupMethodForDefinition(
									name, parameters);
						else if (lookupType == LookupType.FORFRIENDSHIP) {
							result = ((IDerivableContainerSymbol) startingScope)
									.lookupFunctionForFriendship(name,
											parameters);
						}
					}
				else
	...
	protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
			char[] name, List references, boolean throwOnError,
			LookupType lookup) throws ASTSemanticException {
		return lookupQualifiedName(startingScope, name, ITypeInfo.t_any, null,
				0, references, throwOnError, lookup);
	}

	protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
			char[] name, ITypeInfo.eType type, List parameters, int offset,
			List references, boolean throwOnError, LookupType lookup)
			throws ASTSemanticException {
		ISymbol result = null;
	...
	protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
			ITokenDuple name, List references, boolean throwOnError)
			throws ASTSemanticException {
		return lookupQualifiedName(startingScope, name, references,
				throwOnError, LookupType.UNQUALIFIED);
	}
	...
	protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
			...
			LookupType lookup) throws ASTSemanticException {
		return lookupQualifiedName(startingScope, name, ITypeInfo.t_any, null,
				references, throwOnError, lookup);
	}

	protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
			ITokenDuple name, ITypeInfo.eType type, List parameters,
			List references, boolean throwOnError) throws ASTSemanticException {
		return lookupQualifiedName(startingScope, name, type, parameters,
				references, throwOnError, LookupType.UNQUALIFIED);
	}
	...
	protected ISymbol lookupQualifiedName(IContainerSymbol startingScope,
		...
		case 1:
			...
			args = (templateArgLists != null) ? getTemplateArgList(templateArgLists[0])
					: null;
			try {
				result = lookupElement(startingScope, image, type, parameters,
						args, lookup);
			} catch (ASTSemanticException e) {
				if (e.getProblem() == null
						|| e.getProblem().getSourceLineNumber() == -1) {
					handleProblem(e.getProblem().getID(), image, name
							.getStartOffset(), name.getEndOffset(), name
							.getLineNumber(), e.getProblem().isError());
				} else {
					throw e;
				}
			}
			...
			if (result != null) {
				if (lookup == LookupType.FORPARENTSCOPE
						&& startingScope instanceof ITemplateFactory) {
					if (args != null)
						((ITemplateFactory) startingScope).pushTemplateId(
								result, args);
					else
						((ITemplateFactory) startingScope).pushSymbol(result);
				}
				if (references != null)
					addReference(references, createReference(result, image,
							name.getStartOffset()));
				...
				if (startingScope.getASTExtension() != null
						&& startingScope.getASTExtension()
								.getPrimaryDeclaration() instanceof IASTCodeScope) {
					if (((IASTCodeScope) startingScope.getASTExtension()
							.getPrimaryDeclaration()).getContainingFunction() instanceof IASTMethod) {
						IASTClassSpecifier classSpecifier = ((IASTMethod) ((IASTCodeScope) startingScope
								.getASTExtension().getPrimaryDeclaration())
								.getContainingFunction())
								.getOwnerClassSpecifier();
						if (classSpecifier != null)
							((ASTClassSpecifier) classSpecifier)
									.addUnresolvedReference(new UnresolvedReferenceDuple(
											startingScope, name));
						break;
					}
				}
				if (throwOnError)
					handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND, image, name
							.getStartOffset(), name.getEndOffset(), name
							.getLineNumber(), true);
				return null;
			}
			...
			for (;;) {
				...
				if (templateArgLists != null && templateArgLists[idx] != null) {
					if (t != last && t.getNext().getType() == IToken.tLT)
						t = TokenFactory.consumeTemplateIdArguments(
								t.getNext(), last);
				}
				...
				try {
					while (!(result instanceof IContainerSymbol)) {
						if (result.getTypeInfo().checkBit(ITypeInfo.isTypedef)) {
							...
							result = ((IDeferredTemplateInstance) result)
									.getTemplate().getTemplatedSymbol();
						} else {
							if (throwOnError)
								handleProblem(
										IProblem.SEMANTIC_NAME_NOT_PROVIDED,
										image, t.getOffset(), t.getEndOffset(),
										t.getLineNumber(), true);
							else
								return null;
						}
					}
					args = (templateArgLists != null) ? getTemplateArgList(templateArgLists[idx])
							: null;
					if (t == last)
						result = lookupElement((IContainerSymbol) result,
								image, type, parameters, args,
								(lookup == LookupType.FORDEFINITION) ? lookup
										: LookupType.QUALIFIED);
					else if (templateArgLists != null
							&& templateArgLists[idx] != null)
						result = ((IContainerSymbol) result).lookupTemplateId(
								image, args);
					else
						result = ((IContainerSymbol) result)
								.lookupNestedNameSpecifier(image);
					...
					if (result != null) {
						if (lookup == LookupType.FORPARENTSCOPE
							...
							if (templateArgLists != null
									&& templateArgLists[idx] != null)
								((ITemplateFactory) startingScope)
										.pushTemplateId(result, args);
							else
								((ITemplateFactory) startingScope)
										.pushSymbol(result);
						}
						if (references != null)
							addReference(references, createReference(result,
									image, offset));
						if (references != null && templateArgLists != null
								&& templateArgLists[idx] != null) {
							addTemplateIdReferences(references,
									templateArgLists[idx]);
							name.freeReferences();
						}
					} else
						break;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createUsingDirective(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ITokenDuple, int, int)
	 */
	public IASTUsingDirective createUsingDirective(IASTScope scope,
			ITokenDuple duple, int startingOffset, int startingLine,
			int endingOffset, int endingLine) throws ASTSemanticException {
		setProblemInfo(duple);
		List references = new ArrayList();
		ISymbol symbol = lookupQualifiedName(scopeToSymbol(scope), duple,
				references, true);

		IUsingDirectiveSymbol usingDirective = null;
		if (symbol != null)
			try {
				usingDirective = ((ASTScope) scope).getContainerSymbol()
						.addUsingDirective((IContainerSymbol) symbol);
			} catch (ParserSymbolTableException pste) {
				handleProblem(pste.createProblemID(), duple.toCharArray(),
						startingOffset, endingOffset, startingLine, true);
			}

		ASTUsingDirective using = new ASTUsingDirective(scopeToSymbol(scope),
				usingDirective, startingOffset, startingLine, endingOffset,
				endingLine, references, filename, duple.getStartOffset(), duple
						.getEndOffset(), duple.getLineNumber());
		attachSymbolExtension(usingDirective, using);

		return using;
	}

	protected IContainerSymbol getScopeToSearchUpon(IASTScope currentScope,
			IToken firstToken) {
	...
	protected IContainerSymbol scopeToSymbol(IASTScope currentScope) {
		if (currentScope instanceof ASTScope)
			...
			return ((ASTTemplateInstantiation) currentScope)
					.getContainerSymbol();
		else
			return scopeToSymbol(((ASTAnonymousDeclaration) currentScope)
					.getOwnerScope());
	}
	 ...
	 *      boolean, edu.pdx.svl.coDoc.cdt.core.parser.ITokenDuple, int, int)
	 */
	public IASTUsingDeclaration createUsingDeclaration(IASTScope scope,
		...
		if (name.getSegmentCount() > 1) {
			...
			if (duple == null) {
				...
				containerSymbol = scopeToSymbol(scope).getSymbolTable()
						.getCompilationUnit();
			} else {
				ISymbol symbol = lookupQualifiedName(scopeToSymbol(scope),
						duple, references, true);
				...
				if (symbol instanceof IContainerSymbol)
					...
					containerSymbol = ((IDeferredTemplateInstance) symbol)
							.getTemplate().getTemplatedSymbol();
			}
			try {
				endResult = scopeToSymbol(scope).addUsingDeclaration(
						name.getLastToken().getCharImage(), containerSymbol);
			} catch (ParserSymbolTableException e) {
				handleProblem(e.createProblemID(), name.getLastToken()
						.getCharImage(), startingOffset, endingOffset,
						startingLine, true);
			}
		} else
			try {
				endResult = scopeToSymbol(scope).addUsingDeclaration(
						name.getLastToken().getCharImage());
			} catch (ParserSymbolTableException e) {
				handleProblem(e.createProblemID(), name.getLastToken()
						.getCharImage(), startingOffset, endingOffset,
						startingLine, true);
		...
		if (endResult != null) {
			...
			for (int i = 0; i < numRefs; i++)
				addReference(references, createReference((ISymbol) refs.get(i),
						name.getLastToken().getCharImage(), name.getLastToken()
								.getOffset()));

		}
		ASTUsingDeclaration using = new ASTUsingDeclaration(scope, name
				.getLastToken().getCharImage(), endResult
				.getReferencedSymbols(), isTypeName, startingOffset,
				startingLine, endingOffset, endingLine, references, filename,
				name.getStartOffset(), name.getEndOffset(), name
						.getLineNumber());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createASMDefinition(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	...
	public IASTASMDefinition createASMDefinition(IASTScope scope,
		...
		return new ASTASMDefinition(scopeToSymbol(scope), assembly,
				startingOffset, startingLine, endingOffset, endingLine, fn);
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createNamespaceDefinition(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      java.lang.String, int, int)
	 */
	public IASTNamespaceDefinition createNamespaceDefinition(IASTScope scope,
			char[] identifier, int startingOffset, int startingLine,
			int nameOffset, int nameEndOffset, int nameLineNumber, char[] fn)
			throws ASTSemanticException {
		// TODO - set filename
		...
		if (!identifier.equals(EMPTY_STRING)) {
			try {
				...
				handleProblem(e.createProblemID(), identifier, nameOffset,
						nameEndOffset, nameLineNumber, true);
			}
		}

		if (namespaceSymbol != null) {
			if (namespaceSymbol.getType() != ITypeInfo.t_namespace)
				handleProblem(IProblem.SEMANTIC_INVALID_OVERLOAD, identifier,
						nameOffset, nameEndOffset, nameLineNumber, true);
		} else {
			namespaceSymbol = pst.newContainerSymbol(identifier,
					ITypeInfo.t_namespace);
			if (identifier.equals(EMPTY_STRING))
		...
		ASTNamespaceDefinition namespaceDef = new ASTNamespaceDefinition(
				namespaceSymbol, startingOffset, startingLine, nameOffset,
				nameEndOffset, nameLineNumber, fn);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createCompilationUnit()
	 */
	...
	protected void attachSymbolExtension(IExtensibleSymbol symbol,
			ASTNode astNode) {
		ISymbolASTExtension symbolExtension = new ExtensibleSymbolExtension(
				symbol, astNode);
		symbol.setASTExtension(symbolExtension);
	}

	protected void attachSymbolExtension(ISymbol symbol, ASTSymbol astSymbol,
		...
		if (symbolExtension == null) {
			if (astSymbol instanceof IASTNamespaceDefinition)
				symbolExtension = new NamespaceSymbolExtension(symbol,
						astSymbol);
			else if (astSymbol instanceof IASTFunction
					|| astSymbol instanceof IASTMethod
				...
				symbolExtension = new ForewardDeclaredSymbolExtension(symbol,
						astSymbol);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createLinkageSpecification(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	...
	public IASTLinkageSpecification createLinkageSpecification(IASTScope scope,
		...
		return new ASTLinkageSpecification(scopeToSymbol(scope), spec,
				startingOffset, startingLine, fn);
	}
	 ...
	 *      java.lang.String, edu.pdx.svl.coDoc.cdt.core.parser.ast.ASTClassKind,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTClassSpecifier.ClassNameType,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.ASTAccessVisibility,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTemplate, int, int)
	 */
	public IASTClassSpecifier createClassSpecifier(IASTScope scope,
		...
		if (name != null) {
			...
			if (name.getSegmentCount() != 1) // qualified name
			{
				...
				if (temp instanceof IDeferredTemplateInstance)
					currentScopeSymbol = ((IDeferredTemplateInstance) temp)
							.getTemplate().getTemplatedSymbol();
				else
					currentScopeSymbol = (IContainerSymbol) temp;
				if (currentScopeSymbol == null)
					handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND,
							containerSymbolName.toCharArray(),
							containerSymbolName.getFirstToken().getOffset(),
		...
		if (!CharArrayUtils.equals(newSymbolName, EMPTY_STRING)
				&& !isTemplateId) {
			try {
				classSymbol = currentScopeSymbol.lookupMemberForDefinition(
						newSymbolName, pstType);
			} catch (ParserSymbolTableException e) {
				handleProblem(IProblem.SEMANTIC_UNIQUE_NAME_PREDEFINED, name
						.toCharArray(), nameOffset, nameEndOffset, nameLine,
						true);
			}

			if (classSymbol != null && !classSymbol.isForwardDeclaration())
				handleProblem(IProblem.SEMANTIC_UNIQUE_NAME_PREDEFINED,
						newSymbolName, nameOffset, nameEndOffset, nameLine,
						true);

			if (classSymbol != null && classSymbol.getType() != pstType) {
				boolean isError = true;
				if (classSymbol.isType(ITypeInfo.t_class, ITypeInfo.t_union)) {
					if ((pstType == ITypeInfo.t_class
							|| pstType == ITypeInfo.t_struct || pstType == ITypeInfo.t_union))
						isError = false;
		...
		IDerivableContainerSymbol newSymbol = pst.newDerivableContainerSymbol(
				newSymbolName, pstType);
		...
		try {
			...
			if (e2.reason == ParserSymbolTableException.r_InvalidOverload)
				handleProblem(e2.createProblemID(), newSymbolName, nameOffset,
						nameEndOffset, nameLine, false); // to fix 65569 for
															// this
			// patch
			else
				handleProblem(e2.createProblemID(), newSymbolName, nameOffset,
						nameEndOffset, nameLine, true);
		}

		if (name != null && name.getTemplateIdArgLists() != null) {
			for (int i = 0; i < name.getTemplateIdArgLists().length; ++i)
				addTemplateIdReferences(references, name
						.getTemplateIdArgLists()[i]);
			name.freeReferences();
		}
		ASTClassSpecifier classSpecifier = new ASTClassSpecifier(newSymbol,
				kind, type, access, startingOffset, startingLine, nameOffset,
	...
	protected void handleProblem(int id, char[] attribute)
			throws ASTSemanticException {
		handleProblem(null, id, attribute, problemStartOffset,
				problemEndOffset, problemLineNumber, true);
	}

	protected void handleProblem(IASTScope scope, int id, char[] attribute)
			throws ASTSemanticException {
		handleProblem(scope, id, attribute, problemStartOffset,
				problemEndOffset, problemLineNumber, true);
	}

	protected void handleProblem(int id, char[] attribute, int startOffset,
			int endOffset, int lineNumber, boolean isError)
			throws ASTSemanticException {
	 ...
	 *      boolean, edu.pdx.svl.coDoc.cdt.core.parser.ast.ASTAccessVisibility,
	 *      java.lang.String)
	 */
	public void addBaseSpecifier(IASTClassSpecifier astClassSpec,
		...
		// Its possible that the parent is not an IContainerSymbol if its a
		// template parameter or some kinds of template instances
		ISymbol symbol = lookupQualifiedName(classSymbol, parentClassName,
				references, true);

		if (symbol == null)
			handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND, parentClassName
					.toCharArray(), parentClassName.getStartOffset(),
					parentClassName.getEndOffset(), parentClassName
							.getLineNumber(), true);
		if (symbol instanceof ITemplateSymbol
				&& !(symbol instanceof UndefinedTemplateSymbol))
			handleProblem(IProblem.SEMANTIC_INVALID_TEMPLATE_ARGUMENT,
					parentClassName.toCharArray(), parentClassName
							.getStartOffset(), parentClassName.getEndOffset(),
					parentClassName.getLineNumber(), true);
		...
		classSymbol.addParent(symbol, isVirtual, visibility, parentClassName
				.getFirstToken().getOffset(), references);
	 ...
	 * @return
	 */
	protected IASTReference createReference(ISymbol symbol,
			char[] referenceElementName, int offset)
			throws ASTSemanticException {
		if (mode != ParserMode.COMPLETE_PARSE)
			return null;
		// referenced symbol doesn't have an attached AST node, could happen say
		// for the copy constructor added
		// by the symbol table.
		...
		// assert (symbol != null ) : "createReference cannot be called on null
		// symbol ";
		if (symbol.getTypeInfo().checkBit(ITypeInfo.isTypedef)
			...
			return new ASTTypedefReference(offset,
			...
			if (declaration instanceof IASTNamespaceDefinition)
				return new ASTNamespaceReference(offset,
						(IASTNamespaceDefinition) declaration);
			else if (declaration instanceof IASTNamespaceAlias) {
				IASTNamespaceDefinition namespace = null;
				try {
					namespace = ((IASTNamespaceAlias) declaration)
							.getNamespace();
				} catch (ASTNotImplementedException e) {
					return null;
				}
				return new ASTNamespaceReference(offset, namespace);
			} else
				return null;
		} else if (symbol.getType() == ITypeInfo.t_class
				|| symbol.getType() == ITypeInfo.t_struct
				|| symbol.getType() == ITypeInfo.t_union)
			return new ASTClassReference(offset, (IASTTypeSpecifier) symbol
					.getASTExtension().getPrimaryDeclaration());
		else if (symbol.getType() == ITypeInfo.t_enumeration)
			return new ASTEnumerationReference(offset,
					(IASTEnumerationSpecifier) symbol.getASTExtension()
							.getPrimaryDeclaration());
		else if (symbol.getType() == ITypeInfo.t_enumerator)
			return new ASTEnumeratorReference(offset,
					(IASTEnumerator) declaration);
		else if ((symbol.getType() == ITypeInfo.t_function)
				|| (symbol.getType() == ITypeInfo.t_constructor)) {
			ASTNode referenced = (definition != null) ? definition
					: declaration;
...

		{
			if (symbol.getContainingSymbol().getType() == ITypeInfo.t_class
				...
				return new ASTFieldReference(offset,
						(IASTField) (definition != null ? definition
								: declaration));
			} else if ((symbol.getContainingSymbol().getType() == ITypeInfo.t_function || symbol
					.getContainingSymbol().getType() == ITypeInfo.t_constructor)
					&& symbol.getContainingSymbol() instanceof IParameterizedSymbol
					&& ((IParameterizedSymbol) symbol.getContainingSymbol())
							.getParameterList() != null
					&& ((IParameterizedSymbol) symbol.getContainingSymbol())
							.getParameterList().contains(symbol)) {
				return new ASTParameterReference(offset,
				...
				if (s instanceof IASTVariable)
					...
					return new ASTParameterReference(offset,
							(IASTParameterDeclaration) s);
				else if (s instanceof IASTTemplateParameter)
					return new ASTTemplateParameterReference(offset,
							(IASTTemplateParameter) s);
			}
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createEnumerationSpecifier(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      java.lang.String, int, int)
	 */
	public IASTEnumerationSpecifier createEnumerationSpecifier(IASTScope scope,
		...
		IDerivableContainerSymbol classSymbol = pst
				.newDerivableContainerSymbol(name, pstType);
		try {
		...
		ASTEnumerationSpecifier enumSpecifier = new ASTEnumerationSpecifier(
				classSymbol, startingOffset, startingLine, nameOffset,
				nameEndOffset, nameLine, fn);
	...
	public IASTEnumerator addEnumerator(IASTEnumerationSpecifier enumeration,
			...
			IASTExpression initialValue, char[] fn) throws ASTSemanticException {
		setFilename(fn);
		IContainerSymbol enumerationSymbol = (IContainerSymbol) ((ISymbolOwner) enumeration)
				.getSymbol();
		...
		try {
			...
			if (e1.reason == ParserSymbolTableException.r_InvalidOverload)
				handleProblem(IProblem.SEMANTIC_INVALID_OVERLOAD, name,
						startingOffset, endingOffset, startingLine, true);
			// assert false : e1;
		}
		ASTEnumerator enumerator = new ASTEnumerator(enumeratorSymbol,
				enumeration, startingOffset, startingLine, nameOffset,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createExpression(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression.Kind,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression,
	...
	public IASTExpression createExpression(IASTScope scope, Kind kind,
		...
		if (idExpression != null && logService.isTracing()) {
			TraceUtil.outputTrace(logService,
					...
					null, kind.getKindName(), " idexpression=", //$NON-NLS-1$
					idExpression.toString());
		} else if (literal != null && literal.length > 0
				&& logService.isTracing()) {
			TraceUtil.outputTrace(logService,
					"Entering createExpression with Kind=", //$NON-NLS-1$
					null, kind.getKindName(), " literal=", //$NON-NLS-1$
					String.valueOf(literal));
		}

		List references = new ArrayList();
		ISymbol symbol = getExpressionSymbol(scope, kind, lhs, rhs,
				idExpression, references);

		// Try to figure out the result that this expression evaluates to
		ExpressionResult expressionResult = getExpressionResultType(scope,
				kind, lhs, rhs, thirdExpression, typeId, literal, symbol, extra);
		...
		// assert expressionResult != null : expressionResult; //throw new
		// ASTSemanticException();

		// create the ASTExpression
		ASTExpression expression = null;
		if (extension.overrideCreateExpressionMethod())
			expression = (ASTExpression) extension.createExpression(scope,
					kind, lhs, rhs, thirdExpression, typeId, idExpression,
					literal, newDescriptor, references);
		else
			expression = ExpressionFactory.createExpression(kind, lhs, rhs,
					thirdExpression, typeId, idExpression, literal,
					newDescriptor, references);
	...
	private void createConstructorReference(
			IASTNewExpressionDescriptor descriptor, IASTTypeId typeId,
			List references) {
		ISymbol symbol = null;
		...
		List initializers = ((ASTNewDescriptor) descriptor)
				.getNewInitializerExpressionsList();

		ASTExpression exp = (initializers.size() > 0) ? (ASTExpression) initializers
				.get(0)
				: null;
		...
		if (createConstructorReference(symbol, exp, duple, references)) {
			// if we have a constructor reference, get rid of the class
			// reference.
			List refs = ((ASTTypeId) typeId).getReferences();
			int size = refs.size();
			for (int i = 0; i < size; i++) {
				ASTReference ref = (ASTReference) refs.get(i);
				if (CharArrayUtils.equals(ref.getNameCharArray(), duple
						.toCharArray())
	...
	private boolean createConstructorReference(ISymbol classSymbol,
			ASTExpression expressionList, ITokenDuple duple, List references) {
		if (classSymbol != null
				&& classSymbol.getTypeInfo().checkBit(ITypeInfo.isTypedef)) {
		...
		if (classSymbol == null
				|| !(classSymbol instanceof IDerivableContainerSymbol)) {
			return false;
		}
		...
		while (expressionList != null) {
			...
			if (expressionList.getExpressionKind() == IASTExpression.Kind.EXPRESSIONLIST)
				expressionList = (ASTExpression) expressionList
						.getRHSExpression();
			else
				expressionList = null;
		}
		...
		try {
			constructor = ((IDerivableContainerSymbol) classSymbol)
					.lookupConstructor(parameters);
		} catch (ParserSymbolTableException e1) {
			return false;
		}
		...
		if (constructor != null) {
			...
			try {
				reference = createReference(constructor, duple.toCharArray(),
	...
	private void purgeBadReferences(Kind kind, IASTExpression rhs) {
		if (rhs == null)
			return;
		if (kind == Kind.POSTFIX_ARROW_IDEXPRESSION
			...
			if (idExpression.length > 0) //$NON-NLS-1$
			{
				...
				for (int i = 0; i < size; i++) {
					...
					if (CharArrayUtils.equals(r.getNameCharArray(),
							idExpression)) {
						refs.remove(i--);
						size--;
					}
	 ...
	 */
	private ISymbol getExpressionSymbol(IASTScope scope, Kind kind,
			IASTExpression lhs, IASTExpression rhs, ITokenDuple idExpression,
		...
		if (idExpression != null)
			symbol = lookupQualifiedName(startingScope, idExpression,
					references, false);

		// If the expression is lookup symbol if it is in the scope of a type
		// after a "." or an "->"
		IContainerSymbol searchScope = getSearchScope(kind, lhs, startingScope);
		if (searchScope != null && !searchScope.equals(startingScope))
			symbol = lookupQualifiedName(searchScope, ((ASTIdExpression) rhs)
					.getIdExpressionTokenDuple(), references, false,
					LookupType.QUALIFIED);
		...
		if (kind == IASTExpression.Kind.POSTFIX_FUNCTIONCALL) {
			...
			IContainerSymbol functionScope = getSearchScope(lhs
					.getExpressionKind(), lhs.getLHSExpression(), startingScope);
			...
			if (functionScope.equals(startingScope))
				symbol = lookupQualifiedName(functionScope, functionId,
						ITypeInfo.t_function, parameters, references, false);
			else
				symbol = lookupQualifiedName(functionScope, functionId,
						ITypeInfo.t_function, parameters, references, false,
						LookupType.QUALIFIED);
	...
	private ITokenDuple getFunctionId(IASTExpression expression) {
		if (expression.getExpressionKind().isPostfixMemberReference()
				&& expression.getRHSExpression() instanceof ASTIdExpression)
			return ((ASTIdExpression) expression.getRHSExpression())
					.getIdExpressionTokenDuple();
		else if (expression instanceof ASTIdExpression)
			return ((ASTIdExpression) expression).getIdExpressionTokenDuple();
		return null;
	}

	private IContainerSymbol getSearchScope(Kind kind, IASTExpression lhs,
			IContainerSymbol startingScope) throws ASTSemanticException {
		if (kind.isPostfixMemberReference()) {
			ITypeInfo lhsInfo = ((ASTExpression) lhs).getResultType()
					.getResult();
			if (lhsInfo != null) {
				TypeInfoProvider provider = pst.getTypeInfoProvider();
				ITypeInfo info = null;
				...
				if (containingScope instanceof IDeferredTemplateInstance)
					return ((IDeferredTemplateInstance) containingScope)
							.getTemplate().getTemplatedSymbol();
				return (containingScope instanceof IContainerSymbol) ? (IContainerSymbol) containingScope
						: null;
			}
	 ...
	 */
	protected ITypeInfo conditionalExpressionConversions(ITypeInfo second,
			ITypeInfo third) {
		ITypeInfo info = null;
		...
		if ((second.getType() == ITypeInfo.t_void)
				&& (third.getType() != ITypeInfo.t_void)) {
			info = third;
			return info;
		}
		if ((second.getType() != ITypeInfo.t_void)
	...
	/*
	 * Apply the usual arithmetic conversions to find out the result of an
	 * expression that has a lhs and a rhs as indicated in the specs (section
	 * 5.Expressions, page 64)
	 */
	protected ITypeInfo usualArithmeticConversions(IASTScope scope,
			ASTExpression lhsExp, ASTExpression rhsExp, Kind kind)
			throws ASTSemanticException {
		...
		// if you have a variable of type basic type, then we need to go to the
		// basic type first
		while ((lhs.getType() == ITypeInfo.t_type)
				&& (lhs.getTypeSymbol() != null)) {
			if (!isLhsPointer)
				isLhsPointer = lhs.hasPtrOperators();
			lhs = lhs.getTypeSymbol().getTypeInfo();
		}
		while ((rhs.getType() == ITypeInfo.t_type)
				&& (rhs.getTypeSymbol() != null)) {
			if (!isRhsPointer)
				isRhsPointer = rhs.hasPtrOperators();
		...
		while (expLeftTest != null
				&& !(expLeftTest instanceof ASTEmptyExpression)) {
			expLeftTest = expLeftTest.getLHSExpression();
		}

		while (expRightTest != null
				&& !(expRightTest instanceof ASTEmptyExpression)) {
			expRightTest = expRightTest.getLHSExpression();
		}

		if ((expLeftTest != null && expLeftTest instanceof ASTEmptyExpression)
				|| (expRightTest != null && expRightTest instanceof ASTEmptyExpression))
			checkInvalidConversion = false;

		if (checkInvalidConversion) {
			// invalid arithmetic detection TODO add support for 4.5 Integral
			// Promotions/4.7 Integral Conversions if necessary
			// 5.6 Multiplicative Operators: The operands of * and / shall have
			// arithmetic or enumeration type; the operands of % shall have
			...
			if (kind == IASTExpression.Kind.MULTIPLICATIVE_MULTIPLY
				...
				if (!lhs.isType(ITypeInfo.t__Bool, ITypeInfo.t_enumerator))
					handleProblem(scope,
							IProblem.SEMANTIC_INVALID_CONVERSION_TYPE, null,
							lhsExp.getStartingOffset(), lhsExp
									.getEndingOffset(), lhsExp
									.getStartingLine(), true);
				if (!rhs.isType(ITypeInfo.t__Bool, ITypeInfo.t_enumerator))
					handleProblem(scope,
							IProblem.SEMANTIC_INVALID_CONVERSION_TYPE, null,
							rhsExp.getStartingOffset(), rhsExp
									.getEndingOffset(), rhsExp
									.getStartingLine(), true);
			} else if (kind == IASTExpression.Kind.MULTIPLICATIVE_MODULUS) {
				if (!(isIntegralType(lhs, isLhsPointer) || lhs.isType(
						ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))
					handleProblem(scope,
							IProblem.SEMANTIC_INVALID_CONVERSION_TYPE, null,
							lhsExp.getStartingOffset(), lhsExp
									.getEndingOffset(), lhsExp
									.getStartingLine(), true);
				if (!(isIntegralType(rhs, isRhsPointer) || rhs.isType(
						ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))
					handleProblem(scope,
							IProblem.SEMANTIC_INVALID_CONVERSION_TYPE, null,
							rhsExp.getStartingOffset(), rhsExp
				...
				// enumeration type, or one operand shall be a
				// pointer to a completely defined object type and the other
				// shall
				...
				// - the left operand is a pointer to a completely defined
				// object
				// type and the right operand has integral or
				// enumeration type.
			} else if (kind == IASTExpression.Kind.ADDITIVE_PLUS) {
				if (!((isLhsPointer && (isIntegralType(rhs, isRhsPointer) || rhs
						.isType(ITypeInfo.t__Bool, ITypeInfo.t_enumerator))) || (isRhsPointer && (isIntegralType(
						lhs, isRhsPointer) || lhs.isType(ITypeInfo.t__Bool,
						ITypeInfo.t_enumerator))))) {
					if (!(isIntegralType(lhs, isLhsPointer) || lhs.isType(
							ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))
						handleProblem(scope,
								IProblem.SEMANTIC_INVALID_CONVERSION_TYPE,
								null, lhsExp.getStartingOffset(), lhsExp
										.getEndingOffset(), lhsExp
										.getStartingLine(), true);
					if (!(isIntegralType(rhs, isRhsPointer) || rhs.isType(
							ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))
						handleProblem(scope,
								IProblem.SEMANTIC_INVALID_CONVERSION_TYPE,
								null, rhsExp.getStartingOffset(), rhsExp
										.getEndingOffset(), rhsExp
										.getStartingLine(), true);
				}
			} else if (kind == IASTExpression.Kind.ADDITIVE_MINUS) {
				if (!(isLhsPointer && (isIntegralType(rhs, isRhsPointer) || rhs
						.isType(ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))) {
					if (!(isIntegralType(lhs, isLhsPointer) || lhs.isType(
							ITypeInfo.t__Bool, ITypeInfo.t_enumerator)))
						handleProblem(scope,
								IProblem.SEMANTIC_INVALID_CONVERSION_TYPE,
				...
				// 5.11, 5.12, 5.13: The operator applies only to integral or
				// enumeration operands.
			} else if (kind == IASTExpression.Kind.ANDEXPRESSION
		...
		if ((lhs.checkBit(ITypeInfo.isLong) && lhs.getType() == ITypeInfo.t_double)
			...
			return info;
		} else if ((lhs.getType() == ITypeInfo.t_double)
				|| (rhs.getType() == ITypeInfo.t_double)) {
			info.setType(ITypeInfo.t_double);
			return info;
		} else if ((lhs.getType() == ITypeInfo.t_float)
		...
		if ((lhs.checkBit(ITypeInfo.isUnsigned) && lhs
				.checkBit(ITypeInfo.isLong))
				|| (rhs.checkBit(ITypeInfo.isUnsigned) && rhs
						.checkBit(ITypeInfo.isLong))) {
			info.setBit(true, ITypeInfo.isUnsigned);
			info.setBit(true, ITypeInfo.isLong);
			return info;
		} else if ((lhs.checkBit(ITypeInfo.isUnsigned) && rhs
				.checkBit(ITypeInfo.isLong))
				|| (rhs.checkBit(ITypeInfo.isUnsigned) && lhs
						.checkBit(ITypeInfo.isLong))) {
			info.setBit(true, ITypeInfo.isUnsigned);
			info.setBit(true, ITypeInfo.isLong);
			return info;
		} else if ((lhs.checkBit(ITypeInfo.isLong))
				|| (rhs.checkBit(ITypeInfo.isLong))) {
			info.setBit(true, ITypeInfo.isLong);
			return info;
		} else if ((lhs.checkBit(ITypeInfo.isUnsigned))
	...
	protected ExpressionResult getExpressionResultType(IASTScope scope,
			Kind kind, IASTExpression lhs, IASTExpression rhs,
			IASTExpression thirdExpression, IASTTypeId typeId, char[] literal,
		...
		if (kind.isBasicType()) {
			...
			if (kind == IASTExpression.Kind.PRIMARY_STRING_LITERAL
					|| (literal.length > 3 && (kind == IASTExpression.Kind.PRIMARY_CHAR_LITERAL || kind == IASTExpression.Kind.POSTFIX_SIMPLETYPE_CHAR))) {
				info.addPtrOperator(new ITypeInfo.PtrOp(
						ITypeInfo.PtrOp.t_pointer));
			}
		...
		if (kind == IASTExpression.Kind.UNARY_AMPSND_CASTEXPRESSION) {
			...
			if (left == null)
				handleProblem(scope, IProblem.SEMANTIC_MALFORMED_EXPRESSION,
						null);
			...
			if (info != null) {
				...
				info
						.applyOperatorExpression(ITypeInfo.OperatorExpression.addressof);
			} else
				handleProblem(scope, IProblem.SEMANTIC_MALFORMED_EXPRESSION,
						null);
		...
		if (kind == IASTExpression.Kind.UNARY_STAR_CASTEXPRESSION) {
			...
			if (info != null) {
				info = info.getFinalType(null);
				info
						.applyOperatorExpression(ITypeInfo.OperatorExpression.indirection);
			} else
				handleProblem(scope, IProblem.SEMANTIC_MALFORMED_EXPRESSION,
						null);
		...
		if (kind == IASTExpression.Kind.POSTFIX_SUBSCRIPT) {
			...
			if ((info != null)) {
				info = info.getFinalType(null);
				info
						.applyOperatorExpression(ITypeInfo.OperatorExpression.subscript);
			} else {
				handleProblem(scope, IProblem.SEMANTIC_MALFORMED_EXPRESSION,
						null);
			}
		...
		if (kind == IASTExpression.Kind.CONDITIONALEXPRESSION) {
			...
			if ((right != null) && (third != null)) {
				...
				if ((rightType != null) && (thirdType != null)) {
					info = conditionalExpressionConversions(rightType,
							thirdType);
				} else
					handleProblem(scope,
							IProblem.SEMANTIC_MALFORMED_EXPRESSION, null);
		...
		if (kind == IASTExpression.Kind.EXPRESSIONLIST) {
			...
			if (lhs != null) {
				ExpressionResult resultType = ((ASTExpression) lhs)
						.getResultType();
				if (resultType instanceof ExpressionResultList)
					((ExpressionResultList) result)
							.setResult((ExpressionResultList) resultType);
				else
					result.setResult(resultType.getResult());
			}
			if (rhs != null) {
				ITypeInfo rightType = ((ASTExpression) rhs).getResultType()
						.getResult();
		...
		if (kind == IASTExpression.Kind.POSTFIX_FUNCTIONCALL) {
			if (symbol != null) {
				...
				if (returnTypeSymbol != null) {
					info = TypeInfoProvider.newTypeInfo(returnTypeSymbol
							.getTypeInfo());
	...
	protected void getExpressionReferences(IASTExpression expression,
			List references) {
		if (expression != null) {
			...
			if (expression.getLHSExpression() != null)
				getExpressionReferences(expression.getLHSExpression(),
						references);
			if (expression.getRHSExpression() != null)
				getExpressionReferences(expression.getRHSExpression(),
						references);
		}
	}
	...
	public IASTNewExpressionDescriptor createNewDescriptor(
			List newPlacementExpressions, List newTypeIdExpressions,
			List newInitializerExpressions) {
		return new ASTNewDescriptor(newPlacementExpressions,
				newTypeIdExpressions, newInitializerExpressions);
	}
	...
	public IASTExceptionSpecification createExceptionSpecification(
			IASTScope scope, List typeIds) throws ASTSemanticException {
	...
	public IASTConstructorMemberInitializer createConstructorMemberInitializer(
		...
		if (duple != null) {
			try {
				symbol = lookupQualifiedName(scopeSymbol, duple, references,
						true);
			} catch (ASTSemanticException ase) {
				// use the class specifier's unresolved reference mechanism to
				// resolve these references.
				// TODO: resolve unresolved references in the expressionList
				// using
				// resolveLeftoverConstructorInitializerMembers
		...
		if (symbol != null) {
			createConstructorReference(symbol, (ASTExpression) expressionList,
					duple, references);
		}

		getExpressionReferences(expressionList, references);
		return new ASTConstructorMemberInitializer(expressionList,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createSimpleTypeSpecifier(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTSimpleTypeSpecifier.Type,
	...
	public IASTSimpleTypeSpecifier createSimpleTypeSpecifier(IASTScope scope,
			...
			Map extensionParms) throws ASTSemanticException {
		setProblemInfo(typeName);
		if (extension.overrideCreateSimpleTypeSpecifierMethod(kind))
			return extension.createSimpleTypeSpecifier(pst, scope, kind,
					typeName, isShort, isLong, isSigned, isUnsigned,
					isTypename, isComplex, isImaginary, isGlobal,
					extensionParms);
		...
		List references = (kind == Type.CLASS_OR_TYPENAME) ? new ArrayList(
				DEFAULT_QUALIFIEDNAME_REFERENCE_SIZE) : null;
		ISymbol s = pst.newSymbol(EMPTY_STRING, type);
		if (kind == IASTSimpleTypeSpecifier.Type.CLASS_OR_TYPENAME) {
			// lookup the duple
			IToken last = typeName.getLastToken();
			IToken current = null;
			ISymbol typeSymbol = getScopeToSearchUpon(scope, typeName
					.getFirstToken());
			...
			for (;;) {
				if (current == last)
					break;
				current = (current != null) ? current.getNext() : typeName
						.getFirstToken();
				...
				if (argLists != null && argLists[idx] != null) {
					if (current != last
							&& current.getNext().getType() == IToken.tLT)
						current = TokenFactory.consumeTemplateIdArguments(
				...
				while (!(typeSymbol instanceof IContainerSymbol)) {
					if (typeSymbol.getTypeInfo().checkBit(ITypeInfo.isTypedef)) {
						...
						typeSymbol = ((IDeferredTemplateInstance) typeSymbol)
								.getTemplate().getTemplatedSymbol();
					} else {
						handleProblem(IProblem.SEMANTIC_INVALID_TYPE, image,
								current.getOffset(), current.getEndOffset(),
								current.getLineNumber(), true);
					}
				}
				try {
					if (argLists != null && argLists[idx] != null)
						typeSymbol = ((IContainerSymbol) typeSymbol)
								.lookupTemplateId(image,
										getTemplateArgList(argLists[idx]));
					else if (current != last)
						typeSymbol = ((IContainerSymbol) typeSymbol)
								.lookupNestedNameSpecifier(image);
					else if (typeName.getSegmentCount() == 1) // a single
																// segment
						typeSymbol = ((IContainerSymbol) typeSymbol)
								.lookup(image);
					else
						typeSymbol = ((IContainerSymbol) typeSymbol)
								.qualifiedLookup(image);

					if (typeSymbol != null) {
						addReference(references, createReference(typeSymbol,
								image, offset));
						if (argLists != null && argLists[idx] != null) {
					...
					handleProblem(e.createProblemID(), image, typeName
							.getStartOffset(), typeName.getEndOffset(),
							typeName.getLineNumber(), true);
				}
			}
		...
		IASTSimpleTypeSpecifier result = new ASTSimpleTypeSpecifier(s, false,
				typeNameAsString, references);
		if (kind != Type.CLASS_OR_TYPENAME)
	 ...
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTemplate)
	 */
	public IASTFunction createFunction(IASTScope scope, ITokenDuple name,
		...
		if (name.getSegmentCount() > 1) {
			ISymbol symbol = lookupQualifiedName(ownerScope, name
					.getLeadingSegments(), references, false,
			...
			if (symbol instanceof IContainerSymbol)
				...
				parentScope = ((IDeferredTemplateInstance) symbol)
						.getTemplate().getTemplatedSymbol();

			if ((parentScope != null)
					&& ((parentScope.getType() == ITypeInfo.t_class)
							|| (parentScope.getType() == ITypeInfo.t_struct) || (parentScope
							.getType() == ITypeInfo.t_union))) {
				if (parentScope.getASTExtension().getPrimaryDeclaration() instanceof IASTElaboratedTypeSpecifier) {
					// we are trying to define a member of a class for which we
					// only
					// have a forward declaration
					handleProblem(scope, IProblem.SEMANTICS_RELATED, name
							.toCharArray(), startOffset, nameEndOffset,
							startLine, true);
				}
				IASTScope methodParentScope = (IASTScope) parentScope
						.getASTExtension().getPrimaryDeclaration();
				ITokenDuple newName = name.getLastSegment();
				return createMethod(methodParentScope, newName, parameters,
						returnType, exception, isInline, isFriend, isStatic,
						startOffset, startLine, newName.getFirstToken()
								.getOffset(), nameEndOffset, nameLine,
						ownerTemplate, isConst, isVolatile, isVirtual,
						isExplicit, isPureVirtual, ASTAccessVisibility.PRIVATE,
						constructorChain, references, isFunctionDefinition,
						hasFunctionTryBlock, hasVariableArguments);
			}
		}

		IParameterizedSymbol symbol = pst.newParameterizedSymbol(name
				.extractNameFromTemplateId(), ITypeInfo.t_function);
		...
		for (int i = 0; i < size; i++) {
			ASTParameterDeclaration param = (ASTParameterDeclaration) parameters
					.get(i);
			if (param.getSymbol() == null)
				handleProblem(IProblem.SEMANTICS_RELATED, param
						.getNameCharArray(), param.getNameOffset(), param
						.getEndingOffset(), param.getStartingLine(), true);
			functionParameters.add(param.getSymbol().getTypeInfo());
		}

		IParameterizedSymbol functionDeclaration = null;

		functionDeclaration = (IParameterizedSymbol) lookupQualifiedName(
				ownerScope, name.extractNameFromTemplateId(),
				ITypeInfo.t_function, functionParameters, 0, null, false,
				LookupType.FORDEFINITION);
		...
		if (previouslyDeclared == false || isFunctionDefinition) {
			try {
				...
				handleProblem(e.createProblemID(), name.toCharArray(),
						nameOffset, nameEndOffset, nameLine, true);
			}
		} else {
			symbol = functionDeclaration;
		}

		ASTFunction function = new ASTFunction(symbol, nameEndOffset,
				parameters, returnType, exception, startOffset, startLine,
				nameOffset, nameLine, ownerTemplate, references,
				previouslyDeclared, hasFunctionTryBlock, isFriend, filename);
	...
	protected void setFunctionTypeInfoBits(boolean isInline, boolean isFriend,
			boolean isStatic, IParameterizedSymbol symbol) {
	 ...
	 * @param iterator
	 */
	protected void setParameters(IParameterizedSymbol symbol, List references,
			List params) throws ASTSemanticException {
		int size = params.size();
		for (int i = 0; i < size; i++) {
			setParameter(symbol, (IASTParameterDeclaration) params.get(i),
					true, references);
		}
	}

	protected ITypeInfo getParameterTypeInfo(IASTAbstractDeclaration absDecl)
			throws ASTSemanticException {
		TypeInfoProvider provider = pst.getTypeInfoProvider();
		provider.beginTypeConstruction();
		if (absDecl.getTypeSpecifier() instanceof IASTSimpleTypeSpecifier) {
			IASTSimpleTypeSpecifier simpleType = ((IASTSimpleTypeSpecifier) absDecl
					.getTypeSpecifier());
			...
			provider.setType(ITypeInfo.t_type);
			provider.setTypeSymbol(((ASTClassSpecifier) absDecl
					.getTypeSpecifier()).getSymbol());
		} else if (absDecl.getTypeSpecifier() instanceof IASTEnumerationSpecifier) {
			provider.setType(ITypeInfo.t_type);
			provider.setTypeSymbol(((ASTEnumerationSpecifier) absDecl
					.getTypeSpecifier()).getSymbol());
		} else if (absDecl.getTypeSpecifier() instanceof IASTElaboratedTypeSpecifier) {
			provider.setType(ITypeInfo.t_type);
			provider.setTypeSymbol(((ASTElaboratedTypeSpecifier) absDecl
					.getTypeSpecifier()).getSymbol());
		}
		// else
		// assert false : this;
	 ...
	 * @param simpleType
	 */
	private void setTypeBitFlags(TypeInfoProvider provider,
			IASTSimpleTypeSpecifier simpleType) {
	 ...
	 * @param returnType
	 */
	protected void setParameter(IParameterizedSymbol symbol,
			IASTAbstractDeclaration absDecl, boolean isParameter,
			List references) throws ASTSemanticException {
		...
		int infoBits = 0;
		if (absDecl.getTypeSpecifier() instanceof IASTSimpleTypeSpecifier) {
			if (((IASTSimpleTypeSpecifier) absDecl.getTypeSpecifier())
					.getType() == IASTSimpleTypeSpecifier.Type.CLASS_OR_TYPENAME) {
				xrefSymbol = ((ASTSimpleTypeSpecifier) absDecl
						.getTypeSpecifier()).getSymbol();
				newReferences = ((ASTSimpleTypeSpecifier) absDecl
						.getTypeSpecifier()).getReferences();
			}

			infoBits = ((ASTSimpleTypeSpecifier) absDecl.getTypeSpecifier())
					.getSymbol().getTypeInfo().getTypeBits();
		} else if (absDecl.getTypeSpecifier() instanceof ASTElaboratedTypeSpecifier) {
			ASTElaboratedTypeSpecifier elab = (ASTElaboratedTypeSpecifier) absDecl
					.getTypeSpecifier();
			...
			if (xrefSymbol != null)
				addReference(newReferences, createReference(xrefSymbol, elab
						.getNameCharArray(), elab.getNameOffset()));
		}
		...
		paramSymbol.getTypeInfo().setBit(absDecl.isVolatile(),
				ITypeInfo.isVolatile);

		List ptrs = null, arrayMods = null;
		if (absDecl instanceof ASTParameterDeclaration) {
			ptrs = ((ASTParameterDeclaration) absDecl)
					.getPointerOperatorsList();
			arrayMods = ((ASTParameterDeclaration) absDecl)
					.getArrayModifiersList();
		} else {
			ptrs = ((ASTAbstractDeclaration) absDecl).getPointerOperatorsList();
			arrayMods = ((ASTAbstractDeclaration) absDecl)
					.getArrayModifiersList();
		}
	 ...
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExceptionSpecification,
	 *      boolean, boolean, boolean, int, int,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTemplate, boolean, boolean,
	...
	public IASTMethod createMethod(IASTScope scope, ITokenDuple name,
			...
			boolean hasVariableArguments) throws ASTSemanticException {
		return createMethod(scope, name, parameters, returnType, exception,
				isInline, isFriend, isStatic, startOffset, startLine,
				nameOffset, nameEndOffset, nameLine, ownerTemplate, isConst,
				isVolatile, isVirtual, isExplicit, isPureVirtual, visibility,
				constructorChain, null, isFunctionDefinition,
				hasFunctionTryBlock, hasVariableArguments);
	}

	public IASTMethod createMethod(IASTScope scope, ITokenDuple nameDuple,
			List parameters, IASTAbstractDeclaration returnType,
			IASTExceptionSpecification exception, boolean isInline,
			...
			boolean isVirtual, boolean isExplicit, boolean isPureVirtual,
			ASTAccessVisibility visibility, List constructorChain,
			List references, boolean isFunctionDefinition,
			boolean hasFunctionTryBlock, boolean hasVariableArguments)
			throws ASTSemanticException {
		...
		IContainerSymbol ownerScope = scopeToSymbol(ownerTemplate != null ? (IASTScope) ownerTemplate
				: scope);
		...
		if (references == null) {
			...
			if (nameDuple.length() > 2) // destructor
			{
				...
				ISymbol test = lookupQualifiedName(ownerScope, leadingSegments,
						references, false);
				if (test == ownerScope)
		...
		setFunctionTypeInfoBits(isInline, isFriend, isStatic, symbol);
		setMethodTypeInfoBits(symbol, isConst, isVolatile, isVirtual,
				isExplicit);
		symbol.setHasVariableArgs(hasVariableArguments);
		...
		if (scope instanceof IASTTemplateDeclaration) {
			classifier = (IASTClassSpecifier) ((IASTTemplateDeclaration) scope)
					.getOwnerScope();
		} else {
		...
		if (returnType.getTypeSpecifier() == null) {
			if (CharArrayUtils.indexOf(DOUBLE_COLON, parentName) != -1) {
				parentName = CharArrayUtils.lastSegment(parentName,
						DOUBLE_COLON);
			}
			if (CharArrayUtils.equals(parentName, methodName)) {
				isConstructor = true;
			} else if (methodName[0] == '~'
					&& CharArrayUtils.equals(methodName, 1,
							methodName.length - 1, parentName)) { //$NON-NLS-1$
				isDestructor = true;
			}
		}
		...
		if (isFunctionDefinition || isFriend) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (param.getSymbol() == null)
					handleProblem(IProblem.SEMANTICS_RELATED, param
							.getNameCharArray(), param.getNameOffset(), param
							.getEndingOffset(), param.getNameLineNumber(), true);
				functionParameters.add(param.getSymbol().getTypeInfo());
			}

			functionDeclaration = (IParameterizedSymbol) lookupQualifiedName(
					ownerScope, nameDuple,
					isConstructor ? ITypeInfo.t_constructor
							: ITypeInfo.t_function, functionParameters, null,
					false, isFriend ? LookupType.FORFRIENDSHIP
							: LookupType.FORDEFINITION);

			previouslyDeclared = (functionDeclaration != null)
					&& functionDeclaration
							.isType(isConstructor ? ITypeInfo.t_constructor
									: ITypeInfo.t_function);
			...
			if (isFriend) {
				...
				} else if (ownerScope.getContainingSymbol().isType(
						ITypeInfo.t_constructor)
						|| ownerScope.getContainingSymbol().isType(
								ITypeInfo.t_function)
						|| ownerScope.getContainingSymbol().isType(
								ITypeInfo.t_block)) {
					// only needs to be previously declared if we are in a local
					// class
					handleProblem(IProblem.SEMANTIC_ILLFORMED_FRIEND, nameDuple
							.toCharArray(), nameDuple.getStartOffset(),
							nameDuple.getEndOffset(),
							nameDuple.getLineNumber(), true);
				}
				...
				visibility = ((IASTMethod) (functionDeclaration
						.getASTExtension().getPrimaryDeclaration()))
						.getVisiblity();
			}
		}
		...
		try {
			if (isFriend) {
				if (functionDeclaration != null)
					((IDerivableContainerSymbol) ownerScope)
							.addFriend(functionDeclaration);
				else
				...
				if (templateIdArgList == null)
					...
					ownerScope.addTemplateId(symbol,
							getTemplateArgList(templateIdArgList));
			} else {
				symbol.setType(ITypeInfo.t_constructor);
				((IDerivableContainerSymbol) ownerScope).addConstructor(symbol);
			}
		} catch (ParserSymbolTableException e) {
			handleProblem(e.createProblemID(), nameDuple.toCharArray(),
					nameDuple.getStartOffset(), nameDuple.getEndOffset(),
					nameDuple.getLineNumber(), true);
		}

		resolveLeftoverConstructorInitializerMembers(symbol, constructorChain);

		ASTMethod method = new ASTMethod(symbol, parameters, returnType,
				exception, startOffset, startingLine, nameOffset,
				nameEndOffset, nameLine, ownerTemplate, references,
				previouslyDeclared, isConstructor, isDestructor, isPureVirtual,
				visibility, constructorChain, hasFunctionTryBlock, isFriend,
				filename);
		if (functionDeclaration != null && isFunctionDefinition)
			attachSymbolExtension(symbol, (ASTSymbol) functionDeclaration
					.getASTExtension().getPrimaryDeclaration(), false);
	 ...
	 */
	protected void resolveLeftoverConstructorInitializerMembers(
			IParameterizedSymbol symbol, List constructorChain)
			throws ASTSemanticException {
		if (constructorChain != null) {
			int size = constructorChain.size();
			for (int i = 0; i < size; i++) {
				IASTConstructorMemberInitializer initializer = (IASTConstructorMemberInitializer) constructorChain
						.get(i);
				if (initializer.getNameCharArray().length > 0
						&& initializer instanceof ASTConstructorMemberInitializer
						&& ((ASTConstructorMemberInitializer) initializer)
								.requiresNameResolution()) {
					ASTConstructorMemberInitializer realInitializer = ((ASTConstructorMemberInitializer) initializer);
					IDerivableContainerSymbol container = (IDerivableContainerSymbol) symbol
							.getContainingSymbol();
					lookupQualifiedName(container, realInitializer
							.getNameCharArray(), ITypeInfo.t_any, null,
							realInitializer.getNameOffset(), realInitializer
									.getReferences(), false,
							LookupType.QUALIFIED);
					// TODO try and resolve parameter references now in the
					// expression list
				}
			}
		}
	 ...
	 * @param isPureVirtual
	 */
	protected void setMethodTypeInfoBits(IParameterizedSymbol symbol,
			boolean isConst, boolean isVolatile, boolean isVirtual,
			boolean isExplicit) {
	...
	public IASTVariable createVariable(IASTScope scope, ITokenDuple name,
		...
		if (name == null)
			handleProblem(IProblem.SEMANTIC_NAME_NOT_PROVIDED, null,
					startingOffset, startingOffset + 1, startingLine, true);
		...
		if (name.getSegmentCount() > 1) {
			...
			if ((parentScope != null)
					&& ((parentScope.getType() == ITypeInfo.t_class)
							|| (parentScope.getType() == ITypeInfo.t_struct) || (parentScope
							.getType() == ITypeInfo.t_union))) {
				IASTScope fieldParentScope = (IASTScope) parentScope
						.getASTExtension().getPrimaryDeclaration();

				ITokenDuple newName = name.getLastSegment();

				return createField(fieldParentScope, newName, isAuto,
						initializerClause, bitfieldExpression,
						abstractDeclaration, isMutable, isExtern, isRegister,
						isStatic, startingOffset, startingLine, newName
								.getStartOffset(), nameEndOffset, nameLine,
						constructorExpression, ASTAccessVisibility.PRIVATE,
						references, fn);
			}
		}

		ISymbol newSymbol = cloneSimpleTypeSymbol(name.getFirstToken()
				.getCharImage(), abstractDeclaration, references);
		if (newSymbol == null)
			handleProblem(IProblem.SEMANTICS_RELATED, name.toCharArray());

		setVariableTypeInfoBits(isAuto, abstractDeclaration, isMutable,
				isExtern, isRegister, isStatic, newSymbol);

		int numPtrOps = ((ASTAbstractDeclaration) abstractDeclaration)
				.getNumArrayModifiers()
				+ ((ASTAbstractDeclaration) abstractDeclaration)
						.getNumPointerOperators();
		newSymbol.preparePtrOperatros(numPtrOps);
		setPointerOperators(newSymbol,
				((ASTAbstractDeclaration) abstractDeclaration)
						.getPointerOperatorsList(),
				((ASTAbstractDeclaration) abstractDeclaration)
						.getArrayModifiersList());
		...
		boolean previouslyDeclared = false;
		if (!isStatic) {
			ISymbol variableDeclaration = lookupQualifiedName(ownerScope, name
					.toCharArray(), null, false, LookupType.UNQUALIFIED);

			if (variableDeclaration != null
					&& newSymbol.getType() == variableDeclaration.getType()) {
				TypeInfoProvider provider = pst.getTypeInfoProvider();

				ITypeInfo newInfo = newSymbol.getTypeInfo().getFinalType(
						provider);
				ITypeInfo varInfo = variableDeclaration.getTypeInfo()
						.getFinalType(provider);
		...
		try {
			...
			handleProblem(e.createProblemID(), name.getFirstToken()
					.getCharImage(), name.getFirstToken().getOffset(), name
					.getFirstToken().getEndOffset(), name.getFirstToken()
					.getLineNumber(), true);
		}

		ASTVariable variable = new ASTVariable(newSymbol, abstractDeclaration,
				initializerClause, bitfieldExpression, startingOffset,
				startingLine, nameOffset, nameEndOffset, nameLine, references,
				constructorExpression, previouslyDeclared, filename);
		if (variable.getInitializerClause() != null) {
			variable.getInitializerClause().setOwnerVariableDeclaration(
					variable);
			addDesignatorReferences((ASTInitializerClause) variable
	...
	protected void addDesignatorReferences(ASTInitializerClause clause) {
		if (clause.getKind() == IASTInitializerClause.Kind.DESIGNATED_INITIALIZER_LIST
			...
			ISymbol variableSymbol = ((ASTVariable) clause
					.getOwnerVariableDeclaration()).getSymbol();
			ISymbol currentSymbol = variableSymbol.getTypeSymbol();
			if (currentSymbol == null)
				return;

			ITypeInfo currentTypeInfo = TypeInfoProvider
					.newTypeInfo(currentSymbol.getTypeInfo());
			...
			for (int i = 0; i < size; i++) {
				...
				if (designator.getKind() == IASTDesignator.DesignatorKind.FIELD) {
					...
					try {
						lookup = ((IContainerSymbol) currentSymbol)
								.lookup(designator.fieldNameCharArray());
					} catch (ParserSymbolTableException e) {
						break;
					}

					if (lookup == null
							|| lookup.getContainingSymbol() != currentSymbol)
						break;

					try {
						if (lookup != null)
							addReference(clause.getReferences(),
									createReference(lookup, designator
											.fieldNameCharArray(), designator
											.fieldOffset()));
					...
					currentTypeInfo = TypeInfoProvider.newTypeInfo(lookup
							.getTypeInfo());
		...
		if (clause.getKind() == IASTInitializerClause.Kind.DESIGNATED_INITIALIZER_LIST
			...
			for (int i = 0; i < size; i++)
				addDesignatorReferences((ASTInitializerClause) subInitializers
						.get(i));
		}
	}

	protected void setVariableTypeInfoBits(boolean isAuto,
			IASTAbstractDeclaration abstractDeclaration, boolean isMutable,
			boolean isExtern, boolean isRegister, boolean isStatic,
			ISymbol newSymbol) {
		...
		newSymbol.getTypeInfo().setBit(abstractDeclaration.isConst(),
				ITypeInfo.isConst);
		newSymbol.getTypeInfo().setBit(abstractDeclaration.isVolatile(),
				ITypeInfo.isVolatile);
	}

	protected ISymbol cloneSimpleTypeSymbol(char[] name,
			IASTAbstractDeclaration abstractDeclaration, List references)
			throws ASTSemanticException {
		...
		if (abstractDeclaration.getTypeSpecifier() instanceof ASTSimpleTypeSpecifier) {
			symbolToBeCloned = ((ASTSimpleTypeSpecifier) abstractDeclaration
					.getTypeSpecifier()).getSymbol();
			if (references != null) {
				List absRefs = ((ASTSimpleTypeSpecifier) abstractDeclaration
						.getTypeSpecifier()).getReferences();
			...
			symbolToBeCloned
					.setTypeSymbol(((ASTClassSpecifier) abstractDeclaration
							.getTypeSpecifier()).getSymbol());
		} else if (abstractDeclaration.getTypeSpecifier() instanceof ASTElaboratedTypeSpecifier) {
			ASTElaboratedTypeSpecifier elab = ((ASTElaboratedTypeSpecifier) abstractDeclaration
					.getTypeSpecifier());
			symbolToBeCloned = pst.newSymbol(name, ITypeInfo.t_type);
			symbolToBeCloned.setTypeSymbol(elab.getSymbol());
			if (elab.getSymbol() != null && references != null)
				addReference(references, createReference(elab.getSymbol(), elab
						.getNameCharArray(), elab.getNameOffset()));
		} else if (abstractDeclaration.getTypeSpecifier() instanceof ASTEnumerationSpecifier) {
			symbolToBeCloned = pst.newSymbol(name, ITypeInfo.t_type);
			symbolToBeCloned
					.setTypeSymbol(((ASTEnumerationSpecifier) abstractDeclaration
							.getTypeSpecifier()).getSymbol());
		}
	...
	public IASTField createField(IASTScope scope, ITokenDuple name,
			...
			throws ASTSemanticException {
		return createField(scope, name, isAuto, initializerClause,
				bitfieldExpression, abstractDeclaration, isMutable, isExtern,
				isRegister, isStatic, startingOffset, startingLine, nameOffset,
				nameEndOffset, nameLine, constructorExpression, visibility,
				null, fn);
	}
	...
	public IASTField createField(IASTScope scope, ITokenDuple name,
			...
			int nameEndOffset, int nameLine,
			IASTExpression constructorExpression,
			ASTAccessVisibility visibility, List references, char[] fn)
			throws ASTSemanticException {
		setFilename(fn);
		IContainerSymbol ownerScope = scopeToSymbol(scope);
		...
		if (references == null)
			references = new ArrayList();
		ISymbol newSymbol = cloneSimpleTypeSymbol(image, abstractDeclaration,
				references);
		if (newSymbol == null)
		...
		if (!isStatic && !image.equals(EMPTY_STRING)) {
			ISymbol fieldDeclaration = lookupQualifiedName(ownerScope, image,
					null, false, LookupType.FORDEFINITION);
			...
			if (fieldDeclaration != null
				...
				ITypeInfo newInfo = newSymbol.getTypeInfo().getFinalType(
						provider);
				ITypeInfo fieldInfo = fieldDeclaration.getTypeInfo()
						.getFinalType(provider);
				...
				if (newInfo.equals(fieldInfo)) {
					...
					// set the definition visibility = declaration visibility
					visibility = ((IASTField) fieldDeclaration
							.getASTExtension().getPrimaryDeclaration())
							.getVisiblity();
				}
				provider.returnTypeInfo(newInfo);
		...
		ASTField field = new ASTField(newSymbol, abstractDeclaration,
				initializerClause, bitfieldExpression, startingOffset,
				startingLine, nameOffset, nameEndOffset, nameLine, references,
				previouslyDeclared, constructorExpression, visibility, filename);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createTemplateDeclaration(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	...
	public IASTTemplateDeclaration createTemplateDeclaration(IASTScope scope,
			...
			int startingLine, char[] fn) throws ASTSemanticException {
		setFilename(fn);
		ITemplateSymbol template = pst
				.newTemplateSymbol(ParserSymbolTable.EMPTY_NAME_ARRAY);

		// the lookup requires a list of type infos
		// instead of a list of IASTParameterDeclaration
		int size = templateParameters.size();
		for (int i = 0; i < size; i++) {
			ASTTemplateParameter param = (ASTTemplateParameter) templateParameters
					.get(i);
			try {
				template.addTemplateParameter(param.getSymbol());
			} catch (ParserSymbolTableException e) {
				handleProblem(e.createProblemID(), param.getNameCharArray(),
						startingOffset, -1, startingLine, true);
			}
		}

		ASTTemplateDeclaration ast = new ASTTemplateDeclaration(template,
				scope, templateParameters, filename);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createTemplateParameter(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTemplateParameter.ParamKind,
	 *      java.lang.String, java.lang.String,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTParameterDeclaration,
	 *      java.util.List)
	 */
	public IASTTemplateParameter createTemplateParameter(ParamKind kind,
			char[] identifier, IASTTypeId defaultValue,
			IASTParameterDeclaration parameter, List parms,
			IASTCodeScope parameterScope, int startingOffset, int startingLine,
			int nameOffset, int nameEndOffset, int nameLine, int endingOffset,
			int endingLine, char[] fn) throws ASTSemanticException {
		// TODO filename
		ISymbol symbol = null;
		...
		if (defaultValue != null) {
			provider.setDefaultObj(((ASTTypeId) defaultValue).getTypeSymbol()
					.getTypeInfo());
		}
		if (kind == ParamKind.TEMPLATE_LIST) {
			...
			int size = parms.size();
			for (int i = 0; i < size; i++) {
				ASTTemplateParameter param = (ASTTemplateParameter) parms
						.get(i);
				try {
					template.addTemplateParameter(param.getSymbol());
				} catch (ParserSymbolTableException e) {
					handleProblem(e.createProblemID(),
							param.getNameCharArray(),
							param.getStartingOffset(), param.getEndingOffset(),
							param.getStartingLine(), true); //$NON-NLS-1$
				}
			}
			symbol = template;
		} else {
			...
			if (kind == ParamKind.CLASS || kind == ParamKind.TYPENAME) {
				...
				symbol = cloneSimpleTypeSymbol(
						((ASTParameterDeclaration) parameter)
								.getNameCharArray(), parameter, null);
		...
		IContainerSymbol codeScope = ((ASTCodeScope) parameterScope)
				.getContainerSymbol();
		try {
		...
		ASTTemplateParameter ast = new ASTTemplateParameter(symbol,
				defaultValue, parameter, parms, startingOffset, startingLine,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createTemplateInstantiation(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTFactory#createTemplateSpecialization(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      int)
	 */
	public IASTTemplateSpecialization createTemplateSpecialization(
		...
		ASTTemplateSpecialization ast = new ASTTemplateSpecialization(template,
				scope, fn);
		ast.setStartingOffsetAndLineNumber(startingOffset, startingLine);
		attachSymbolExtension(template, ast, false);
	 ...
	 *      java.lang.String,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTAbstractDeclaration, int, int)
	 */
	public IASTTypedefDeclaration createTypedef(IASTScope scope, char[] name,
		...
		if (typeSymbol == null)
			handleProblem(scope, IProblem.SEMANTICS_RELATED, name, nameOffset,
					nameEndOffset, nameLine, true);

		setPointerOperators(typeSymbol, ((ASTAbstractDeclaration) mapping)
				.getPointerOperatorsList(), ((ASTAbstractDeclaration) mapping)
				.getArrayModifiersList());
		...
		if (mapping.getTypeSpecifier() instanceof ASTSimpleTypeSpecifier) {
			List mappingReferences = ((ASTSimpleTypeSpecifier) mapping
					.getTypeSpecifier()).getReferences();
		...
		try {
			...
			handleProblem(e.createProblemID(), name, nameOffset, nameEndOffset,
					nameLine, true);
		}
		ASTTypedef d = new ASTTypedef(typeSymbol, mapping, startingOffset,
				startingLine, nameOffset, nameEndOffset, nameLine, references,
				filename);
	 ...
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTypeSpecifier,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTemplate, int, int)
	 */
	public IASTAbstractTypeSpecifierDeclaration createTypeSpecDeclaration(
		...
		return new ASTAbstractTypeSpecifierDeclaration(scopeToSymbol(scope),
				typeSpecifier, template, startingOffset, startingLine,
				endingOffset, endingLine, isFriend, fn);
	}

	public IASTElaboratedTypeSpecifier createElaboratedTypeSpecifier(
			IASTScope scope, ASTClassKind kind, ITokenDuple name,
			int startingOffset, int startingLine, int endOffset,
			int endingLine, boolean isForewardDecl, boolean isFriend)
			throws ASTSemanticException {
		setProblemInfo(name);
		IContainerSymbol currentScopeSymbol = scopeToSymbol(scope);
		...
		if (name.getSegmentCount() != 1) // qualified name
		{
			...
			if (containerSymbolName == null) {
				...
				currentScopeSymbol = currentScopeSymbol.getSymbolTable()
						.getCompilationUnit();
			} else {
				currentScopeSymbol = (IContainerSymbol) lookupQualifiedName(
						currentScopeSymbol, containerSymbolName, references,
						true);
			}
			if (currentScopeSymbol == null)
				handleProblem(IProblem.SEMANTIC_NAME_NOT_FOUND,
		...
		if (!isTemplateId) {
			try {
				if (isFriend
						&& isForewardDecl
						&& currentScopeSymbol instanceof IDerivableContainerSymbol) {
					checkSymbol = ((IDerivableContainerSymbol) currentScopeSymbol)
							.lookupForFriendship(newSymbolName);
				} else {
					if (!isForewardDecl)
						checkSymbol = currentScopeSymbol.elaboratedLookup(
								pstType, newSymbolName); // for using - goes
															// outside scope
															// with
					// filter
					else
						checkSymbol = currentScopeSymbol
								.lookupMemberForDefinition(newSymbolName,
										pstType); // for declaring - stays
													// inside
					// scope with filter
				}
			} catch (ParserSymbolTableException e) {
		...
		if (scope instanceof IASTTemplateInstantiation) {
			if (isTemplateId) {
				...
				try {
					...
					handleProblem(e.createProblemID(),
							nameToken.getCharImage(), nameToken.getOffset(),
							nameToken.getEndOffset(),
							nameToken.getLineNumber(), true);
				}
			} else {
			...
			try {
				if (isFriend && isForewardDecl) {
					((IDerivableContainerSymbol) originalScope)
		...
		if (checkSymbol != null) {
			...
			if (checkSymbol instanceof ITemplateSymbol) {
				checkSymbol = ((ITemplateSymbol) checkSymbol)
						.getTemplatedSymbol();
			}
	...
	public IASTNamespaceAlias createNamespaceAlias(IASTScope scope,
			...
			int startingLine, int nameOffset, int nameEndOffset, int nameLine,
			int endOffset, int endingLine) throws ASTSemanticException {
		setProblemInfo(alias);
		IContainerSymbol startingSymbol = scopeToSymbol(scope);
		List references = new ArrayList();

		ISymbol namespaceSymbol = lookupQualifiedName(startingSymbol, alias,
				references, true);

		if (namespaceSymbol.getType() != ITypeInfo.t_namespace)
			handleProblem(IProblem.SEMANTIC_INVALID_OVERLOAD, alias
					.toCharArray(), startingOffset, endOffset, startingLine,
					true);

		ISymbol newSymbol = pst.newContainerSymbol(identifier,
				ITypeInfo.t_namespace);
		...
		try {
			...
			handleProblem(e.createProblemID(), identifier, startingOffset,
					endOffset, startingLine, true);
		}

		ASTNamespaceAlias astAlias = new ASTNamespaceAlias(newSymbol, alias
				.toCharArray(), (IASTNamespaceDefinition) namespaceSymbol
				.getASTExtension().getPrimaryDeclaration(), startingOffset,
				startingLine, nameOffset, nameEndOffset, nameLine, endOffset,
				endingLine, references, filename);
	...
	public IASTCodeScope createNewCodeBlock(IASTScope scope) {
		...
		IContainerSymbol newScope = pst.newContainerSymbol(EMPTY_STRING,
				ITypeInfo.t_block);
	...
	public IASTScope getDeclaratorScope(IASTScope scope, ITokenDuple duple) {
		if (duple != null && duple.getSegmentCount() > 1) {
			...
			try {
				symbol = lookupQualifiedName(ownerScope, duple
						.getLeadingSegments(), null, false,
						LookupType.FORDEFINITION);
			} catch (ASTSemanticException e) {
				return scope;
			}
			...
			if (parentScope != null && parentScope.getASTExtension() != null) {
				if (scope instanceof IASTTemplateDeclaration
						|| scope instanceof IASTTemplateSpecialization) {
				...
				return (IASTScope) parentScope.getASTExtension()
						.getPrimaryDeclaration();
			}
		}
	...
	public boolean queryIsTypeName(IASTScope scope, ITokenDuple nameInQuestion) {
		...
		try {
			lookupSymbol = lookupQualifiedName(scopeToSymbol(scope),
					nameInQuestion, null, false);
		} catch (ASTSemanticException e) {
			// won't get thrown
		}
		...
		if (lookupSymbol.isType(ITypeInfo.t_type, ITypeInfo.t_enumeration)
				|| (lookupSymbol.isType(ITypeInfo.t_templateParameter) && lookupSymbol
						.getTypeInfo().getTemplateParameterType() == ITypeInfo.t_typeName)
				|| (lookupSymbol.getASTExtension() != null && lookupSymbol
						.getASTExtension().getPrimaryDeclaration() instanceof IASTTypedefDeclaration)) {
			return true;
		}
		return false;
	}
	...
	public IASTParameterDeclaration createParameterDeclaration(boolean isConst,
			...
			int endingOffset, int endingLine, char[] fn) {
		setFilename(fn);
		return new ASTParameterDeclaration(null, isConst, isVolatile,
				typeSpecifier, pointerOperators, arrayModifiers, parameters,
				pointerOp, parameterName, initializerClause, startingOffset,
				startingLine, nameOffset, nameEndOffset, nameLine,
				endingOffset, endingLine, filename);
	}
	...
	public IASTTypeId createTypeId(IASTScope scope, Type kind, boolean isConst,
			...
			char[] completeSignature) throws ASTSemanticException {
		if (kind != Type.CLASS_OR_TYPENAME) {
			IASTTypeId check = (IASTTypeId) typeIdCache.get(completeSignature);
			if (check != null)
				return check;
		}
		ASTTypeId result = new ASTTypeId(kind, name, pointerOps, arrayMods,
				completeSignature, isConst, isVolatile, isUnsigned, isSigned,
				isShort, isLong, isTypename);
	...
	public static ITypeInfo.eType getTypeKind(IASTTypeId id) {
		...
		else if (id.isShort() || id.isLong() || id.isUnsigned()
				|| id.isSigned())
			return ITypeInfo.t_int;
		else
	...
	protected ISymbol createSymbolForTypeId(IASTScope scope, IASTTypeId id)
			throws ASTSemanticException {
		if (id == null)
			return null;

		ASTTypeId typeId = (ASTTypeId) id;
		ISymbol result = pst.newSymbol(EMPTY_STRING, CompleteParseASTFactory
				.getTypeKind(id));
		...
		if (result.getType() == ITypeInfo.t_type) {
			ISymbol typeSymbol = lookupQualifiedName(scopeToSymbol(scope),
					typeId.getTokenDuple(), refs, true);
			if (typeSymbol == null /*
									 * || typeSymbol.getType() ==
									 * TypeInfo.t_type
									 */) {
				freeReferences(refs);
				handleProblem(scope, IProblem.SEMANTIC_INVALID_TYPE, typeId
						.getTypeOrClassNameCharArray());
			}
			result.setTypeSymbol(typeSymbol);
			typeId.addReferences(refs);
		}

		setPointerOperators(result, ((ASTTypeId) id).getPointerOperatorsList(),
				((ASTTypeId) id).getArrayModifiersList());
		return result;
	}
	...
	public void signalEndOfClassSpecifier(IASTClassSpecifier astClassSpecifier) {
		...
		try {
			((IDerivableContainerSymbol) (astImplementation).getSymbol())
					.addCopyConstructor();
		} catch (ParserSymbolTableException e) {
		...
		for (int i = 0; i < size; i++) {
			UnresolvedReferenceDuple duple = (UnresolvedReferenceDuple) unresolved
					.get(i);

			try {
				lookupQualifiedName(duple.getScope(), duple.getName(),
						references, false);
			} catch (ASTSemanticException ase) {
			}
		}
	...
	public IASTInitializerClause createInitializerClause(IASTScope scope,
			...
			List designators) {
		return new ASTInitializerClause(kind, assignmentExpression,
	...
	public IASTNode lookupSymbolInContext(IASTScope scope, ITokenDuple duple,
		...
		if (reference == null) {
			try {
				s = lookupQualifiedName(scopeToSymbol(scope), duple, null,
						false);
			} catch (ASTSemanticException e) {
			}
		} else {
			if (reference instanceof ASTExpression) {
				...
				if (expression.getExpressionKind() == IASTExpression.Kind.ID_EXPRESSION
						&& CharArrayUtils.equals(expression
								.getIdExpressionCharArray(), dupleAsCharArray)) {
					try {
						s = lookupQualifiedName(scopeToSymbol(scope), duple,
								null, false);
					} catch (ASTSemanticException e1) {
					}
					...
					s = lookupSymbolInNewExpression(scope, duple, expression);
						...
						&& CharArrayUtils.equals(expression.getLHSExpression()
								.getIdExpressionCharArray(), dupleAsCharArray)) {
					try {
						ISymbol symbol = getExpressionSymbol(scope, expression
								.getExpressionKind(), expression
								.getLHSExpression(), expression
								.getRHSExpression(), null, null);
						if (symbol == null)
							return null;
					...
					ASTExpression ownerExpression = expression
							.findOwnerExpressionForIDExpression(duple);
					if (ownerExpression == null)
						return null;
					if (ownerExpression.getExpressionKind()
							.isPostfixMemberReference()) {
						try {
							s = lookupQualifiedName(getSearchScope(
									ownerExpression.getExpressionKind(),
									ownerExpression.getLHSExpression(),
									scopeToSymbol(scope)), duple, null, false);
						} catch (ASTSemanticException e) {
							return null;
						}
						...
						s = lookupSymbolInNewExpression(scope, duple,
								ownerExpression);
					} else {
						try {
							s = lookupQualifiedName(scopeToSymbol(scope),
									duple, null, false);
	...
	public ISymbol lookupSymbolInNewExpression(IASTScope scope,
			ITokenDuple duple, ASTExpression expression) {
		IContainerSymbol classSymbol = null;
		try {
			classSymbol = (IContainerSymbol) lookupQualifiedName(
					scopeToSymbol(scope), duple, null, false);
		} catch (ASTSemanticException e) {
			return null;
		}
		if (classSymbol != null
				&& classSymbol.getTypeInfo().checkBit(ITypeInfo.isTypedef)) {
			ITypeInfo info = classSymbol.getTypeInfo().getFinalType(
					pst.getTypeInfoProvider());
		...
		List parameters = new ArrayList();
		ASTNewDescriptor newDescriptor = (ASTNewDescriptor) expression
				.getNewExpressionDescriptor();
		List newInitializerExpressions = newDescriptor
				.getNewInitializerExpressionsList();
		...
		try {
			return ((IDerivableContainerSymbol) classSymbol)
					.lookupConstructor(parameters);
		} catch (ParserSymbolTableException e1) {
	...
	public IASTNode expressionToMostPreciseASTNode(IASTScope scope,
		...
		if (expression.getExpressionKind() == IASTExpression.Kind.ID_EXPRESSION) {
			if (expression instanceof ASTExpression) {
				try {
					return lookupSymbolInContext(scope,
							((ASTIdExpression) expression)
									.getIdExpressionTokenDuple(), null);
				} catch (ASTNotImplementedException e) {
					// assert false : e;
				}
			}
	...
	public boolean validateIndirectMemberOperation(IASTNode node) {
		...
		if (pointerOps == null || pointerOps.isEmpty())
			return false;
		ITypeInfo.PtrOp lastOperator = (ITypeInfo.PtrOp) pointerOps
				.get(pointerOps.size() - 1);
		if (lastOperator.getType() == ITypeInfo.PtrOp.t_array
				|| lastOperator.getType() == ITypeInfo.PtrOp.t_pointer)
			return true;
		return false;
	}
	...
	public boolean validateDirectMemberOperation(IASTNode node) {
		...
		ITypeInfo.PtrOp lastOperator = (ITypeInfo.PtrOp) pointerOps
				.get(pointerOps.size() - 1);
		if (lastOperator.getType() == ITypeInfo.PtrOp.t_reference)
			return true;
		return false;
	}
	...
	private boolean isIntegralType(ITypeInfo info, boolean isPointer) {
		if (isPointer)
			return true;
		if (info.getType() == ITypeInfo.t_bool
				|| info.getType() == ITypeInfo.t_char
				|| info.getType() == ITypeInfo.t_wchar_t
				|| info.getType() == ITypeInfo.t_int)
			return true;
