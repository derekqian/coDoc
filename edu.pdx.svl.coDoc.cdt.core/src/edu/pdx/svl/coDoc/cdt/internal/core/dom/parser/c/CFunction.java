 ...
 * @author aniefer
 */
public class CFunction extends PlatformObject implements IFunction,
		ICInternalFunction {
	...
	public void addDeclarator(IASTFunctionDeclarator fnDeclarator) {
		...
		if (fnDeclarator.getParent() instanceof IASTFunctionDefinition
				|| fnDeclarator instanceof ICASTKnRFunctionDeclarator)
	...
	public IParameter[] getParameters() {
		...
		if (dtor instanceof IASTStandardFunctionDeclarator) {
			IASTParameterDeclaration[] params = ((IASTStandardFunctionDeclarator) dtor)
					.getParameters();
			...
			IASTName[] names = ((ICASTKnRFunctionDeclarator) dtor)
					.getParameterNames();
			...
			if (names.length > 0) {
				...
				for (int i = 0; i < names.length; i++) {
					...
					if (decl != null) {
						result[i] = (IParameter) decl.getName()
								.resolveBinding();
					} else {
						result[i] = new CParameter.CParameterProblem(
								names[i],
	...
	public String getName() {
		IASTFunctionDeclarator dtor = (definition != null) ? definition
				: declarators[0];
	...
	public IScope getFunctionScope() {
		if (definition != null) {
			IASTFunctionDefinition def = (IASTFunctionDefinition) definition
					.getParent();
			return def.getScope();
		}
		return null;
	...
	public IBinding resolveParameter(IASTName paramName) {
		...
		while (parent instanceof IASTDeclarator
				&& !(parent instanceof ICASTKnRFunctionDeclarator))
			parent = parent.getParent();

		ICASTKnRFunctionDeclarator fKnRDtor = null;
		IASTDeclarator knrParamDtor = null;
		if (parent instanceof IASTParameterDeclaration) {
			IASTStandardFunctionDeclarator fdtor = (IASTStandardFunctionDeclarator) parent
					.getParent();
			...
			knrParamDtor = CVisitor.getKnRParameterDeclarator(fKnRDtor,
					paramName);
		...
		if (definition != null) {
			if (definition instanceof IASTStandardFunctionDeclarator) {
				...
				if (parameterNames.length > idx) {
					...
					IASTDeclarator dtor = CVisitor.getKnRParameterDeclarator(
							fKnRDtor, n);
					if (dtor != null) {
	...
	protected void updateParameterBindings(IASTFunctionDeclarator fdtor) {
		...
		if (fdtor instanceof IASTStandardFunctionDeclarator) {
			IASTParameterDeclaration[] nps = ((IASTStandardFunctionDeclarator) fdtor)
					.getParameters();
			...
			IASTName[] ns = ((ICASTKnRFunctionDeclarator) fdtor)
					.getParameterNames();
			...
			for (int i = 0; i < params.length; i++) {
				...
				name.setBinding(params[i]);
				IASTDeclarator dtor = CVisitor.getKnRParameterDeclarator(
						(ICASTKnRFunctionDeclarator) fdtor, name);
				if (dtor != null) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IFunction#isInline()
	 */
	...
	public boolean takesVarArgs() {
		...
		if (definition != null) {
			if (definition instanceof IASTStandardFunctionDeclarator)
				return ((IASTStandardFunctionDeclarator) definition)
						.takesVarArgs();
			return false;
		}
