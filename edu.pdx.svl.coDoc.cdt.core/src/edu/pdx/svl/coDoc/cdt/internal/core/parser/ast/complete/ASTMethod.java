...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.TypeFilter;
...
public class ASTMethod extends ASTFunction implements IASTMethod {
	 ...
	 * @param filename
	 */
	public ASTMethod(IParameterizedSymbol symbol, List parameters,
			IASTAbstractDeclaration returnType,
			IASTExceptionSpecification exception, int startOffset,
			int startLine, int nameOffset, int nameEndOffset, int nameLine,
			IASTTemplate ownerTemplate, List references,
			boolean previouslyDeclared, boolean isConstructor,
			boolean isDestructor, boolean isPureVirtual,
			ASTAccessVisibility visibility, List constructorChain,
			boolean hasFunctionTryBlock, boolean isFriend, char[] filename) {
		super(symbol, nameEndOffset, parameters, returnType, exception,
				startOffset, startLine, nameOffset, nameLine, ownerTemplate,
				references, previouslyDeclared, hasFunctionTryBlock, isFriend,
				filename);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#isVirtual()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#isExplicit()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#isConstructor()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#isDestructor()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#isConst()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#isVolatile()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#isPureVirtual()
	 */
	...
	protected void processConstructorChain(ISourceElementRequestor requestor) {
		if (constructorChain != null) {
			...
			for (int i = 0; i < initializers.size(); i++) {
				IASTConstructorMemberInitializer c = (IASTConstructorMemberInitializer) initializers
						.get(i);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#getConstructorChainInitializers()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTMethod#getOwnerClassSpecifier()
	 */
	public IASTClassSpecifier getOwnerClassSpecifier() {
		if (getOwnerScope() instanceof IASTTemplateDeclaration)
			return (IASTClassSpecifier) ((IASTTemplateDeclaration) getOwnerScope())
					.getOwnerScope();
	 ...
	 * @throws LookupError
	 */
	protected List performPrefixLookup(char[] prefix,
			IContainerSymbol thisContainer, IContainerSymbol qualification,
			TypeFilter filter, List paramList) throws LookupError {
		if (filter.isLookingInThis()) {
			try {
				ISymbol thisPointer = thisContainer
						.lookup(ParserSymbolTable.THIS);
				ISymbol thisClass = (thisPointer != null) ? thisPointer
						.getTypeSymbol() : null;
				if (thisClass != null && thisClass instanceof IContainerSymbol) {
					return ((IContainerSymbol) thisClass).prefixLookup(filter,
							prefix, true, paramList);
				}
			...
			return super.performPrefixLookup(prefix, thisContainer,
					qualification, filter, paramList);
		}
