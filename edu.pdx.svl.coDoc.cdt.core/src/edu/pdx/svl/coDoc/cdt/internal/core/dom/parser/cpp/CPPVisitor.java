...
public class CPPVisitor {
	...
	public static IBinding createBinding(IASTName name) {
		...
		if (parent instanceof IASTIdExpression) {
			...
			return CPPTemplates
					.createBinding((ICPPASTTemplateParameter) parent);
		}
	...
	private static IBinding createBinding(
			ICPPASTElaboratedTypeSpecifier elabType) {
		...
		if (parent instanceof IASTSimpleDeclaration) {
			IASTDeclarator[] dtors = ((IASTSimpleDeclaration) parent)
					.getDeclarators();
			ICPPASTDeclSpecifier declSpec = (ICPPASTDeclSpecifier) ((IASTSimpleDeclaration) parent)
					.getDeclSpecifier();
		...
		if (binding != null
				&& (!(binding instanceof IProblemBinding) || ((IProblemBinding) binding)
						.getID() != IProblemBinding.SEMANTIC_NAME_NOT_FOUND)) {
			return binding;
		}

		// 7.1.5.3-2 ... If name lookup does not find a declaration for the
		// name, the elaborated-type-specifier is ill-formed
		...
		if (mustBeSimple) {
			// 3.3.1-5 ... the identifier is declared in the smallest non-class
			// non-function-prototype scope that contains
			// the declaration
			while (scope instanceof ICPPClassScope
					|| scope instanceof ICPPFunctionScope) {
				try {
	...
	private static IBinding createBinding(ICPPASTCompositeTypeSpecifier compType) {
		...
		try {
			if (name.toCharArray().length > 0 && scope != null) // can't lookup
																// anonymous
																// things
	...
	private static IBinding createBinding(IASTDeclaration declaration) {
		if (declaration instanceof ICPPASTNamespaceDefinition) {
			...
			return CPPSemantics
					.resolveBinding(((ICPPASTUsingDirective) declaration)
							.getQualifiedName());
			...
			try {
				...
				if (binding == null) {
					IBinding namespace = alias.getMappingName()
							.resolveBinding();
					if (namespace instanceof IProblemBinding) {
						IProblemBinding problem = (IProblemBinding) namespace;
						namespace = new CPPNamespace.CPPNamespaceProblem(
								problem.getASTNode(), problem.getID(), alias
										.getMappingName().toCharArray());
					}
					if (namespace instanceof ICPPNamespace) {
						binding = new CPPNamespaceAlias(alias.getAlias(),
								(ICPPNamespace) namespace);
						scope.addName(alias.getAlias());
					} else {
						binding = new ProblemBinding(alias.getAlias(),
								IProblemBinding.SEMANTIC_NAME_NOT_FOUND, alias
										.getAlias().toCharArray());
					}
				}
			} catch (DOMException e) {
				binding = e.getProblem();
	...
	private static IBinding createBinding(IASTDeclarator declarator) {
		...
		IASTSimpleDeclaration simpleDecl = (parent instanceof IASTSimpleDeclaration) ? (IASTSimpleDeclaration) parent
				: null;
		if (parent instanceof ICPPASTParameterDeclaration) {
			ICPPASTParameterDeclaration param = (ICPPASTParameterDeclaration) parent;
			parent = param.getParent();
			if (parent instanceof IASTStandardFunctionDeclarator) {
				IASTStandardFunctionDeclarator fDtor = (IASTStandardFunctionDeclarator) param
						.getParent();
				if ( /* fDtor.getParent() instanceof IASTDeclarator || */fDtor
						.getNestedDeclarator() != null)
					return null;
				IBinding temp = fDtor.getName().resolveBinding();
				if (temp instanceof ICPPInternalFunction) {
					binding = ((ICPPInternalFunction) temp)
			...
			if (binding != null && binding instanceof ITypedef) {
				...
				return new ProblemBinding(name,
						IProblemBinding.SEMANTIC_INVALID_REDECLARATION, name
								.toCharArray());
			}
			...
			if (scope instanceof ICPPClassScope) {
				if (isConstructor(scope, declarator))
					binding = template ? (ICPPConstructor) new CPPConstructorTemplate(
							name)
							: new CPPConstructor(
									(ICPPASTFunctionDeclarator) declarator);
				else
					binding = template ? (ICPPMethod) new CPPMethodTemplate(
							name) : new CPPMethod(
							(ICPPASTFunctionDeclarator) declarator);
			} else {
				binding = template ? (ICPPFunction) new CPPFunctionTemplate(
						name) : new CPPFunction(
						(ICPPASTFunctionDeclarator) declarator);
			}
			...
			if (t1 != null && t2 != null) {
				if (t1.isSameType(t2)) {
					...
					binding = new ProblemBinding(name,
							...
							declarator.getName().toCharArray());
				}
	...
	public static boolean isConstructor(IScope containingScope,
			IASTDeclarator declarator) {
		if (containingScope == null
				|| !(containingScope instanceof ICPPClassScope))
			return false;

		ICPPASTCompositeTypeSpecifier clsTypeSpec;
		try {
			IASTNode node = ((ICPPClassScope) containingScope)
					.getPhysicalNode();
	...
	public static boolean isConstructor(IASTName parentName,
			IASTDeclarator declarator) {
		if (declarator == null
		...
		if (!CharArrayUtils
				.equals(name.toCharArray(), parentName.toCharArray()))
			return false;
	...
	public static IScope getContainingScope(IASTNode node) {
		...
		while (node != null) {
			if (node instanceof IASTName
					&& !(node instanceof ICPPASTQualifiedName)) {
			...
			if (node instanceof IASTDeclaration) {
				...
				if (node.getPropertyInParent() == ICPPASTTemplateId.TEMPLATE_ID_ARGUMENT) {
					ICPPASTTemplateDeclaration decl = CPPTemplates
							.getTemplateDeclaration((IASTName) node.getParent());
					if (decl == null) {
						node = node.getParent();
				...
				if (parent instanceof IASTForStatement) {
					...
					if (temp instanceof IASTFunctionDefinition) {
						...
						|| parent instanceof IASTInitializer) {
					...
					IASTName name = dtor.getName();
					if (name instanceof ICPPASTQualifiedName) {
						IASTName[] ns = ((ICPPASTQualifiedName) name)
								.getNames();
						return getContainingScope(ns[ns.length - 1]);
					}
				}
				...
				ICPPASTCompositeTypeSpecifier compSpec = (ICPPASTCompositeTypeSpecifier) node
						.getParent();
	...
	public static IScope getContainingScope(IASTName name) {
		...
		try {
			...
			if (parent instanceof ICPPASTQualifiedName) {
				...
				if (i > 0) {
					...
					if (binding instanceof ICPPClassType) {
						...
						return ((ICPPInternalUnknown) binding)
								.getUnknownScope();
				...
				IASTExpression owner = ((ICPPASTFieldReference) parent)
						.getFieldOwner();
				...
				if (((ICPPASTFieldReference) parent).isPointerDereference()) {
					...
					if (type instanceof ICPPClassType) {
						ICPPFunction op = CPPSemantics.findOperator(
								(IASTFieldReference) parent,
								(ICPPClassType) type);
						if (op != null) {
			...
			if (problem instanceof ICPPScope)
				return problem;
			return new CPPScope.CPPScopeProblem(problem.getASTNode(), problem
					.getID(), problem.getNameCharArray());
		}
	...
	public static IASTNode getContainingBlockItem(IASTNode node) {
		if (node == null)
			return null;
		if (node.getPropertyInParent() == CPPSemantics.STRING_LOOKUP_PROPERTY)
			return null;
		IASTNode parent = node.getParent();
		if (parent == null)
			return null;
	...
	static private IBinding resolveBinding(IASTNode node) {
		...
		while (node != null) {
			if (node instanceof IASTIdExpression) {
				...
				node = ((IASTFunctionCallExpression) node)
		...
		if (name != null) {
			...
			if (binding == null) {
				...
				if (name instanceof ICPPASTTemplateId
						&& binding instanceof ICPPSpecialization) {
					((ICPPASTTemplateId) name).getTemplateName().setBinding(
							((ICPPSpecialization) binding)
		...
		public int visit(IASTName name) {
			...
			case KIND_COMPOSITE:
				if (prop == IASTCompositeTypeSpecifier.TYPE_NAME
					...
					IASTNode p = name.getParent().getParent();
					if (p instanceof IASTSimpleDeclaration) {
						IASTDeclSpecifier declSpec = ((IASTSimpleDeclaration) p)
								.getDeclSpecifier();
			...
			if (binding != null) {
				...
				while (candidate != null) {
					...
					if (binding instanceof ICPPUsingDeclaration) {
						...
						try {
							delegates = ((ICPPUsingDeclaration) binding)
									.getDelegates();
						} catch (DOMException e1) {
						}
						if (delegates != null) {
						...
						// found = ( ((ICPPDelegate)candidate).getBinding() ==
						// binding );
						// else
		...
		public int visit(IASTName name) {
			...
			if (binding != null) {
				...
				while (candidate != null) {
					...
					if (binding instanceof ICPPUsingDeclaration) {
						try {
							found = ArrayUtil.contains(
									((ICPPUsingDeclaration) binding)
											.getDelegates(), candidate);
						} catch (DOMException e) {
						}
					} else if (potential instanceof ICPPUsingDeclaration) {
						found = (binding == ((ICPPDelegate) candidate)
								.getBinding());
					} else {
						found = (binding == candidate);
					}
	...
	/**
	 * Generate a function type for an implicit function. NOTE: This does not
	 * currectly handle parameters with typedef types.
	 * 
	 * @param returnType
	 * @param parameters
	 * @return
	...
	public static IFunctionType createImplicitFunctionType(IType returnType,
		...
		for (int i = 0; i < parameters.length; i++) {
			...
			if (lastType instanceof IArrayType) {
				try {
					lastType = new CPPPointerType(((IArrayType) lastType)
							.getType());
				} catch (DOMException e1) {
	...
	private static IType createType(IType returnType,
			ICPPASTFunctionDeclarator fnDtor) {
		...
		for (int i = 0; i < params.length; i++) {
			...
			// so only create the base type from the declspec and not the
			// qualifiers
			...
			// any parameter to type function returning T is adjusted to be
			// pointer to function
		...
		IType type = new CPPFunctionType(returnType, pTypes, fnDtor.isConst(),
				fnDtor.isVolatile());
	...
	private static IType getPointerTypes(IType type, IASTDeclarator declarator) {
		...
		for (int i = 0; i < ptrOps.length; i++) {
			if (ptrOps[i] instanceof IGPPASTPointerToMember)
				type = new GPPPointerToMemberType(type,
						(IGPPASTPointerToMember) ptrOps[i]);
			else if (ptrOps[i] instanceof ICPPASTPointerToMember)
				type = new CPPPointerToMemberType(type,
						(ICPPASTPointerToMember) ptrOps[i]);
	...
	private static IType getArrayTypes(IType type,
	...
	public static IType createType(IASTDeclSpecifier declSpec) {
		...
		if (type != null && (declSpec.isConst() || declSpec.isVolatile())) {
			type = new CPPQualifierType(type, declSpec.isConst(), declSpec
					.isVolatile());
		}
		return type;
	}
	...
	private static IType getBaseType(IASTDeclSpecifier declSpec) {
		...
		if (declSpec instanceof ICPPASTCompositeTypeSpecifier) {
			...
			if (spec instanceof IGPPASTSimpleDeclSpecifier) {
				...
				if (gspec.getTypeofExpression() != null) {
					...
					type = new GPPBasicType(spec.getType(), bits,
							getExpressionType(gspec.getTypeofExpression()));
				}
			} else {
		...
		if (name != null) {
			...
			if (binding instanceof IType)
				...
				// this is to help with the ambiguity between typeid &
				// idexpression in template arguments
				try {
			...
			if (node != null
				...
				if (s instanceof ICPPClassScope) {
					...
					if (type instanceof ICPPClassTemplate) {
						type = (IType) CPPTemplates
								.instantiateWithinClassTemplate((ICPPClassTemplate) type);
					}
					if (dtor.isConst() || dtor.isVolatile())
						type = new CPPQualifierType(type, dtor.isConst(), dtor
								.isVolatile());
					type = new CPPPointerType(type);
					return type;
				}
	...
	public static IType getExpressionType(IASTExpression expression) {
		...
		if (expression instanceof IASTIdExpression) {
			...
			if (binding instanceof ICPPConstructor) {
				...
				try {
					scope = (ICPPClassScope) ((ICPPConstructor) binding)
							.getScope();
				} catch (DOMException e) {
					return e.getProblem();
				}
				...
				try {
					...
					if (t instanceof IPointerType
							&& ((IPointerType) t).getType() instanceof IFunctionType) {
						IFunctionType ftype = (IFunctionType) ((IPointerType) t)
								.getType();
						if (ftype != null)
							return ftype.getReturnType();
					}
			...
			case ICPPASTBinaryExpression.op_pmdot:
				...
				if (type instanceof ICPPPointerToMemberType) {
					...
					result = new ProblemBinding(binary,
							IProblemBinding.SEMANTIC_INVALID_TYPE, new char[0]);
				}
				break;
			default:
				result = getExpressionType(((IASTBinaryExpression) expression)
			...
			if (op == IASTUnaryExpression.op_sizeof) {
				...
				return new CPPBasicType(IBasicType.t_int, CPPBasicType.IS_LONG
			...
			if (op == IASTUnaryExpression.op_star
				...
				if (type instanceof ICPPReferenceType)
					try {
						return new CPPPointerType(((ICPPReferenceType) type)
								.getType());
					} catch (DOMException e) {
					}
				return new CPPPointerType(type);
			...
			try {
				...
				if (t instanceof ICPPClassType) {
					ICPPFunction op = CPPSemantics.findOperator(expression,
							(ICPPClassType) t);
					if (op != null) {
			...
			IBinding binding = ((ICPPASTTypenameExpression) expression)
	...
	public static IASTName[] getDeclarations(IASTTranslationUnit tu,
		...
		if (found.length == 0 && binding instanceof ICPPSpecialization
				&& binding instanceof ICPPInternalBinding) {
			IASTNode node = ((ICPPInternalBinding) binding).getDefinition();
			if (node == null) {
				IASTNode[] nds = ((ICPPInternalBinding) binding)
						.getDeclarations();
