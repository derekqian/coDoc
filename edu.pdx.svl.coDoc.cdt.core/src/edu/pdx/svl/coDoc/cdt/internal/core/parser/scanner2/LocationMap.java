...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFunctionStyleMacroParameter;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTMacroExpansion;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTName;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTNode;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTNodeLocation;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorElifStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorElseStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorEndifStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorErrorStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorFunctionStyleMacroDefinition;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorIfStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorIfdefStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorIfndefStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorObjectStyleMacroDefinition;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorPragmaStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorUndefStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTTranslationUnit.IDependencyTree.IASTInclusionNode;
import edu.pdx.svl.coDoc.cdt.core.parser.CodeReader;
import edu.pdx.svl.coDoc.cdt.core.parser.IMacro;
...
public class LocationMap implements ILocationResolver, IScannerPreprocessorLog {
	...
	public static class ASTElif extends ASTNode implements
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorElifStatement#taken()
		 */
	...
	public static class ASTInclusionStatement extends ASTNode implements
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPreprocessorIncludeStatement#getPath()
		 */
	...
	public static class ASTFunctionMacro extends ASTNode implements
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFunctionStyleMacroDefinition#getParameters()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFunctionStyleMacroDefinition#addParameter(edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFunctionStyleMacroParameter)
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTMacroDefinition#getName()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTMacroDefinition#setName(edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTName)
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTMacroDefinition#getExpansion()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTMacroDefinition#setExpansion(java.lang.String)
		 */
	...
	protected class _Undef extends _Context implements _IPreprocessorDirective {
		...
		public IASTName getName() {
			if (expansionName == null) {
				...
				expansionName
						.setPropertyInParent(IASTPreprocessorUndefStatement.MACRO_NAME);
				((ASTNode) expansionName).setOffsetAndLength(
						context_directive_start, name.length);
			}
	...
	public abstract static class Location implements IASTNodeLocation {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTNodeLocation#getNodeOffset()
		 */
		public int getNodeOffset() {
			return nodeOffset;
		}
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTNodeLocation#getNodeLength()
		 */
		public int getNodeLength() {
			return nodeLength;
		}
	...
	public static class ASTFunctionMacroParameter extends ASTNode implements
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFunctionStyleMacroParameter#getParameter()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFunctionStyleMacroParameter#setParameter(java.lang.String)
		 */
	...
	public class FileLocation extends Location implements IASTFileLocation {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTFileLocation#getFileName()
		 */
	...
	protected static class _TranslationUnit extends _CompositeFileContext {
		...
		public void addBuiltinMacro(IMacroDefinition def) {
			if (def != null) {
				builtinsPos++;
				builtins = (IMacroDefinition[]) ArrayUtil.append(
						IMacroDefinition.class, builtins, def);
			}
		}

		public IMacroDefinition[] getBuiltinMacroDefinitions() {
			builtins = (IMacroDefinition[]) ArrayUtil.removeNullsAfter(
					IMacroDefinition.class, builtins, builtinsPos);
			return builtins;
		}
	...
	protected class _MacroExpansion extends _CompositeContext {
		...
		public IASTName getName() {
			if (expansionName == null) {
				...
				expansionName.setParent(rootNode);
				expansionName
						.setPropertyInParent(IASTTranslationUnit.EXPANSION_NAME);
				((ASTNode) expansionName).setOffsetAndLength(
						context_directive_start, context_directive_end
								- context_directive_start + 1);
			}
			return expansionName;
		}
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getMacroDefinitions()
	 */
	...
	public IMacroBinding resolveBindingForMacro(char[] name, int offset) {
		...
		if (search instanceof _MacroDefinition) {
			...
			if (CharArrayUtils.equals(name, macroDef.name)
					&& offset == macroDef.nameOffset)
		...
		if (macroDefinition.getBinding() == null)
			macroDefinition.setBinding(new MacroBinding(name, rootNode
					.getScope(), macroDefinition));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getIncludeDirectives()
	 */
	...
	private IASTPreprocessorIncludeStatement createASTInclusion(_Inclusion inc) {
		IASTPreprocessorIncludeStatement result = new ASTInclusionStatement(
				inc.reader.filename);
		((ASTNode) result).setOffsetAndLength(inc.context_directive_start,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getAllPreprocessorStatements()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getLocations(int,
	 *      int)
	 */
	...
	private IASTNodeLocation[] createMegaLocationArray(int offset, int length,
		...
		return (IASTNodeLocation[]) ArrayUtil.removeNulls(
				IASTNodeLocation.class, result);
	}
	...
	protected IASTNodeLocation createSoleLocation(_Context c, int offset,
		...
		if (c instanceof _MacroExpansion) {
			...
			// first check to see if we are in the directive rather than the
			// expansion
			if (c.containsInDirective(offset, length)) {
				_CompositeContext parent = c.parent;
				while (!(parent instanceof _CompositeFileContext))
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#startTranslationUnit()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#endTranslationUnit(int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#startInclusion(char[],
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#endInclusion(char[],
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#enterObjectStyleMacroExpansion(char[],
	 *      char[], int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#exitObjectStyleMacroExpansion(char[],
	 *      int)
	 */
	public void endObjectStyleMacroExpansion(IMacroDefinition macro, int offset) {
		if (currentContext instanceof _MacroExpansion
				&& ((_MacroExpansion) currentContext).definition == macro) {
		...
		_CompositeContext test = currentContext;
		while ((test = test.getParent()) != tu) {
			if (test instanceof _MacroExpansion
					&& ((_MacroExpansion) test).definition == macro) {
				currentContext = test;
				((_ObjectMacroExpansion) currentContext).context_ends = offset;
				currentContext = currentContext.getParent();
				return;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#enterFunctionStyleExpansion(char[],
	 *      char[][], char[], int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#exitFunctionStyleExpansion(char[],
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#defineObjectStyleMacro(edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ObjectStyleMacro,
	 *      int, int, int, int)
	 */
	public IMacroDefinition defineObjectStyleMacro(ObjectStyleMacro m,
		...
		_ObjectMacroDefinition objectMacroDefinition = new _ObjectMacroDefinition(
				currentContext, startOffset, endOffset, m.name, nameOffset, m
						.getExpansion());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#defineFunctionStyleMacro(edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.FunctionStyleMacro,
	 *      int, int, int, int)
	 */
	public IMacroDefinition defineFunctionStyleMacro(FunctionStyleMacro m,
		...
		final _FunctionMacroDefinition functionMacroDefinition = new _FunctionMacroDefinition(
				currentContext, startOffset, endOffset, m.name, nameOffset, m
						.getExpansion(), removeNullArguments(m.arglist));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundIf(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundPragma(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundError(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundIfdef(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundUndef(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundElse(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundElif(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundEndIf(int,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getTranslationUnitPath()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#cleanup()
	 */
	public void cleanup() {
		tu = null;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getScannerProblems()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterIProblem(edu.pdx.svl.coDoc.cdt.core.parser.IProblem)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getSignature(edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTNodeLocation[])
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerPreprocessorLog#encounterPoundIfndef(int,
	 *      int, boolean)
	 */
	...
	_Inclusion findInclusion(_CompositeContext context, String path) {
		...
		for (int i = 0; foundContext == null && i < contexts.length; i++) {
			if (contexts[i] instanceof _Inclusion) {
				...
				if (CharArrayUtils.equals(tempContext.reader.filename, path
						.toCharArray())) {
	...
	private ASTPreprocessorSelectionResult getPreprocessorNode(
		...
		for (int i = 0; result == null && i < contexts.length; i++) {
			...
			} else if (context instanceof _MacroExpansion
					&& globalOffset == context.context_directive_start) {
			...
			if ((globalOffset >= context.context_directive_start && globalOffset <= context.context_ends)
					&& context instanceof _MacroDefinition) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ILocationResolver#getPreprocessorNode(int,
	 *      int)
	 */
	...
	private IASTFileLocation flatten(IASTFileLocation[] result) {
		...
		for (int i = 0; i < result.length; ++i) {
			if (i == 0) {
				...
				if (result[i] != null
						&& !result[i].getFileName().equals(filename))
					return null;
		...
		for (int i = 0; i < subs.length; ++i) {
			if (subs[i] instanceof _MacroExpansion) {
				if (((_MacroExpansion) subs[i]).definition == d)
					results = (_Context[]) ArrayUtil.append(_Context.class,
							results, subs[i]);
			...
			if (subs[i] instanceof _CompositeContext) {
				...
				if (s.length > 0)
					results = (_Context[]) ArrayUtil.addAll(_Context.class,
							results, s);
			}
		}
	...
	protected void buildDependencyTree(IDependencyNodeHost result,
			_CompositeFileContext context) {
	...
	public IMacroDefinition registerBuiltinObjectStyleMacro(
			ObjectStyleMacro macro) {
		IMacroDefinition result = new _ObjectMacroDefinition(tu, -1, -1,
				macro.name, -1, macro);
		tu.addBuiltinMacro(result);
		return result;
	}

	public IMacroDefinition registerBuiltinFunctionStyleMacro(
			FunctionStyleMacro macro) {
		IMacroDefinition result = new _FunctionMacroDefinition(tu, -1, -1,
				macro.name, -1, macro, removeNullArguments(macro.arglist));
		tu.addBuiltinMacro(result);
		return result;
	}

	public IMacroDefinition registerBuiltinDynamicFunctionStyleMacro(
			DynamicFunctionStyleMacro macro) {
		IMacroDefinition result = new _MacroDefinition(tu, -1, -1, macro.name,
				-1, macro.getExpansion());
		tu.addBuiltinMacro(result);
		return result;
	}

	public IMacroDefinition registerBuiltinDynamicStyleMacro(
			DynamicStyleMacro macro) {
		IMacroDefinition result = new _MacroDefinition(tu, -1, -1, macro.name,
				-1, macro.execute());
		tu.addBuiltinMacro(result);
		return result;
