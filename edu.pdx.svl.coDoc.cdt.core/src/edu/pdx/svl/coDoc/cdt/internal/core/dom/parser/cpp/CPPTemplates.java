...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPClassTemplatePartialSpecialization;
...
import edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.cpp.CPPSemantics.Cost;
...
public class CPPTemplates {
	...
	public static IASTName getTemplateParameterName(
		...
		if (param instanceof ICPPASTSimpleTypeTemplateParameter)
			...
			return ((ICPPASTParameterDeclaration) param).getDeclarator()
					.getName();
		return null;
	}

	private static ICPPTemplateDefinition getContainingTemplate(
			ICPPASTTemplateParameter param) {
		IASTNode parent = param.getParent();
		IBinding binding = null;
		if (parent instanceof ICPPASTTemplateDeclaration) {
			// IASTName name = getTemplateName( (ICPPASTTemplateDeclaration)
			// parent );
			// if( name != null ){
			// if( name instanceof ICPPASTTemplateId && !(name.getParent()
			// instanceof ICPPASTQualifiedName) )
			...
			while (parent.getParent() instanceof ICPPASTTemplateDeclaration) {
				...
				templates = (ICPPASTTemplateDeclaration[]) ArrayUtil.append(
						ICPPASTTemplateDeclaration.class, templates, parent);
			}
			templates = (ICPPASTTemplateDeclaration[]) ArrayUtil.trim(
					ICPPASTTemplateDeclaration.class, templates);
			...
			if (decl instanceof IASTSimpleDeclaration) {
				...
				if (dtors.length == 0) {
					...
					if (spec instanceof ICPPASTCompositeTypeSpecifier) {
						...
						name = ((ICPPASTElaboratedTypeSpecifier) spec)
								.getName();
					}
				} else {
				...
				IASTDeclarator dtor = ((IASTFunctionDefinition) decl)
						.getDeclarator();
				while (dtor.getNestedDeclarator() != null)
					dtor = dtor.getNestedDeclarator();
				name = dtor.getName();
			...
			if (name instanceof ICPPASTQualifiedName) {
				...
				for (int j = 0; j < ns.length; j++) {
					if (ns[j] instanceof ICPPASTTemplateId) {
						++i;
						if (i == idx) {
							binding = ((ICPPASTTemplateId) ns[j])
		...
		return (binding instanceof ICPPTemplateDefinition) ? (ICPPTemplateDefinition) binding
				: null;
	}
	...
	public static IBinding createBinding(
		...
		if (template instanceof CPPTemplateDefinition) {
			binding = ((CPPTemplateDefinition) template)
					.resolveTemplateParameter(templateParameter);
		} else if (template instanceof CPPTemplateTemplateParameter) {
			binding = ((CPPTemplateTemplateParameter) template)
					.resolveTemplateParameter(templateParameter);
		}
	...
	public static IBinding createBinding(ICPPASTTemplateId id) {
		...
		} else if ((parent instanceof ICPPASTElaboratedTypeSpecifier || parent instanceof ICPPASTCompositeTypeSpecifier)
				&& segment != 0) {
		...
		if (parent instanceof ICPPASTNamedTypeSpecifier
				|| parent instanceof ICPPASTBaseSpecifier || segment == 0) {
			// class template
			IASTName templateName = id.getTemplateName();
			template = templateName.resolveBinding();
			if (template instanceof ICPPClassTemplatePartialSpecialization) {
				// specializations are selected during the instantiation, start
				// with the primary template
				try {
					template = ((ICPPClassTemplatePartialSpecialization) template)
							.getPrimaryClassTemplate();
				} catch (DOMException e) {
					return e.getProblem();
				}
			} else if (template instanceof ICPPSpecialization
					&& !(template instanceof ICPPTemplateDefinition)) {
				template = ((ICPPSpecialization) template)
			...
			if (template instanceof ICPPTemplateInstance) {
				IASTName templateName = id.getTemplateName();
				templateName.setBinding(((ICPPTemplateInstance) template)
	...
	protected static IBinding createClassExplicitInstantiation(
			ICPPASTElaboratedTypeSpecifier elabSpec) {
		...
		if (classTemplate instanceof ICPPInternalTemplate) {
			IBinding binding = ((ICPPInternalTemplate) classTemplate)
					.instantiate(args);
			return binding;
		}
		return null;
	...
	protected static IBinding createClassSpecialization(
			ICPPASTDeclSpecifier compSpec) {
		IASTName name = null;
		...
		if (templateDecl instanceof ICPPASTTemplateSpecialization) {
			...
			if (spec == null) {
				...
				((ICPPInternalTemplate) template).addSpecialization(args,
		...
		((CPPClassTemplate) template)
				.addPartialSpecialization((ICPPClassTemplatePartialSpecialization) spec);
		return spec;
	}
	...
	protected static IBinding createFunctionSpecialization(IASTName name) {
		...
		ICPPFunctionTemplate function = resolveTemplateFunctions(
				(Object[]) data.foundItems, name);
		if (function == null)
			return new ProblemBinding(name,
					IProblemBinding.SEMANTIC_NAME_NOT_FOUND, name.toCharArray());
		...
		IASTParameterDeclaration[] ps = ((ICPPASTFunctionDeclarator) parent)
				.getParameters();
		Object[] map_types;
		try {
			map_types = deduceTemplateFunctionArguments(function, ps,
					data.templateArguments);
		} catch (DOMException e) {
			return e.getProblem();
		}
		if (map_types != null) {
			...
			if (parent.getParent() instanceof ICPPASTExplicitTemplateInstantiation) {
				spec = ((ICPPInternalTemplate) function)
						.getInstance((IType[]) map_types[1]);
				if (spec == null)
					spec = (ICPPSpecialization) CPPTemplates.createInstance(
							scope, function, (ObjectMap) map_types[0],
							(IType[]) map_types[1]);
			} else {
				spec = ((ICPPInternalTemplate) function)
						.getInstance((IType[]) map_types[1]);
				if (spec == null) {
					if (function instanceof ICPPConstructor)
						spec = new CPPConstructorSpecialization(function,
								scope, (ObjectMap) map_types[0]);
					else if (function instanceof ICPPMethod)
						spec = new CPPMethodSpecialization(function, scope,
								(ObjectMap) map_types[0]);
					else
						spec = new CPPFunctionSpecialization(function, scope,
								(ObjectMap) map_types[0]);
				}
			...
			((ICPPInternalTemplate) function).addSpecialization(
					(IType[]) map_types[1], spec);
			return spec;
		}
		// TODO problem?
		return null;
	...
	static protected ICPPFunctionTemplate resolveTemplateFunctions(
			Object[] items, IASTName name) {
		if (items == null)
			return null;
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (temp instanceof ICPPFunctionTemplate)
				templates = (ICPPFunctionTemplate[]) ArrayUtil.append(
						ICPPFunctionTemplate.class, templates, temp);
		}
		...
		if (name instanceof ICPPASTTemplateId) {
			templateArguments = createTypeArray(((ICPPASTTemplateId) name)
					.getTemplateArguments());
		}
		int numArgs = (templateArguments != null) ? templateArguments.length
				: 0;
			...
			for (int j = 0; j < numParams; j++) {
				...
				if (map.containsKey(param)) {
					...
					if (arg == null)
						...
						|| !matchTemplateParameterAndArgument(param, arg, map)) {
					continue outer;
				}
			}
			// made it this far, its a match
			if (result != null) {
				return new CPPFunctionTemplate.CPPFunctionTemplateProblem(name,
						IProblemBinding.SEMANTIC_AMBIGUOUS_LOOKUP, name
								.toCharArray());
			}
			result = tmpl;
		}
	 ...
	 * @throws DOMException
	 */
	static protected Object[] deduceTemplateFunctionArguments(
			ICPPFunctionTemplate primaryTemplate,
			IASTParameterDeclaration[] ps, IASTNode[] specArgs)
			throws DOMException {
		ICPPTemplateParameter[] templateParameters = primaryTemplate
	 ...
	 * @return
	 */
	public static IBinding createInstance(ICPPScope scope, IBinding decl,
	...
	public static ICPPSpecialization createSpecialization(ICPPScope scope,
			IBinding decl, ObjectMap argMap) {
		ICPPSpecialization spec = null;
		if (decl instanceof ICPPClassTemplate) {
			...
			if (decl instanceof ICPPConstructor)
				spec = new CPPConstructorTemplateSpecialization(decl, scope,
						argMap);
			else if (decl instanceof ICPPMethod)
				spec = new CPPMethodTemplateSpecialization(decl, scope, argMap);
			else
				spec = new CPPFunctionTemplateSpecialization(decl, scope,
						argMap);
	...
	public static IType instantiateType(IType type, ObjectMap argMap) {
		...
		if (type instanceof IFunctionType) {
			...
			IType[] params = null;
			try {
				ret = instantiateType(((IFunctionType) type).getReturnType(),
						argMap);
	...
	public static ICPPASTTemplateDeclaration getTemplateDeclaration(
		...
		if (parent instanceof IASTDeclaration
				&& parent.getParent() instanceof ICPPASTTemplateDeclaration) {
			parent = parent.getParent();
		} else {
			return null;
		}
		...
		if (parent instanceof ICPPASTTemplateDeclaration) {
			...
			while (templateDecl.getParent() instanceof ICPPASTTemplateDeclaration)
				templateDecl = (ICPPASTTemplateDeclaration) templateDecl
						.getParent();
			...
			if (ns != null) {
				...
				for (int j = 0; j < ns.length; j++) {
					...
					if (ns[j] instanceof ICPPASTTemplateId) {
						if (currDecl instanceof ICPPASTTemplateDeclaration)
							currDecl = ((ICPPASTTemplateDeclaration) currDecl)
									.getDeclaration();
						else
							return null; // ??? this would imply bad ast or
											// code
					}
				}
			} else {
	...
	public static IASTName getTemplateName(
		...
		while (nestedDecl instanceof ICPPASTTemplateDeclaration) {
			nestedDecl = ((ICPPASTTemplateDeclaration) nestedDecl)
					.getDeclaration();
		}
		...
		if (nestedDecl instanceof IASTSimpleDeclaration) {
			...
			name = ((IASTFunctionDefinition) nestedDecl).getDeclarator()
					.getName();
		}
		if (name != null) {
			if (name instanceof ICPPASTQualifiedName) {
		...
		public int visit(IASTName name) {
			if (name.getBinding() != null) {
				...
				if (!clear && binding instanceof ICPPTemplateInstance) {
					IType[] args = ((ICPPTemplateInstance) binding)
							.getArguments();
	 ...
	 * @return
	 */
	public static boolean isSameTemplate(ICPPTemplateDefinition definition,
			IASTName name) {
		...
		ICPPASTTemplateParameter[] templateParams = templateDecl
				.getTemplateParameters();
		...
		if (parent instanceof ICPPASTFunctionDeclarator) {
			try {
				IASTParameterDeclaration[] params = ((ICPPASTFunctionDeclarator) parent)
						.getParameters();
				IParameter[] ps = ((ICPPFunction) definition).getParameters();
				if (ps.length == params.length) {
					int i = 0;
					for (; i < ps.length; i++) {
						IType t1 = CPPVisitor.createType(params[i]
								.getDeclarator());
			...
			if (name instanceof ICPPASTTemplateId) {
				if (definition instanceof ICPPClassTemplatePartialSpecialization) {
					...
					IASTNode[] args = ((ICPPASTTemplateId) name)
							.getTemplateArguments();
					IType[] specArgs = null;
					try {
				...
				result = CharArrayUtils.equals(definition.getNameCharArray(),
						name.toCharArray());
			}
		}
	...
	static protected IFunction[] selectTemplateFunctions(ObjectSet templates,
			Object[] functionArguments, IASTName name) {// IASTNode[]
														// templateArguments ){
		...
		if (name instanceof ICPPASTTemplateId) {
			templateArguments = ((ICPPASTTemplateId) name)
					.getTemplateArguments();
		...
		outer: for (int idx = 0; idx < size; idx++) {
			ICPPFunctionTemplate template = (ICPPFunctionTemplate) templates
					.keyAt(idx);
			...
			for (int i = 0; i < numTemplateParams; i++) {
				IType arg = (i < numTemplateArgs) ? CPPVisitor
						.createType(templateArguments[i]) : null;
				IType mapped = (IType) map.get(templateParams[i]);

				if (arg != null && mapped != null)
					if (arg.isSameType(mapped))
						instanceArgs = (IType[]) ArrayUtil.append(IType.class,
								instanceArgs, arg);
					else
						continue outer;
					...
					try {
						if (templateParams[i] instanceof ICPPTemplateTypeParameter) {
							def = ((ICPPTemplateTypeParameter) templateParams[i])
									.getDefault();
						} else if (templateParams[i] instanceof ICPPTemplateTemplateParameter) {
							def = ((ICPPTemplateTemplateParameter) templateParams[i])
									.getDefault();
						} else if (templateParams[i] instanceof ICPPTemplateNonTypeParameter) {
							def = CPPVisitor
									.getExpressionType(((ICPPTemplateNonTypeParameter) templateParams[i])
											.getDefault());
						}
					} catch (DOMException e) {
						continue outer;
					...
					instanceArgs = (IType[]) ArrayUtil.append(IType.class,
							instanceArgs, (arg != null) ? arg : mapped);
			}
			instanceArgs = (IType[]) ArrayUtil.trim(IType.class, instanceArgs);
			ICPPSpecialization temp = (ICPPSpecialization) ((ICPPInternalTemplate) template)
					.instantiate(instanceArgs);
			if (temp != null)
				instances = (IFunction[]) ArrayUtil.append(IFunction.class,
						instances, temp);
		}
	 ...
	 * @throws DOMException
	 */
	static private ObjectMap deduceTemplateArguments(
			ICPPFunctionTemplate template, IType[] arguments)
			throws DOMException {
		...
		for (int i = 0; i < numArgs && i < numParams; i++) {
			if (!deduceTemplateArgument(map, functionParameters[i].getType(),
					arguments[i])) {
				return null;
			}
		}
	 ...
	 * @return
	 */
	static private IType getArgumentTypeForDeduction(IType aType,
			boolean pIsAReferenceType) {
	...
	static private boolean expressionsEquivalent(IASTExpression p,
			IASTExpression a) {
		if (p == null)
			return true;

		if (p instanceof IASTLiteralExpression
				&& a instanceof IASTLiteralExpression) {
			return p.toString().equals(a.toString());
		}
		return false;
	}

	static protected boolean deduceTemplateArgument(ObjectMap map, IType p,
			IType a) throws DOMException {
		...
		if (p instanceof IBasicType) {
			if (p.isSameType(a) && a instanceof IBasicType) {
				return expressionsEquivalent(((IBasicType) p).getValue(),
						((IBasicType) a).getValue());
			}
		} else {
			while (p != null) {
				...
				if (p instanceof IBasicType) {
					...
					if (!deduceTemplateArgument(map,
							((ICPPPointerToMemberType) p).getMemberOfClass(),
					...
					if (!(a instanceof IQualifierType))
						return false;
					a = ((IQualifierType) a).getType(); // TODO a = strip
														// qualifiers from p out
														// of a
					p = ((IQualifierType) p).getType();
				} else if (p instanceof IFunctionType) {
					if (!(a instanceof IFunctionType))
						return false;
					if (!deduceTemplateArgument(map, ((IFunctionType) p)
							.getReturnType(), ((IFunctionType) a)
							.getReturnType()))
						return false;
					...
					if (aMap != null
							&& !(aInst.getTemplateDefinition() instanceof ICPPClassTemplatePartialSpecialization)) {
						ICPPTemplateParameter[] aParams = aInst
								.getTemplateDefinition()
								.getTemplateParameters();
						...
						for (int i = 0; i < pArgs.length; i++) {
							...
							if (t == null
									|| !deduceTemplateArgument(map, pArgs[i], t))
								return false;
						}
					} else {
	...
	static private IType[] createArgsForFunctionTemplateOrdering(
			ICPPFunctionTemplate template) throws DOMException {
	...
	static protected int orderTemplateFunctions(ICPPFunctionTemplate f1,
			ICPPFunctionTemplate f2) throws DOMException {
		// Using the transformed parameter list, perform argument deduction
		// against the other
		// function template
		IType[] args = createArgsForFunctionTemplateOrdering(f1);
		ICPPFunction function = (ICPPFunction) ((ICPPInternalTemplate) f1)
				.instantiate(args);

		ObjectMap m1 = null;
		if (function != null)
			m1 = deduceTemplateArguments(f2, function.getType()
					.getParameterTypes());
		...
		ObjectMap m2 = null;
		if (function != null)
			m2 = deduceTemplateArguments(f1, function.getType()
					.getParameterTypes());
	...
	static protected ICPPTemplateDefinition matchTemplatePartialSpecialization(
			ICPPClassTemplate template, IType[] args) throws DOMException {
		if (template == null) {
			return null;
		}

		ICPPClassTemplatePartialSpecialization[] specializations = template
		...
		if (!bestMatchIsBest) {
			// TODO problem
			return new CPPTemplateDefinition.CPPTemplateProblem(null,
					IProblemBinding.SEMANTIC_AMBIGUOUS_LOOKUP, null);
		}
	 ...
	 * @throws DOMException
	 */
	static private int orderSpecializations(
			ICPPClassTemplatePartialSpecialization spec1,
			ICPPClassTemplatePartialSpecialization spec2) throws DOMException {
		if (spec1 == null) {
			return -1;
		}
	...
	public static final class CPPImplicitFunctionTemplate extends
			CPPFunctionTemplate {
		IParameter[] functionParameters = null;
		 ...
		 * @param name
		 */
		public CPPImplicitFunctionTemplate(
		...
		public IFunctionType getType() {
			if (type == null) {
				type = CPPVisitor.createImplicitFunctionType(new CPPBasicType(
						IBasicType.t_void, 0), functionParameters);
			}
			return type;
		}
	}
	 ...
	 */
	static private ICPPFunctionTemplate classTemplateSpecializationToFunctionTemplate(
		...
		IType paramType = (IType) ((ICPPInternalTemplate) template)
				.instantiate(args);
		IParameter[] functionParameters = new IParameter[] { new CPPParameter(
				paramType) };

		try {
			return new CPPImplicitFunctionTemplate(specialization
	...
	static private boolean isValidArgument(ICPPTemplateParameter param,
			IType argument) {
		// TODO
		return true;
	}
	...
	static protected boolean matchTemplateParameterAndArgument(
		...
		if (param instanceof ICPPTemplateTypeParameter)
			...
			try {
				pParams = ((ICPPTemplateTemplateParameter) param)
						.getTemplateParameters();
				aParams = ((ICPPTemplateDefinition) argument)
						.getTemplateParameters();
			} catch (DOMException e) {
				return false;
			}
			...
			try {
				...
				Cost cost = CPPSemantics.checkStandardConversionSequence(
						argument, pType);
	...
	public static IBinding instantiateWithinClassTemplate(
			ICPPClassTemplate template) throws DOMException {
		IType[] args = null;
		if (template instanceof ICPPClassTemplatePartialSpecialization) {
			args = ((ICPPClassTemplatePartialSpecialization) template)
					.getArguments();
		} else {
			ICPPTemplateParameter[] templateParameters = template
					.getTemplateParameters();
			...
			for (int i = 0; i < templateParameters.length; i++) {
				if (templateParameters[i] instanceof IType) {
					...
					args[i] = ((ICPPTemplateNonTypeParameter) templateParameters[i])
							.getType();
				}
			}
		}
	...
	public static IBinding instantiateTemplate(ICPPTemplateDefinition template,
			IType[] arguments, ObjectMap specializedArgs) {
		if (template == null) {
			template = null;
		}
		...
		for (int i = 0; i < numParams; i++) {
			...
			if (i < numArgs) {
				arg = arguments[i];
				// If the argument is a template parameter, we can't instantiate
				// yet, defer for later
				if (typeContainsTemplateParameter(arg)) {
					return ((ICPPInternalTemplate) template)
							.deferredInstance(arguments);
				}
			} else {
				IType defaultType = null;
				try {
					if (param instanceof ICPPTemplateTypeParameter)
						defaultType = ((ICPPTemplateTypeParameter) param)
								.getDefault();
					else if (param instanceof ICPPTemplateTemplateParameter)
						defaultType = ((ICPPTemplateTemplateParameter) param)
								.getDefault();
					else if (param instanceof ICPPTemplateNonTypeParameter)
						defaultType = CPPVisitor
								.getExpressionType(((ICPPTemplateNonTypeParameter) param)
										.getDefault());
				} catch (DOMException e) {
		...
		ICPPSpecialization instance = ((ICPPInternalTemplate) template)
				.getInstance(actualArgs);
		if (instance != null) {
			return instance;
		}
		...
		instance = (ICPPTemplateInstance) CPPTemplates.createInstance(scope,
				template, map, arguments);
		((ICPPInternalTemplate) template)
