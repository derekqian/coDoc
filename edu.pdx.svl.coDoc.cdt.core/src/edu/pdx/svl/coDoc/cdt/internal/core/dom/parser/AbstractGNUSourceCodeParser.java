...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.IGNUASTTypeIdExpression;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.IGNUASTUnaryExpression;
...
public abstract class AbstractGNUSourceCodeParser implements ISourceCodeParser {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.ISourceCodeParser#getCompletionNode()
	 */
		...
		if (LT(1) == IToken.tQUESTION) {
			...
			if (thirdExpression != null) {
				...
				((ASTNode) result).setOffsetAndLength(
						...
						calculateEndOffset(thirdExpression)
								- ((ASTNode) firstExpression).getOffset());
			}
	 ...
	 */
	protected void lookAheadForDeclarator(Flags flags)
			throws FoundDeclaratorException {
		if (flags.typeId)
			return;
		IToken mark = null;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.ISourceCodeParser#encounteredError()
	 */
	...
	protected IASTStatement parseDeclarationOrExpressionStatement()
		...
		if (ds.getDeclaration() instanceof IASTAmbiguousDeclaration) {
			IASTAmbiguousDeclaration amb = (IASTAmbiguousDeclaration) ds
					.getDeclaration();
			...
			for (int i = 0; i < ambDs.length; ++i) {
				if (ambDs[i] instanceof IASTSimpleDeclaration
						&& ((IASTSimpleDeclaration) ambDs[i])
								.getDeclSpecifier() instanceof IASTSimpleDeclSpecifier
						&& ((IASTSimpleDeclSpecifier) ((IASTSimpleDeclaration) ambDs[i])
								.getDeclSpecifier()).getType() == IASTSimpleDeclSpecifier.t_unspecified) {
					++ambCount;
				}
			}
	...
	protected IASTStatement parseLabelStatement() throws EndOfFileException,
		...
		nestedStatement
				.setPropertyInParent(IASTLabelStatement.NESTED_STATEMENT);
	...
	protected IASTStatement parseWhileStatement() throws EndOfFileException,
		...
		((ASTNode) while_statement).setOffsetAndLength(startOffset,
				(while_body != null ? calculateEndOffset(while_body) : LA(1)
						.getEndOffset())
						- startOffset);
	...
	protected abstract IASTDeclaration simpleDeclaration()
			throws BacktrackException, EndOfFileException;

	/**
	 * @throws BacktrackException
	 */
	protected IASTStatement forInitStatement() throws BacktrackException,
			EndOfFileException {
	...
	protected void __attribute__() throws BacktrackException,
			EndOfFileException {
		IToken token = LA(1);
						...
						whileLoop: while (true) {
							...
							case IToken.tIDENTIFIER:
								if (comma1 || first) {
									...
									throwBacktrack(token.getOffset(), token
											.getLength());
								}
								consume();
								break;
