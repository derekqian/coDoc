...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTConditionalExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTProblemHolder;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTTypeIdExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTArrayModifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTCompositeTypeSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTDeclSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTElaboratedTypeSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTEnumerationSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTFieldDesignator;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTPointer;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTSimpleDeclSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTTypedefNameSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICCompositeTypeScope;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICFunctionScope;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICScope;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.IGNUASTCompoundStatementExpression;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.c.ICASTKnRFunctionDeclarator;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.c.IGCCASTSimpleDeclSpecifier;
...
public class CVisitor {
	...
	public static class CollectProblemsAction extends CASTVisitor {
		...
		private IASTProblem[] removeNullFromProblems() {
			if (problems[problems.length - 1] != null) { // if the last
															// element in the
															// list is not null
															// then return the
															// list
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.c.CVisitor.CBaseVisitorAction#processDeclaration(edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTDeclaration)
		 */
	...
	public static class CollectDeclarationsAction extends CASTVisitor {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.c.CVisitor.CBaseVisitorAction#processDeclarator(edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTDeclarator)
		 */
		public int visit(IASTDeclarator declarator) {
			...
			if (parent instanceof IASTDeclaration) {
				if (parent != null && parent instanceof IASTFunctionDefinition) {
					...
					if ((declarator.getName() != null && declarator.getName()
		...
		public int visit(IASTDeclSpecifier declSpec) {
			if (compositeTypeDeclared
					&& declSpec instanceof ICASTTypedefNameSpecifier)
				return PROCESS_CONTINUE;
			...
			if (binding instanceof ICompositeType
				...
				if (((IASTCompositeTypeSpecifier) declSpec).getName()
						.resolveBinding() == binding) {
				...
				if (((IASTEnumerationSpecifier) declSpec).getName()
						.resolveBinding() == binding) {
					compositeTypeDeclared = true;
				...
				if (compositeTypeDeclared) {
					...
					if (!(parent instanceof IASTSimpleDeclaration)
							|| ((IASTSimpleDeclaration) parent)
									.getDeclarators().length > 0) {
						return PROCESS_CONTINUE;
					}
				}
				if (((IASTElaboratedTypeSpecifier) declSpec).getName()
						.resolveBinding() == binding) {
					compositeTypeDeclared = true;
		...
		public int visit(IASTEnumerator enumerator) {
			if (binding instanceof IEnumerator
					&& enumerator.getName().resolveBinding() == binding) {
	...
	public static class CollectReferencesAction extends CASTVisitor {
		...
		public int visit(IASTName name) {
			...
			if (CharArrayUtils.equals(name.toCharArray(), binding
					.getNameCharArray())
		...
		if (binding != null) {
			if (binding instanceof CEnumeration)
				...
				return new ProblemBinding(name,
						IProblemBinding.SEMANTIC_INVALID_OVERLOAD, name
								.toCharArray());
		} else {
	...
	private static IBinding createBinding(IASTStatement statement) {
		if (statement instanceof IASTGotoStatement) {
			char[] gotoName = ((IASTGotoStatement) statement).getName()
					.toCharArray();
			...
			if (scope != null && scope instanceof ICFunctionScope) {
				...
				// label not found
				return new CLabel.CLabelProblem(((IASTGotoStatement) statement)
						.getName(),
						IProblemBinding.SEMANTIC_LABEL_STATEMENT_NOT_FOUND,
						gotoName);
			}
	...
	private static IBinding createBinding(
			ICASTElaboratedTypeSpecifier elabTypeSpec) {
		...
		if (parent instanceof IASTDeclaration) {
			...
			return binding;
				...
				|| parent instanceof IASTParameterDeclaration) {
			IASTNode blockItem = getContainingBlockItem(parent);
			try {
				return (IBinding) findBinding(blockItem,
						elabTypeSpec.getName(), COMPLETE | TAGS);
			} catch (DOMException e) {
				return null;
			}
		}
	 ...
	 * @return
	 */
	private static Object findBinding(IASTFieldReference fieldReference,
			boolean prefix) {
		IASTExpression fieldOwner = fieldReference.getFieldOwner();
		IType type = null;
		if (fieldOwner instanceof IASTArraySubscriptExpression) {
			type = getExpressionType(((IASTArraySubscriptExpression) fieldOwner)
					.getArrayExpression());
		} else {
		...
		if (type != null && type instanceof ICompositeType) {
			if (prefix) {
				...
				try {
					...
					for (int i = 0; i < fields.length; i++) {
						if (CharArrayUtils.equals(fields[i].getNameCharArray(),
								0, p.length, p)) {
							result = (IBinding[]) ArrayUtil.append(
									IBinding.class, result, fields[i]);
						}
					}
			...
			try {
				return ((ICompositeType) type).findField(fieldReference
	...
	public static IType getExpressionType(IASTExpression expression) {
		try {
			if (expression instanceof IASTIdExpression) {
				IBinding binding = ((IASTIdExpression) expression).getName()
						.resolveBinding();
				...
				IBinding binding = ((IASTFieldReference) expression)
						.getFieldName().resolveBinding();
				if (binding instanceof IVariable) {
					return ((IVariable) binding).getType();
				}
			} else if (expression instanceof IASTFunctionCallExpression) {
				IType type = getExpressionType(((IASTFunctionCallExpression) expression)
				...
				IType type = getExpressionType(((IASTUnaryExpression) expression)
						.getOperand());
				...
				case IASTLiteralExpression.lk_string_literal:
					IType type = new CBasicType(IBasicType.t_char, 0,
							expression);
				...
				case IASTBinaryExpression.op_minus:
					IType t = getExpressionType(((IASTBinaryExpression) expression)
							.getOperand1());
				...
				if (op == IASTUnaryExpression.op_sizeof) {
					...
					return new CBasicType(IBasicType.t_int, CBasicType.IS_LONG
							| CBasicType.IS_UNSIGNED, expression);
				}
				IType type = getExpressionType(((IASTUnaryExpression) expression)
						.getOperand());
				...
				IBinding binding = (IBinding) findBinding(
						(IASTFieldReference) expression, false);
				if (binding instanceof IVariable)
					return ((IVariable) binding).getType();
				...
				IASTExpression[] exps = ((IASTExpressionList) expression)
						.getExpressions();
				...
				IType t = getExpressionType(((IASTArraySubscriptExpression) expression)
						.getArrayExpression());
				if (t instanceof IPointerType)
				...
				IASTCompoundStatement compound = ((IGNUASTCompoundStatementExpression) expression)
						.getCompoundStatement();
				IASTStatement[] statements = compound.getStatements();
				if (statements.length > 0) {
					IASTStatement st = statements[statements.length - 1];
					if (st instanceof IASTExpressionStatement)
						return getExpressionType(((IASTExpressionStatement) st)
								.getExpression());
				}
			} else if (expression instanceof IASTConditionalExpression) {
				IASTConditionalExpression conditional = (IASTConditionalExpression) expression;
				IType t2 = getExpressionType(conditional
						.getPositiveResultExpression());
				IType t3 = getExpressionType(conditional
	...
	private static IBinding createBinding(IASTDeclarator declarator,
		...
		if (declarator instanceof ICASTKnRFunctionDeclarator) {
			IASTNode parent = declarator.getParent();
			if (CharArrayUtils.equals(declarator.getName().toCharArray(), name
					.toCharArray())) {
				binding = resolveBinding(parent, CURRENT_SCOPE);
				if (binding != null) {
					if (binding instanceof ICInternalFunction)
						((ICInternalFunction) binding)
								.addDeclarator((ICASTKnRFunctionDeclarator) declarator);
					else
					...
					binding = createBinding(declarator);
						...
						// ICASTKnRFunctionDeclarator's parameterNames
				...
				if (declarator.getParent() instanceof IASTFunctionDefinition) {
					ICScope scope = (ICScope) ((IASTCompoundStatement) ((IASTFunctionDefinition) declarator
							.getParent()).getBody()).getScope();
	...
	private static IBinding createBinding(IASTDeclarator declarator) {
		...
		if (parent instanceof IASTParameterDeclaration
			...
			IASTFunctionDeclarator fdtor = (IASTFunctionDeclarator) parent
					.getParent();
			...
			try {
				if (scope != null
						&& scope.getPhysicalNode() instanceof IASTTranslationUnit) {
					return binding;
				}
			} catch (DOMException e) {
			}
			...
			if (binding != null) {
				if (binding instanceof IFunction) {
					...
					if (function instanceof CFunction)
						((CFunction) function)
								.addDeclarator((IASTFunctionDeclarator) declarator);
					return function;
				}
				binding = new ProblemBinding(name,
						IProblemBinding.SEMANTIC_INVALID_OVERLOAD, name
					...
					&& ((IASTSimpleDeclaration) parent).getDeclSpecifier()
			...
			if (simpleDecl.getDeclSpecifier().getStorageClass() == IASTDeclSpecifier.sc_typedef) {
				...
				if (binding != null) {
					if (binding instanceof IParameter) {
						return new ProblemBinding(name,
								IProblemBinding.SEMANTIC_INVALID_REDECLARATION,
								name.toCharArray());
	...
	private static IBinding createBinding(
			ICASTCompositeTypeSpecifier compositeTypeSpec) {
		ICScope scope = null;
		IBinding binding = null;
	...
	protected static IBinding resolveBinding(IASTNode node, int bits) {
		if (node instanceof IASTFunctionDefinition) {
			...
			IASTFunctionDeclarator functionDeclartor = functionDef
					.getDeclarator();
			...
			IASTNode blockItem = getContainingBlockItem(node);
			try {
				IBinding binding = (IBinding) findBinding(blockItem,
						((IASTIdExpression) node).getName(), bits);
				if (binding instanceof IType
						&& !(binding instanceof IProblemBinding)) {
					return new ProblemBinding(node,
							IProblemBinding.SEMANTIC_INVALID_TYPE, binding
									.getNameCharArray());
				}
				return binding;
			} catch (DOMException e) {
			...
			try {
				IBinding binding = (IBinding) findBinding(blockItem,
						((ICASTTypedefNameSpecifier) node).getName(), bits);
				if (binding instanceof IType)
					return binding;
			...
			try {
				return (IBinding) findBinding(blockItem,
						((ICASTElaboratedTypeSpecifier) node).getName(), bits);
			} catch (DOMException e) {
				return null;
			}
			...
			try {
				return (IBinding) findBinding(blockItem,
						((ICASTCompositeTypeSpecifier) node).getName(), bits);
			} catch (DOMException e) {
				return null;
			}
			...
			if ((blockItem instanceof IASTSimpleDeclaration || (blockItem instanceof IASTDeclarationStatement && ((IASTDeclarationStatement) blockItem)
				...
				if (blockItem instanceof IASTDeclarationStatement
						&& ((IASTDeclarationStatement) blockItem)
								.getDeclaration() instanceof IASTSimpleDeclaration)
			...
			if (node instanceof IASTDeclaration) {
				...
				if (parent instanceof IASTTranslationUnit) {
					...
					if (parent instanceof IASTFunctionDefinition) {
						return ((IASTCompoundStatement) ((IASTFunctionDefinition) parent)
								.getBody()).getScope();
					}
				}
	...
	public static IScope getContainingScope(IASTStatement statement) {
		...
		if (parent instanceof IASTCompoundStatement) {
			...
			IASTFunctionDeclarator fnDeclarator = ((IASTFunctionDefinition) parent)
					.getDeclarator();
	 ...
	 * IBinding
	 */
	protected static Object findBinding(IASTNode blockItem, IASTName name,
			int bits) throws DOMException {
		boolean prefix = (bits & PREFIX_LOOKUP) != 0;
		Object binding = prefix ? new ObjectSet(2) : null;

		CharArrayObjectMap prefixMap = prefix ? new CharArrayObjectMap(2)
				: null;
		...
		while (blockItem != null) {
			...
			if (parent instanceof IASTCompoundStatement) {
				...
				if (parent.getParent() instanceof IASTFunctionDefinition) {
					IASTFunctionDeclarator dtor = ((IASTFunctionDefinition) parent
							.getParent()).getDeclarator();
					if (dtor instanceof IASTStandardFunctionDeclarator)
						nodes = ((IASTStandardFunctionDeclarator) dtor)
								.getParameters();
					else if (dtor instanceof ICASTKnRFunctionDeclarator)
						nodes = ((ICASTKnRFunctionDeclarator) dtor)
			...
			if (scope != null && scope.isFullyCached()) {
				...
				if (nodes != null) {
					int idx = -1;
					IASTNode node = (nodes != null ? (nodes.length > 0 ? nodes[++idx]
							: null)
							: parent);
					while (node != null) {
						Object candidate = null;
						try {
							candidate = checkForBinding(scope, node, name,
									typesOnly, prefixMap);
						} catch (DOMException e) {
							continue;
						}
						...
						if (idx > -1 && ++idx < nodes.length) {
							...
							if (nodes[0].getPropertyInParent() == ICASTKnRFunctionDeclarator.FUNCTION_PARAMETER
								...
								// function body, we were looking at parameters,
								// now check the body itself
								...
								if (parent instanceof IASTCompoundStatement) {
									...
									if (n instanceof IASTFunctionDefinition) {
										compound = (IASTCompoundStatement) ((IASTFunctionDefinition) n)
												.getBody();
									}
								}
								if (compound != null) {
					...
					try {
						result = checkForBinding(scope, parent, name,
								typesOnly, prefixMap);
					} catch (DOMException e) {
					}
				}
		...
		if (prefixMap != null) {
			...
			for (int i = 0; i < vals.length; i++) {
				result = (IBinding[]) ArrayUtil.append(IBinding.class, result,
						((IASTName) vals[i]).resolveBinding());
			}
			return ArrayUtil.trim(IBinding.class, result);
		}
		if (blockItem != null) {
	...
	private static IBinding externalBinding(IASTTranslationUnit tu,
			IASTName name) {
		IASTNode parent = name.getParent();
		...
		if (parent instanceof IASTIdExpression) {
			if (parent.getPropertyInParent() == IASTFunctionCallExpression.FUNCTION_NAME) {
				...
				// ((CScope)tu.getScope()).addName( name );
				external = new ProblemBinding(name,
						IProblemBinding.SEMANTIC_NAME_NOT_FOUND, name
								.toCharArray());
			}
		}
		return external;
	...
	private static IASTName checkForBinding(ICScope scope,
			IASTDeclSpecifier declSpec, IASTName name, boolean typesOnly,
			CharArrayObjectMap prefixMap) throws DOMException {
		...
		if (declSpec instanceof ICASTElaboratedTypeSpecifier) {
			...
			if (typesOnly) {
				if (prefixMap != null)
					prefixMap = (CharArrayObjectMap) collectResult(tempName, n,
							prefixMap);
			...
			IASTDeclaration[] nested = ((ICASTCompositeTypeSpecifier) declSpec)
					.getMembers();
			for (int i = 0; i < nested.length; i++) {
				if (nested[i] instanceof IASTSimpleDeclaration) {
					IASTDeclSpecifier d = ((IASTSimpleDeclaration) nested[i])
							.getDeclSpecifier();
					if (d instanceof ICASTCompositeTypeSpecifier
							|| d instanceof IASTEnumerationSpecifier) {
						Object obj = checkForBinding(scope, d, name, typesOnly,
								prefixMap);
			...
			// check enumerators
			IASTEnumerator[] list = ((ICASTEnumerationSpecifier) declSpec)
					.getEnumerators();
			for (int i = 0; i < list.length; i++) {
				...
				if (!typesOnly) {
					if (prefixMap != null)
						prefixMap = (CharArrayObjectMap) collectResult(
								tempName, n, prefixMap);
					else if (collectResult(tempName, n, prefixMap) != null)
						resultName = tempName;
				}
			}
	...
	private static Object collectResult(IASTName candidate, char[] name,
			CharArrayObjectMap prefixMap) {
		char[] c = candidate.toCharArray();
		if (prefixMap == null && CharArrayUtils.equals(c, name)) {
			return candidate;
		} else if (prefixMap != null
				&& CharArrayUtils.equals(c, 0, name.length, name)
				&& !prefixMap.containsKey(c)) {
	...
	private static IASTName checkForBinding(ICScope scope,
			IASTParameterDeclaration paramDecl, IASTName name,
			boolean typesOnly, CharArrayObjectMap prefixMap)
			throws DOMException {
		if (paramDecl == null)
			return null;
		...
		if (!typesOnly) {
			...
			if (prefixMap == null && CharArrayUtils.equals(c, n))
				return tempName;
					...
					&& CharArrayUtils.equals(c, 0, n.length, n)
					&& !prefixMap.containsKey(c))
				prefixMap.put(c, tempName);
		} else {
			return checkForBinding(scope, paramDecl.getDeclSpecifier(), name,
					typesOnly, prefixMap);
		}
		return null;
	}
	...
	/**
	 * if not a prefix lookup, returns IASTName if doing prefix lookup, results
	 * are in prefixMap, returns null
	 */
	private static IASTName checkForBinding(ICScope scope, IASTNode node,
			IASTName name, boolean typesOnly, CharArrayObjectMap prefixMap)
			throws DOMException {
		if (node instanceof IASTDeclaration) {
			return checkForBinding(scope, (IASTDeclaration) node, name,
					typesOnly, prefixMap);
		} else if (node instanceof IASTParameterDeclaration) {
			return checkForBinding(scope, (IASTParameterDeclaration) node,
					name, typesOnly, prefixMap);
		} else if (node instanceof IASTDeclarationStatement) {
			return checkForBinding(scope, ((IASTDeclarationStatement) node)
					.getDeclaration(), name, typesOnly, prefixMap);
			...
			if (forStatement.getInitializerStatement() instanceof IASTDeclarationStatement) {
				return checkForBinding(scope,
						((IASTDeclarationStatement) forStatement
	...
	private static IASTName checkForBinding(ICScope scope,
			IASTDeclaration declaration, IASTName name, boolean typesOnly,
			CharArrayObjectMap prefixMap) throws DOMException {
		char[] n = name.toCharArray();
		IASTName tempName = null;
		IASTName resultName = null;
		if (declaration instanceof IASTSimpleDeclaration) {
			...
			tempName = checkForBinding(scope, simpleDeclaration
					.getDeclSpecifier(), name, typesOnly, prefixMap);
			...
			tempName = checkForBinding(scope, functionDef.getDeclSpecifier(),
					name, typesOnly, prefixMap);
			if (prefixMap == null && tempName != null) {
				resultName = tempName;
			}
	...
	protected static IASTDeclarator findDefinition(IASTDeclarator declarator,
			int beginAtLoc) {
		return (IASTDeclarator) findDefinition(declarator, declarator.getName()
				.toCharArray(), beginAtLoc);
	}

	protected static IASTFunctionDeclarator findDefinition(
			IASTFunctionDeclarator declarator) {
		return (IASTFunctionDeclarator) findDefinition(declarator, declarator
				.getName().toCharArray(), AT_NEXT);
	}

	protected static IASTDeclSpecifier findDefinition(
			ICASTElaboratedTypeSpecifier declSpec) {
		return (IASTDeclSpecifier) findDefinition(declSpec, declSpec.getName()
				.toCharArray(), AT_BEGINNING);
	}

	private static IASTNode findDefinition(IASTNode decl, char[] declName,
			int beginAtLoc) {
			...
			for (int i = 0; i < list.length; i++) {
				...
				if (begun) {
					if (node instanceof IASTDeclarationStatement) {
						node = ((IASTDeclarationStatement) node)
								.getDeclaration();
					}

					if (node instanceof IASTFunctionDefinition
							&& decl instanceof IASTFunctionDeclarator) {
						IASTFunctionDeclarator dtor = ((IASTFunctionDefinition) node)
								.getDeclarator();
						...
						if (name.toString().equals(declName)) {
							...
							&& decl instanceof ICASTElaboratedTypeSpecifier) {
						IASTSimpleDeclaration simpleDecl = (IASTSimpleDeclaration) node;
						IASTDeclSpecifier declSpec = simpleDecl
								.getDeclSpecifier();
						IASTName name = null;

						if (declSpec instanceof ICASTCompositeTypeSpecifier) {
							name = ((ICASTCompositeTypeSpecifier) declSpec)
									.getName();
						} else if (declSpec instanceof ICASTEnumerationSpecifier) {
							name = ((ICASTEnumerationSpecifier) declSpec)
									.getName();
						}
						if (name != null) {
							if (CharArrayUtils.equals(name.toCharArray(),
									declName)) {
								return declSpec;
							}
						}
					} else if (node instanceof IASTSimpleDeclaration
							&& decl instanceof IASTDeclarator) {
						IASTSimpleDeclaration simpleDecl = (IASTSimpleDeclaration) node;
						IASTDeclarator[] dtors = simpleDecl.getDeclarators();
						for (int j = 0; dtors != null && j < dtors.length; j++) {
							if (CharArrayUtils.equals(dtors[j].getName()
									.toCharArray(), declName)) {
								return dtors[j];
							}
						}
					}
	...
	public static IType createType(IASTDeclarator declarator) {
		...
		boolean isParameter = (node instanceof IASTParameterDeclaration || node
				.getParent() instanceof ICASTKnRFunctionDeclarator);
		...
		if (isParameter
				&& node.getParent().getParent() instanceof IASTFunctionDefinition) {
	...
	public static IType createType(IType returnType,
			IASTFunctionDeclarator declarator) {

		IType[] pTypes = getParmTypes(declarator);
		returnType = setupPointerChain(declarator.getPointerOperators(),
				returnType);
	 ...
	 * 
	 * @param declarator
	 *            the IASTDeclarator whose base IType will be created
	 * @param declSpec
	 *            the IASTDeclSpecifier used to determine if the base type is a
	 *            CQualifierType or not
	 * @param isParm
	 *            is used to specify whether the IASTDeclarator is a parameter
	 *            of a declaration
	...
	public static IType createBaseType(IASTDeclSpecifier declSpec) {
		if (declSpec instanceof IGCCASTSimpleDeclSpecifier) {
			IASTExpression exp = ((IGCCASTSimpleDeclSpecifier) declSpec)
					.getTypeofExpression();
			if (exp != null)
	...
	private static IType[] getParmTypes(IASTFunctionDeclarator decltor) {
		if (decltor instanceof IASTStandardFunctionDeclarator) {
			IASTParameterDeclaration parms[] = ((IASTStandardFunctionDeclarator) decltor)
					.getParameters();
			...
			IASTName parms[] = ((ICASTKnRFunctionDeclarator) decltor)
					.getParameterNames();
			IType parmTypes[] = new IType[parms.length];

			for (int i = 0; i < parms.length; i++) {
				IASTDeclarator dtor = getKnRParameterDeclarator(
						(ICASTKnRFunctionDeclarator) decltor, parms[i]);
				if (dtor != null)
	...
	protected static IASTDeclarator getKnRParameterDeclarator(
			ICASTKnRFunctionDeclarator fKnRDtor, IASTName name) {
		...
		for (int i = 0; i < decls.length; i++) {
			...
			IASTDeclarator[] dtors = ((IASTSimpleDeclaration) decls[i])
					.getDeclarators();
	...
	private static IType setupArrayChain(IASTDeclarator decl, IType lastType) {
		if (decl instanceof IASTArrayDeclarator) {
			int i = 0;
			IASTArrayModifier[] mods = ((IASTArrayDeclarator) decl)
					.getArrayModifiers();

			CArrayType arrayType = new CArrayType(lastType);
			if (mods[i] instanceof ICASTArrayModifier) {
				arrayType
						.setModifiedArrayModifier((ICASTArrayModifier) mods[i++]);
			}
	 ...
	 *         chain
	 */
	private static IType setupPointerChain(IASTPointerOperator[] ptrs,
			IType lastType) {
		...
		if (ptrs != null && ptrs.length > 0) {
			...
			if (ptrs.length == 1) {
				...
				pointerType
						.setQualifiers((((ICASTPointer) ptrs[0]).isConst() ? CPointerType.IS_CONST
								: 0)
								| (((ICASTPointer) ptrs[0]).isRestrict() ? CPointerType.IS_RESTRICT
										: 0)
								| (((ICASTPointer) ptrs[0]).isVolatile() ? CPointerType.IS_VOLATILE
										: 0));
			} else {
				CPointerType tempType = new CPointerType();
				pointerType.setType(tempType);
				pointerType
						.setQualifiers((((ICASTPointer) ptrs[ptrs.length - 1])
								.isConst() ? CPointerType.IS_CONST : 0)
								| (((ICASTPointer) ptrs[ptrs.length - 1])
										.isRestrict() ? CPointerType.IS_RESTRICT
										: 0)
								| (((ICASTPointer) ptrs[ptrs.length - 1])
										.isVolatile() ? CPointerType.IS_VOLATILE
										: 0));
				int i = ptrs.length - 2;
				for (; i > 0; i--) {
					tempType.setType(new CPointerType());
					tempType
							.setQualifiers((((ICASTPointer) ptrs[i]).isConst() ? CPointerType.IS_CONST
									: 0)
									| (((ICASTPointer) ptrs[i]).isRestrict() ? CPointerType.IS_RESTRICT
											: 0)
									| (((ICASTPointer) ptrs[i]).isVolatile() ? CPointerType.IS_VOLATILE
											: 0));
	...
	public static IASTName[] getDeclarations(IASTTranslationUnit tu,
			IBinding binding) {
		CollectDeclarationsAction action = new CollectDeclarationsAction(
				binding);
		tu.accept(action);
	 ...
	 * @return
	 */
	public static IASTName[] getReferences(IASTTranslationUnit tu,
			IBinding binding) {
	...
	public static IBinding findTypeBinding(IASTNode startingPoint, IASTName name)
		...
		if (startingPoint instanceof IASTTranslationUnit) {
			IASTDeclaration[] declarations = ((IASTTranslationUnit) startingPoint)
					.getDeclarations();
			if (declarations.length > 0)
				return (IBinding) findBinding(
						declarations[declarations.length - 1], name, COMPLETE
								| INCLUDE_BLOCK_ITEM);
		}
		if (startingPoint instanceof IASTCompoundStatement) {
			IASTStatement[] statements = ((IASTCompoundStatement) startingPoint)
					.getStatements();
			if (statements.length > 0)
				return (IBinding) findBinding(
						statements[statements.length - 1], name, COMPLETE
								| INCLUDE_BLOCK_ITEM);
		}
		return null;
	}
	...
	public static IBinding[] prefixLookup(IASTName name) {
		...
		if (prop == IASTFieldReference.FIELD_NAME) {
			result = (IBinding[]) findBinding((IASTFieldReference) name
					.getParent(), true);
		} else {
