...
public class ASTNode implements IASTNode {
	...
	public ILookupResult lookup(String prefix, LookupKind[] kind,
		...
		IContainerSymbol qualification = (context != null) ? ((ASTNode) context)
				.getLookupQualificationSymbol()
				: null;
		...
		List lookupResults = performPrefixLookup(prefix.toCharArray(),
				thisContainer, qualification, filter, parameters);

		if (lookupResults == null)
			return null;

		// filter out things that are not visible and things that don't have AST
		// nodes attached
		...
		while (iter.hasNext()) {
			...
			if (context != null
					&& ((ASTNode) context).shouldFilterLookupResult(s))
				iter.remove();
