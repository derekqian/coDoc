...
import edu.pdx.svl.coDoc.cdt.core.parser.OffsetLimitReachedException;
import edu.pdx.svl.coDoc.cdt.core.parser.ParseError;
...
import edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTQualifiedNameElement;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.ast.ASTCompletionNode;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.token.OffsetDuple;
...
public class Parser implements IParserData, IParser {
			...
			if (!completedArg) {
				try {
					...
					if (expression.getExpressionKind() == IASTExpression.Kind.PRIMARY_EMPTY) {
						throwBacktrack(so, (lastToken != null) ? lastToken
		...
		try {
			...
			case IToken.tIDENTIFIER:
				...
				if (startsWithColonColon)
					...
					setCompletionValues(scope, kind, first, prev, argumentList
							.getTemplateArgumentsList(), KeywordSetKey.EMPTY);
				else
	...
	public IASTExpression expression(IASTScope scope, CompletionKind kind,
		...
		// extension.supportsStatementsInExpressions() )
		// {
	 ...
	 * @throws BacktrackException
	 */
	private IASTExpression compoundStatementExpression(IASTScope scope,
			IToken la, IASTExpression resultExpression)
			throws EndOfFileException, BacktrackException {
		int startingOffset = la.getOffset();
		int ln = la.getLineNumber();
		char[] fn = la.getFilename();
		...
		try {
			...
			try {
				resultExpression = astFactory.createExpression(scope, extension
						.getExpressionKindForStatement(), null, null, null,
	...
	protected IASTExpression equalityExpression(IASTScope scope,
		...
		for (;;) {
			...
			case IToken.tNOTEQUAL:
				IToken t = consume();
				IASTExpression secondExpression = relationalExpression(scope,
						kind, key);
				...
				try {
					firstExpression = astFactory
							.createExpression(
									scope,
									(t.getType() == IToken.tEQUAL) ? IASTExpression.Kind.EQUALITY_EQUALS
	...
	protected IASTExpression relationalExpression(IASTScope scope,
		...
		for (;;) {
			...
			case IToken.tGTEQUAL:
				...
				IASTExpression secondExpression = shiftExpression(scope, kind,
						key);
				...
				try {
					firstExpression = astFactory.createExpression(scope,
							expressionKind, firstExpression, secondExpression,
							null, null, null, EMPTY_STRING, null,
							(ITokenDuple) la);
				} catch (ASTSemanticException e) {
					throwBacktrack(e.getProblem());
			...
			default:
				if (extension.isValidRelationalExpressionStart(language, LT(1))) {
					IASTExpression extensionExpression = extension
							.parseRelationalExpression(scope, this, kind, key,
									firstExpression);
			...
			case IToken.tSHIFTR:
				...
				try {
					firstExpression = astFactory
							.createExpression(
									scope,
									((t.getType() == IToken.tSHIFTL) ? IASTExpression.Kind.SHIFT_LEFT
			...
			case IToken.tMINUS:
				...
				try {
					firstExpression = astFactory
							.createExpression(
									scope,
									((t.getType() == IToken.tPLUS) ? IASTExpression.Kind.ADDITIVE_PLUS
	...
	protected IASTExpression pmExpression(IASTScope scope, CompletionKind kind,
		...
		for (;;) {
			...
			case IToken.tARROWSTAR:
				IToken t = consume();
				IASTExpression secondExpression = castExpression(scope, kind,
						key);
				int endOffset = (lastToken != null) ? lastToken.getEndOffset()
						: 0;
				try {
					firstExpression = astFactory
							.createExpression(
									scope,
									((t.getType() == IToken.tDOTSTAR) ? IASTExpression.Kind.PM_DOTSTAR
		...
		if (LT(1) == IToken.tLPAREN) {
			...
			try {
				...
				if (castExpression != null
						&& castExpression.getExpressionKind() == IASTExpression.Kind.PRIMARY_EMPTY) {
					backup(mark);
					if (typeId != null)
						typeId.freeReferences();
	...
	private CompletionKind getCastExpressionKind(CompletionKind kind) {
		return ((kind == CompletionKind.SINGLE_NAME_REFERENCE || kind == CompletionKind.FUNCTION_REFERENCE) ? kind
				: CompletionKind.TYPE_REFERENCE);
	}
	...
	protected IASTExpression deleteExpression(IASTScope scope,
		...
		try {
			return astFactory.createExpression(scope,
					(vectored ? IASTExpression.Kind.DELETE_VECTORCASTEXPRESSION
		...
		case IToken.t_sizeof:
			...
			try {
				return astFactory.createExpression(scope,
						...
						unaryExpression, null, null, null, null, EMPTY_STRING,
						null, (ITokenDuple) la);
	...
	protected IASTExpression postfixExpression(IASTScope scope,
		...
		case IToken.t_typename:
			...
			try {
				firstExpression = astFactory
						.createExpression(
								scope,
								(templateTokenConsumed ? IASTExpression.Kind.POSTFIX_TYPENAME_TEMPLATEID
		...
		case IToken.t_typeid:
			...
			try {
				firstExpression = astFactory
						.createExpression(
								scope,
								(isTypeId ? IASTExpression.Kind.POSTFIX_TYPEID_TYPEID
										: IASTExpression.Kind.POSTFIX_TYPEID_EXPRESSION),
								lhs, null, null, typeId, null, EMPTY_STRING,
								null, (ITokenDuple) la);
		...
		for (;;) {
			...
			case IToken.tLPAREN:
				...
				if (firstExpression != null) {
					if (firstExpression.getExpressionKind() == IASTExpression.Kind.ID_EXPRESSION)
						...
						cntext = astFactory.expressionToMostPreciseASTNode(
								scope, firstExpression.getLHSExpression());
					}
				}
				...
				setCompletionValues(scope, CompletionKind.FUNCTION_REFERENCE,
						cntext);
				secondExpression = expression(scope,
			...
			case IToken.tDOT:
				...
				Kind memberCompletionKind = (isTemplate ? IASTExpression.Kind.POSTFIX_DOT_TEMPL_IDEXPRESS
				...
				if (secondExpression != null
						...
						&& CharArrayUtils.indexOf('~', secondExpression
								.getIdExpressionCharArray()) != -1)
					memberCompletionKind = Kind.POSTFIX_DOT_DESTRUCTOR;

				try {
					firstExpression = astFactory.createExpression(scope,
							memberCompletionKind, firstExpression,
							secondExpression, null, null, null, EMPTY_STRING,
							null, (ITokenDuple) la);
			...
			case IToken.tARROW:
				...
				Kind arrowCompletionKind = (isTemplate ? IASTExpression.Kind.POSTFIX_ARROW_TEMPL_IDEXP
	...
	protected IASTExpression primaryExpression(IASTScope scope,
		...
		case IToken.tFLOATINGPT:
			...
			try {
				return astFactory.createExpression(scope,
						IASTExpression.Kind.PRIMARY_FLOAT_LITERAL, null, null,
						null, null, null, t.getCharImage(), null,
						(ITokenDuple) t);
			} catch (ASTSemanticException e2) {
				throwBacktrack(e2.getProblem());
			} catch (Exception e) {
		...
		case IToken.tLSTRING:
			...
			try {
				return astFactory.createExpression(scope,
						IASTExpression.Kind.PRIMARY_STRING_LITERAL, null, null,
						null, null, null, t.getCharImage(), null,
						(ITokenDuple) t);
			} catch (ASTSemanticException e5) {
				throwBacktrack(e5.getProblem());
		...
		case IToken.tLCHAR:
			...
			try {
				return astFactory.createExpression(scope,
						IASTExpression.Kind.PRIMARY_CHAR_LITERAL, null, null,
						null, null, null, t.getCharImage(), null,
						(ITokenDuple) t);
			} catch (ASTSemanticException e4) {
				throwBacktrack(e4.getProblem());
		...
		case IToken.tLPAREN:
			...
			try {
				return astFactory.createExpression(scope,
						IASTExpression.Kind.PRIMARY_BRACKETED_EXPRESSION, lhs,
		...
		case IToken.tCOMPL:
			...
			try {
				return astFactory.createExpression(scope,
						IASTExpression.Kind.ID_EXPRESSION, null, null, null,
						null, duple, EMPTY_STRING, null, duple);
		...
		default:
			...
			try {
				empty = astFactory.createExpression(scope,
						IASTExpression.Kind.PRIMARY_EMPTY, null, null, null,
						null, null, EMPTY_STRING, null, (ITokenDuple) la);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.IParserData#getLastToken()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.IParserData#getParserLanguage()
	 */
	...
	public IToken identifier() throws EndOfFileException, BacktrackException {
		...
		if (first instanceof ITokenDuple)
			setGreaterNameContext((ITokenDuple) first);
		return first;
	}
	 ...
	 * 
	 */
	public Parser(IScanner scanner, ParserMode mode,
			ISourceElementRequestor callback, ParserLanguage language,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#failParse()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.IParser#parse()
	 */
	...
	protected void translationUnit() {
		...
		try {
			setCompletionValues(compilationUnit, CompletionKind.VARIABLE_TYPE,
	...
	protected IASTDeclaration usingClause(IASTScope scope)
		...
		setCompletionValues(scope, CompletionKind.TYPE_REFERENCE,
				KeywordSetKey.POST_USING);

		if (LT(1) == IToken.t_namespace) {
			// using-directive
			consume(IToken.t_namespace);

			setCompletionValues(scope, CompletionKind.NAMESPACE_REFERENCE,
					KeywordSetKey.EMPTY);
			...
			if (LT(1) == IToken.tSEMI) {
				...
				try {
					astUD = astFactory.createUsingDirective(scope, duple,
		...
		if (LT(1) == IToken.tSEMI) {
			...
			try {
				declaration = astFactory.createUsingDeclaration(scope,
						typeName, name, firstToken.getOffset(), firstToken
								.getLineNumber(), last.getEndOffset(), last
								.getLineNumber());
			} catch (Exception e1) {
			...
			setCompletionValues(scope, getCompletionKindForDeclaration(scope,
					null), KeywordSetKey.DECLARATION);
			return declaration;
		}
		int endOffset = (lastToken != null) ? lastToken.getEndOffset() : 0;
		throwBacktrack(firstToken.getOffset(), endOffset, firstToken
	...
	protected IASTDeclaration linkageSpecification(IASTScope scope)
		...
		if (LT(1) == IToken.tLBRACE) {
			...
			try {
				linkage = astFactory.createLinkageSpecification(scope, spec
						.getCharImage(), firstToken.getOffset(), firstToken
						.getLineNumber(), firstToken.getFilename());
			} catch (Exception e) {
				logException(
			...
			try {
				...
				linkage.setEndingOffsetAndLineNumber(lastTokenConsumed
						.getEndOffset(), lastTokenConsumed.getLineNumber());
			} finally {
	...
	protected IASTDeclaration templateDeclaration(IASTScope scope)
		...
		if (LT(1) == IToken.t_export) {
			...
			if (extension.supportsExtendedTemplateInstantiationSyntax()
					&& extension.isValidModifierForInstantiation(LA(1))) {
		...
		if (LT(1) != IToken.tLT) {
			...
			try {
				templateInstantiation = astFactory.createTemplateInstantiation(
						scope, firstToken.getOffset(), firstToken
								.getLineNumber(), firstToken.getFilename());
			} catch (Exception e) {
				logException(
			...
			try {
				declaration(templateInstantiation, templateInstantiation, null,
						KeywordSetKey.DECLARATION);
				templateInstantiation.setEndingOffsetAndLineNumber(lastToken
		...
		if (LT(1) == IToken.tGT) {
			...
			try {
				templateSpecialization = astFactory
						.createTemplateSpecialization(scope, firstToken
								.getOffset(), firstToken.getLineNumber(),
			...
			try {
				declaration(templateSpecialization, templateSpecialization,
						null, KeywordSetKey.DECLARATION);
				templateSpecialization.setEndingOffsetAndLineNumber(lastToken
						.getEndOffset(), lastToken.getLineNumber());
			} finally {
		...
		try {
			...
			try {
				templateDecl = astFactory.createTemplateDeclaration(scope,
						parms, exported, firstToken.getOffset(), firstToken
								.getLineNumber(), firstToken.getFilename());
			} catch (Exception e) {
			...
			try {
				declaration(templateDecl, templateDecl, null,
						KeywordSetKey.DECLARATION);
				templateDecl.setEndingOffsetAndLineNumber(lastToken
						.getEndOffset(), lastToken.getLineNumber());
			} finally {
				...
				try {
					if (LT(1) == IToken.tIDENTIFIER) // optional identifier
					{
						...
						if (LT(1) == IToken.tASSIGN) // optional = type-id
						{
							...
							typeId = typeId(parameterScope, false,
									CompletionKind.TYPE_REFERENCE); // type-id
						}
					}
					...
					returnValue
							.add(astFactory
									.createTemplateParameter(
											kind,
											(id == null) ? EMPTY_STRING : id
													.getCharImage(), //$NON-NLS-1$
											typeId,
											null,
											null,
											(parameterScope instanceof IASTCodeScope) ? (IASTCodeScope) parameterScope
													: null,
											...
											(lastToken != null) ? lastToken
													.getEndOffset() : nameEnd,
											(lastToken != null) ? lastToken
													.getLineNumber() : nameLine,
							...
							.add(astFactory
									.createTemplateParameter(
											...
											(optionalId == null) ? EMPTY_STRING
													: optionalId.getCharImage(), //$NON-NLS-1$
											optionalTypeId,
											null,
											subResult,
											(parameterScope instanceof IASTCodeScope) ? (IASTCodeScope) parameterScope
											...
											(optionalId != null) ? optionalId
													.getLineNumber() : 0,
											lastToken.getEndOffset(), lastToken
													.getLineNumber(), lastToken
													.getFilename()));
				} catch (ASTSemanticException ase) {
					throwBacktrack(ase.getProblem());
				} catch (Exception e) {
							...
							.add(astFactory
									.createTemplateParameter(
											...
											(parameterScope instanceof IASTCodeScope) ? (IASTCodeScope) parameterScope
													: null, wrapper
													.getStartingOffset(),
											wrapper.getStartingLine(),
											declarator.getNameStartOffset(),
											declarator.getNameEndOffset(),
											declarator.getNameLine(), wrapper
													.getEndOffset(), wrapper
													.getEndLine(), fn));
				} catch (ASTSemanticException ase) {
					throwBacktrack(ase.getProblem());
				} catch (Exception e) {
	 ...
	 *             request a backtrack
	 */
	protected void declaration(IASTScope scope, IASTTemplate ownerTemplate,
			CompletionKind overideKind, KeywordSetKey overideKey)
			throws EndOfFileException, BacktrackException {

		IASTCompletionNode.CompletionKind kind = getCompletionKindForDeclaration(
				scope, overideKind);
		...
		case IToken.t_asm:
			...
			try {
				resultDeclaration = astFactory.createASMDefinition(scope,
						assembly, first.getOffset(), first.getLineNumber(),
						last.getEndOffset(), last.getLineNumber(), last
								.getFilename());
			} catch (Exception e) {
		...
		default:
			if (extension.supportsExtendedTemplateInstantiationSyntax()
				...
				resultDeclaration = simpleDeclarationStrategyUnion(scope,
						ownerTemplate, overideKind, overideKey);
		}
		setCompletionValues(scope, kind, KeywordSetKey.DECLARATION);
		endDeclaration(resultDeclaration);
	}

	protected IASTDeclaration simpleDeclarationStrategyUnion(IASTScope scope,
			IASTTemplate ownerTemplate, CompletionKind overrideKind,
			KeywordSetKey overrideKey) throws EndOfFileException,
			BacktrackException {
		...
		try {
			return simpleDeclaration(SimpleDeclarationStrategy.TRY_CONSTRUCTOR,
					scope, ownerTemplate, overrideKind, false, overrideKey);
	...
	protected IASTDeclaration namespaceDefinition(IASTScope scope)
		...
		if (LT(1) == IToken.tLBRACE) {
			...
			try {
				namespaceDefinition = astFactory.createNamespaceDefinition(
						scope,
						(identifier == null ? EMPTY_STRING : identifier
								.getCharImage()), //$NON-NLS-1$
						first.getOffset(), first.getLineNumber(),
						(identifier == null ? first.getOffset() : identifier
								.getOffset()), (identifier == null ? first
								.getEndOffset() : identifier.getEndOffset()),
						(identifier == null ? first.getLineNumber()
								: identifier.getLineNumber()), first
								.getFilename());
			} catch (Exception e1) {
			...
			try {
				setCompletionValues(scope, CompletionKind.VARIABLE_TYPE,
						KeywordSetKey.DECLARATION);
				...
				namespaceDefinition.setEndingOffsetAndLineNumber(last
						.getOffset()
						+ last.getLength(), last.getLineNumber());
				setCompletionValues(scope, kind, KeywordSetKey.DECLARATION);
			} finally {
			...
			try {
				alias = astFactory.createNamespaceAlias(scope, identifier
						.getCharImage(), duple, first.getOffset(), first
						.getLineNumber(), identifier.getOffset(), identifier
						.getEndOffset(), identifier.getLineNumber(), duple
						.getLastToken().getEndOffset(), duple.getLastToken()
						.getLineNumber());
			} catch (Exception e1) {
	...
	protected IASTDeclaration simpleDeclaration(
			SimpleDeclarationStrategy strategy, IASTScope scope,
			IASTTemplate ownerTemplate, CompletionKind overideKind,
			boolean fromCatchHandler, KeywordSetKey overrideKey)
			throws BacktrackException, EndOfFileException {
		...
		CompletionKind completionKindForDeclaration = getCompletionKindForDeclaration(
				scope, overideKind);
		...
		declSpecifierSeq(sdw, false,
				...
				completionKindForDeclaration, overrideKey);
		IASTSimpleTypeSpecifier simpleTypeSpecifier = null;
		if (sdw.getTypeSpecifier() == null
				&& sdw.getSimpleType() != IASTSimpleTypeSpecifier.Type.UNSPECIFIED)
			try {
				simpleTypeSpecifier = astFactory.createSimpleTypeSpecifier(
						scope, sdw.getSimpleType(), sdw.getName(), sdw
								.isShort(), sdw.isLong(), sdw.isSigned(), sdw
								.isUnsigned(), sdw.isTypeNamed(), sdw
								.isComplex(), sdw.isImaginary(), sdw
								.isGloballyQualified(), sdw
								.getExtensionParameters());
				sdw.setTypeSpecifier(simpleTypeSpecifier);
		...
		try {
			...
			try {
				...
				if (e.getProblem() == null) {
					IProblem p = problemFactory.createProblem(
							IProblem.SYNTAX_ERROR, sdw.getStartingOffset(),
							lastToken != null ? lastToken.getEndOffset() : 0,
							sdw.getStartingLine(), fn, EMPTY_STRING, false,
							true);
					throwBacktrack(p);
				} else {
					throwBacktrack(e.getProblem());
			...
			if (!l.isEmpty()) // no need to do this unless we have a
								// declarator
			{
			...
			try {
				if (sdw.getTypeSpecifier() != null) {
					IASTAbstractTypeSpecifierDeclaration declaration = astFactory
							.createTypeSpecDeclaration(sdw.getScope(), sdw
									.getTypeSpecifier(), ownerTemplate, sdw
									.getStartingOffset(),
									sdw.getStartingLine(), lastToken
											.getEndOffset(), lastToken
											.getLineNumber(), sdw.isFriend(),
	...
	protected void handleFunctionBody(IASTScope scope)
			throws BacktrackException, EndOfFileException {
		if (mode == ParserMode.QUICK_PARSE
				|| mode == ParserMode.STRUCTURAL_PARSE)
			skipOverCompoundStatement();
	...
	protected void ctorInitializer(Declarator d) throws EndOfFileException,
		...
		for (;;) {
			...
			ITokenDuple duple = name(scope,
					CompletionKind.SINGLE_NAME_REFERENCE, KeywordSetKey.EMPTY);
			...
			try {
				d.addConstructorMemberInitializer(astFactory
						.createConstructorMemberInitializer(scope, duple,
								expressionList));
			} catch (Exception e1) {
				logException(
	...
	protected void parameterDeclaration(IParameterCollection collection,
			IASTScope scope) throws BacktrackException, EndOfFileException {
		IToken current = LA(1);
		...
		if (sdw.getTypeSpecifier() == null
				&& sdw.getSimpleType() != IASTSimpleTypeSpecifier.Type.UNSPECIFIED)
			try {
				sdw.setTypeSpecifier(astFactory.createSimpleTypeSpecifier(
						scope, sdw.getSimpleType(), sdw.getName(), sdw
								.isShort(), sdw.isLong(), sdw.isSigned(), sdw
								.isUnsigned(), sdw.isTypeNamed(), sdw
								.isComplex(), sdw.isImaginary(), sdw
								.isGloballyQualified(), null));
			} catch (ASTSemanticException e) {
				throwBacktrack(e.getProblem());
			} catch (Exception e) {
				int endOffset = (lastToken != null) ? lastToken.getEndOffset()
	...
	private boolean lookAheadForConstructorOrConversion(Flags flags,
		...
		try {
			...
			boolean result = CharArrayUtils.equals(className.getCharImage(),
					duple.getLastToken().getCharImage());
			backup(mark);
			return result;
		} finally {
			...
			default:
				if (extension.canHandleDeclSpecifierSequence(LT(1))) {
					IParserExtension.IDeclSpecifierExtensionResult declSpecExtResult = extension
							.parseDeclSpecifierSequence(this, flags, sdw, kind,
									key);
	...
	protected IASTInitializerClause cInitializerClause(IASTScope scope,
		...
		if (LT(1) == IToken.tLBRACE) {
			...
			return createInitializerClause(
					scope,
					((designators.size() == 0) ? IASTInitializerClause.Kind.INITIALIZER_LIST
		...
		try {
			...
			try {
				return createInitializerClause(
						scope,
						((designators.size() == 0) ? IASTInitializerClause.Kind.ASSIGNMENT_EXPRESSION
		...
		if (LT(1) == IToken.tLBRACE) {
			...
			try {
				return createInitializerClause(scope,
						...
						initializerClauses == null ? Collections.EMPTY_LIST
		...
		overallLoop: do {
			...
			for (;;) {
				...
				case IToken.tLPAREN:
					...
					boolean failed = false;
					IASTScope parameterScope = astFactory.getDeclaratorScope(
							scope, d.getNameDuple());
						...
						while (!done) {
							...
							default:
								try {
									...
									exceptionTypeId.acceptElement(requestor);
								} catch (BacktrackException e) {
									failParse(e);
									consume();
					...
					if (LT(1) == IToken.tCOLONCOLON
						...
						if (LT(1) == IToken.t_operator)
							operatorId(d11, start,
									(hasTemplateId ? argumentList : null), kind);
						else {
							int endOffset = (lastToken != null) ? lastToken
									.getEndOffset() : 0;
							backup(mark);
			...
			try {
				enumeration = astFactory.createEnumerationSpecifier(
						...
						((identifier == null) ? mark.getOffset() : identifier
								...
								.getEndOffset() : identifier.getEndOffset()),
						((identifier == null) ? mark.getLineNumber()
								: identifier.getLineNumber()), mark
								.getFilename());
			} catch (ASTSemanticException e) {
				throwBacktrack(e.getProblem());
			} catch (Exception e) {
	...
	protected void classSpecifier(DeclarationWrapper sdw)
		...
		if (LT(1) == IToken.tIDENTIFIER) {
			// Bug 66496
			// special magic to handle macros that resolve to
			// dllimport/dllexport in quick parse
		...
		try {
			astClassSpecifier = astFactory.createClassSpecifier(sdw.getScope(),
					...
					duple == null ? classKey.getEndOffset() : duple
							.getFirstToken().getEndOffset(),
					duple == null ? classKey.getLineNumber() : duple
							.getFirstToken().getLineNumber(), classKey
							.getFilename());
		} catch (ASTSemanticException e) {
			throwBacktrack(e.getProblem());
		} catch (Exception e) {
		...
		if (LT(1) == IToken.tLBRACE) {
			...
			try {
				...
				try {
					...
					throwBacktrack(lt.getOffset(), lt.getEndOffset(), lt
							.getLineNumber(), lt.getFilename());
				}
	 ...
	 * @throws BacktrackException
	 */
	protected void baseSpecifier(IASTClassSpecifier astClassSpec)
			throws EndOfFileException, BacktrackException {
		IToken la = LA(1);
		char[] fn = la.getFilename();
		int startingOffset = la.getOffset();
		...
		setCompletionValues(astClassSpec.getOwnerScope(),
				CompletionKind.CLASS_REFERENCE, KeywordSetKey.BASE_SPECIFIER);
		...
		baseSpecifierLoop: for (;;) {
			...
			case IToken.t_virtual:
				...
				setCompletionValues(astClassSpec.getOwnerScope(),
						CompletionKind.CLASS_REFERENCE, KeywordSetKey.EMPTY);
				isVirtual = true;
				break;
			case IToken.t_public:
				consume();
			...
			case IToken.tIDENTIFIER:
				...
				nameDuple = name(astClassSpec, CompletionKind.CLASS_REFERENCE,
						KeywordSetKey.BASE_SPECIFIER);
				break;
			case IToken.tCOMMA:
				// because we are using the class as the scope to get the name,
				// we need to postpone adding the base
				...
				consume();
				setCompletionValues(astClassSpec.getOwnerScope(),
						CompletionKind.CLASS_REFERENCE,
						KeywordSetKey.BASE_SPECIFIER);
		...
		try {
			...
			astFactory.addBaseSpecifier(astClassSpec, isVirtual, visibility,
					nameDuple);
		} catch (ASTSemanticException e) {
		...
		case IToken.t_if:
			if_loop: while (true) {
				...
				try {
					...
					// if the problem has no offset info, make a new one that
					// does
					if (b.getProblem() != null
							&& b.getProblem().getSourceLineNumber() == -1) {
						IProblem p = b.getProblem();
						IProblem p2 = problemFactory.createProblem(p.getID(),
								start.getOffset(),
								lastToken != null ? lastToken.getEndOffset()
										: start.getEndOffset(), start
										.getLineNumber(), p
										.getOriginatingFileName(), p
										.getArguments() != null ? p
										.getArguments().toCharArray() : null, p
										.isWarning(), p.isError());
						b.initialize(p2);
					}
					failParse(b);
				...
				if (LT(1) == IToken.t_else) {
					...
					if (LT(1) == IToken.t_if) {
						// an else if, don't recurse, just loop and do another
						// if
						cleanupLastToken();
						continue if_loop;
	...
	protected void catchHandlerSequence(IASTScope scope)
		...
		if (LT(1) != IToken.t_catch) {
			IToken la = LA(1);
			throwBacktrack(la.getOffset(), la.getEndOffset(), la
					.getLineNumber(), la.getFilename()); // error, need at
															// least one of
															// these
		}
		while (LT(1) == IToken.t_catch) {
			...
			setCompletionValues(scope, CompletionKind.NO_SUCH_KIND,
					KeywordSetKey.EMPTY);
			consume(IToken.tLPAREN);
			setCompletionValues(scope, CompletionKind.EXCEPTION_REFERENCE,
					KeywordSetKey.DECL_SPECIFIER_SEQUENCE);
			try {
				if (LT(1) == IToken.tELLIPSIS)
					...
					simpleDeclaration(SimpleDeclarationStrategy.TRY_VARIABLE,
							scope, null, CompletionKind.EXCEPTION_REFERENCE,
							true, KeywordSetKey.DECLARATION);
				consume(IToken.tRPAREN);
	...
	protected void compoundStatement(IASTScope scope, boolean createNewScope)
		...
		try {
			...
			while (LT(1) != IToken.tRBRACE) {
				int checkToken = LA(1).hashCode();
				try {
					statement((IASTCodeScope) (createNewScope ? newScope
							: scope));
				} catch (BacktrackException b) {
					failParse(b);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.IParser#getLanguage()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.IParser#setLanguage(Language)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.IParser#getLastErrorOffset()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.IParser#parse(int, int)
	 */
	public ISelectionParseResult parse(int startingOffset, int endingOffset)
			throws ParseError {
		if (mode != ParserMode.SELECTION_PARSE)
			throw new ParseError(
					ParseError.ParseErrorKind.METHOD_NOT_IMPLEMENTED);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.IParser#parse(int)
	 */
	public IASTCompletionNode parse(int offset) throws ParseError {
		...
		// System.out.println("Completion Parse time: " + (stopTime - startTime)
		// + "ms");
		return new ASTCompletionNode(getCompletionKind(), getCompletionScope(),
				getCompletionContext(), getCompletionPrefix(),
				reconcileKeywords(getKeywordSet(), getCompletionPrefix()),
				String.valueOf(getCompletionFunctionName()),
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#setupASTFactory(edu.pdx.svl.coDoc.cdt.core.parser.IScanner,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ParserLanguage)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#getCompliationUnit()
	 */
	...
	/**
	 *  /* (non-Javadoc)
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.IParser#cancel()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#validateCaches()
	 */
	...
	protected String getCompletionPrefix() {
		return (finalToken == null ? String.valueOf(EMPTY_STRING) : finalToken
				.getImage());
	}
	 ...
	 * @param string
	 */
	protected void setCompletionValues(CompletionKind kind, Set keywordSet,
			String prefix) {
		if (mode == ParserMode.COMPLETION_PARSE
				|| mode == ParserMode.SELECTION_PARSE) {
			...
			setCompletionToken(TokenFactory.createStandAloneToken(
					IToken.tIDENTIFIER, prefix));
		}
	}
	...
	protected void setCompletionValues(IASTScope scope, CompletionKind kind,
			KeywordSetKey key, IASTNode node, String prefix)
			throws EndOfFileException {
		if (mode == ParserMode.COMPLETION_PARSE
				|| mode == ParserMode.SELECTION_PARSE) {
			setCompletionToken(TokenFactory.createStandAloneToken(
	...
	protected void setCompletionValues(IASTScope scope, CompletionKind kind,
			KeywordSetKey key) throws EndOfFileException {
		if (mode == ParserMode.COMPLETION_PARSE
				|| mode == ParserMode.SELECTION_PARSE)
	...
	protected void setCompletionValues(IASTScope scope, CompletionKind kind,
			IToken first, IToken last, List arguments, KeywordSetKey key)
			throws EndOfFileException {
		if (mode == ParserMode.COMPLETION_PARSE
				|| mode == ParserMode.SELECTION_PARSE) {
			setCompletionScope(scope);
			...
			ITokenDuple duple = TokenFactory.createTokenDuple(first, last,
					arguments);
			try {
				setCompletionContext(astFactory.lookupSymbolInContext(scope,
						duple, null));
	...
	protected void setCompletionValues(IASTScope scope, CompletionKind kind,
		...
		if (mode == ParserMode.COMPLETION_PARSE
				|| mode == ParserMode.SELECTION_PARSE) {
			IASTNode node = astFactory.expressionToMostPreciseASTNode(scope,
					firstExpression);
	...
	private boolean validMemberOperation(IASTNode node, Kind expressionKind) {
		if (expressionKind == Kind.POSTFIX_ARROW_IDEXPRESSION
				|| expressionKind == Kind.POSTFIX_ARROW_TEMPL_IDEXP)
			return astFactory.validateIndirectMemberOperation(node);
		else if (expressionKind == Kind.POSTFIX_DOT_IDEXPRESSION
				|| expressionKind == Kind.POSTFIX_DOT_TEMPL_IDEXPRESS)
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#setCompletionValues(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTCompletionNode.CompletionKind)
	 */
	protected void setCompletionValues(IASTScope scope, CompletionKind kind)
			throws EndOfFileException {
		if (mode == ParserMode.COMPLETION_PARSE
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#setCurrentFunctionName(java.lang.String)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#setCompletionValuesNoContext(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTCompletionNode.CompletionKind,
	 *      edu.pdx.svl.coDoc.cdt.internal.core.parser.token.KeywordSets.Key)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#setParameterListExpression(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.Parser#handleNewToken(edu.pdx.svl.coDoc.cdt.core.parser.IToken)
	 */
	protected void handleNewToken(IToken value) {
		if (mode != ParserMode.SELECTION_PARSE)
			return;
		if (value != null
				&& CharArrayUtils.equals(value.getFilename(),
						parserStartFilename)) {
			TraceUtil
					.outputTrace(
							log,
							"IToken provided w/offsets ", null, value.getOffset(), " & ", value.getEndOffset()); //$NON-NLS-1$ //$NON-NLS-2$
			boolean change = false;
			if (value.getOffset() == offsetRange.getFloorOffset()) {
				TraceUtil
						.outputTrace(
								log,
								"Offset Floor Hit w/token \"", null, value.getCharImage(), "\"", null); //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (value.getEndOffset() == offsetRange.getCeilingOffset()) {
				TraceUtil
						.outputTrace(
								log,
								"Offset Ceiling Hit w/token \"", null, value.getCharImage(), "\"", null); //$NON-NLS-1$ //$NON-NLS-2$
				change = true;
	...
	protected boolean tokenDupleCompleted() {
		return lastTokenOfDuple != null
				&& lastTokenOfDuple.getEndOffset() >= offsetRange
						.getCeilingOffset();
	}
	...
	protected ISelectionParseResult reconcileTokenDuple() throws ParseError {
		if (firstTokenOfDuple == null || lastTokenOfDuple == null)
			throw new ParseError(
					ParseError.ParseErrorKind.OFFSET_RANGE_NOT_NAME);

		if (getCompletionKind() == IASTCompletionNode.CompletionKind.UNREACHABLE_CODE)
			throw new ParseError(
					ParseError.ParseErrorKind.OFFSETDUPLE_UNREACHABLE);

		ITokenDuple duple = TokenFactory.createTokenDuple(firstTokenOfDuple,
				lastTokenOfDuple);
	...
	protected ISelectionParseResult provideSelectionNode(ITokenDuple duple) {
		...
		if (!duple.equals(greaterContextDuple)) {
			...
			if (greaterContextDuple == null
					|| duple.getFirstToken().equals(
							greaterContextDuple.getFirstToken()))
				finalDuple = duple; // => do not use greaterContextDuple
			else if (duple.getLastSegment().getFirstToken().equals(
					greaterContextDuple.getLastSegment().getFirstToken()))
				finalDuple = greaterContextDuple; // => use
													// greaterContextDuple
			else
				throw new ParseError(
						ParseError.ParseErrorKind.OFFSET_RANGE_NOT_NAME);
		...
		if (!(node instanceof IASTOffsetableNamedElement))
			return null;
		return new SelectionParseResult((IASTOffsetableNamedElement) node,
				new String(((IASTOffsetableElement) node).getFilename()));
	}
	...
	protected IASTNode lookupNode(ITokenDuple finalDuple) {
		...
		if (contextNode instanceof IASTDeclaration) {
			if (contextNode instanceof IASTOffsetableNamedElement
					&& !(contextNode instanceof IASTUsingDirective)
					&& !(contextNode instanceof IASTUsingDeclaration)) {
				if (contextNode instanceof IASTFunction) {
					Iterator i = ((IASTFunction) contextNode).getParameters();
					while (i.hasNext()) {
						IASTParameterDeclaration parm = (IASTParameterDeclaration) i
								.next();
						if (parm.getName().equals(finalDuple.toString())
								&& parm.getNameOffset() == finalDuple
										.getStartOffset()
								&& parm.getStartingLine() == finalDuple
										.getLineNumber()
								&& parm instanceof IASTNode) {
				...
				if (contextNode instanceof IASTMethod) {
					...
					while (parms.hasNext()) {
						...
						if (parm instanceof IASTParameterDeclaration
								&& ((IASTParameterDeclaration) parm).getName()
										.equals(finalDuple.toString())
								&& ((IASTParameterDeclaration) parm)
										.getNameOffset() == finalDuple
										.getStartOffset()) {
							return (IASTNode) parm;
						}
					}
				...
				if (((IASTOffsetableNamedElement) contextNode).getName()
						.equals(finalDuple.toString())
						&& ((IASTOffsetableNamedElement) contextNode)
								.getNameOffset() == finalDuple.getStartOffset()) // 75731
																					// needs
																					// to
																					// include
																					// offset
			...
			if (contextNode instanceof IASTQualifiedNameElement) {
				String[] elementQualifiedName = ((IASTQualifiedNameElement) contextNode)
						.getFullyQualifiedName();
				if (Arrays.equals(elementQualifiedName, finalDuple
						.toQualifiedName())) {
					IASTNode declNode = null;
					if (contextNode instanceof ISymbolOwner) {
						ISymbolOwner owner = (ISymbolOwner) contextNode;
						if (owner.getSymbol() != null
								&& owner.getSymbol().getASTExtension() != null) {
							declNode = owner.getSymbol().getASTExtension()
			...
			try {
				if (ourKind == IASTCompletionNode.CompletionKind.NEW_TYPE_REFERENCE) {
					if (contextNode instanceof IASTVariable) {
						IASTInitializerClause initializer = ((IASTVariable) contextNode)
								.getInitializerClause();
						if (initializer != null) {
							IASTExpression ownerExpression = initializer
									.findExpressionForDuple(finalDuple);
							return astFactory.lookupSymbolInContext(ourScope,
									finalDuple, ownerExpression);
						}
					}
				}

				return astFactory.lookupSymbolInContext(ourScope, finalDuple,
						null);
			} catch (ASTNotImplementedException e) {
				return null;
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.ExpressionParser#setGreaterNameContext(edu.pdx.svl.coDoc.cdt.core.parser.ITokenDuple)
	 */
	protected void setGreaterNameContext(ITokenDuple tokenDuple) {
		if (mode != ParserMode.SELECTION_PARSE)
			return;
		if (pastPointOfSelection)
			return;
		if (greaterContextDuple == null
				&& lastTokenOfDuple != null
				&& firstTokenOfDuple != null
				&& CharArrayUtils.equals(tokenDuple.getFilename(),
						parserStartFilename)) {
			...
			for (IToken token = tokenDuple.getFirstToken(); token != null; token = token
					.getNext()) {
				if (token == firstTokenOfDuple)
					++tokensFound;
				if (token == lastTokenOfDuple)
					++tokensFound;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.Parser#endExpressionStatement(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTExpression)
	 */
	...
	public static class SelectionParseResult implements ISelectionParseResult {
		...
		public SelectionParseResult(IASTOffsetableNamedElement node,
				String fileName) {
			this.node = node;
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.parser.IParser.ISelectionParseResult#getNode()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.parser.IParser.ISelectionParseResult#getFilename()
		 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.Parser#endEnumerator(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTEnumerator)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.Parser#endClassSpecifier(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTClassSpecifier)
	 */
	...
	protected void handleEnumeration(IASTEnumerationSpecifier enumeration)
			throws EndOfFileException {
		if (mode != ParserMode.SELECTION_PARSE || !tokenDupleCompleted())
			cleanupLastToken();
		else {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.Parser#handleOffsetLimitException()
	 */
	protected void handleOffsetLimitException(
		...
		if (exception.getCompletionNode() == null) {
			...
			ASTCompletionNode node = (ASTCompletionNode) exception
					.getCompletionNode();
			setCompletionValues(node.getCompletionKind(), node.getKeywordSet(),
					node.getCompletionPrefix());
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.Parser#getCompletionKindForDeclaration(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope)
	 */
	protected CompletionKind getCompletionKindForDeclaration(IASTScope scope,
			CompletionKind overide) {
		if (mode != ParserMode.COMPLETION_PARSE)
			return null;
	 ...
	 */
	public static void processReferences(List references,
