...
public class CPPClassScope extends CPPScope implements ICPPClassScope {
	...
	// 12.1 The default constructor, copy constructor, copy assignment operator,
	// and destructor are
	...
	private void createImplicitMembers() {
		// create bindings for the implicit members, if the user declared them
		// then those declarations
		...
		if (clsType instanceof ICPPClassTemplate) {
			try {
				IBinding within = CPPTemplates
						.instantiateWithinClassTemplate((ICPPClassTemplate) clsType);
		...
		IParameter[] voidPs = new IParameter[] { new CPPParameter(
				CPPSemantics.VOID_TYPE) };
		...
		IType pType = new CPPReferenceType(new CPPQualifierType(clsType, true,
				false));
		...
		m = new CPPImplicitMethod(this, ICPPASTOperatorName.OPERATOR_ASSIGN,
				refType, ps); //$NON-NLS-1$
		implicits[2] = m;
		addBinding(m);

		// destructor: ~A()
		char[] dtorName = CharArrayUtils.concat("~".toCharArray(), className); //$NON-NLS-1$
		m = new CPPImplicitMethod(this, dtorName, new CPPBasicType(
				IBasicType.t_unspecified, 0), voidPs);
		implicits[3] = m;
		addBinding(m);
	}

	private boolean hasNonStandardDefaultConstructor(
		...
		for (int i = 0; i < members.length; i++) {
			...
			if (!(dtor instanceof ICPPASTFunctionDeclarator)
					|| !(spec instanceof IASTSimpleDeclSpecifier)
					|| ((IASTSimpleDeclSpecifier) spec).getType() != IASTSimpleDeclSpecifier.t_unspecified
					|| !CharArrayUtils.equals(dtor.getName().toCharArray(),
							name)) {
				continue;
			}

			IASTParameterDeclaration[] ps = ((ICPPASTFunctionDeclarator) dtor)
					.getParameters();
			if (ps.length >= 1) {
				...
				if (s instanceof IASTSimpleDeclSpecifier
						...
						&& !(d instanceof IASTArrayDeclarator)) {
					continue; // A(void)
				}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPScope#addBinding(edu.pdx.svl.coDoc.cdt.core.dom.ast.IBinding)
	 */
	...
	private void addConstructor(Object constructor) {
		...
		if (constructor instanceof IASTName
				&& ((IASTName) constructor).getBinding() != null) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPScope#getBinding(int, char[])
	 */
	public IBinding getBinding(IASTName name, boolean resolve)
		...
		if (CharArrayUtils.equals(c, compName.toCharArray())) {
			...
			// 9.2 ... The class-name is also inserted into the scope of the
			// class itself
	...
	static protected ICPPConstructor[] getConstructors(
			CharArrayObjectMap bindings, boolean forceResolve) {
		if (bindings == null)
			...
			if (o instanceof ObjectSet) {
				...
				for (int i = 0; i < set.size(); i++) {
					...
					if (obj instanceof IASTName) {
						...
						binding = forceResolve ? n.resolveBinding() : n
								.getBinding();
						if (binding != null) {
							set.remove(n);
							set.put(binding);
						...
						bs = (IBinding[]) ArrayUtil.append(
								ICPPConstructor.class, bs, obj);
				}
				return (ICPPConstructor[]) ArrayUtil.trim(
	...
	public IBinding[] find(String name) throws DOMException {
		...
		if (CharArrayUtils.equals(n, compName.toCharArray())) {
			return (IBinding[]) ArrayUtil.addAll(IBinding.class, null,
					getConstructors(bindings, true));
		}
	...
	static protected boolean isConstructorReference(IASTName name) {
		...
		if (node instanceof IASTDeclSpecifier) {
			...
			if (parent instanceof IASTTypeId
					&& parent.getParent() instanceof ICPPASTNewExpression)
				return true;
			return false;
		}
		return true;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPClassScope#getClassType()
	 */
	public ICPPClassType getClassType() {
		...
		return new CPPClassType.CPPClassTypeProblem(compSpec.getName(),
				IProblemBinding.SEMANTIC_BAD_SCOPE, compSpec.getName()
						.toCharArray());
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPClassScope#getImplicitMethods()
	 */
	public ICPPMethod[] getImplicitMethods() {
		if (implicits == null)
			implicits = new ICPPMethod[] { new CPPMethod.CPPMethodProblem(null,
					IProblemBinding.SEMANTIC_INVALID_TYPE,
					CPPSemantics.EMPTY_NAME_ARRAY) };
		return implicits;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPScope#removeBinding(edu.pdx.svl.coDoc.cdt.core.dom.ast.IBinding)
	 */
