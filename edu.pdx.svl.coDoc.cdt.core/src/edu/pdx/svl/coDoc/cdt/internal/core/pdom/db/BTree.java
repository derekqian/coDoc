 ...
 *******************************************************************************/
package edu.pdx.svl.coDoc.cdt.internal.core.pdom.db;
...
public class BTree {
	...
	protected static final int OFFSET_CHILDREN = NUM_RECORDS
			* Database.INT_SIZE;
	 ...
	 * 
	 * @param db
	 *            the database containing the btree
	 * @param root
	 *            offset into database of the pointer to the root node
	 */
	...
	/**
	 * Inserts the record into the b-tree. We don't insert if the key was
	 * already there, in which case we return the record that matched. In other
	 * cases, we just return the record back.
	 * 
	 * @param offset
	 *            of the record
	 * @return
	 */
	public int insert(int record, IBTreeComparator comparator)
	...
	private int insert(Chunk pChunk, int parent, int iParent, int node,
			int record, IBTreeComparator comparator) throws CoreException {
		...
		if (getRecord(chunk, node, NUM_RECORDS - 1) != 0) {
			...
			if (median == record)
				...
				for (int i = 0; i < MEDIAN_RECORD; ++i) {
					putRecord(newchunk, newnode, i, getRecord(chunk, node,
							MEDIAN_RECORD + 1 + i));
					putRecord(chunk, node, MEDIAN_RECORD + 1 + i, 0);
					putChild(newchunk, newnode, i, getChild(chunk, node,
							MEDIAN_RECORD + 1 + i));
					putChild(chunk, node, MEDIAN_RECORD + 1 + i, 0);
				}
				putChild(newchunk, newnode, MEDIAN_RECORD, getChild(chunk,
						node, NUM_RECORDS));
				...
				if (parent == 0) {
					...
					for (int i = NUM_RECORDS - 2; i >= iParent; --i) {
						...
						if (r != 0) {
							...
							putChild(pChunk, parent, i + 2, getChild(pChunk,
									parent, i + 1));
						}
					}
				}
	...
	/**
	 * Visit all nodes beginning when the visitor comparator returns >= 0 until
	 * the visitor visit returns falls.
	 * 
	 * @param visitor
	 */
	...
	private boolean accept(int node, IBTreeVisitor visitor, boolean found)
			throws CoreException {
