...
import edu.pdx.svl.coDoc.cdt.core.dom.IPDOMWriter;
...
import edu.pdx.svl.coDoc.cdt.internal.core.pdom.dom.PDOMFile.Comparator;
import edu.pdx.svl.coDoc.cdt.internal.core.pdom.dom.PDOMFile.Finder;
 ...
 */
public class PDOM extends PlatformObject implements IPDOM, IPDOMResolver,
		IPDOMWriter {
	...
	public void accept(IPDOMVisitor visitor) throws CoreException {
		for (PDOMLinkage linkage = getFirstLinkage(); linkage != null; linkage = linkage
				.getNextLinkage())
	...
	public IASTName[] getDeclarations(IBinding binding) {
		try {
			if (binding instanceof PDOMBinding) {
				...
				for (PDOMName name = ((PDOMBinding) binding)
	...
	public PDOMLinkage getLinkage(ILanguage language) throws CoreException {
		...
		// Need to create it
		IPDOMLinkageFactory factory = (IPDOMLinkageFactory) language
				.getAdapter(IPDOMLinkageFactory.class);
	...
	public PDOMLinkage getLinkage(int record) throws CoreException {
		...
		// First check the cache. We do a linear search since there will be very
		// few linkages
		// in a given database.
	...
	public PDOMBinding getBinding(int record) throws CoreException {
		if (record == 0)
			...
			PDOMNode node = PDOMLinkage.getLinkage(this, record)
					.getNode(record);
