...
public class CPPClassInstanceScope implements ICPPClassScope {
	...
	public IBinding getBinding(IASTName name, boolean forceResolve) {
		...
		if (CharArrayUtils.equals(c, instance.getNameCharArray())
		...
		if (cache != null) {
			...
			ObjectSet set = (cache instanceof ObjectSet) ? (ObjectSet) cache
					: null;
			...
			while (obj != null) {
				if (obj instanceof IASTName) {
					...
					if (instanceMap.containsKey(n)) {
						...
						if (binding != null) {
							binding = CPPTemplates.createSpecialization(this,
									binding, instance.getArgumentMap());
					...
					if (instanceMap.containsKey(obj)) {
						...
						binding = CPPTemplates.createSpecialization(this,
								(IBinding) obj, instance.getArgumentMap());
						if (instanceMap == ObjectMap.EMPTY_MAP)
							instanceMap = new ObjectMap(2);
				...
				if (binding != null) {
					if (i == -1)
						return binding;
					bs = (IBinding[]) ArrayUtil.append(IBinding.class, bs,
							binding);
					binding = null;
				}
				if (i != -1 && ++i < set.size()) {
	...
	public void addName(IASTName name) {
		...
		if (parent instanceof IASTDeclarator
				&& CPPVisitor.isConstructor(this, (IASTDeclarator) parent)) {
			c = CONSTRUCTOR_KEY;
		}
	...
	protected ICPPConstructor[] getConstructors() {
		...
		if (!doneConstructors) {
			...
			try {
				ctors = ((ICPPClassType) instance.getTemplateDefinition())
						.getConstructors();
		...
		for (int i = 0; i < ctors.length; i++) {
			if (instanceMap.containsKey(ctors[i])) {
				...
				IBinding b = CPPTemplates.createSpecialization(this, ctors[i],
						instance.getArgumentMap());
				if (instanceMap == ObjectMap.EMPTY_MAP)
					instanceMap = new ObjectMap(2);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPScope#setFullyCached(boolean)
	 */
	...
	public void addBinding(IBinding binding) {
		if (bindings == null)
			bindings = new CharArrayObjectMap(1);
		char[] c = (binding instanceof ICPPConstructor) ? CONSTRUCTOR_KEY
				: binding.getNameCharArray();
		Object o = bindings.get(c);
		if (o != null) {
			if (o instanceof ObjectSet) {
