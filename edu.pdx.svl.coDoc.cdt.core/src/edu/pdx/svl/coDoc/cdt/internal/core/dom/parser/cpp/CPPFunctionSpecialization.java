...
import edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.cpp.CPPFunction.CPPFunctionDelegate;

/**
 * @author aniefer
 */
public class CPPFunctionSpecialization extends CPPSpecialization implements
		ICPPFunction, ICPPInternalFunction {
	private IFunctionType type = null;

	private IParameter[] specializedParams = null;

	public CPPFunctionSpecialization(IBinding orig, ICPPScope scope,
			ObjectMap argMap) {
	...
	public IParameter[] getParameters() throws DOMException {
		if (specializedParams == null) {
			...
			for (int i = 0; i < params.length; i++) {
				specializedParams[i] = new CPPParameterSpecialization(
						(ICPPParameter) params[i], (ICPPScope) getScope(),
						argumentMap);
			}
		}
	...
	public IFunctionType getType() throws DOMException {
		if (type == null) {
			...
			type = (IFunctionType) CPPTemplates.instantiateType(type,
					argumentMap);
		}
	...
	protected void updateParameterBindings(ICPPASTFunctionDeclarator fdtor) {
		...
		for (int i = 0; i < nps.length; i++) {
			// temp = (CPPParameter)
			// ops[i].getDeclarator().getName().getBinding();
			if (params[i] != null) {
