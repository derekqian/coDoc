 ...
 */
public class DeferredTemplateInstance extends BasicSymbol implements
		IDeferredTemplateInstance {

	public DeferredTemplateInstance(ParserSymbolTable table,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IDeferredTemplateInstance#getTemplate()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IDeferredTemplateInstance#getArgumentMap()
	 */
	...
	public ISymbol instantiate(ITemplateSymbol template, ObjectMap argMap)
		...
		for (int i = 0; i < size; i++) {
			...
			newArgs.add(TemplateEngine.instantiateTypeInfo(arg, template,
					argMap));
		}

		ITemplateSymbol deferredTemplate = getTemplate();
		if (deferredTemplate.isType(ITypeInfo.t_templateParameter)
				&& argMap.containsKey(deferredTemplate)) {
