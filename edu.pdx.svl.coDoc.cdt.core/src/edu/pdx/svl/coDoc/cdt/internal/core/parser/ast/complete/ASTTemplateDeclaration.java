 ...
 * 
 */
public class ASTTemplateDeclaration extends ASTSymbol implements
	...
	private ITemplateSymbol getTemplateSymbol() {
		return (ITemplateSymbol) ((getSymbol() instanceof ITemplateSymbol) ? getSymbol()
				: null);
	}
	 ...
	 * 
	 */
	public ASTTemplateDeclaration(ITemplateSymbol template, IASTScope scope,
			List parameters, char[] filename) {
		super(template);
		...
		templateParameters = (parameters != null) ? parameters
	...
	public IASTDeclaration getOwnedDeclaration() {
		...
		if (ownedSymbol != null && ownedSymbol.getASTExtension() != null) {
			ASTNode node = ownedSymbol.getASTExtension()
					.getPrimaryDeclaration();
			return (node instanceof IASTDeclaration) ? (IASTDeclaration) node
					: null;
		}
		return null;
	}
	...
	public void enterScope(ISourceElementRequestor requestor) {
		...
		for (int i = 0; i < templateParameters.size(); ++i)
			((IASTTemplateParameter) templateParameters.get(i))
