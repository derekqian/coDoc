...
import edu.pdx.svl.coDoc.cdt.core.parser.ExtendedScannerInfo;
...
import edu.pdx.svl.coDoc.cdt.core.parser.NullLogService;
...
import edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTTypeSpecifierOwner;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.ParserException;
...
public class CModelBuilder {
	...
	private edu.pdx.svl.coDoc.cdt.internal.core.model.TranslationUnit translationUnit;
	...
	class ProblemCallback extends StructuralParseCallback {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.QuickParseCallback#acceptProblem(edu.pdx.svl.coDoc.cdt.core.parser.IProblem)
		 */
	...
	public CModelBuilder(
			edu.pdx.svl.coDoc.cdt.internal.core.model.TranslationUnit tu,
			Map newElements) {
		this.translationUnit = tu;
		this.newElements = newElements;
	}

	public IASTCompilationUnit parse(boolean quickParseMode,
			boolean throwExceptionOnError) throws ParserException {
		...
		final IProblemRequestor problemRequestor = translationUnit
				.getProblemRequestor();
		// use quick or structural parse mode
		ParserMode mode = quickParseMode ? ParserMode.QUICK_PARSE
				: ParserMode.STRUCTURAL_PARSE;
		if (problemRequestor == null) {
			quickParseCallback = (quickParseMode) ? ParserFactory
					.createQuickParseCallback() : ParserFactory
					.createStructuralParseCallback();
		} else {
		...
		if (hasCppNature) {
			...
			language = translationUnit.isCXXLanguage() ? ParserLanguage.CPP
					: ParserLanguage.C;
		}
		...
		try {
			...
			if (provider != null) {
				...
				// Technically we should not do this. But the managed scanner
				// providers
				// may rely on this behaviour i.e. only provide scannerInfo for
				// projects
				if (buildScanInfo == null) {
					buildScanInfo = provider
							.getScannerInformation(currentProject);
				}
				if (buildScanInfo != null) {
			...
			CodeReader reader = resource != null
					&& resource.getLocation() != null ? new CodeReader(
					translationUnit.getUnderlyingResource().getLocation()
							.toOSString(), code) : new CodeReader(code);
			parser = ParserFactory.createParser(ParserFactory.createScanner(
					reader, scanInfo, mode, language, quickParseCallback,
					quickParseMode ? new NullLogService() : ParserUtil
							.getScannerLogService(), null), quickParseCallback,
					mode, language, ParserUtil.getParserLogService());
		} catch (ParserFactoryError pfe) {
			throw new ParserException(
					CCorePlugin
							.getResourceString("CModelBuilder.Parser_Construction_Failure")); //$NON-NLS-1$
		}
		// call parse
		if (problemRequestor != null) {
		...
		if ((!hasNoErrors) && throwExceptionOnError)
			throw new ParserException(CCorePlugin
					.getResourceString("CModelBuilder.Parse_Failure")); //$NON-NLS-1$
	...
	public Map parse(boolean quickParseMode) throws Exception {
		...
		try {
			...
			Util
					.debugLog(
							"Parse Exception in CModelBuilder", IDebugLogConstants.MODEL); //$NON-NLS-1$
			// e.printStackTrace();
		}
		Util
				.debugLog(
						"CModel parsing: " + (System.currentTimeMillis() - startTime) + "ms", IDebugLogConstants.MODEL); //$NON-NLS-1$ //$NON-NLS-2$

		startTime = System.currentTimeMillis();
		try {
			generateModelElements();
			// important to know if the unit has parse errors or not
			translationUnit.getElementInfo().setIsStructureKnown(
					hasNoErrors && quickParseCallback.hasNoProblems());
		} catch (NullPointerException npe) {
			Util
					.debugLog(
							"NullPointer exception in CModelBuilder", IDebugLogConstants.MODEL); //$NON-NLS-1$
		}
		...
		Util
				.debugLog(
						"CModel building: " + (System.currentTimeMillis() - startTime) + "ms", IDebugLogConstants.MODEL); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void generateModelElements() throws CModelException {
		...
		while (i.hasNext()) {
			...
			if (offsetable instanceof IASTInclusion) {
				...
				try {
					generateModelElements(translationUnit,
							(IASTDeclaration) offsetable);
	...
	private void generateModelElements(Parent parent,
			IASTDeclaration declaration) throws CModelException,
			ASTNotImplementedException {
		...
		if (declaration instanceof IASTLinkageSpecification) {
			generateModelElements(parent,
					(IASTLinkageSpecification) declaration);
		}
	...
	private void generateModelElements(Parent parent,
			IASTNamespaceDefinition declaration) throws CModelException,
			ASTNotImplementedException {
	...
	private void generateModelElements(Parent parent,
			IASTLinkageSpecification declaration) throws CModelException,
			ASTNotImplementedException {
	...
	private void generateModelElements(Parent parent,
			...
			throws CModelException, ASTNotImplementedException {
	...
	private void generateModelElements(Parent parent,
		...
		if (declaration instanceof IASTAbstractTypeSpecifierDeclaration) {
			...
			element = createAbstractElement(parent, abstractDeclaration, true,
					true);
			String[] parameterTypes = ASTUtil
					.getTemplateParameters(templateDeclaration);
		...
		if (element instanceof SourceManipulation) {
			...
			sourceRef.setPos(templateDeclaration.getStartingOffset(),
					templateDeclaration.getEndingOffset()
							- templateDeclaration.getStartingOffset());
			sourceRef.setLines(templateDeclaration.getStartingLine(),
					templateDeclaration.getEndingLine());
		}
	}

	private void generateModelElements(Parent parent,
			IASTTypedefDeclaration declaration) throws CModelException,
			ASTNotImplementedException {
		TypeDef typeDef = createTypeDef(parent, declaration);
		IASTAbstractDeclaration abstractDeclaration = declaration
				.getAbstractDeclarator();
		CElement element = createAbstractElement(parent, abstractDeclaration,
				false, true);
	}

	private CElement createClassSpecifierElement(Parent parent,
			IASTClassSpecifier classSpecifier, boolean isTemplate)
			throws ASTNotImplementedException, CModelException {
		CElement element = null;
	...
	private CElement createAbstractElement(Parent parent,
			IASTTypeSpecifierOwner abstractDeclaration, boolean isTemplate,
			boolean isDeclaration) throws ASTNotImplementedException,
			CModelException {
		CElement element = null;
		if (abstractDeclaration != null) {
			...
			if (typeSpec instanceof IASTEnumerationSpecifier) {
				...
				element = createClassSpecifierElement(parent, classSpecifier,
						isTemplate);
			} else if (isDeclaration
					&& typeSpec instanceof IASTElaboratedTypeSpecifier) {
				// This is not a model element, so we don't create anything
				// here.
	...
	private CElement createSimpleElement(Parent parent,
			IASTDeclaration declaration, boolean isTemplate)
			throws CModelException, ASTNotImplementedException {

		CElement element = null;
		if (declaration instanceof IASTVariable) {
			element = createVariableSpecification(parent,
					(IASTVariable) declaration, isTemplate);
		}
		// function or method
		else if (declaration instanceof IASTFunction) {
			element = createFunctionSpecification(parent,
					(IASTFunction) declaration, isTemplate);
		}
		return element;
	}

	private StructureDeclaration createElaboratedTypeSpecifier(Parent parent,
			IASTElaboratedTypeSpecifier typeSpec, boolean isTemplate)
			throws CModelException {
		// create element
		ASTClassKind classkind = typeSpec.getClassKind();
		int kind = -1;
		if (classkind == ASTClassKind.CLASS) {
			kind = (isTemplate) ? ICElement.C_TEMPLATE_CLASS_DECLARATION
					: ICElement.C_CLASS_DECLARATION;
		} else if (classkind == ASTClassKind.STRUCT) {
			kind = (isTemplate) ? ICElement.C_TEMPLATE_STRUCT_DECLARATION
					: ICElement.C_STRUCT_DECLARATION;
		} else if (classkind == ASTClassKind.UNION) {
			kind = (isTemplate) ? ICElement.C_TEMPLATE_UNION_DECLARATION
					: ICElement.C_UNION_DECLARATION;
		}
		String className = (typeSpec.getName() == null) ? "" //$NON-NLS-1$
		...
		// set position
		element.setIdPos(typeSpec.getNameOffset(), typeSpec.getNameEndOffset()
				- typeSpec.getNameOffset());
		element.setPos(typeSpec.getStartingOffset(), typeSpec.getEndingOffset()
				- typeSpec.getStartingOffset());
	...
	private Include createInclusion(Parent parent, IASTInclusion inclusion)
			throws CModelException {
		// create element
		Include element = new Include(parent, inclusion.getName(), !inclusion
				.isLocal());
		element.setFullPathName(inclusion.getFullFileName());
		// add to parent
		parent.addChild(element);
		// set position
		element.setIdPos(inclusion.getNameOffset(), inclusion
				.getNameEndOffset()
				- inclusion.getNameOffset());
		element.setPos(inclusion.getStartingOffset(), inclusion
				.getEndingOffset()
				- inclusion.getStartingOffset());
		element
				.setLines(inclusion.getStartingLine(), inclusion
						.getEndingLine());
		this.newElements.put(element, element.getElementInfo());
		return element;
	}
	...
	private Macro createMacro(Parent parent, IASTMacro macro)
			throws CModelException {
		// create element
		edu.pdx.svl.coDoc.cdt.internal.core.model.Macro element = new Macro(parent,
				macro.getName());
		// add to parent
		parent.addChild(element);
		// set position
		element.setIdPos(macro.getNameOffset(), macro.getNameEndOffset()
				- macro.getNameOffset());
		element.setPos(macro.getStartingOffset(), macro.getEndingOffset()
	...
	private Namespace createNamespace(Parent parent,
			IASTNamespaceDefinition nsDef) throws CModelException {
		// create element
		String type = "namespace"; //$NON-NLS-1$
		String nsName = (nsDef.getName() == null) ? "" //$NON-NLS-1$
				: nsDef.getName().toString();
		Namespace element = new Namespace(parent, nsName);
		// add to parent
		parent.addChild(element);
		element.setIdPos(nsDef.getNameOffset(), (nsName.length() == 0) ? type
				.length() : (nsDef.getNameEndOffset() - nsDef.getNameOffset()));
		element.setPos(nsDef.getStartingOffset(), nsDef.getEndingOffset()
				- nsDef.getStartingOffset());
	...
	private Enumeration createEnumeration(Parent parent,
			IASTEnumerationSpecifier enumSpecifier) throws CModelException {
		// create element
		String type = "enum"; //$NON-NLS-1$
		String enumName = (enumSpecifier.getName() == null) ? "" //$NON-NLS-1$
		...
		element.setIdPos(enumSpecifier.getNameOffset(),
				(enumName.length() == 0) ? type.length() : (enumSpecifier
						.getNameEndOffset() - enumSpecifier.getNameOffset()));
		element.setPos(enumSpecifier.getStartingOffset(), enumSpecifier
				.getEndingOffset()
				- enumSpecifier.getStartingOffset());
		element.setLines(enumSpecifier.getStartingLine(), enumSpecifier
				.getEndingLine());
		element.setTypeName(type);
	...
	private Enumerator createEnumerator(Parent enumarator,
			IASTEnumerator enumDef) throws CModelException {
		Enumerator element = new Enumerator(enumarator, enumDef.getName()
				.toString());
		IASTExpression initialValue = enumDef.getInitialValue();
		if (initialValue != null) {
			element.setConstantExpression(ASTUtil
					.getExpressionString(initialValue));
		}
		// add to parent
		enumarator.addChild(element);
		// set enumerator position
		element.setIdPos(enumDef.getStartingOffset(), (enumDef
				.getNameEndOffset() - enumDef.getNameOffset()));
		element.setPos(enumDef.getStartingOffset(), enumDef.getEndingOffset()
				- enumDef.getStartingOffset());
		element.setLines(enumDef.getStartingLine(), enumDef.getEndingLine());
		this.newElements.put(element, element.getElementInfo());
		return element;
	}

	private Structure createClass(Parent parent,
			IASTClassSpecifier classSpecifier, boolean isTemplate)
			throws CModelException {
		// create element
		...
		if (classkind == ASTClassKind.CLASS) {
			kind = (isTemplate) ? ICElement.C_TEMPLATE_CLASS
					: ICElement.C_CLASS;
			type = "class"; //$NON-NLS-1$
		} else if (classkind == ASTClassKind.STRUCT) {
			kind = (isTemplate) ? ICElement.C_TEMPLATE_STRUCT
					: ICElement.C_STRUCT;
			type = "struct"; //$NON-NLS-1$
		} else if (classkind == ASTClassKind.UNION) {
			kind = (isTemplate) ? ICElement.C_TEMPLATE_UNION
					: ICElement.C_UNION;
		...
		if (!isTemplate) {
			...
			StructureTemplate classTemplate = new StructureTemplate(parent,
					kind, className);
		...
		while (baseClauses.hasNext()) {
			...
			element.addSuperClass(baseSpec.getParentClassName(), baseSpec
					.getAccess());
		}
		...
		element.setIdPos(classSpecifier.getNameOffset(),
				(className.length() == 0) ? type.length() : (classSpecifier
						.getNameEndOffset() - classSpecifier.getNameOffset()));
		element.setTypeName(type);
		if (!isTemplate) {
			// set the element position
			element.setPos(classSpecifier.getStartingOffset(), classSpecifier
					.getEndingOffset()
					- classSpecifier.getStartingOffset());
		}
		element.setLines(classSpecifier.getStartingLine(), classSpecifier
				.getEndingLine());
	...
	private TypeDef createTypeDef(Parent parent,
			IASTTypedefDeclaration typeDefDeclaration) throws CModelException {
		// create the element
		String name = typeDefDeclaration.getName();

		TypeDef element = new TypeDef(parent, name);

		StringBuffer typeName = new StringBuffer(ASTUtil
				.getType(typeDefDeclaration.getAbstractDeclarator()));
		...
		// set positions
		element.setIdPos(typeDefDeclaration.getNameOffset(),
				(typeDefDeclaration.getNameEndOffset() - typeDefDeclaration
						.getNameOffset()));
		element.setPos(typeDefDeclaration.getStartingOffset(),
				typeDefDeclaration.getEndingOffset()
						- typeDefDeclaration.getStartingOffset());
		element.setLines(typeDefDeclaration.getStartingLine(),
				typeDefDeclaration.getEndingLine());
		this.newElements.put(element, element.getElementInfo());
		return element;
	}

	private VariableDeclaration createVariableSpecification(Parent parent,
			IASTVariable varDeclaration, boolean isTemplate)
			throws CModelException, ASTNotImplementedException {
		...
		IASTAbstractDeclaration abstractDeclaration = varDeclaration
				.getAbstractDeclaration();
		CElement abstractElement = createAbstractElement(parent,
				abstractDeclaration, isTemplate, false);
		...
		if (varDeclaration instanceof IASTField) {
			...
			if (isTemplate) {
				// variable
				VariableTemplate newElement = new VariableTemplate(parent,
						variableName);
				element = newElement;
			} else {
				if (varDeclaration.isExtern()) {
					// variableDeclaration
					VariableDeclaration newElement = new VariableDeclaration(
							parent, variableName);
					element = newElement;
				} else {
					// variable
		...
		element.setTypeName(ASTUtil.getType(varDeclaration
				.getAbstractDeclaration()));
		element.setConst(varDeclaration.getAbstractDeclaration().isConst());
		element.setVolatile(varDeclaration.getAbstractDeclaration()
				.isVolatile());
		...
		// set position
		element.setIdPos(varDeclaration.getNameOffset(), (varDeclaration
				.getNameEndOffset() - varDeclaration.getNameOffset()));
		if (!isTemplate) {
			// set element position
			element.setPos(varDeclaration.getStartingOffset(), varDeclaration
					.getEndingOffset()
					- varDeclaration.getStartingOffset());
		}
		element.setLines(varDeclaration.getStartingLine(), varDeclaration
				.getEndingLine());
		this.newElements.put(element, element.getElementInfo());
		return element;
	}

	private FunctionDeclaration createFunctionSpecification(Parent parent,
			IASTFunction functionDeclaration, boolean isTemplate)
			throws CModelException {
		...
		if (functionDeclaration instanceof IASTMethod
			...
			if (methodDeclaration.hasFunctionBody()) {
				...
				// method declaration
				if (!isTemplate) {
					MethodDeclaration newElement = new MethodDeclaration(
							parent, name);
					methodElement = newElement;
				} else {
					MethodTemplateDeclaration newElement = new MethodTemplateDeclaration(
							parent, name);
					methodElement = newElement;
				}
			...
			methodElement.setReturnType(ASTUtil.getType(functionDeclaration
					.getReturnType()));
		...
		} else // instance of IASTFunction
		{
			...
			if (functionDeclaration.hasFunctionBody()) {
				...
				if (!isTemplate) {
					...
					FunctionTemplate newElement = new FunctionTemplate(parent,
							name);
					functionElement = newElement;
				}
			} else {
				// functionDeclaration
				if (!isTemplate) {
					FunctionDeclaration newElement = new FunctionDeclaration(
							parent, name);
					functionElement = newElement;
				} else {
					FunctionTemplateDeclaration newElement = new FunctionTemplateDeclaration(
							parent, name);
					functionElement = newElement;
				}
			}
			functionElement.setParameterTypes(parameterTypes);
			functionElement.setReturnType(ASTUtil.getType(functionDeclaration
					.getReturnType()));
		...
		// hook up the offsets
		element.setIdPos(functionDeclaration.getNameOffset(),
				(functionDeclaration.getNameEndOffset() - functionDeclaration
						.getNameOffset()));
		if (!isTemplate) {
			// set the element position
			element.setPos(functionDeclaration.getStartingOffset(),
					functionDeclaration.getEndingOffset()
							- functionDeclaration.getStartingOffset());
		}
		element.setLines(functionDeclaration.getStartingLine(),
				functionDeclaration.getEndingLine());

		this.newElements.put(element, element.getElementInfo());
		return element;
	}

	private Using createUsingDirective(Parent parent,
			IASTUsingDirective usingDirDeclaration) throws CModelException {
		// create the element
		...
		// set positions
		element.setIdPos(usingDirDeclaration.getNameOffset(),
				(usingDirDeclaration.getNameEndOffset() - usingDirDeclaration
						.getNameOffset()));
		element.setPos(usingDirDeclaration.getStartingOffset(),
				usingDirDeclaration.getEndingOffset()
						- usingDirDeclaration.getStartingOffset());
		element.setLines(usingDirDeclaration.getStartingLine(),
				usingDirDeclaration.getEndingLine());
		this.newElements.put(element, element.getElementInfo());
		return element;
	}

	private Using createUsingDeclaration(Parent parent,
			IASTUsingDeclaration usingDeclaration) throws CModelException {
		// create the element
		...
		// set positions
		element.setIdPos(usingDeclaration.getNameOffset(), (usingDeclaration
				.getNameEndOffset() - usingDeclaration.getNameOffset()));
		element.setPos(usingDeclaration.getStartingOffset(), usingDeclaration
				.getEndingOffset()
				- usingDeclaration.getStartingOffset());
		element.setLines(usingDeclaration.getStartingLine(), usingDeclaration
				.getEndingLine());
		this.newElements.put(element, element.getElementInfo());
		return element;
	}
