...
public class ASTClassSpecifier extends ASTScope implements IASTClassSpecifier {
	...
	public class BaseIterator implements Iterator {
		...
		public Object next() {
			...
			return new ASTBaseSpecifier(pw.getParent(), pw.isVirtual(), pw
					.getAccess(), pw.getOffset(), pw.getReferences());
	 ...
	 * @param symbol
	 */
	public ASTClassSpecifier(ISymbol symbol, ASTClassKind kind,
			ClassNameType type, ASTAccessVisibility access, int startingOffset,
			int startingLine, int nameOffset, int nameEndOffset, int nameLine,
			List references, char[] filename) {
		super(symbol);
		classKind = kind;
		classNameType = type;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTClassSpecifier#getCurrentVisibilityMode()
	 */
	...
	public void enterScope(ISourceElementRequestor requestor) {
		...
		for (int i = 0; i < size; i++) {
			...
			IASTBaseSpecifier baseSpec = new ASTBaseSpecifier(pw.getParent(),
					pw.isVirtual(), pw.getAccess(), pw.getOffset(), pw
							.getReferences());
	...
	public void addUnresolvedReference(UnresolvedReferenceDuple duple) {
		// avoid a ConcurrentModificationException by not adding more references
		// when we are
