...
public class CPPClassInstance extends CPPInstance implements ICPPClassType,
	 ...
	 * @param arguments
	 */
	public CPPClassInstance(ICPPScope scope, IBinding decl, ObjectMap argMap,
			IType[] args) {
	...
	public ICPPBase[] getBases() throws DOMException {
		...
		if (cls != null) {
			...
			for (int i = 0; i < bases.length; i++) {
				...
				if (T instanceof ICPPTemplateTypeParameter
						&& argumentMap.containsKey(T)) {
					IType t = (IType) argumentMap.get(T);
					if (t instanceof ICPPClassType)
						((CPPBaseClause) bases[i])
								.setBaseClass((ICPPClassType) argumentMap
										.get(T));
				}
			}
			return bases;
	...
	public boolean isSameType(IType type) {
		...
		if (type instanceof CPPDeferredClassInstance)
			return type.isSameType(this); // the CPPDeferredClassInstance has
											// some fuzziness

		if (type instanceof ICPPTemplateInstance) {
			if (getSpecializedBinding() != ((ICPPTemplateInstance) type)
					.getTemplateDefinition())
				return false;

			ObjectMap m1 = getArgumentMap(), m2 = ((ICPPTemplateInstance) type)
					.getArgumentMap();
