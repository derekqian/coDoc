...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.Parser.Flags;
...
public class GCCParserExtension implements IParserExtension {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#isValidCVModifier(edu.pdx.svl.coDoc.cdt.core.parser.ParserLanguage,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#getPointerOperator(edu.pdx.svl.coDoc.cdt.core.parser.ParserLanguage,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#isValidUnaryExpressionStart(int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#parseUnaryExpression(edu.pdx.svl.coDoc.cdt.internal.core.parser.IParserData)
	 */
	public IASTExpression parseUnaryExpression(IASTScope scope,
			IParserData data, IASTCompletionNode.CompletionKind kind,
			KeywordSetKey key) {
		try {
			switch (data.LT(1)) {
			case IGCCToken.t___alignof__:
				return performUnaryExpression(data, scope, kind, key,
						UnaryExpressionKind.ALIGNOF);
			case IGCCToken.t_typeof:
				return performUnaryExpression(data, scope, kind, key,
						UnaryExpressionKind.TYPEOF);
			default:
				break;
			}
	...
	protected static class UnaryExpressionKind extends Enum {
		public static final UnaryExpressionKind ALIGNOF = new UnaryExpressionKind(
				1);

		public static final UnaryExpressionKind TYPEOF = new UnaryExpressionKind(
				2);
	 ...
	 * @return
	 */
	protected IASTExpression performUnaryExpression(IParserData data,
			IASTScope scope, CompletionKind kind, KeywordSetKey key,
			UnaryExpressionKind type) {
		...
		try {
			...
			if (data.LT(1) == IToken.tLPAREN) {
				try {
					...
					d = data
							.typeId(scope, false, CompletionKind.TYPE_REFERENCE);
			...
			if (d != null & unaryExpression == null)
				try {
					...
					return data.getAstFactory().createExpression(scope,
							expKind, null, null, null, d, null, EMPTY_STRING,
							null, null);
					...
					data.logException(
							"unaryExpression_1::createExpression()", e); //$NON-NLS-1$
					data.backup(startingPoint);
					return null;
				}
				...
				try {
					...
					return data.getAstFactory().createExpression(scope,
							expKind, unaryExpression, null, null, null, null,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#canHandleDeclSpecifierSequence(int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#handleDeclSpecifierSequence(edu.pdx.svl.coDoc.cdt.internal.core.parser.IParserData,
	 *      edu.pdx.svl.coDoc.cdt.core.model.Flags,
	 *      edu.pdx.svl.coDoc.cdt.internal.core.parser.DeclarationWrapper)
	 */
	public IDeclSpecifierExtensionResult parseDeclSpecifierSequence(
			IParserData data, Parser.Flags flags, DeclarationWrapper sdw,
		...
		try {
			switch (data.LT(1)) {
			case IGCCToken.t_typeof:
				IASTExpression typeOfExpression = performUnaryExpression(data,
						sdw.getScope(), kind, key, UnaryExpressionKind.TYPEOF);
				if (typeOfExpression != null) {
					sdw.setSimpleType(IASTGCCSimpleTypeSpecifier.Type.TYPEOF);
					flags.setEncounteredRawType(true);
					Hashtable params = new Hashtable();
					params.put(IASTGCCSimpleTypeSpecifier.TYPEOF_EXRESSION,
							typeOfExpression);
					sdw.setExtensionParameter(
							IASTGCCSimpleTypeSpecifier.TYPEOF_EXRESSION,
							typeOfExpression);
					return new GCCDeclSpecifierExtensionResult(startingPoint,
							data.getLastToken(), flags, params);
				}
				data.backup(startingPoint);
				return null;
			default:
	...
	public class GCCDeclSpecifierExtensionResult implements
			IDeclSpecifierExtensionResult {
		 ...
		 * @param flags
		 */
		public GCCDeclSpecifierExtensionResult(IToken startingPoint,
				IToken token, Flags flags, Hashtable params) {
			first = startingPoint;
			last = token;
			this.flags = flags;
		}
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension.IDeclSpecifierExtensionResult#getFirstToken()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension.IDeclSpecifierExtensionResult#getLastToken()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension.IDeclSpecifierExtensionResult#getFlags()
		 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#isValidRelationalExpressionStart(int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#parseRelationalExpression(edu.pdx.svl.coDoc.cdt.core.parser.ast.IASTScope,
	 *      edu.pdx.svl.coDoc.cdt.internal.core.parser.IParserData,
	...
	public IASTExpression parseRelationalExpression(IASTScope scope,
		...
		try {
			...
			try {
				IASTExpression resultExpression = data.getAstFactory()
						.createExpression(scope, expressionKind, lhsExpression,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#canHandleCDesignatorInitializer(int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#parseDesignator(edu.pdx.svl.coDoc.cdt.internal.core.parser.IParserData)
	 */
	public IASTDesignator parseDesignator(IParserData parserData,
		...
		try {
			if (parserData.LT(1) == IToken.tIDENTIFIER) {
				...
				return parserData.getAstFactory().createDesignator(
						IASTDesignator.DesignatorKind.FIELD, null, identifier,
						null);
			}
			if (parserData.LT(1) == IToken.tLBRACKET) {
				parserData.consume(IToken.tLBRACKET);
				IASTExpression constantExpression1 = parserData.expression(
						scope, CompletionKind.SINGLE_NAME_REFERENCE,
						KeywordSetKey.EXPRESSION);
				parserData.consume(IToken.tELLIPSIS);
				IASTExpression constantExpression2 = parserData.expression(
						scope, CompletionKind.SINGLE_NAME_REFERENCE,
						KeywordSetKey.EXPRESSION);
				parserData.consume(IToken.tRBRACKET);
				Map extensionParms = new Hashtable();
				extensionParms.put(IASTGCCDesignator.SECOND_EXRESSION,
						constantExpression2);
				return parserData.getAstFactory().createDesignator(
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#supportsStatementsInExpressions()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#getExpressionKindForStatement()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#supportsExtendedTemplateInstantiationSyntax()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.parser.extension.IParserExtension#isValidModifierForInstantiation(edu.pdx.svl.coDoc.cdt.core.parser.IToken)
	 */
