	...
	public boolean equals(Object t) {
		...
		if (_typeDeclaration != null && symbol != null) {
			if (_typeDeclaration.isType(t__Bool, t_void)
				...
				// if typeDeclaration is a basic type, then only need the types
				// the same
				result &= (_typeDeclaration.getType() == symbol.getType());
			} else if (_typeDeclaration.isType(t_function)
					&& symbol.isType(t_function)) {
				// function pointers... functions must have same parameter lists
				// and return types
				...
				if (f1.getReturnType() != null && f2.getReturnType() != null)
					result &= f1.getReturnType().getTypeInfo().equals(
							f2.getReturnType().getTypeInfo());
				else
				...
				// template parameters
				result &= TemplateEngine.templateParametersAreEquivalent(
				...
				result &= TemplateEngine.deferedInstancesAreEquivalent(
						(IDeferredTemplateInstance) _typeDeclaration,
						(IDeferredTemplateInstance) symbol);
			} else {
