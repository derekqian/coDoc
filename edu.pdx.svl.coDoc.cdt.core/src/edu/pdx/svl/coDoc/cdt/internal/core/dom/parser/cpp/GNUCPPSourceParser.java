...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTASMDeclaration;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTBreakStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTCaseStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTContinueStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTDefaultStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTInitializerList;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTNullStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTParameterDeclaration;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPointer;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTPointerOperator;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTProblem;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTProblemDeclaration;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTProblemExpression;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTProblemHolder;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTProblemStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTProblemTypeId;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTReturnStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTSimpleDeclSpecifier;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTSwitchStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTWhileStatement;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTBinaryExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTConversionName;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTDeclSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTDeleteExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTReferenceOperator;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTSimpleDeclSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTSimpleTypeConstructorExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTTryBlockStatement;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTTypeIdExpression;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTTypenameExpression;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPASTUnaryExpression;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.cpp.IGPPASTBinaryExpression;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.cpp.IGPPASTDeclSpecifier;
import edu.pdx.svl.coDoc.cdt.core.dom.ast.gnu.cpp.IGPPASTExplicitTemplateInstantiation;
...
import edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.AbstractGNUSourceCodeParser;
import edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.BacktrackException;
import edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.GCCBuiltinSymbolProvider;
import edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.IASTAmbiguousDeclaration;
...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.SimpleDeclarationStrategy;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.TemplateParameterManager;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.token.OperatorTokenDuple;
...
public class GNUCPPSourceParser extends AbstractGNUSourceCodeParser {
	...
	protected ITokenDuple name() throws BacktrackException, EndOfFileException {
		...
		try {
			...
			case IToken.tEOC:
				last = consume();
				IToken templateLast = consumeTemplateArguments(last,
						argumentList);
	...
	protected void consumePointerOperators(List collection)
		...
		for (;;) {
			// having __attribute__ inbetween pointers is not yet supported by
			// the GCC compiler
			// if (LT(1) == IGCCToken.t__attribute__ &&
			// supportAttributeSpecifiers) // if __attribute__ is after a *
			// __attribute__();
		...
		if (LT(1) == IToken.tLPAREN) {
			...
			if (typeId != null && LT(1) == IToken.tRPAREN) {
				...
				try {
					...
					mark = null; // clean up mark so that we can garbage
									// collect
					return buildTypeIdUnaryExpression(
							IASTCastExpression.op_cast, typeId, castExpression,
							startingOffset, calculateEndOffset(castExpression));
					...
					try {
						...
						if (LT(1) == IToken.tLPAREN) {
							...
							mark = null; // clean up mark so that we can
											// garbage collect
							return buildTypeIdUnaryExpression(
									IASTCastExpression.op_cast, typeId,
									castExpression, startingOffset,
	...
	protected IASTTypeId typeId(boolean forNewExpression)
		...
		try {
			...
			return null;
			/*
			 * backup(mark); if (declarator != null || declSpecifier != null)
			 * throwBacktrack(startingOffset, figureEndOffset(declSpecifier,
			 * declarator) - startingOffset); else
		...
		if (declarator != null) {
			if (declarator.getName().toCharArray().length > 0) {
				return null;
				/*
				 * backup(mark); throwBacktrack(startingOffset,
				 * figureEndOffset(declSpecifier, declarator) - startingOffset);
				 */
			}
					...
					if (typeId == null) {
						...
						try {
							...
							if (LT(1) == IToken.tLPAREN
								...
								// Worst-case scenario - this cannot be resolved
								// w/o
								// more semantic information.
								// Luckily, we don't need to know what was that
								// - we
								// only know that
								...
								if (newPlacementExpressions != null) {
									result
											.setNewPlacement(newPlacementExpressions);
			...
			case IToken.tDOT:
				...
				if (name == null)
					throwBacktrack(((ASTNode) firstExpression).getOffset(),
							((ASTNode) firstExpression).getLength()
									+ dot.getLength());
		...
		try {
			...
			if (LT(1) == IToken.tCOLONCOLON || LT(1) == IToken.tIDENTIFIER) {
				...
				if (LT(1) == IToken.t_operator)
					...
					backup(mark);
					throwBacktrack(start.getOffset(), (end != null ? end
							.getEndOffset() : start.getEndOffset())
							- start.getOffset());
				}
	...
	public GNUCPPSourceParser(IScanner scanner, ParserMode mode,
		...
		super(scanner, log, mode, config.supportStatementsInExpressions(),
				...
				config.supportGCCOtherBuiltinSymbols(), config
	...
	protected IASTDeclaration templateDeclaration() throws EndOfFileException,
		...
		try {
			...
			try {
				IASTDeclaration d = declaration();
				((ASTNode) templateDecl).setOffsetAndLength(firstToken
						.getOffset(), calculateEndOffset(d)
						- firstToken.getOffset());
				...
				d
						.setPropertyInParent(ICPPASTTemplateDeclaration.OWNED_DECLARATION);
				for (int i = 0; i < parms.size(); ++i) {
					...
					parm
							.setPropertyInParent(ICPPASTTemplateDeclaration.PARAMETER);
				}
			} finally {
				--templateCount;
			}
		...
		if (d1 != null) {
			...
			if (d1 instanceof IASTSimpleDeclaration) {
				...
				if (sd.getDeclSpecifier() instanceof ICPPASTDeclSpecifier
						&& ((ICPPASTDeclSpecifier) sd.getDeclSpecifier())
								.isFriend())
					return d1;
				if (sd.getDeclarators().length != 1)
					return d1;
				if (sd.getDeclSpecifier() instanceof IASTSimpleDeclSpecifier) {
					IASTSimpleDeclSpecifier simpleSpec = (IASTSimpleDeclSpecifier) sd
							.getDeclSpecifier();
					if (simpleSpec.getType() == IASTSimpleDeclSpecifier.t_void
							&& sd.getDeclarators()[0].getPointerOperators().length == 0)
						return d1;
				}
	...
	protected IASTDeclaration simpleDeclaration(
		...
		try {
			...
			declarators = (IASTDeclarator[]) ArrayUtil.append(
					IASTDeclarator.class, declarators, e.declarator);
			backup(e.currToken);
		}
	...
	protected ICPPASTDeclSpecifier declSpecifierSeq(boolean parm,
				...
				try {
					...
					if (last == null && typeofExpression != null) {
						((ASTNode) simpleDeclSpec)
								.setOffsetAndLength((ASTNode) typeofExpression);
					} else {
						int l = last != null ? last.getEndOffset()
								- firstToken.getOffset() : 0;
						((ASTNode) simpleDeclSpec).setOffsetAndLength(
								firstToken.getOffset(), l);
					}
					simpleDeclSpec.setConst(isConst);
					simpleDeclSpec.setVolatile(isVolatile);
					if (simpleDeclSpec instanceof IGPPASTDeclSpecifier)
						((IGPPASTDeclSpecifier) simpleDeclSpec)
								.setRestrict(isRestrict);
		...
		if (last == null && typeofExpression != null) {
			...
			int l = last != null ? last.getEndOffset() - firstToken.getOffset()
					: 0;
			((ASTNode) simpleDeclSpec).setOffsetAndLength(firstToken
					.getOffset(), l);
		}
		simpleDeclSpec.setConst(isConst);
		simpleDeclSpec.setVolatile(isVolatile);
	...
	protected IASTDeclarator declarator(SimpleDeclarationStrategy strategy,
		...
		overallLoop: do {
			...
			// if __attribute__ is after the pointer ops and before the
			// declarator ex: void * __attribute__((__cdecl__)) foo();
			if (LT(1) == IGCCToken.t__attribute__ && supportAttributeSpecifiers) // if
																					// __attribute__
																					// is
																					// after
																					// the
			...
			for (;;) {
				...
				case IToken.tLPAREN:
					...
					if (LT(1) == IGCCToken.t__attribute__
							&& supportAttributeSpecifiers) // if __attribute__
															// is after the
															// parameters
						__attribute__();
						...
						while (!done) {
							...
							default:
								...
								if (typeId != null) {
									...
									IASTProblem p = createProblem(
											IASTProblem.SYNTAX_ERROR, before
													.getOffset(), before
													.getLength());
	...
	protected void catchHandlerSequence(List collection)
		...
		while (nextToken == IToken.t_catch) {
			...
			try {
				...
				// if EOF is reached, then return here and let it be encountered
				// elsewhere
				// (i.e. try/catch won't be added to the declaration if the
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#getTranslationUnit()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createCompoundStatement()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createBinaryExpression()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createConditionalExpression()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createUnaryExpression()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createCompoundStatementExpression()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createExpressionList()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createName(edu.pdx.svl.coDoc.cdt.core.parser.IToken)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createName()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createEnumerator()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#buildTypeIdExpression(int,
	 *      edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTTypeId, int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser2.AbstractGNUSourceCodeParser#createEnumerationSpecifier()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.AbstractGNUSourceCodeParser#nullifyTranslationUnit()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.AbstractGNUSourceCodeParser#createProblemStatement()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.AbstractGNUSourceCodeParser#createProblemExpression()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.AbstractGNUSourceCodeParser#createProblem(int,
	 *      int, int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.AbstractGNUSourceCodeParser#parseWhileStatement()
	 */
	 ...
	 * @return
	 */
	protected IASTNode cppStyleCondition(boolean expectSemi)
			throws BacktrackException, EndOfFileException {
		IToken mark = mark();
		try {
	...
	protected IASTStatement parseIfStatement() throws EndOfFileException,
		...
		if_loop: while (true) {
			...
			if (condition != null
					&& (condition instanceof IASTExpression || condition instanceof IASTDeclaration)) // shouldn't
			// be
			// possible
			// but
			// failure
	...
	protected void checkTokenVsDeclarator(IToken la, IASTDeclarator d)
			throws FoundDeclaratorException {
		switch (la.getType()) {
		case IToken.tCOLON:
		case IToken.t_try:
	...
	protected IASTExpression unaryOperatorCastExpression(int operator)
		...
		if (castExpression instanceof IASTLiteralExpression
				&& (operator == IASTUnaryExpression.op_amper || operator == IASTUnaryExpression.op_star)) {
	...
	protected IASTStatement parseSwitchStatement() throws EndOfFileException,
		...
		if (switch_condition instanceof IASTExpression) {
			switch_statement
					.setControllerExpression((IASTExpression) switch_condition);
			switch_condition.setParent(switch_statement);
			switch_condition
					.setPropertyInParent(IASTSwitchStatement.CONTROLLER_EXP);
		} else if (switch_condition instanceof IASTDeclaration) {
			switch_statement
					.setControllerDeclaration((IASTDeclaration) switch_condition);
			switch_condition.setParent(switch_statement);
			switch_condition
					.setPropertyInParent(ICPPASTSwitchStatement.CONTROLLER_DECLARATION);
		}
	...
	protected IASTStatement parseForStatement() throws EndOfFileException,
		...
		if (for_condition != null) {
			...
			if (for_condition instanceof IASTExpression) {
				for_statement
						.setConditionExpression((IASTExpression) for_condition);
				for_condition.setPropertyInParent(IASTForStatement.CONDITION);
			} else if (for_condition instanceof IASTDeclaration) {
				for_statement
						.setConditionDeclaration((IASTDeclaration) for_condition);
				for_condition
						.setPropertyInParent(ICPPASTForStatement.CONDITION_DECLARATION);
			}
		}
