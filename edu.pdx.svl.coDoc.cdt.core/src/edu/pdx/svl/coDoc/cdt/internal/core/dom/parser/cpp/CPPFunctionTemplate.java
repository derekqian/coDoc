 ...
 * @author aniefer
 */
public class CPPFunctionTemplate extends CPPTemplateDefinition implements
		ICPPFunctionTemplate, ICPPFunction, ICPPInternalFunction {
	...
	public static class CPPFunctionTemplateDelegate extends
			CPPFunction.CPPFunctionDelegate implements ICPPFunctionTemplate,
		...
		public ICPPTemplateParameter[] getTemplateParameters()
				throws DOMException {
			return ((ICPPFunctionTemplate) getBinding())
	...
	private void updateFunctionParameterBindings(IASTName paramName) {
		...
		ICPPASTFunctionDeclarator orig = (ICPPASTFunctionDeclarator) defName
				.getParent();
		IASTParameterDeclaration[] ops = orig.getParameters();
		IASTParameterDeclaration[] nps = ((ICPPASTFunctionDeclarator) paramName
	 ...
	 * @return
	 */
	// public IBinding resolveParameter(ICPPASTTemplateParameter
	// templateParameter) {
	// return null;
	// }
	/*
	...
	public IBinding resolveParameter(IASTParameterDeclaration param) {
		...
		ICPPASTFunctionDeclarator fdtor = (ICPPASTFunctionDeclarator) param
				.getParent();
		...
		if (declarations != null) {
			for (int j = 0; j < declarations.length && declarations[j] != null; j++) {
				temp = ((ICPPASTFunctionDeclarator) declarations[j].getParent())
						.getParameters()[i];
				IASTName n = temp.getDeclarator().getName();
				if (n != name) {
					n.setBinding(binding);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.dom.parser.cpp.CPPTemplateDefinition#deferredInstance(edu.pdx.svl.coDoc.cdt.core.dom.ast.IType[])
	 */
	public ICPPSpecialization deferredInstance(IType[] arguments) {
	...
	public boolean takesVarArgs() {
		IASTName name = (IASTName) getDefinition();
		if (name != null) {
			ICPPASTFunctionDeclarator dtor = (ICPPASTFunctionDeclarator) name
					.getParent();
			return dtor.takesVarArgs();
		}
		IASTName[] ns = (IASTName[]) getDeclarations();
		if (ns != null && ns.length > 0) {
			ICPPASTFunctionDeclarator dtor = (ICPPASTFunctionDeclarator) ns[0]
					.getParent();
			return dtor.takesVarArgs();
		}
		return false;
