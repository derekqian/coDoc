...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ParserSymbolTable.Cost;
...
public final class TemplateEngine {
	...
	static protected ITypeInfo instantiateTypeInfo(ITypeInfo info,
			ITemplateSymbol template, ObjectMap argMap)
		...
		if (info.isType(ITypeInfo.t_type)) {
			...
			if (typeSymbol instanceof IDeferredTemplateInstance) {
				IDeferredTemplateInstance deferred = (IDeferredTemplateInstance) info
						.getTypeSymbol();
				ITypeInfo newInfo = TypeInfoProvider.newTypeInfo(info);
				// newInfo.setTypeSymbol( deferred.instantiate( template, argMap
				// ) );
				template.registerDeferredInstatiation(newInfo, deferred,
						ITemplateSymbol.DeferredKind.TYPE_SYMBOL, argMap);
				newInfo.setTypeSymbol(deferred);
				return newInfo;
			} else if (typeSymbol instanceof UndefinedTemplateSymbol
					&& (typeSymbol.isType(ITypeInfo.t_template) || typeSymbol
							.isType(ITypeInfo.t_undef))) {
				ITemplateSymbol deferred = (ITemplateSymbol) info
						.getTypeSymbol();
				ITypeInfo newInfo = TypeInfoProvider.newTypeInfo(info);
				template.registerDeferredInstatiation(newInfo, deferred,
						ITemplateSymbol.DeferredKind.TYPE_SYMBOL, argMap);
				...
				ITypeInfo targetInfo = TypeInfoProvider
						.newTypeInfo((ITypeInfo) argMap.get(info
								.getTypeSymbol()));
				...
				ITypeInfo newInfo = TypeInfoProvider.newTypeInfo(info);
				newInfo.setTypeSymbol(info.getTypeSymbol().instantiate(
						template, argMap));
				return newInfo;
			}
		}
	...
	static protected void instantiateDeferredTypeInfo(ITypeInfo info,
			ITemplateSymbol template, ObjectMap argMap)
			throws ParserSymbolTableException {
	 ...
	 * @param map
	 */
	public static void discardDeferredTypeInfo(ITypeInfo info,
			ITemplateSymbol template, ObjectMap map) {
	...
	static protected ITemplateSymbol matchTemplatePartialSpecialization(
			ITemplateSymbol template, List args)
			throws ParserSymbolTableException {
		if (template == null) {
			return null;
		}
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < specArgsSize; j++) {
				...
				ISymbol sym1 = template.getSymbolTable().newSymbol(
						ParserSymbolTable.EMPTY_NAME_ARRAY);
		...
		// 14.5.4.1 If none of the specializations is more specialized than all
		// the other matchnig
	...
	static protected boolean matchTemplateParameterAndArgument(ISymbol param,
			ITypeInfo arg) {
		...
		if (param.getTypeInfo().getTemplateParameterType() == ITypeInfo.t_typeName) {
			...
			if (!arg.isType(ITypeInfo.t_type) || symbol == null
					|| !symbol.isType(ITypeInfo.t_template)) {
				return false;
			}
			...
			for (int i = 0; i < size; i++) {
				...
				if (pParam.getType() != aParam.getType()
						|| pParam.getTypeInfo().getTemplateParameterType() != aParam
								.getTypeInfo().getTemplateParameterType()) {
					return false;
				}
			}
			...
			Cost cost = null;
			TypeInfoProvider provider = param.getSymbolTable()
					.getTypeInfoProvider();
			try {
				ITypeInfo info = provider.getTypeInfo(param.getTypeInfo()
						.getTemplateParameterType());
				try {
					info.copy(param.getTypeInfo());
					info.setType(info.getTemplateParameterType());
					cost = param.getSymbolTable()
							.checkStandardConversionSequence(arg, info);
	...
	static private boolean isValidArgument(ISymbol param, ITypeInfo arg) {
		if (param.getTypeInfo().getTemplateParameterType() == ITypeInfo.t_typeName) {
			...
			if (arg.isType(ITypeInfo.t_type) && arg.getTypeSymbol() != null) {
				ISymbol symbol = arg.getTypeSymbol();
				if (CharArrayUtils.equals(symbol.getName(),
						ParserSymbolTable.EMPTY_NAME_ARRAY)) {
					return false;
			...
			ITypeInfo.PtrOp op = (ptrs.size() > 0) ? (ITypeInfo.PtrOp) ptrs
					.get(0) : null;

			// if the parameter has reference type
			if (op != null && op.getType() == ITypeInfo.PtrOp.t_reference) {
				if (arg.isType(ITypeInfo.t_type) && arg.getTypeSymbol() != null) {
					if (CharArrayUtils.equals(arg.getTypeSymbol().getName(),
							ParserSymbolTable.EMPTY_NAME_ARRAY)) {
						return false;
					}
				}
			...
			ITypeInfo.PtrOp argOp = (argPtrs.size() > 0) ? (ITypeInfo.PtrOp) argPtrs
					.get(0)
					: null;

			// address of an object with external linkage exluding nonstatic
			// class members
			// name of an object with external linkage excluding nonstatic class
			// members
			if ((argOp != null && argOp.getType() == ITypeInfo.PtrOp.t_pointer)
				...
				if (symbol != null
						&& symbol.getContainingSymbol().isType(
								ITypeInfo.t_class, ITypeInfo.t_union)) {
			...
			if (op == null
					&& (arg.isType(ITypeInfo.t_bool, ITypeInfo.t_int) || arg
	...
	/**
	 * 14.8.2.1-2 If P is a cv-qualified type, the top level cv-qualifiers of
	 * P's type are ignored for type deduction. If P is a reference type, the
	 * type referred to by P is used for Type deduction.
	 * 
	 * @param pSymbol
	 * @return
	 */
	static private ITypeInfo getParameterTypeForDeduction(ISymbol pSymbol) {
		...
		if (pPtrs.size() > 0) {
			...
			if (pOp.getType() == ITypeInfo.PtrOp.t_reference
					|| pOp.getType() == ITypeInfo.PtrOp.t_undef_ptr) {
				pPtrs.remove(0);
			} else {
				ITypeInfo.PtrOp newOp = new ITypeInfo.PtrOp(pOp.getType(),
						false, false);
				pPtrs.set(0, newOp);
			}
		} else {
	...
	/**
	 * 14.8.2.1-2 if P is not a reference type - If A is an array type, the
	 * pointer type produced by the array-to-pointer conversion is used instead -
	 * If A is a function type, the pointer type produced by the
	 * function-to-pointer conversion is used instead - If A is a cv-qualified
	 * type, the top level cv-qualifiers are ignored for type deduction
	 * 
	 * @param aInfo
	 * @return
	 */
	static private ITypeInfo getArgumentTypeForDeduction(ITypeInfo aInfo,
			boolean pIsAReferenceType) throws ParserSymbolTableException {
		...
		if (!pIsAReferenceType) {
			...
			if (a.getType() == ITypeInfo.t_type) {
				if (aSymbol == null) {
					throw new ParserSymbolTableException(
							ParserSymbolTableException.r_BadTemplateArgument);
				} else if (aSymbol.isType(ITypeInfo.t_function)
						&& a.getPtrOperators().size() == 0) {
					a.addPtrOperator(new ITypeInfo.PtrOp(
			...
			if (aPtrs.size() > 0) {
				...
				if (pOp.getType() == ITypeInfo.PtrOp.t_array) {
					aPtrs.set(0, new ITypeInfo.PtrOp(ITypeInfo.PtrOp.t_pointer,
							false, false));
				} else {
					aPtrs.set(0, new ITypeInfo.PtrOp(pOp.getType(), false,
							false));
				}
			} else {
	...
	static private List getTargetList(ISymbol symbol) {
		...
		if (instantiated != null) {
			ITemplateSymbol template = (ITemplateSymbol) instantiated
					.getContainingSymbol();
	 ...
	 * @return
	 */
	private static ISymbol findMatchingBaseClass(ISymbol p,
			IDerivableContainerSymbol a) {
		...
		if (p instanceof IDeferredTemplateInstance) {
			...
			pTemplate = (ITemplateSymbol) p.getInstantiatedSymbol()
					.getContainingSymbol();
		}
		...
		for (int i = 0; i < size; i++) {
			...
			if (parent instanceof IDeferredTemplateInstance) {
				...
				if (parentTemplate instanceof ISpecializedSymbol) {
					parentTemplate = ((ISpecializedSymbol) parentTemplate)
							.getPrimaryTemplate();
				}
				...
				// In general, we don't have enough information to proceed
				// further down this branch
			} else {
				parentTemplate = (ITemplateSymbol) parent
				...
				aSymbol = findMatchingBaseClass(p,
	...
	static private boolean deduceTemplateArgument(ObjectMap map,
			ISymbol pSymbol, ITypeInfo a) throws ParserSymbolTableException {
		ISymbol symbol;
		...
		if (ptrOps.size() > 0
				&& ((ITypeInfo.PtrOp) ptrOps.get(0)).getType() == ITypeInfo.PtrOp.t_reference) {
		...
		if (p.isType(ITypeInfo.t_type)) {
			...
			ISymbol aSymbol = a.getTypeSymbol();
			if (symbol == null
					|| (a.isType(ITypeInfo.t_type) && aSymbol == null)
					|| a.isType(ITypeInfo.t_undef))
				throw new ParserSymbolTableException(
			...
			if (symbol.isType(ITypeInfo.t_templateParameter)) {
				if (symbol.getTypeInfo().getTemplateParameterType() == ITypeInfo.t_typeName) {
					...
					// non-type parameter
					if (symbol.getTypeInfo().getTemplateParameterType() == a
							.getType()) {
						return deduceArgument(map, symbol, a);
					}
					return false;
			...
			// T (*) ( ), T ( T::* ) ( T ), & variations
				...
				if (!deduceTemplateArgument(map, pFunction.getReturnType(),
						aFunction.getReturnType().getTypeInfo())) {
					return false;
				}
				...
				for (int i = 0; i < size; i++) {
					...
					if (!deduceTemplateArgument(map, (ISymbol) pParams.get(i),
							info)) {
						return false;
					}
				}
		...
		if (p.getType() == a.getType()) {
			if (p.getDefault() != null) {
				if (p.getDefault() instanceof char[]
						&& a.getDefault() instanceof char[])
					return CharArrayUtils.equals((char[]) p.getDefault(),
							(char[]) a.getDefault());
	 ...
	 * @return
	 */
	private static boolean deduceFromTemplateTemplateArguments(ObjectMap map,
			ISymbol pSymbol, ISymbol aSymbol) {
		// template-name<T> or template-name<i>, where template-name is a class
		// template
		ITemplateSymbol p = (pSymbol instanceof IDeferredTemplateInstance) ? (ITemplateSymbol) ((IDeferredTemplateInstance) pSymbol)
				.getTemplate()
				: (ITemplateSymbol) pSymbol.getInstantiatedSymbol()
						.getContainingSymbol();

		if (p instanceof ISpecializedSymbol)
			p = ((ISpecializedSymbol) p).getPrimaryTemplate();

		ISymbol a = (aSymbol.isTemplateInstance()) ? aSymbol
				.getInstantiatedSymbol().getContainingSymbol() : aSymbol
				.getContainingSymbol();
		...
		if (p != a) {
			if (aSymbol instanceof IDerivableContainerSymbol) {
				aSymbol = findMatchingBaseClass(pSymbol,
						(IDerivableContainerSymbol) aSymbol);
			} else {
				aSymbol = null;
			}
			if (aSymbol == null) {
	 ...
	 * @return
	 * 
	 * A type that is specified in terms of template parameters (P) is compared
	 * with an actual type (A), and an attempt is made to find template argument
	 * vaules that will make P, after substitution of the deduced values,
	 * compatible with A.
	 */
	static private ObjectMap deduceTemplateArgumentsUsingParameterList(
			ITemplateSymbol template, IParameterizedSymbol function) {
	 ...
	 * compatible with A.
	 */
	static private ObjectMap deduceTemplateArguments(ITemplateSymbol template,
			List arguments) {
		if (template.getContainedSymbols() == CharArrayObjectMap.EMPTY_MAP
				|| template.getContainedSymbols().size() != 1) {
			return null;
		}
		...
		// TODO: ellipses?
		if (pList == null || arguments == null
				|| pList.size() != arguments.size()) {
			return null;
		}
	...
	/**
	 * Compare spec1 to spec2. Return > 0 if spec1 is more specialized, < 0 if
	 * spec2 is more specialized, = 0 otherwise.
	 * 
	 * @param spec1
	 * @param spec2
	 * @return
	 */
	static private int orderSpecializations(ISpecializedSymbol spec1,
			ISpecializedSymbol spec2) throws ParserSymbolTableException {
		if (spec1 == null) {
			return -1;
		}
		...
		// to order class template specializations, we need to transform them
	 ...
	 * 
	 */
	static protected int orderTemplateFunctions(ITemplateSymbol spec1,
			ITemplateSymbol spec2) throws ParserSymbolTableException {
		// Using the transformed parameter list, perform argument deduction
		// against the other
		// function template
		...
		ObjectMap m1 = deduceTemplateArgumentsUsingParameterList(spec2,
				function);
		...
		ObjectMap m2 = deduceTemplateArgumentsUsingParameterList(spec1,
				function);

		// The transformed template is at least as specialized as the other iff
		// the deduction
		// succeeds and the deduced parameter types are an exact match
		// A template is more specialized than another iff it is at least as
		// specialized as the
		// other template and that template is not at least as specialized as
		// the first.
		boolean d1 = (m1 != null);
		boolean d2 = (m2 != null);
	...
	/**
	 * transform a function template for use in partial ordering, as described
	 * in the spec 14.5.5.2-3
	 * 
	 * @param template
	 * @return -for each type template parameter, synthesize a unique type and
	 *         substitute that for each occurence of that parameter in the
	 *         function parameter list -for each non-type template parameter,
	 *         synthesize a unique value of the appropriate type and susbstitute
	 *         that for each occurence of that parameter in the function
	 *         parameter list for each template template parameter, synthesize a
	 *         unique class template and substitute that for each occurence of
	 *         that parameter in the function parameter list
	 */

	static private ObjectMap createMapForFunctionTemplateOrdering(
		...
		for (int i = 0; i < size; i++) {
			...
			if (param.getTypeInfo().getTemplateParameterType() == ITypeInfo.t_typeName) {
				val = TypeInfoProvider.newTypeInfo(ITypeInfo.t_type, 0,
						template.getSymbolTable().newSymbol(
								ParserSymbolTable.EMPTY_NAME_ARRAY,
								ITypeInfo.t_class)); //$NON-NLS-1$
			}
			// template parameter
			else if (param.getTypeInfo().getTemplateParameterType() == ITypeInfo.t_template) {
				// TODO
			...
			// non type parameter
				...
				val = TypeInfoProvider.newTypeInfo(param.getTypeInfo()
						.getTemplateParameterType());
			}
			map.put(param, val);
		}
	...
	/**
	 * transform the class template to a function template as described in the
	 * spec 14.5.4.2-1
	 * 
	 * @param template
	 * @return IParameterizedSymbol the function template has the same template
	 *         parameters as the partial specialization and has a single
	 *         function parameter whose type is a class template specialization
	 *         with the template arguments of the partial specialization
	 */
	static private ITemplateSymbol classTemplateSpecializationToFunctionTemplate(
			ISpecializedSymbol specialization)
			throws ParserSymbolTableException {
		ISpecializedSymbol transformed = (ISpecializedSymbol) specialization
				.clone();
		...
		IParameterizedSymbol function = specialization.getSymbolTable()
				.newParameterizedSymbol(transformed.getName(),
						ITypeInfo.t_function);
		try {
			transformed.addSymbol(function);
		} catch (ParserSymbolTableException e) {
			// we shouldn't get this because there aren't any other symbols in
			// the template
		}
		ISymbol param = specialization.getSymbolTable().newSymbol(
				ParserSymbolTable.EMPTY_NAME_ARRAY, ITypeInfo.t_type); //$NON-NLS-1$

		param.setTypeSymbol(specialization.instantiate(specialization
				.getArgumentList()));
	...
	static private ITypeInfo transformTypeInfo(Object obj, ObjectMap argumentMap) {
		ITypeInfo info = null;
		if (obj instanceof ISymbol) {
			info = TypeInfoProvider.newTypeInfo(ITypeInfo.t_type, 0,
					(ISymbol) obj);
		} else {
			info = (ITypeInfo) obj;
		}
		...
		if (info.isType(ITypeInfo.t_type)
			...
			ITypeInfo newType = TypeInfoProvider
					.newTypeInfo((ITypeInfo) argumentMap.get(info
							.getTypeSymbol()));
			if (info.hasPtrOperators())
	...
	static protected List selectTemplateFunctions(ObjectSet templates,
			List functionArguments, List templateArguments)
			throws ParserSymbolTableException {
		...
		outer: for (int idx = 0; idx < size; idx++) {
			IParameterizedSymbol fn = (IParameterizedSymbol) templates
					.keyAt(idx);
			ITemplateSymbol template = (ITemplateSymbol) fn
					.getContainingSymbol();
			...
			int numTemplateArgs = (templateArguments != null) ? templateArguments
					.size()
					: 0;
			...
			for (int i = 0; i < numTemplateParams; i++) {
				...
				ITypeInfo arg = (ITypeInfo) (i < numTemplateArgs ? templateArguments
						.get(i)
						: null);
			...
			IContainerSymbol instance = (IContainerSymbol) template
					.instantiate(instanceArgs);
	 ...
	 * @throws ParserSymbolTableException
	 */
	static protected ITemplateSymbol selectTemplateOrSpecialization(
			ITemplateSymbol template, List parameters, List arguments)
			throws ParserSymbolTableException {
		if (template != null) {
			...
			if (parameters.size() == 0) {
				...
				if (arguments.size() != parameters.size()) {
					...
					for (int i = 0; i < size; i++) {
						if (parameters.get(i) != ((ITypeInfo) arguments.get(i))
								.getTypeSymbol()) {
							forPrimary = false;
							break;
						}
			...
			if (forPrimary) {
				...
				if (checkTemplateParameterListsAreEquivalent(primary
						.getParameterList(), parameters)) {
					return template;
				}
				throw new ParserSymbolTableException(
						ParserSymbolTableException.r_BadTemplateParameter);
			}
			...
			ISpecializedSymbol spec = findPartialSpecialization(template,
			...
			// TODO
			throw new ParserSymbolTableException(
					ParserSymbolTableException.r_BadTemplate);

		}
		return null;
	}

	static private boolean checkTemplateParameterListsAreEquivalent(List p1,
			List p2) {
		if (p1.size() != p2.size()) {
			return false;
		}
		...
		for (int i = 0; i < size; i++) {
			...
			if (param1.getTypeInfo().getTemplateParameterType() != param2
					.getTypeInfo().getTemplateParameterType()) {
				return false;
			}
		}
	...
	static private boolean checkTemplateArgumentListsAreEquivalent(List p1,
			List p2, List a1, List a2) {
		...
		for (int i = 0; i < a1Size; i++) {
			...
			if (t1.equals(t2)) {
				continue;
			} else if (t1.isType(ITypeInfo.t_type)
					&& t2.isType(ITypeInfo.t_type)) {
				ISymbol s1 = t1.getTypeSymbol(), s2 = t2.getTypeSymbol();
				if (m[0].containsKey(s1) && m[1].containsKey(s2)
						&& m[0].get(s1).equals(m[1].get(s2)))
					continue;
			}
	...
	static private ISpecializedSymbol findPartialSpecialization(
			ITemplateSymbol template, List parameters, List arguments) {
		List specs = template.getSpecializations();
		int size = specs.size();
		ISpecializedSymbol spec = null;
		for (int i = 0; i < size; i++) {
			...
			if (!checkTemplateParameterListsAreEquivalent(spec
					.getParameterList(), parameters)) {
				continue;
			}

			if (checkTemplateArgumentListsAreEquivalent(
					spec.getParameterList(), parameters,
	...
	static protected ISymbol translateParameterForDefinition(
			ISymbol templatedSymbol, ISymbol param, ObjectMap defnMap) {
	...
	/**
	 * 14.6.1-1 (2) Within the scope of a class template (class template
	 * specialization or partial specialization), when the name of the template
	 * is neither qualified nor followed by <. it is equivalent to the name of
	 * the template followed by the template-parameters (template-arguments)
	 * enclosed in <>
	 * 
	 * @param symbol
	 * @return
	 * @throws ParserSymbolTableException
	 */
	static protected ISymbol instantiateWithinTemplateScope(
			IContainerSymbol container, ITemplateSymbol symbol)
			throws ParserSymbolTableException {
		if (symbol.getTemplatedSymbol() == null
				|| symbol.getTemplatedSymbol().isType(ITypeInfo.t_function)) {
			return symbol;
		}
		...
		while (containing != null) {
			if (containing == symbol
					|| (containing instanceof ISpecializedSymbol && ((ISpecializedSymbol) containing)
							.getPrimaryTemplate() == symbol)) {
				instantiate = true;
			...
			if (containing != null && !containing.isTemplateMember()
					|| !containing.isType(ITypeInfo.t_template)) {
				break;
			}
		}
		...
		if (instantiate) {
			if (template instanceof ISpecializedSymbol) {
				...
				for (int i = 0; i < size; i++) {
					args.add(TypeInfoProvider.newTypeInfo(ITypeInfo.t_type, 0,
							(ISymbol) params.get(i)));
				}
	...
	static protected boolean alreadyHasTemplateParameter(
			IContainerSymbol container, char[] name) {
	...
	static protected boolean canAddTemplate(IContainerSymbol containing,
			ITemplateSymbol template) {
		// 14-2 A template-declaration can appear only as a namespace scope or
		// class scope declaration
		if (!containing.isType(ITypeInfo.t_namespace)
				&& !containing.isType(ITypeInfo.t_class, ITypeInfo.t_union)) {
			return false;
		}

		// 14.5.2-3 A member function template shall not be virtual
		if (containing.isTemplateMember()
				&& containing.getContainingSymbol()
						.isType(ITypeInfo.t_template)) {
			ISymbol symbol = template.getTemplatedSymbol();
			if (symbol != null && symbol.isType(ITypeInfo.t_function)
					&& symbol.getTypeInfo().checkBit(ITypeInfo.isVirtual)) {
				return false;
			}
		}
	...
	static protected List verifyExplicitArguments(ITemplateSymbol template,
			List arguments, ISymbol symbol) throws ParserSymbolTableException {
		List params = template.getParameterList();
		...
		for (int i = 0; i < numParams; i++) {
			...
			if (i < numArgs) {
				...
				if (template.getTemplatedSymbol() instanceof IParameterizedSymbol
						&& symbol instanceof IParameterizedSymbol
						&& CharArrayUtils.equals(template.getTemplatedSymbol()
								.getName(), symbol.getName())) {
					ObjectMap map = deduceTemplateArgumentsUsingParameterList(
							template, (IParameterizedSymbol) symbol);
	...
	static protected ITemplateSymbol resolveTemplateFunctions(
			ObjectSet functions, List args, ISymbol symbol)
			throws ParserSymbolTableException {
		ITemplateSymbol template = null;
		...
		outer: for (int i = 0; i < functions.size(); i++) {
			...
			ObjectMap map = deduceTemplateArgumentsUsingParameterList(tmpl,
	...
	static protected List resolveTemplateFunctionArguments(List args,
			ITemplateSymbol template, IParameterizedSymbol fn) {
		...
		for (int i = 0; i < numParams; i++) {
			...
			if (i < numArgs) {
				...
				if (map == null) {
					map = deduceTemplateArgumentsUsingParameterList(template,
							fn);
					if (map == null)
						return null;
				}
	...
	static protected ISymbol checkForTemplateExplicitSpecialization(
			ITemplateSymbol template, ISymbol symbol, List arguments) {
		if (!template.getExplicitSpecializations().isEmpty()) {
			...
			for (int i = 0; i < specs.size(); i++) {
				...
				if (args.equals(arguments)) {
					ObjectMap explicitMap = (ObjectMap) template
							.getExplicitSpecializations().get(args);
	...
	static protected boolean templateParametersAreEquivalent(ISymbol p1,
			ISymbol p2) {
		if (!p1.isType(ITypeInfo.t_templateParameter)
				|| !p2.isType(ITypeInfo.t_templateParameter)
				|| p1.getTypeInfo().getTemplateParameterType() != p2
						.getTypeInfo().getTemplateParameterType()) {
			return false;
		}
		...
		if (p1.getTypeInfo().getTemplateParameterType() == ITypeInfo.t_typeName) {
			...
			return (l1 != null && l2 != null && l1.indexOf(p1) == l2
					.indexOf(p2));
			...
			return checkTemplateParameterListsAreEquivalent(pt1
					.getParameterList(), pt2.getParameterList());
		} else {
	 ...
	 * @return
	 */
	protected static boolean deferedInstancesAreEquivalent(
			IDeferredTemplateInstance instance,
			IDeferredTemplateInstance instance2) {
