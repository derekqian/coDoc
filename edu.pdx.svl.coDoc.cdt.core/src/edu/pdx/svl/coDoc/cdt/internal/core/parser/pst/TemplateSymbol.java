 ...
 */
public class TemplateSymbol extends ParameterizedSymbol implements
		ITemplateSymbol {
	...
	public Object clone() {
		...
		copy._defnParameterMap = (_defnParameterMap != ObjectMap.EMPTY_MAP) ? (ObjectMap) _defnParameterMap
				.clone()
				: _defnParameterMap;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IContainerSymbol#instantiate(java.util.List)
	 */
	public ISymbol instantiate(List arguments)
		...
		ITemplateSymbol template = TemplateEngine
				.matchTemplatePartialSpecialization(this, arguments);
		...
		for (int i = 0; i < numParams; i++) {
			...
			param = TemplateEngine.translateParameterForDefinition(
					templatedSymbol, param, getDefinitionParameterMap());
			...
			if (i < numArgs) {
				...
				if (arg.isType(ITypeInfo.t_type)) {
					...
					else if (arg.getTypeSymbol().isType(
							ITypeInfo.t_templateParameter)
				...
				if (obj != null && obj instanceof ITypeInfo) {
					...
					} else if (arg.isType(ITypeInfo.t_type)
							&& arg.getTypeSymbol() instanceof IDeferredTemplateInstance) {
						IDeferredTemplateInstance deferred = (IDeferredTemplateInstance) arg
								.getTypeSymbol();
						arg = TypeInfoProvider.newTypeInfo(arg);
						arg.setTypeSymbol(deferred.instantiate(this, map));
						// // IDeferredTemplateInstance deferred =
						// (IDeferredTemplateInstance) info.getTypeSymbol();
						// ITypeInfo newInfo = TypeInfoProvider.newTypeInfo( arg
						// );
						// //newInfo.setTypeSymbol( deferred.instantiate(
						// template, argMap ) );
						// template.registerDeferredInstatiation( newInfo,
						// deferred, ITemplateSymbol.DeferredKind.TYPE_SYMBOL,
						// map );
						// newInfo.setTypeSymbol( deferred );
						// // process any accumulated deferred instances, we may
						// need them
		...
		ISymbol temp = TemplateEngine.checkForTemplateExplicitSpecialization(
				template, symbol, actualArgs);
	...
	public ISymbol instantiate(ITemplateSymbol template, ObjectMap argMap)
		...
		TemplateSymbol newTemplate = (TemplateSymbol) super.instantiate(
				template, argMap);

		// we don't want to instantiate the template parameters, just the
		...
		for (int i = 0; i < size; i++) {
			...
			if (obj instanceof ITypeInfo) {
				param.getTypeInfo().setDefault(
						TemplateEngine.instantiateTypeInfo((ITypeInfo) obj,
								template, argMap));
			}
		}
	...
	private boolean isAllowableTemplateParameter(ISymbol param) {
		...
		if (!CharArrayUtils.equals(getName(),
	...
	private void modifyTemplateParameter(ISymbol param) {
		...
		} else if (param.isType(ITypeInfo.t_type)
				&& param.getTypeSymbol().isType(ITypeInfo.t_function)) {
			param
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IParameterizedSymbol#hasSpecializations()
	 */
	...
	public void addExplicitSpecialization(ISymbol symbol, List args)
		...
		List actualArgs = TemplateEngine.verifyExplicitArguments(this, args,
				symbol);
		...
		try {
			if (symbol.isType(ITypeInfo.t_function)
					|| symbol.isType(ITypeInfo.t_constructor)) {
				List params = ((IParameterizedSymbol) symbol)
						.getParameterList();
				...
				found = getTemplatedSymbol().lookupMethodForDefinition(
						symbol.getName(), fnArgs);
			} else {
				found = getTemplatedSymbol().lookupMemberForDefinition(
						symbol.getName());
			}
		} catch (ParserSymbolTableException e) {
			/* nothing */
		...
		if (found == null
				&& CharArrayUtils.equals(getTemplatedSymbol().getName(), symbol
						.getName())) {
		...
		if (found != null) {
			// in defining the explicit specialization for a member function,
			// the factory would have set
			// the specialization as the definition of the original declaration,
			// which it is not
			if (found.isForwardDeclaration()
					&& found.getForwardSymbol() == symbol)
				found.setForwardSymbol(null);

			// TODO, once we can instantiate members as we need them instead of
			// at the same time as the class
			// then found should stay as the instance, for now though, we need
			// the original (not 100% correct
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IParameterizedSymbol#addSpecialization(edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IParameterizedSymbol)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.IParameterizedSymbol#getSpecializations()
	 */
	...
	protected void addToDefinitionParameterMap(ISymbol newSymbol,
			ObjectMap defnMap) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ITemplateSymbol#registerDeferredInstatiation(edu.pdx.svl.coDoc.cdt.internal.core.parser.pst.ParameterizedSymbol,
	...
	protected void processDeferredInstantiations()
			throws ParserSymbolTableException {
		if (_deferredInstantiations == Collections.EMPTY_LIST)
			return;
		...
		while (numDeferred > numProcessed) {
			for (int i = numProcessed; i < numDeferred; i++) {
				...
				if (kind == DeferredKind.PARENT) {
					...
					d.instantiateDeferredParent((ISymbol) objs[1], this,
							(ObjectMap) objs[3]);
				} else if (kind == DeferredKind.RETURN_TYPE) {
					ParameterizedSymbol p = (ParameterizedSymbol) objs[0];
					p.instantiateDeferredReturnType((ISymbol) objs[1], this,
							(ObjectMap) objs[3]);
				} else if (kind == DeferredKind.TYPE_SYMBOL) {
					TemplateEngine.instantiateDeferredTypeInfo(
							(ITypeInfo) objs[0], this, (ObjectMap) objs[3]);
				}
				numProcessed++;
			}
			...
			if (++loopCount > ParserSymbolTable.TEMPLATE_LOOP_THRESHOLD) {
				...
				throw new ParserSymbolTableException(
						ParserSymbolTableException.r_RecursiveTemplate);
			}
		}
	...
	private void discardDeferredInstantiations() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (kind == DeferredKind.PARENT) {
				DerivableContainerSymbol d = (DerivableContainerSymbol) objs[0];
				d.discardDeferredParent((IDeferredTemplateInstance) objs[1],
						this, (ObjectMap) objs[3]);
			} else if (kind == DeferredKind.RETURN_TYPE) {
				ParameterizedSymbol p = (ParameterizedSymbol) objs[0];
				p.discardDeferredReturnType((ISymbol) objs[1], this,
						(ObjectMap) objs[3]);
			} else if (kind == DeferredKind.TYPE_SYMBOL) {
				TemplateEngine.discardDeferredTypeInfo((ITypeInfo) objs[0],
						this, (ObjectMap) objs[3]);
			}
		}
		_deferredInstantiations.clear();
	...
	private List _specializations = Collections.EMPTY_LIST; // template
															// specializations

	private ObjectMap _explicitSpecializations = ObjectMap.EMPTY_MAP; // explicit
																		// specializations

	private ObjectMap _defnParameterMap = ObjectMap.EMPTY_MAP; // members could
																// be defined
																// with
																// different
																// template
	...
	private List _deferredInstantiations = Collections.EMPTY_LIST; // used to
																	// avoid
																	// recursive
																	// loop
