...
public abstract class PDOMFastIndexerJob implements IPDOMIndexerTask {
	...
	protected void addTU(ITranslationUnit tu) throws InterruptedException,
			CoreException {
		...
		IASTTranslationUnit ast = language.getASTTranslationUnit(tu,
				codeReaderFactory, ILanguage.AST_USE_INDEX
	...
	protected void addSymbols(ILanguage language, IASTTranslationUnit ast)
		...
		for (int i = 0; i < includes.length; ++i) {
			...
			String sourcePath = sourceLoc != null ? sourceLoc.getFileName()
		...
		ast.accept(new ASTVisitor() {
			...
			public int visit(IASTName name) {
				try {
					...
					if (nameLoc != null)
						linkage.addName(name, codeReaderFactory
								.getCachedFile(nameLoc.getFileName()));
					return PROCESS_CONTINUE;
				} catch (Throwable e) {
					CCorePlugin.log(e);
					return ++errorCount > MAX_ERRORS ? PROCESS_ABORT
							: PROCESS_CONTINUE;
				}
			}
		});
