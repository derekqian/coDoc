 ...
 * @author aniefer
 */
public class CPPFunction extends PlatformObject implements ICPPFunction,
		ICPPInternalFunction {

	public static class CPPFunctionDelegate extends CPPDelegate implements
			ICPPFunction, ICPPInternalFunction {
		...
		public IBinding resolveParameter(IASTParameterDeclaration param) {
			return ((ICPPInternalFunction) getBinding())
					.resolveParameter(param);
		}
	}

	public static class CPPFunctionProblem extends ProblemBinding implements
			ICPPFunction {
	...
	private void resolveAllDeclarations() {
		if ((bits & (FULLY_RESOLVED | RESOLUTION_IN_PROGRESS)) == 0) {
			...
			declarations = (ICPPASTFunctionDeclarator[]) ArrayUtil.trim(
					ICPPASTFunctionDeclarator.class, declarations);
			bits |= FULLY_RESOLVED;
			bits &= ~RESOLUTION_IN_PROGRESS;
		}
	}
	...
	public void addDeclaration(IASTNode node) {
		...
		if (declarations.length > 0
				&& ((ASTNode) node).getOffset() < ((ASTNode) declarations[0])
						.getOffset()) {
			declarations = (ICPPASTFunctionDeclarator[]) ArrayUtil.prepend(
					ICPPASTFunctionDeclarator.class, declarations, dtor);
		} else {
			declarations = (ICPPASTFunctionDeclarator[]) ArrayUtil.append(
					ICPPASTFunctionDeclarator.class, declarations, dtor);
		}
	}
	...
	public IParameter[] getParameters() {
		IASTStandardFunctionDeclarator dtor = (definition != null) ? definition
				: declarations[0];
		...
		if (size > 0) {
			for (int i = 0; i < size; i++) {
				...
				result[i] = (IParameter) p.getDeclarator().getName()
						.resolveBinding();
			}
		}
		return result;
	...
	public IScope getScope() {
		IASTName n = definition != null ? definition.getName()
				: declarations[0].getName();
	...
	public IFunctionType getType() {
		if (type == null)
			type = (IFunctionType) CPPVisitor
					.createType((definition != null) ? definition
							: declarations[0]);
		return type;
	}
	...
	public IBinding resolveParameter(IASTParameterDeclaration param) {
		...
		if (declarations != null) {
			for (int j = 0; j < declarations.length && declarations[j] != null; j++) {
				IASTParameterDeclaration[] paramDecls = declarations[j]
						.getParameters();
	...
	protected void updateParameterBindings(ICPPASTFunctionDeclarator fdtor) {
		ICPPASTFunctionDeclarator orig = definition != null ? definition
				: declarations[0];
		...
		for (int i = 0; i < ops.length; i++) {
			...
			if (temp != null && nps.length > i) { // length could be
													// different, ie 0 or 1 with
													// void
	...
	// //2 state bits, most significant = whether or not we've figure this out
	// yet
	...
	// }
	//        
	// IASTFunctionDeclarator[] dtors = (IASTFunctionDeclarator[])
	// getDeclarations();
	// if( dtors != null ) {
	...
	static public boolean hasStorageClass(ICPPInternalFunction function,
			int storage) {
		ICPPASTFunctionDeclarator dtor = (ICPPASTFunctionDeclarator) function
				.getDefinition();
		ICPPASTFunctionDeclarator[] ds = (ICPPASTFunctionDeclarator[]) function
				.getDeclarations();
		int i = -1;
		do {
			if (dtor != null) {
				...
				if (parent instanceof IASTSimpleDeclaration)
					declSpec = ((IASTSimpleDeclaration) parent)
							.getDeclSpecifier();
				else if (parent instanceof IASTFunctionDefinition)
					declSpec = ((IASTFunctionDefinition) parent)
							.getDeclSpecifier();
