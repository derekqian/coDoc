...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.FunctionStyleMacro;
import edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.ObjectStyleMacro;
import edu.pdx.svl.coDoc.cdt.internal.core.pdom.PDOM;
import edu.pdx.svl.coDoc.cdt.internal.core.pdom.db.Database;
import edu.pdx.svl.coDoc.cdt.internal.core.pdom.db.IString;
...
public class PDOMMacro {
	...
	public PDOMMacro(PDOM pdom, IASTPreprocessorMacroDefinition macro)
			throws CoreException {
		this.pdom = pdom;

		Database db = pdom.getDB();
		this.record = db.malloc(RECORD_SIZE);
		db.putInt(record + NAME, db.newString(macro.getName().toCharArray())
				.getRecord());
		db.putInt(record + EXPANSION, db.newString(macro.getExpansion())
				.getRecord());
		setNextMacro(0);
		...
		if (macro instanceof IASTPreprocessorFunctionStyleMacroDefinition) {
			...
			for (int i = params.length - 1; i >= 0; --i) {
				...
				PDOMMacroParameter pdomParam = new PDOMMacroParameter(pdom,
						param.getParameter());
				if (last != null)
		...
		db
				.putInt(record + FIRST_PARAMETER, last != null ? last
						.getRecord() : 0);
	}
	...
	public IMacro getMacro() throws CoreException {
		...
		if (param != null) {
			...
			char[][] params = (char[][]) paramList.toArray(new char[paramList
					.size()][]);
