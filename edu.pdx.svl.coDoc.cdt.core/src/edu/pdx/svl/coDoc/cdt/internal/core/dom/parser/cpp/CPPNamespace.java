 ...
 * @author aniefer
 */
public class CPPNamespace extends PlatformObject implements ICPPNamespace,
		ICPPInternalBinding {
	public static class CPPNamespaceDelegate extends CPPDelegate implements
			ICPPNamespace {
	...
	public static class CPPNamespaceProblem extends ProblemBinding implements
			ICPPNamespace {
	...
	static private class NamespaceCollector extends CPPASTVisitor {
		...
		public int visit(ICPPASTNamespaceDefinition namespace) {
			...
			while (candidate != null) {
				if (!CharArrayUtils.equals(orig.getName().toCharArray(),
						candidate.getName().toCharArray()))
				...
				if (orig.getParent() instanceof ICPPASTNamespaceDefinition) {
					...
					candidate = (ICPPASTNamespaceDefinition) candidate
							.getParent();
			...
			namespaces = (IASTName[]) ArrayUtil.append(IASTName.class,
	...
	static private class NamespaceMemberCollector extends CPPASTVisitor {
		...
		public int visit(IASTDeclSpecifier declSpec) {
			if (declSpec instanceof ICPPASTCompositeTypeSpecifier) {
				IBinding binding = ((ICPPASTCompositeTypeSpecifier) declSpec)
				...
				if (parent instanceof IASTSimpleDeclaration) {
					...
					IBinding binding = ((ICPPASTElaboratedTypeSpecifier) declSpec)
							.getName().resolveBinding();
					if (binding != null
		...
		public int visit(IASTDeclaration declaration) {
			if (declaration instanceof ICPPASTUsingDeclaration) {
				IBinding binding = ((ICPPASTUsingDeclaration) declaration)
						.getName().resolveBinding();
				if (binding != null && !(binding instanceof IProblemBinding))
					members.put(binding);
				return PROCESS_SKIP;
	...
	public ICPPNamespaceScope getNamespaceScope() {
		if (scope == null) {
			if (tu != null)
				...
				scope = new CPPNamespaceScope(namespaceDefinitions[0]
						.getParent());
		}
		return scope;
	}
	...
	public char[] getNameCharArray() {
		return tu != null ? EMPTY_CHAR_ARRAY : namespaceDefinitions[0]
				.toCharArray();
	}
	...
	public IScope getScope() {
		return tu != null ? null : CPPVisitor
				.getContainingScope(namespaceDefinitions[0]);
	}
	...
	public void addDefinition(IASTNode node) {
		...
		if (namespaceDefinitions.length > 0
				&& ((ASTNode) name).getOffset() < ((ASTNode) namespaceDefinitions[0])
						.getOffset()) {
			namespaceDefinitions = (IASTName[]) ArrayUtil.prepend(
					IASTName.class, namespaceDefinitions, name);
		} else {
			namespaceDefinitions = (IASTName[]) ArrayUtil.append(
					IASTName.class, namespaceDefinitions, name);
		}
	}
			...
			for (int i = 0; i < namespaceDefinitions.length; i++) {
				if (node == namespaceDefinitions[i]) {
					if (i == namespaceDefinitions.length - 1)
						...
						System.arraycopy(namespaceDefinitions, i + 1,
								namespaceDefinitions, i,
								namespaceDefinitions.length - 1 - i);
					return;
				}
			}
		}
		...
		if (namespaceDefinitions != null) {
			...
			for (int i = 0; i < namespaceDefinitions.length; i++) {
				...
				if (parent instanceof ICPPASTNamespaceDefinition) {
					IASTDeclaration[] decls = ((ICPPASTNamespaceDefinition) parent)
							.getDeclarations();
