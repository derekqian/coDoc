...
public class ScannerUtility {
	 ...
	 * skip "/./" - skip quotes - process "/../" (skip previous directory level)
	 * 
	 * @param originalPath -
	 *            path to process
				...
				if (noSepBefore && j > 0)
					...
					if (i < len1) {
						...
						// we found "/.." sequence. Look ahead.
							...
							// we found "/../" (or "/.." is at the end of
							// string)
							// we should delete previous segment of output path
							if (i == len1 || ein[i + 2] == SLASH
									|| ein[i + 2] == BSLASH) {
								i += 2;
								noSepBefore = false;
								if (j > 1) { // there is at least 1 segment
												// before
									int k = j - 2;
									while (k >= 0) {
										if (aus[k] == File.separatorChar)
											break;
										k--;
									}
									j = k + 1; // set index to previous segment
												// or to 0
								}
							}
	...
	public static String createReconciledPath(String path, String fileName) {
		...
		return (pathEmpty ? fileName : reconcilePath(path + File.separatorChar
				+ fileName));
	}

	public static CodeReader createReaderDuple(String path,
			ISourceElementRequestor requestor, Iterator workingCopies) {
		return requestor.createReader(path, workingCopies);
	}

	public static class InclusionDirective {
		public InclusionDirective(String fileName, boolean useIncludePaths,
