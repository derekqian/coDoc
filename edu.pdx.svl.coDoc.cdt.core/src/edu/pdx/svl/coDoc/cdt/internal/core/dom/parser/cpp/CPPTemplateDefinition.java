 ...
 * @author aniefer
 */
public abstract class CPPTemplateDefinition extends PlatformObject implements
		ICPPTemplateDefinition, ICPPInternalTemplate {
	public static final class CPPTemplateProblem extends ProblemBinding
		...
		public ICPPClassTemplatePartialSpecialization[] getTemplateSpecializations()
				throws DOMException {
			throw new DOMException(this);
		}
	...
	public IBinding instantiate(ICPPASTTemplateId templateId) {// IASTNode[]
																// arguments) {
	...
	public IBinding instantiate(IType[] arguments) {
		...
		if (this instanceof ICPPClassTemplate) {
			try {
				template = CPPTemplates.matchTemplatePartialSpecialization(
						(ICPPClassTemplate) this, arguments);
	...
	public IBinding resolveTemplateParameter(
		...
		IASTName name = CPPTemplates
				.getTemplateParameterName(templateParameter);
		IBinding binding = name.getBinding();
		if (binding != null)
			return binding;

		ICPPASTTemplateDeclaration templateDecl = (ICPPASTTemplateDeclaration) templateParameter
				.getParent();
		...
		if (definition != null
			...
			IASTName templateName = (definition != null) ? definition
					: declarations[0];
			ICPPASTTemplateDeclaration temp = CPPTemplates
					.getTemplateDeclaration(templateName);
			ICPPASTTemplateParameter[] params = temp.getTemplateParameters();
			if (params.length > i) {
				IASTName paramName = CPPTemplates
						.getTemplateParameterName(params[i]);
		...
		for (; j < length; j++) {
			ICPPASTTemplateDeclaration template = (j == -1) ? CPPTemplates
					.getTemplateDeclaration(definition) : CPPTemplates
					.getTemplateDeclaration(declarations[j]);
			if (template == null)
				continue;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.cpp.ICPPTemplateDefinition#getParameters()
	 */
	public ICPPTemplateParameter[] getTemplateParameters() {
		if (templateParameters == null) {
			ICPPASTTemplateDeclaration template = CPPTemplates
					.getTemplateDeclaration(getTemplateName());
			if (template == null)
				return ICPPTemplateParameter.EMPTY_TEMPLATE_PARAMETER_ARRAY;
			ICPPASTTemplateParameter[] params = template
			...
			for (int i = 0; i < params.length; i++) {
				if (params[i] instanceof ICPPASTSimpleTypeTemplateParameter) {
					p = (ICPPTemplateParameter) ((ICPPASTSimpleTypeTemplateParameter) params[i])
							.getName().resolveBinding();
				} else if (params[i] instanceof ICPPASTParameterDeclaration) {
					p = (ICPPTemplateParameter) ((ICPPASTParameterDeclaration) params[i])
							.getDeclarator().getName().resolveBinding();
				} else if (params[i] instanceof ICPPASTTemplatedTypeTemplateParameter) {
					p = (ICPPTemplateParameter) ((ICPPASTTemplatedTypeTemplateParameter) params[i])
							.getName().resolveBinding();
				}

				if (p != null) {
					result = (ICPPTemplateParameter[]) ArrayUtil.append(
							ICPPTemplateParameter.class, result, p);
				}
			}
			templateParameters = (ICPPTemplateParameter[]) ArrayUtil.trim(
					ICPPTemplateParameter.class, result);
		}
	...
	protected void updateTemplateParameterBindings(IASTName name) {
		...
		ICPPASTTemplateDeclaration origTemplate = CPPTemplates
				.getTemplateDeclaration(orig);
		ICPPASTTemplateDeclaration newTemplate = CPPTemplates
		...
		for (int i = 0; i < nps.length; i++) {
			temp = (ICPPInternalBinding) CPPTemplates.getTemplateParameterName(
					ops[i]).getBinding();
			if (temp != null) {
