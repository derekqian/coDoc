 ...
 * 
 * It performs this operation by locally caching the contents of the C element
 * when it is created. When the method buildDeltas() is called, it creates a
 * delta over the cached contents and the new contents.
 * 
...
public class CElementDeltaBuilder {
	...
	/**
	 * Creates a C element comparator on a C element looking as deep as
	 * necessary.
	 */
	...
	/**
	 * Creates a C element comparator on a C element looking only 'maxDepth'
	 * levels deep.
	 */
	...
	/**
	 * Builds the C element deltas between the old content of the translation
	 ...
	 */
	private void findContentChange(CElementInfo oldInfo, CElementInfo newInfo,
			ICElement newElement) {
		if (oldInfo instanceof SourceManipulationInfo
				&& newInfo instanceof SourceManipulationInfo) {
			...
			if ((oldSourceInfo).getModifiers() != (newSourceInfo)
					.getModifiers()) {
				this.delta.changed(newElement, ICElementDelta.F_MODIFIERS);
			}

			// The element info should be able to tell if the contents are the
			// same.
	...
	private void insertPositions(ICElement[] elements, boolean isNew) {
		...
		ICElement previous = null, current = null, next = (length > 0) ? elements[0]
				: null;
		for (int i = 0; i < length; i++) {
			previous = current;
			current = next;
	...
	/**
	 * Returns true if the given elements represent the an equivalent
	 * declaration.
	 * 
	 * <p>
	 * NOTE: Since this comparison can be done with handle info only, none of
	 * the internal calls need to use the locally cached contents of the old
	 * translation unit.
	 */
	...
	// }
	/**
	 * Answers true if the elements position has not changed. Takes into account
	 * additions so that elements following new elements will not appear out of
	 * place.
	 */
	...
	private void recordElementInfo(ICElement element, int depth) {
		...
		CElementInfo info = (CElementInfo) CModelManager.getDefault().getInfo(
				element);
