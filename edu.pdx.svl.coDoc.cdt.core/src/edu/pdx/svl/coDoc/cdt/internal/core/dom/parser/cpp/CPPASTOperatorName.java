 ...
 * @author dsteffle
 */
public class CPPASTOperatorName extends CPPASTName implements
		ICPPASTOperatorName {
	...
	/**
	 * Primary consturctor that should be used to initialize the
	 * CPPASTOperatorName.
	 * 
	 * @param name
	 *            the char[] name to initialize to
	 */
	...
	/**
	 * Returns the char[] of the name and ensures that the name will be
	 * standardized so that it will be of the form "operator op".
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the String of the name and ensures that the name will be
	 * standardized so that it will be of the form "operator op".
	 * 
	 * @return
	 */
	...
	/**
	 * Sets the name of this CPPASTOperatorName and ensures that the name
	 * follows the standard "operator op".
	 * 
	 * @param name
	 */
	...
	private char[] enforceOpStandard(String name) {
		...
		if (name.indexOf(OP_NEW, FIRST_CHAR_AFTER_OPERATOR) >= FIRST_CHAR_AFTER_OPERATOR
				&& name.indexOf(LBRACKET, FIRST_CHAR_AFTER_OPERATOR) >= FIRST_CHAR_AFTER_OPERATOR
				&& name.indexOf(RBRACKET, FIRST_CHAR_AFTER_OPERATOR) >= FIRST_CHAR_AFTER_OPERATOR) {
		...
		} else if (name.indexOf(LPAREN, FIRST_CHAR_AFTER_OPERATOR) >= FIRST_CHAR_AFTER_OPERATOR
				&& name.indexOf(RPAREN, FIRST_CHAR_AFTER_OPERATOR) >= FIRST_CHAR_AFTER_OPERATOR) {
			return OPERATOR_PAREN;
		} else if (name.indexOf(LBRACKET, FIRST_CHAR_AFTER_OPERATOR) >= FIRST_CHAR_AFTER_OPERATOR
				&& name.indexOf(RBRACKET, FIRST_CHAR_AFTER_OPERATOR) >= FIRST_CHAR_AFTER_OPERATOR) {
