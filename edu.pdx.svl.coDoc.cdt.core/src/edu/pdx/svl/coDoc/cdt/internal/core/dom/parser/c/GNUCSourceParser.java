...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTDesignatedInitializer;
...
import edu.pdx.svl.coDoc.cdt.core.dom.ast.c.ICASTTypeIdInitializerExpression;
...
public class GNUCSourceParser extends AbstractGNUSourceCodeParser {
	...
	public GNUCSourceParser(IScanner scanner, ParserMode parserMode,
		...
		super(scanner, logService, parserMode, config
				...
				.supportAlignOfUnaryExpression(), config.supportKnRC(), config
				.supportGCCOtherBuiltinSymbols(), config
			...
			for (;;) {
				...
				if (newDesignators.isEmpty()) {
					...
					((CASTNode) desigInitializer).setOffsetAndLength(
							...
							((CASTNode) initializer).getOffset()
									+ ((CASTNode) initializer).getLength()
									- ((CASTNode) newDesignators.get(0))
											.getOffset());
		...
		if (LT(1) == IToken.tDOT || LT(1) == IToken.tLBRACKET) {
			...
			if (supportGCCStyleDesignators
				...
				if (LT(1) == IToken.tIDENTIFIER) {
					// fix for 84176: if reach identifier and it's not a
					// designator then return empty designator list
	...
	protected IASTDeclaration simpleDeclaration() throws BacktrackException,
		...
		if (LT(1) != IToken.tSEMI) {
			if (!skipAhead)
				declarators = (IASTDeclarator[]) ArrayUtil.append(
						IASTDeclarator.class, declarators, initDeclarator());
		...
		try {
			...
			if (supportGCCOtherBuiltinSymbols) {
				...
				IBinding[] bindings = new GCCBuiltinSymbolProvider(
						translationUnit.getScope(), ParserLanguage.C)
						.getBuiltinBindings();
					...
					try {
						...
						if (typeId != null && proper && LT(1) == IToken.tLPAREN) {
							...
							return buildTypeIdUnaryExpression(
									IASTCastExpression.op_cast,
									typeId,
									castExpression,
									startingOffset,
									LT(1) == IToken.tEOC ? LA(1).getEndOffset()
	...
	protected IASTDeclarator declarator() throws EndOfFileException,
		...
		overallLoop: do {
			...
			for (;;) {
				...
				case IToken.tLPAREN:
					...
					if (supportKnRC && numKnRCParms > 0) { // KnR C parameters
															// were found so
		...
		if (numKnRCParms > 0) {
			...
			parmDeclarations = (IASTDeclaration[]) ArrayUtil.removeNulls(
					IASTDeclaration.class, parmDeclarations);
