...
public class ASTInitializerClause implements IASTInitializerClause {
	 ...
	 * @param designators
	 */
	public ASTInitializerClause(Kind kind, IASTExpression assignmentExpression,
	...
	public List getInitializersList() {
		return (initializerClauses != null) ? initializerClauses
	...
	public void acceptElement(ISourceElementRequestor requestor) {
		...
		for (int i = 0; i < size; i++)
			((IASTInitializerClause) initializers.get(i))
	...
	public void setOwnerVariableDeclaration(IASTVariable declaration) {
		...
		while (subInitializers.hasNext())
			((IASTInitializerClause) subInitializers.next())
					.setOwnerVariableDeclaration(declaration);
	}
	...
	public IASTExpression findExpressionForDuple(ITokenDuple finalDuple)
		...
		if (kind == IASTInitializerClause.Kind.EMPTY)
			return null;
		if (kind == IASTInitializerClause.Kind.ASSIGNMENT_EXPRESSION
				|| kind == Kind.DESIGNATED_ASSIGNMENT_EXPRESSION)
			return ((ASTExpression) assignmentExpression)
					.findNewDescriptor(finalDuple);
