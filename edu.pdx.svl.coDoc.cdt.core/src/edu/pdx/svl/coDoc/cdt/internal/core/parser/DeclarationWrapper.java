...
public class DeclarationWrapper implements IDeclaratorOwner {
	 ...
	 *            TODO
	 */
	public DeclarationWrapper(IASTScope scope, int startingOffset,
	 ...
	 * @param requestor
	 */
	public List createASTNodes(IASTFactory astFactoryToWorkWith)
			throws ASTSemanticException, BacktrackException {
	...
	private IASTDeclaration createASTNode(Declarator declarator)
			throws ASTSemanticException, BacktrackException {
		boolean isWithinClass = false;// (getScope() instanceof
										// IASTClassSpecifier); //TODO fix this
										// for COMPLETE_PARSE
		if (getScope() instanceof IASTClassSpecifier) {
			isWithinClass = true;
		} else if (getScope() instanceof IASTTemplateDeclaration) {
			isWithinClass = (((IASTTemplateDeclaration) getScope())
					.getOwnerScope() instanceof IASTClassSpecifier);
		}
		...
		if (isWithinClass) {
			...
			else if (declarator.hasFunctionBody())
	 ...
	 * @return
	 */
	private IASTDeclaration createIndirectDeclaration(Declarator declarator)
			throws BacktrackException, ASTSemanticException {
		if (declarator.getOwnedDeclarator().getOwnedDeclarator() == null) {
			...
			List convertedParms = createParameterList(declarator
					.getParameters());
			IASTAbstractDeclaration abs = null;
			abs = astFactory.createAbstractDeclaration(isConst(), isVolatile(),
					getTypeSpecifier(), declarator.getPointerOperators(),
					declarator.getArrayModifiers(), convertedParms,
					(ASTPointerOperator) ptrOps.get(0));

			ITokenDuple nameDuple = (d.getPointerOperatorNameDuple() != null) ? TokenFactory
					.createTokenDuple(d.getPointerOperatorNameDuple(), d
							.getNameDuple())
					: d.getNameDuple();

			if (isTypedef())
				return astFactory.createTypedef(scope, nameDuple.toCharArray(),
						abs, getStartingOffset(), getStartingLine(), d
								.getNameStartOffset(), d.getNameEndOffset(), d
								.getNameLine(), fn);

			if (isWithinClass)
				return astFactory
						.createField(scope, nameDuple, isAuto(), d
								.getInitializerClause(), d
								.getBitFieldExpression(), abs, isMutable(),
								isExtern(), isRegister(), isStatic(),
								getStartingOffset(), getStartingLine(), d
										.getNameStartOffset(), d
										.getNameEndOffset(), d.getNameLine(), d
										.getConstructorExpression(),
								((IASTClassSpecifier) scope)
										.getCurrentVisibilityMode(), fn);

			return astFactory.createVariable(scope, nameDuple, isAuto(), d
					.getInitializerClause(), d.getBitFieldExpression(), abs,
					isMutable(), isExtern(), isRegister(), isStatic(),
					getStartingOffset(), getStartingLine(), d
							.getNameStartOffset(), d.getNameEndOffset(), d
							.getNameLine(), d.getConstructorExpression(), fn);
	...
	private IASTTypedefDeclaration createTypedef(Declarator declarator,
			boolean nested) throws ASTSemanticException {
	 ...
	 * @return
	 */
	private IASTMethod createMethodASTNode(Declarator declarator, boolean nested)
			throws ASTSemanticException {
		IASTScope classifierScope = getScope();
		if (classifierScope instanceof IASTTemplateDeclaration) {
			classifierScope = ((IASTTemplateDeclaration) classifierScope)
					.getOwnerScope();
		}
		return astFactory.createMethod(scope, nested ? declarator
				...
				declarator.getNameLine(), templateDeclaration, declarator
						.isConst(), declarator.isVolatile(), isVirtual(),
				isExplicit(), declarator.isPureVirtual(),
				((IASTClassSpecifier) classifierScope)
	...
	private IASTFunction createFunctionASTNode(Declarator declarator,
		...
		return astFactory.createFunction(scope, nested ? declarator
				...
				declarator.getNameLine(), templateDeclaration, declarator
						.isConst(), declarator.isVolatile(), isVirtual(),
				isExplicit(), declarator.isPureVirtual(), declarator
						.getConstructorMemberInitializers(), declarator
						.hasFunctionBody(), declarator.hasFunctionTryBlock(),
				declarator.isVarArgs());
	}
	 ...
	 * @return
	 */
	private IASTField createFieldASTNode(Declarator declarator, boolean nested)
			throws ASTSemanticException {
		return astFactory.createField(scope, nested ? declarator
				.getOwnedDeclarator().getNameDuple() : declarator
				.getNameDuple(), isAuto(), declarator.getInitializerClause(),
				declarator.getBitFieldExpression(), astFactory
						.createAbstractDeclaration(isConst(), isVolatile(),
								getTypeSpecifier(), declarator
										.getPointerOperators(), declarator
										.getArrayModifiers(), null, null),
				...
				((IASTClassSpecifier) scope).getCurrentVisibilityMode(), fn);
	}
	...
	private List createParameterList(List currentParameters)
		...
		for (int i = 0; i < currentParameters.size(); ++i) {
			DeclarationWrapper wrapper = (DeclarationWrapper) currentParameters
					.get(i);
			List decls = wrapper.getDeclaratorsList();
			for (int j = 0; j < decls.size(); j++) {
				Declarator declarator = (Declarator) decls.get(j);

				result.add(astFactory.createParameterDeclaration(wrapper
						.isConst(), wrapper.isVolatile(), wrapper
						.getTypeSpecifier(), declarator.getPointerOperators(),
						declarator.getArrayModifiers(), null, null, declarator
								.getName(), declarator.getInitializerClause(),
						wrapper.getStartingOffset(), getStartingLine(),
						declarator.getNameStartOffset(), declarator
								.getNameEndOffset(), declarator.getNameLine(),
						wrapper.getEndOffset(), getEndLine(), wrapper.fn));
			}
		}
		return result;
	}
	...
	private IASTVariable createVariableASTNode(Declarator declarator,
			boolean nested) throws ASTSemanticException {
		return astFactory.createVariable(scope, nested ? declarator
				.getOwnedDeclarator().getNameDuple() : declarator
				.getNameDuple(), isAuto(), declarator.getInitializerClause(),
				declarator.getBitFieldExpression(), astFactory
						.createAbstractDeclaration(isConst(), isVolatile(),
								getTypeSpecifier(), declarator
										...
										.getArrayModifiers(), null, null),
				isMutable(), isExtern(), isRegister(), isStatic(),
				getStartingOffset(), getStartingLine(), declarator
						.getNameStartOffset(), declarator.getNameEndOffset(),
				declarator.getNameLine(),
				declarator.getConstructorExpression(), fn);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.IDeclaratorOwner#getDeclarationWrapper()
	 */
