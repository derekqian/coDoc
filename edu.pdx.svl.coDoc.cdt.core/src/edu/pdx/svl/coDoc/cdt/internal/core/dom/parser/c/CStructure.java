 ...
 * @author aniefer
 */
public class CStructure extends PlatformObject implements ICompositeType,
		ICInternalBinding {
	...
	public IASTNode getPhysicalNode() {
		return (definition != null) ? (IASTNode) definition
				: (IASTNode) declarations[0];
	}

	private ICASTCompositeTypeSpecifier checkForDefinition(
			IASTElaboratedTypeSpecifier declSpec) {
		IASTDeclSpecifier spec = CVisitor
				.findDefinition((ICASTElaboratedTypeSpecifier) declSpec);
	...
	public IField[] getFields() throws DOMException {
		if (definition == null) {
			ICASTCompositeTypeSpecifier temp = checkForDefinition((IASTElaboratedTypeSpecifier) declarations[0]
					.getParent());
			if (temp == null)
				return new IField[] { new CField.CFieldProblem(declarations[0],
						IProblemBinding.SEMANTIC_DEFINITION_NOT_FOUND,
						getNameCharArray()) };
			definition = temp.getName();
		}
		ICASTCompositeTypeSpecifier compSpec = (ICASTCompositeTypeSpecifier) definition
				.getParent();
				...
				if (node instanceof IASTSimpleDeclaration) {
					...
					for (int j = 0; j < declarators.length; j++) {
						...
						if (scope != null)
							scope.addName(name);
						if (binding != null)
							fields = (IField[]) ArrayUtil.append(IField.class,
									fields, binding);
					}
				}
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.core.dom.ast.ICompositeType#findField(edu.pdx.svl.coDoc.cdt.core.dom.ast.IASTName)
	 */
	public IField findField(String name) throws DOMException {
		if (definition == null) {
			ICASTCompositeTypeSpecifier temp = checkForDefinition((IASTElaboratedTypeSpecifier) declarations[0]
					.getParent());
			if (temp == null)
				return new CField.CFieldProblem(declarations[0],
						IProblemBinding.SEMANTIC_DEFINITION_NOT_FOUND,
						getNameCharArray());
			definition = temp.getName();
		}
					...
					if (node instanceof IASTSimpleDeclaration) {
						...
						if (declarators.length == 0
							...
							if (n.toCharArray().length == 0) {
								specStack = (ICASTCompositeTypeSpecifier[]) ArrayUtil
										.append(
												ICASTCompositeTypeSpecifier.class,
												specStack, declSpec);
							}
						}
					}
				}
				if (specStack != null && ++stackIdx < specStack.length
						&& specStack[stackIdx] != null) {
	 ...
	 */
	public int getKey() {
		return (definition != null) ? ((IASTCompositeTypeSpecifier) definition
				.getParent()).getKey()
				: ((IASTElaboratedTypeSpecifier) declarations[0].getParent())
						.getKind();
	}
