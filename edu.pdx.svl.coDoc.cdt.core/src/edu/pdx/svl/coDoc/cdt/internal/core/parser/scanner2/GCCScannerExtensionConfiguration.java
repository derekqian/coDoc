...
import edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.BaseScanner.ExpressionEvaluator;

/**
 * @author jcamelon
 */
public class GCCScannerExtensionConfiguration extends
	...
	private final DynamicFunctionStyleMacro __builtin_choose_expr = new DynamicFunctionStyleMacro(
			"__builtin_choose_expr".toCharArray(), //$NON-NLS-1$
			new char[][] { "const_exp".toCharArray(), "exp1".toCharArray(),
					"exp2".toCharArray() }) //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	{
		public char[] execute(CharArrayObjectMap argmap) {
			...
			long exp = 0;
			if (const_exp != null)
				exp = evaluator.evaluate(const_exp, 0, const_exp.length,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.pdx.svl.coDoc.cdt.internal.core.parser.scanner2.IScannerConfiguration#getAdditionalMacros()
	 */
