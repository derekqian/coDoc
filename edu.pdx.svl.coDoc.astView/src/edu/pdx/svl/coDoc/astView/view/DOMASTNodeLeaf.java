...
public class DOMASTNodeLeaf implements IAdaptable {
	...
	public String toString() {
		...
		} else if( node instanceof IASTDeclSpecifier )
		{
			...
			buffer.append(START_OF_LIST);
			//derek buffer.append( ASTStringUtil.getCastOperatorString( (IASTCastExpression)node ) );
			buffer.append(node.toString());
		} else if ( node instanceof IASTUnaryExpression ) {
			buffer.append(START_OF_LIST);
			//derek buffer.append( ASTStringUtil.getUnaryOperatorString( (IASTUnaryExpression)node ) );
			buffer.append(node.toString());
		} else if ( node instanceof IASTBinaryExpression ) {
			buffer.append(START_OF_LIST);
			//derek buffer.append( ASTStringUtil.getBinaryOperatorString( (IASTBinaryExpression)node ) );
			buffer.append(node.toString());
			...
			/*derek if (((IASTPointer)node).isRestrict()) {
				if (!started) {
					started = true;
					buffer.append(START_OF_LIST);
				}
	...
	private static class ASTPropertySource implements IPropertySource {
		...
		private String getValueString(Object obj) {
			...
			if (obj.getClass().isPrimitive()) {
				...
				//derek buffer.append(ASTStringUtil.getExpressionString((IASTExpression)obj));
			} else if (obj instanceof IASTNode) {
				//derek String utilString = DOMAST.getNodeSignature((IASTNode)obj);
				String utilString = null;
