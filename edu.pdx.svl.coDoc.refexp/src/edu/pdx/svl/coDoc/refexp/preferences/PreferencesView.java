package edu.pdx.svl.coDoc.refexp.preferences;
...
import edu.pdx.svl.coDoc.refexp.referencemodel.PDFManager;
import edu.pdx.svl.coDoc.refexp.referencemodel.PDFFile;
...
public class PreferencesView extends Dialog {
	...
	ReferenceExplorerView parentView;

	public void setParentView(ReferenceExplorerView rev) {
		parentView = rev;
	}
	
	public PreferencesView() {
		this(new Shell());
		setParentView(Global.INSTANCE.referenceExplorerView);
	}
	/**
	 * Create the dialog.
	 * @param parentShell
...
//		currentlyActivePDFTxtBox.addKeyListener(keyListener);
		...
		selectPDFButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (parentView != null) {
					parentView.getSelectPDFButton().setToolTipText("Current PDF:\n" + f.getPath());
				}
			}
		});
