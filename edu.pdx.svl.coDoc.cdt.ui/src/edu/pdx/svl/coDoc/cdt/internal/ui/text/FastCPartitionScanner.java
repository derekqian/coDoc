...
/**
 * This scanner recognizes the C multi line comments, C single line comments, C
...
public class FastCPartitionScanner implements IPartitionTokenScanner,
	...
	private final IToken[] fTokens = new IToken[] { new Token(null),
			new Token(C_SINGLE_LINE_COMMENT), new Token(C_MULTILINE_COMMENT),
			new Token(C_CHARACTER), new Token(C_STRING) };
	...
	public IToken nextToken() {
		...
		while (true) {
			...
			if (delim != -1) {
				...
				if (len > 1) {
					// adjust the token length if the delimiter was 2 or more
					// chars
					fTokenLength += (len - 1);
				}
				switch (fState) {
			...
			case CCODE:
				...
				case '*':
					if (fLast == SLASH) {
						if (fTokenLength - getLastLength(fLast) > 0)
							return preFix(CCODE, MULTI_LINE_COMMENT,
									SLASH_STAR, 2);
	 ...
	 */
	private static final int detectLineDelimiter(final int ch,
			final ICharacterScanner scanner, final char[][] delimiters) {
		...
		for (int i = 0; i < delimiters.length; i++) {
			...
			if (ch == delim[0]) {
				...
				if (len > maxLen
						&& (len == 1 || sequenceDetected(scanner, delim, 1, len))) {
					maxLen = len;
					longestDelimiter = i;
				}
			}
	...
	/**
	 * <code>true</code> if sequence matches between beginIndex (inclusive)
	 * and endIndex (exclusive)
	 */
	private static final boolean sequenceDetected(
			final ICharacterScanner scanner, final char[] sequence,
