...
import edu.pdx.svl.coDoc.cdt.internal.ui.editor.IReconcilingParticipant;
...
public class CReconcilingStrategy implements IReconcilingStrategy {
		...
		if ((doc != null) && (!CWordFinder.isGlobal(doc, dOff))) {
			...
			if (dirtyRegion.getType().charAt(2) == 'i') { // insert operation
				...
				if (!CWordFinder.hasCBraces(s)) {
					// CModelManager.getDefault().fireShift(tu, dOff, dLen,
					// CWordFinder.countLFs(s));
					needReconcile = false;
				}
	...
	private void reconcile() {
		try {
			ITranslationUnit tu = fManager.getWorkingCopy(fEditor
					.getEditorInput());
