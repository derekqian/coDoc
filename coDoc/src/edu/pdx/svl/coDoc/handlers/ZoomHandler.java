 ...
 ******************************************************************************/
package edu.pdx.svl.coDoc.handlers;
...
import edu.pdx.svl.coDoc.editors.CDCEditor;
...
public class ZoomHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {		
		if (!CDCEditor.ID.equals(HandlerUtil.getActiveEditorId(event))) return null;
		...
		float zoomFactor = 1f;	
		if (msg.charAt(0) == '+') {
			zoomFactor = ((CDCEditor) editor).pv.getZoomFactor() + Float.parseFloat(msg.substring(1));
		}
			...
			((CDCEditor) editor).fitHorizontal();
			return null;
		}
		else if (msg.toLowerCase().equals(FIT_COMPLETE)) {
			((CDCEditor) editor).fit();
			return null;
		}

		((CDCEditor) editor).pv.setZoomFactor(zoomFactor);
		return null;
	}
}
